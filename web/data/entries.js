// Auto-generated from entries.json by scripts/build.py — do not edit.
window.TECHDEX_ENTRIES = [
 {
  "id": "aws-vpc",
  "name": "AWS VPC",
  "category": "Platform",
  "domain": "Networking",
  "description": "AWS Virtual Private Cloud — logically isolated network within AWS where you define IP ranges, subnets, routing, and firewall rules. Every AWS workload lives inside a VPC. Foundation of AWS network security.",
  "use_cases": [
   "Isolating production, staging, and dev environments in separate VPCs",
   "Public/private subnet design: ALB in public, app/DB in private",
   "VPC peering and Transit Gateway for multi-VPC / multi-account networking",
   "VPC Endpoints for private S3/DynamoDB access without internet",
   "Security Groups and NACLs as layered firewall controls"
  ],
  "used_by": [
   "All AWS customers"
  ],
  "docs": "https://docs.aws.amazon.com/vpc/",
  "tutorial": "https://docs.aws.amazon.com/vpc/latest/userguide/vpc-getting-started.html",
  "added": "2026-06-08",
  "eli5": "Your own private neighborhood inside Amazon's giant city.",
  "dex": 1
 },
 {
  "id": "caddy",
  "name": "Caddy",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Caddy is like a web server that automatically gets its own security certificate and sets it up for you — no manual HTTPS configuration needed.",
  "description": "Modern web server with automatic HTTPS via Let's Encrypt. Written in Go, zero config TLS, with a simple Caddyfile DSL and a JSON API for dynamic configuration.",
  "use_cases": [
   "Automatic TLS for local and production services",
   "Reverse proxy with zero-config HTTPS",
   "Static file serving and PHP applications",
   "API gateway with dynamic config via Admin API"
  ],
  "used_by": [
   "Tailscale",
   "Replicate",
   "Acme Corp deployments"
  ],
  "docs": "https://caddyserver.com/docs/",
  "tutorial": "https://caddyserver.com/docs/getting-started",
  "added": "2026-06-12",
  "dex": 2
 },
 {
  "id": "caddy-v2",
  "name": "Caddy v2",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Caddy is like a smart security guard at a building entrance who automatically gives everyone a special badge (HTTPS certificate) so nobody can spy on them.",
  "description": "Caddy is a modern, open-source web server written in Go that automatically provisions and renews HTTPS certificates via Let's Encrypt. It simplifies deployment with its JSON or Caddyfile configuration format and built-in reverse proxy, load balancing, and API gateway capabilities.",
  "use_cases": [
   "Automatic HTTPS certificate management for web applications",
   "Reverse proxy and API gateway for microservices",
   "Load balancing across multiple backend servers",
   "Quick deployment of static sites and web services"
  ],
  "used_by": [
   "Cloudflare",
   "Fastly",
   "many enterprises using automatic TLS"
  ],
  "docs": "https://caddyserver.com/docs/",
  "tutorial": "https://caddyserver.com/docs/getting-started",
  "added": "2026-08-19",
  "dex": 3
 },
 {
  "id": "reverse-proxy-caddy",
  "name": "Caddy Web Server",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like a smart mail sorter at the post office that automatically routes all your letters to the right houses and even adds special protective envelopes.",
  "description": "Caddy is a production-ready web server that automatically manages HTTPS certificates, supports HTTP/2 and HTTP/3, and provides dynamic configuration without downtime. It simplifies certificate management and serves as a reverse proxy, load balancer, and static file server.",
  "use_cases": [
   "Reverse proxying and load balancing with automatic SSL/TLS",
   "Serving static websites with automatic HTTPS setup",
   "API gateway functionality with dynamic configuration",
   "Edge computing and content delivery with automatic certificate renewal"
  ],
  "used_by": [
   "CloudFlare",
   "Fastly",
   "1.1.1.1"
  ],
  "docs": "https://caddyserver.com/docs/",
  "tutorial": "https://caddyserver.com/docs/quick-start",
  "added": "2026-07-27",
  "dex": 4
 },
 {
  "id": "cilium",
  "name": "Cilium",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Cilium is like a super-smart traffic cop for your Kubernetes cluster that understands what apps are talking about, not just where packets are going.",
  "description": "eBPF-based networking, observability, and security for Kubernetes. Provides high-performance networking with deep visibility into application traffic at the kernel level.",
  "use_cases": [
   "Kubernetes CNI plugin with eBPF dataplane",
   "Network policy enforcement at L3/L4/L7",
   "Service mesh replacement (Cilium Mesh)",
   "Hubble-based flow observability"
  ],
  "used_by": [
   "Google",
   "AWS",
   "Adobe",
   "GitLab",
   "Bell Canada"
  ],
  "docs": "https://docs.cilium.io/",
  "tutorial": "https://docs.cilium.io/en/stable/gettingstarted/",
  "added": "2026-06-11",
  "dex": 5
 },
 {
  "id": "cloudflare",
  "name": "Cloudflare",
  "category": "Platform",
  "domain": "Networking",
  "eli5": "Cloudflare is like a bodyguard and speed booster for your website — it blocks attacks and makes pages load faster by storing copies closer to users.",
  "description": "Global CDN, DDoS protection, DNS, and edge compute platform. Sits in front of web properties to improve performance, security, and availability at scale.",
  "use_cases": [
   "DDoS mitigation and WAF",
   "Global CDN and static asset caching",
   "Authoritative DNS with anycast routing",
   "Edge compute with Cloudflare Workers"
  ],
  "used_by": [
   "Discord",
   "Shopify",
   "DoorDash",
   "Canva"
  ],
  "docs": "https://developers.cloudflare.com/",
  "tutorial": "https://developers.cloudflare.com/fundamentals/get-started/",
  "added": "2026-06-12",
  "dex": 6
 },
 {
  "id": "coredns",
  "name": "CoreDNS",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "CoreDNS is like the phone book inside your Kubernetes cluster — it translates service names into IP addresses so apps can find each other.",
  "description": "Flexible, plugin-based DNS server written in Go. Default DNS server for Kubernetes since v1.13, handling service discovery and internal name resolution.",
  "use_cases": [
   "Kubernetes cluster-internal DNS",
   "Service discovery in microservices",
   "Custom DNS rewrites and forwarding",
   "Split-horizon DNS for on-prem/cloud hybrid"
  ],
  "used_by": [
   "Kubernetes (default)",
   "Red Hat OpenShift",
   "Rancher"
  ],
  "docs": "https://coredns.io/manual/toc/",
  "tutorial": "https://coredns.io/2017/05/08/custom-dns-entries-for-kubernetes/",
  "added": "2026-06-12",
  "dex": 7
 },
 {
  "id": "envoy-gateway",
  "name": "Envoy Gateway",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like a smart traffic director at the entrance of your building that routes visitors to the right room and keeps bad visitors out.",
  "description": "Envoy Gateway is an open-source API gateway built on Envoy proxy that simplifies traffic routing, rate limiting, and observability for Kubernetes environments. It provides a declarative, standards-based approach to gateway management with advanced traffic control features.",
  "use_cases": [
   "API gateway and ingress control in Kubernetes clusters",
   "Request routing, rate limiting, and circuit breaking",
   "TLS termination and mutual authentication",
   "Request/response transformation and traffic manipulation"
  ],
  "used_by": [
   "Kubernetes community",
   "Lyft",
   "ServiceTitan"
  ],
  "docs": "https://gateway.envoyproxy.io/",
  "tutorial": "https://gateway.envoyproxy.io/latest/tasks/traffic-management/",
  "added": "2026-08-20",
  "dex": 8
 },
 {
  "id": "envoy-proxy",
  "name": "Envoy Proxy",
  "category": "Tool",
  "domain": "Networking",
  "description": "Open-source, high-performance L7 proxy and communication bus built for cloud-native microservices architectures. Originally created at Lyft, now a CNCF graduated project. Runs as a sidecar alongside applications, making network concerns (routing, load balancing, observability, security) transparent to application code.",
  "use_cases": [
   "Service mesh data plane (sidecar proxy — backbone of Istio, AWS App Mesh, Consul Connect)",
   "Edge / ingress gateway for Kubernetes clusters",
   "L7 load balancing with header/path-aware routing",
   "Canary deployments and A/B traffic splitting",
   "mTLS between services, TLS termination at edge",
   "Distributed tracing and observability (Prometheus, Jaeger, Zipkin)",
   "Rate limiting and circuit breaking"
  ],
  "used_by": [
   "Lyft",
   "Google",
   "Apple",
   "Airbnb",
   "Pinterest",
   "Booking.com",
   "Dropbox",
   "Microsoft",
   "Netflix",
   "Uber"
  ],
  "docs": "https://www.envoyproxy.io/docs/envoy/latest/intro/what_is_envoy",
  "tutorial": "https://www.envoyproxy.io/docs/envoy/latest/start/quick-start/",
  "added": "2026-06-08",
  "eli5": "A traffic cop between your apps that decides who talks to who and how.",
  "dex": 9
 },
 {
  "id": "frpc",
  "name": "Frp (Fast Reverse Proxy)",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Frp is like a secret tunnel that lets you safely access a computer hidden behind a firewall from the internet without opening ports.",
  "description": "Frp is a fast reverse proxy written in Go that exposes local servers behind NAT or firewalls to the internet via a public server. It's widely used for exposing internal services, port forwarding, and creating secure tunnels.",
  "use_cases": [
   "Exposing local development servers for webhook testing",
   "Accessing internal services from outside networks securely",
   "Creating reverse tunnels for remote access to IoT devices",
   "Load balancing and multiplexing connections across multiple clients"
  ],
  "used_by": [
   "Alibaba Cloud",
   "Tencent",
   "ByteDance"
  ],
  "docs": "https://github.com/fatedier/frp",
  "tutorial": "https://gofrp.org/docs/setup/",
  "added": "2026-07-06",
  "dex": 10
 },
 {
  "id": "haproxy",
  "name": "HAProxy",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "HAProxy is like a traffic cop at a busy intersection who splits cars evenly across many roads so no single road gets jammed.",
  "description": "Battle-tested open-source TCP/HTTP load balancer and proxy. Industry standard for high-availability setups, used in front of databases, APIs, and web clusters.",
  "use_cases": [
   "L4/L7 load balancing for APIs and databases",
   "SSL termination and health-check-based failover",
   "Rate limiting and ACL-based routing",
   "High-availability database clusters (MySQL, PostgreSQL)"
  ],
  "used_by": [
   "GitHub",
   "Reddit",
   "Airbnb",
   "Stack Overflow"
  ],
  "docs": "https://docs.haproxy.org/",
  "tutorial": "https://www.haproxy.com/documentation/haproxy-configuration-tutorials/",
  "added": "2026-06-12",
  "dex": 11
 },
 {
  "id": "hashicorp-consul",
  "name": "HashiCorp Consul",
  "category": "Tool",
  "domain": "Networking",
  "description": "Service mesh and service discovery platform. Provides a distributed key-value store, health checking, and connect (mTLS service mesh). Works across VMs and containers, on-prem and cloud.",
  "use_cases": [
   "Service discovery: services register themselves, clients query DNS/HTTP API",
   "Health checking and automatic removal of unhealthy instances",
   "Consul Connect: mTLS service mesh for non-Kubernetes environments",
   "Distributed KV store for configuration and feature flags",
   "Multi-datacenter federation with WAN gossip"
  ],
  "used_by": [
   "Stripe",
   "SAP",
   "Citrix",
   "Barclays",
   "Booz Allen Hamilton"
  ],
  "docs": "https://developer.hashicorp.com/consul/docs",
  "tutorial": "https://developer.hashicorp.com/consul/tutorials/get-started-vms",
  "added": "2026-06-08",
  "eli5": "A phone book that tells your apps where their friends (other services) live today.",
  "dex": 12
 },
 {
  "id": "istio",
  "name": "Istio",
  "category": "Tool",
  "domain": "Networking",
  "description": "Open-source service mesh that layers traffic management, security, and observability on top of Kubernetes without changing application code. Uses Envoy as its sidecar data plane. CNCF graduated project.",
  "use_cases": [
   "Mutual TLS (mTLS) between all services with zero code changes",
   "Fine-grained traffic control: canary releases, A/B testing, fault injection",
   "Distributed tracing and metrics via Envoy sidecar telemetry",
   "Authorization policies: service-to-service and end-user RBAC",
   "Multi-cluster and multi-cloud service mesh federation"
  ],
  "used_by": [
   "Google",
   "Salesforce",
   "T-Mobile",
   "Airbus",
   "Autodesk"
  ],
  "docs": "https://istio.io/latest/docs/",
  "tutorial": "https://istio.io/latest/docs/setup/getting-started/",
  "added": "2026-06-08",
  "eli5": "A security guard between every pair of apps so they talk safely without trusting strangers.",
  "dex": 13
 },
 {
  "id": "libvirt-hypervisor",
  "name": "libvirt",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "libvirt is like a remote control that lets you manage many virtual computers on a single physical computer.",
  "description": "libvirt is an open-source API and daemon for managing platform virtualization, supporting KVM, QEMU, Xen, and other hypervisors. It's the foundation for infrastructure-as-code tools like Terraform and is widely used in cloud and edge computing environments.",
  "use_cases": [
   "Hypervisor management across heterogeneous platforms",
   "Virtual machine provisioning and orchestration",
   "Cloud infrastructure automation and IaC",
   "Edge computing and on-premises virtualization"
  ],
  "used_by": [
   "Red Hat",
   "OpenStack Foundation",
   "Proxmox",
   "Canonical"
  ],
  "docs": "https://libvirt.org/docs.html",
  "tutorial": "https://libvirt.org/goals.html",
  "added": "2026-09-26",
  "dex": 14
 },
 {
  "id": "linkerd",
  "name": "Linkerd",
  "category": "Tool",
  "domain": "Networking",
  "description": "Ultralight, security-first service mesh for Kubernetes. Uses a Rust-based micro-proxy (linkerd2-proxy) instead of Envoy — lower latency and resource overhead than Istio. CNCF graduated project. Zero-config mTLS by default.",
  "use_cases": [
   "Automatic mTLS for all pod-to-pod traffic with zero config changes",
   "Golden metrics (latency, success rate, RPS) per route via Prometheus",
   "Traffic splitting for canary deployments with SMI TrafficSplit",
   "Retries and timeouts at the mesh level without touching app code",
   "Multi-cluster failover and load balancing across clusters"
  ],
  "used_by": [
   "Buoyant",
   "Walmart",
   "H-E-B",
   "Elkjøp",
   "Entain"
  ],
  "docs": "https://linkerd.io/docs/",
  "tutorial": "https://linkerd.io/2.15/getting-started/",
  "added": "2026-06-08",
  "eli5": "A lightweight traffic cop between apps that also checks everyone's ID.",
  "dex": 15
 },
 {
  "id": "netbird",
  "name": "Netbird",
  "category": "Platform",
  "domain": "Networking",
  "eli5": "Netbird is like a magical tunnel that lets all your computers and phones talk to each other securely, no matter where they are in the world, without needing complicated setup.",
  "description": "Netbird is an open-source VPN and network management platform that uses WireGuard under the hood to create secure mesh networks between devices. It simplifies secure remote access and private networking without the complexity of traditional VPN infrastructure.",
  "use_cases": [
   "Zero-trust network access for remote development teams",
   "Secure mesh networking between microservices across cloud regions",
   "Private network connectivity for distributed teams without VPN appliances",
   "Device-to-device secure communication in IoT deployments"
  ],
  "used_by": [
   "GitLab",
   "Elastic",
   "Teleport"
  ],
  "docs": "https://docs.netbird.io/",
  "tutorial": "https://docs.netbird.io/getting-started",
  "added": "2026-06-22",
  "dex": 16
 },
 {
  "id": "netbox-ipam",
  "name": "NetBox",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "NetBox is like a detailed map of your entire network that tracks every computer, cable, and connection.",
  "description": "NetBox is an open-source infrastructure resource modeling (IRM) and IP address management (IPAM) platform designed for network operations teams to track and document datacenter infrastructure. It serves as the source of truth for network topology, device inventory, and IP allocations.",
  "use_cases": [
   "IP address management (IPAM) and DNS integration",
   "Datacenter asset and equipment tracking",
   "Network topology documentation and visualization",
   "Capacity planning and infrastructure reporting"
  ],
  "used_by": [
   "DigitalOcean",
   "Equinix",
   "Fastly",
   "CoreWeave"
  ],
  "docs": "https://docs.netbox.dev/",
  "tutorial": "https://docs.netbox.dev/en/stable/getting-started/",
  "added": "2026-09-26",
  "dex": 17
 },
 {
  "id": "netmaker",
  "name": "Netmaker",
  "category": "Platform",
  "domain": "Networking",
  "eli5": "It's like a magic tunnel that connects all your computers and servers together securely, no matter where they are in the world.",
  "description": "An open-source platform for creating and managing WireGuard-based virtual networks, enabling secure mesh networking across distributed infrastructure. It simplifies overlay network creation, management, and monitoring for zero-trust networking architectures.",
  "use_cases": [
   "Building zero-trust network architectures across cloud and on-premises",
   "Creating secure VPNs for remote teams and distributed teams",
   "Connecting Kubernetes clusters across multiple regions",
   "Establishing private mesh networks for IoT and edge devices"
  ],
  "used_by": [
   "HashiCorp",
   "Canonical",
   "various enterprises",
   "open source projects"
  ],
  "docs": "https://docs.netmaker.io/",
  "tutorial": "https://docs.netmaker.io/getting-started",
  "added": "2026-08-08",
  "dex": 18
 },
 {
  "id": "nginx",
  "name": "Nginx",
  "category": "Tool",
  "domain": "Networking",
  "description": "High-performance web server, reverse proxy, and load balancer. Event-driven, async architecture handles massive concurrent connections with minimal memory. Powers ~34% of the web. Also used as API gateway and static file server.",
  "use_cases": [
   "Reverse proxy and load balancer in front of app servers",
   "TLS termination and HTTP/2 / HTTP/3 offloading",
   "Static asset serving with efficient sendfile() kernel bypass",
   "Ingress controller for Kubernetes (ingress-nginx)",
   "Rate limiting, basic auth, and IP allowlisting at the edge",
   "API gateway with upstream routing and caching"
  ],
  "used_by": [
   "Dropbox",
   "Netflix",
   "WordPress.com",
   "GitHub",
   "Cloudflare"
  ],
  "docs": "https://nginx.org/en/docs/",
  "tutorial": "https://nginx.org/en/docs/beginners_guide.html",
  "added": "2026-06-08",
  "eli5": "A hotel receptionist that greets visitors and sends them to the right room.",
  "dex": 19
 },
 {
  "id": "nginx-ingress",
  "name": "Nginx Ingress Controller",
  "category": "Tool",
  "domain": "Networking",
  "description": "Kubernetes-native ingress controller built on Nginx. Translates Kubernetes Ingress resources into Nginx config, routing external HTTP/S traffic to in-cluster services. Most widely deployed Kubernetes ingress solution.",
  "use_cases": [
   "HTTP/S routing to Kubernetes services by host and path rules",
   "TLS termination with cert-manager for automatic Let's Encrypt certificates",
   "Rate limiting, IP allowlisting, and basic auth via annotations",
   "Canary deployments by splitting traffic across two Ingress resources",
   "WebSocket and gRPC passthrough for long-lived connections"
  ],
  "used_by": [
   "Widely used across Kubernetes deployments in enterprises and startups"
  ],
  "docs": "https://kubernetes.github.io/ingress-nginx/",
  "tutorial": "https://kubernetes.github.io/ingress-nginx/deploy/",
  "added": "2026-06-08",
  "eli5": "The front door of your Kubernetes building that sends visitors to the right apartment.",
  "dex": 20
 },
 {
  "id": "reverse-tunnel-ngrok",
  "name": "ngrok",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like creating a secret tunnel from your home computer to the internet so friends can visit without knowing your address.",
  "description": "ngrok is a reverse tunneling platform that exposes local servers to the internet securely, enabling rapid development and testing of webhooks, APIs, and integrations. It provides public URLs, traffic inspection, and replay capabilities for developing against production-like conditions.",
  "use_cases": [
   "Testing webhooks from external services (Stripe, GitHub) locally",
   "Exposing internal development servers for partner testing",
   "Mobile app testing against local APIs",
   "IoT device communication with cloud services"
  ],
  "used_by": [
   "Twilio",
   "Stripe",
   "GitHub",
   "Salesforce"
  ],
  "docs": "https://ngrok.com/docs",
  "tutorial": "https://ngrok.com/docs/getting-started",
  "added": "2026-08-26",
  "dex": 21
 },
 {
  "id": "openbsd-pf",
  "name": "OpenBSD PF Firewall",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "PF is like a security guard at a gate who can look at every package passing through and decide whether to let it in or throw it back out.",
  "description": "PF (Packet Filter) is a stateful firewall and network address translator included in OpenBSD, known for its clarity, expressiveness, and minimal memory footprint. It's widely deployed in high-performance networking environments and has been ported to other systems like FreeBSD and pfSense.",
  "use_cases": [
   "Protecting servers and networks at the perimeter firewall level",
   "Implementing advanced routing and traffic shaping",
   "Building ultra-reliable edge gateways for cloud infrastructure",
   "Creating transparent proxies and load balancing layers"
  ],
  "used_by": [
   "Cloudflare",
   "OpenBSD Foundation",
   "pfSense Project",
   "Netflix"
  ],
  "docs": "https://man.openbsd.org/pf",
  "tutorial": "https://www.openbsd.org/faq/pf/",
  "added": "2026-06-29",
  "dex": 22
 },
 {
  "id": "openwrt",
  "name": "OpenWrt",
  "category": "Platform",
  "domain": "Networking",
  "eli5": "It's like replacing the simple brain in your WiFi router with a super-smart custom one that can do way more tricks.",
  "description": "OpenWrt is a free and open-source operating system for embedded devices and network routers. It provides advanced networking capabilities, custom firmware, and extensibility beyond vendor-supplied router software.",
  "use_cases": [
   "Custom router firmware deployment",
   "Network traffic management and QoS",
   "VPN and VPN server setup",
   "IoT gateway and edge computing"
  ],
  "used_by": [
   "TP-Link",
   "Netgear",
   "ASUS",
   "Various ISPs and enterprises"
  ],
  "docs": "https://openwrt.org/docs/start",
  "tutorial": "https://openwrt.org/docs/guide-user/start",
  "added": "2026-07-13",
  "dex": 23
 },
 {
  "id": "pion",
  "name": "Pion WebRTC",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like a direct phone line between two people that works through the internet without needing a central operator, so you can chat with really low delay.",
  "description": "Pion is a pure Go implementation of the WebRTC protocol that enables peer-to-peer audio, video, and data communication directly in browsers and applications. It provides a simple API for building real-time communication systems without external dependencies.",
  "use_cases": [
   "Video conferencing and screen sharing applications",
   "Live streaming with ultra-low latency",
   "Real-time multiplayer gaming",
   "Peer-to-peer data transfer and collaboration tools"
  ],
  "used_by": [
   "Jitsi",
   "LiveKit",
   "Loom"
  ],
  "docs": "https://pion.ly/",
  "tutorial": "https://github.com/pion/webrtc/wiki/Getting-Started",
  "added": "2026-06-28",
  "dex": 24
 },
 {
  "id": "rustdesk",
  "name": "RustDesk",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like having a magic remote control that lets you see and control someone else's computer from yours, even if they're far away.",
  "description": "RustDesk is an open-source remote desktop software written in Rust that provides secure, fast remote access and control. It offers an alternative to commercial solutions like TeamViewer with self-hosting capabilities and no bandwidth limits.",
  "use_cases": [
   "Remote IT support and helpdesk operations",
   "System administrator access to servers and workstations",
   "Educational remote classroom sessions",
   "Self-hosted remote work infrastructure"
  ],
  "used_by": [
   "Canonical",
   "NixOS",
   "System integrators worldwide",
   "Enterprise IT departments"
  ],
  "docs": "https://rustdesk.com/docs",
  "tutorial": "https://rustdesk.com/docs/en/self-host/",
  "added": "2026-07-20",
  "dex": 25
 },
 {
  "id": "tailscale",
  "name": "Tailscale",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Tailscale is like a magic tunnel that connects all your devices and servers into one private network, no matter where they are in the world.",
  "description": "Zero-config VPN built on WireGuard with a control plane. Devices join a mesh network automatically via SSO, enabling secure access to private services without firewall rules.",
  "use_cases": [
   "Secure remote access to internal services",
   "Zero-config VPN for distributed teams",
   "Site-to-site networking without VPN appliances",
   "Kubernetes cluster access via Tailscale operator"
  ],
  "used_by": [
   "GitHub",
   "Mullvad",
   "Notion",
   "ReadMe"
  ],
  "docs": "https://tailscale.com/kb/",
  "tutorial": "https://tailscale.com/kb/1017/install/",
  "added": "2026-06-14",
  "dex": 26
 },
 {
  "id": "tailscale-funnel",
  "name": "Tailscale Funnel",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like having a secret tunnel that lets your friends visit your treehouse without knowing your home address.",
  "description": "Tailscale Funnel is a feature that allows you to share services on your private Tailscale network with the public internet without exposing your infrastructure or running a traditional reverse proxy. It provides encrypted, authenticated access with minimal configuration overhead.",
  "use_cases": [
   "Sharing development environments with stakeholders without VPN setup",
   "Exposing internal services to external partners or customers temporarily",
   "Creating public-facing webhooks from private backend services",
   "Demo access to internal tools without complex firewall rules"
  ],
  "used_by": [
   "Figma",
   "HashiCorp",
   "Vercel",
   "Stripe"
  ],
  "docs": "https://tailscale.com/kb/1312/funnel/",
  "tutorial": "https://tailscale.com/blog/tailscale-funnel/",
  "added": "2026-09-09",
  "dex": 27
 },
 {
  "id": "thrift-rpc-framework",
  "name": "Thrift RPC Framework",
  "category": "Framework",
  "domain": "Networking",
  "eli5": "It's like a translator that lets different programs written in different languages talk to each other really fast over a network.",
  "description": "Thrift is an RPC framework developed by Apache for building scalable cross-language services with efficient binary serialization. It's widely used in distributed systems for service-to-service communication and has proven critical in high-performance environments.",
  "use_cases": [
   "Building microservices that communicate across different programming languages",
   "High-performance inter-service communication in large-scale systems",
   "Efficient data serialization for distributed databases",
   "Real-time data streaming between backend services"
  ],
  "used_by": [
   "Meta",
   "Twitter",
   "Uber",
   "Airbnb"
  ],
  "docs": "https://thrift.apache.org/",
  "tutorial": "https://thrift.apache.org/tutorial/",
  "added": "2026-09-22",
  "dex": 28
 },
 {
  "id": "traefik",
  "name": "Traefik",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "Traefik is like a hotel concierge that automatically knows which room (service) every guest (request) should go to, even as rooms change.",
  "description": "Cloud-native reverse proxy and load balancer that auto-discovers service configuration from Docker, Kubernetes, and other orchestrators with zero manual config.",
  "use_cases": [
   "Kubernetes ingress controller",
   "Automatic TLS via Let's Encrypt",
   "Dynamic routing for microservices",
   "Docker Swarm and Nomad edge routing"
  ],
  "used_by": [
   "Containous",
   "BlaBlaCar",
   "Gitlab",
   "The New York Times"
  ],
  "docs": "https://doc.traefik.io/traefik/",
  "tutorial": "https://doc.traefik.io/traefik/getting-started/quick-start/",
  "added": "2026-06-11",
  "dex": 29
 },
 {
  "id": "veilid-decentralized-network",
  "name": "Veilid",
  "category": "Platform",
  "domain": "Networking",
  "eli5": "Veilid is like a secret underground postal system where messages are passed through many untraceable hands, so no one can see who's sending what to whom.",
  "description": "Veilid is a decentralized network and privacy-preserving platform that enables peer-to-peer communication with built-in routing and privacy guarantees. It provides infrastructure for applications requiring strong privacy, content distribution, and resilient networking without central authority.",
  "use_cases": [
   "Private decentralized messaging and communication applications",
   "Content distribution networks resistant to censorship",
   "Peer-to-peer file sharing with privacy protections",
   "Building resilient applications for unstable network conditions"
  ],
  "used_by": [
   "Cult of the Dead Cow",
   "Privacy-focused startups",
   "Open-source communities",
   "Decentralized application developers"
  ],
  "docs": "https://veilid.com/docs",
  "tutorial": "https://veilid.com/guide",
  "added": "2026-09-23",
  "dex": 30
 },
 {
  "id": "wireshark",
  "name": "Wireshark",
  "category": "Tool",
  "domain": "Networking",
  "eli5": "It's like a spy camera for internet traffic that shows you exactly what data is traveling between your computer and others.",
  "description": "Wireshark is a network protocol analyzer that captures and displays packet data in real-time, allowing engineers to debug network issues, analyze security threats, and understand application communication. It's the industry standard for network troubleshooting and packet inspection.",
  "use_cases": [
   "Debugging network connectivity and protocol issues",
   "Analyzing suspicious network traffic for security threats",
   "Capturing and inspecting API calls between services",
   "Performance analysis and latency troubleshooting"
  ],
  "used_by": [
   "Google",
   "Microsoft",
   "Cisco",
   "Red Hat"
  ],
  "docs": "https://www.wireshark.org/docs/",
  "tutorial": "https://www.wireshark.org/download/docs/Wireshark_User's_Guide.pdf",
  "added": "2026-07-21",
  "dex": 31
 },
 {
  "id": "ansible",
  "name": "Ansible",
  "category": "Tool",
  "domain": "DevOps",
  "description": "Agentless IT automation tool by Red Hat — uses SSH and YAML playbooks to configure servers, deploy apps, and orchestrate multi-tier workflows. No agent to install on managed nodes; idempotent by design.",
  "use_cases": [
   "Server configuration management and hardening at scale",
   "Application deployment and rolling updates across server fleets",
   "Provisioning cloud VMs and configuring them in one playbook",
   "Automated compliance checks and remediation",
   "Network device configuration (routers, switches via Ansible network modules)"
  ],
  "used_by": [
   "NASA",
   "Atlassian",
   "Hootsuite",
   "Eaton",
   "WeWork"
  ],
  "docs": "https://docs.ansible.com/",
  "tutorial": "https://docs.ansible.com/ansible/latest/getting_started/index.html",
  "added": "2026-06-08",
  "eli5": "A recipe book that teaches computers how to set themselves up.",
  "dex": 32
 },
 {
  "id": "apptainer",
  "name": "Apptainer",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Apptainer is like a portable lunchbox for your scientific programs that keeps all the ingredients together so they work the same way on any computer.",
  "description": "Apptainer (formerly Singularity) is a container runtime designed for high-performance computing and scientific workloads, enabling secure and portable execution of applications across diverse HPC environments. It's widely used in research institutions and national labs for reproducible computational science.",
  "use_cases": [
   "Running scientific simulations across different HPC clusters",
   "Packaging bioinformatics pipelines for reproducible research",
   "Distributing complex multi-dependency applications to supercomputers",
   "Creating portable computational workflows for academic research"
  ],
  "used_by": [
   "NERSC",
   "Sandia National Labs",
   "CERN",
   "Purdue University"
  ],
  "docs": "https://apptainer.org/docs/",
  "tutorial": "https://apptainer.org/docs/user/latest/quick-start.html",
  "added": "2026-08-01",
  "dex": 33
 },
 {
  "id": "atmos-orchestration",
  "name": "Atmos (Terraform Orchestration)",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a conductor leading an orchestra of machines—telling each part exactly when and how to play in perfect harmony.",
  "description": "Atmos is an open-source framework for managing, orchestrating, and scaling Terraform and Helm deployments across multiple cloud environments and accounts. It provides structure, reusability, and consistency for Infrastructure-as-Code at enterprise scale.",
  "use_cases": [
   "Multi-environment and multi-account Terraform management",
   "Infrastructure reusability across projects and teams",
   "Complex dependency orchestration between infrastructure components",
   "Centralized configuration management and validation for IaC"
  ],
  "used_by": [
   "CloudPosse",
   "Betterment",
   "Datadog",
   "Zappi"
  ],
  "docs": "https://atmos.tools/",
  "tutorial": "https://atmos.tools/quick-start/",
  "added": "2026-08-23",
  "dex": 34
 },
 {
  "id": "backstage",
  "name": "Backstage",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "A company's internal website where engineers find all their tools, docs, and services in one place.",
  "description": "Open-source Internal Developer Platform (IDP) by Spotify. Unifies services, docs, tooling, and infrastructure into a single developer portal. Software catalog, TechDocs, and scaffolding templates reduce cognitive load and enable self-service. CNCF incubating project.",
  "use_cases": [
   "Software catalog: single source of truth for all services, APIs, and teams",
   "TechDocs: auto-generated, git-backed documentation colocated with code",
   "Scaffolding templates: engineers create new services from golden-path templates",
   "Infrastructure self-service: provision databases, queues, and resources via UI",
   "Plugin ecosystem: 200+ plugins for GitHub, PagerDuty, Datadog, Kubernetes"
  ],
  "used_by": [
   "Spotify",
   "American Airlines",
   "Box",
   "Expedia",
   "LinkedIn",
   "Zalando"
  ],
  "docs": "https://backstage.io/docs/overview/what-is-backstage",
  "tutorial": "https://backstage.io/docs/getting-started/",
  "added": "2026-06-08",
  "dex": 35
 },
 {
  "id": "bash",
  "name": "Bash",
  "category": "Language",
  "domain": "DevOps",
  "eli5": "A way to give your computer a to-do list and have it do every chore in order.",
  "description": "Bourne Again Shell — the default command interpreter on Linux and macOS. Scripts automate sequences of system commands. Ubiquitous in CI/CD pipelines, server provisioning, and DevOps tooling. Not pretty, but runs everywhere UNIX does.",
  "use_cases": [
   "CI/CD pipeline steps: build, test, package, deploy scripts",
   "Server bootstrapping and configuration automation",
   "Log parsing, file manipulation, and text processing with grep/awk/sed",
   "Cron job automation for scheduled maintenance tasks",
   "Docker entrypoint scripts and Kubernetes init containers"
  ],
  "used_by": [
   "Every Linux/macOS-based engineering team"
  ],
  "docs": "https://www.gnu.org/software/bash/manual/bash.html",
  "tutorial": "https://linuxcommand.org/lc3_learning_the_shell.php",
  "added": "2026-06-08",
  "dex": 36
 },
 {
  "id": "brev",
  "name": "Brev",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a magic button that sets up your entire development environment in the cloud instead of on your computer, so you can work from anywhere instantly.",
  "description": "Brev is a cloud development environment platform that allows developers to spin up pre-configured workspaces in seconds. It eliminates local machine setup time and enables developers to write code from anywhere with consistent development environments.",
  "use_cases": [
   "Instant developer onboarding without local setup",
   "Contributing to projects on any device or browser",
   "Collaborative development in ephemeral environments",
   "Standardizing development environments across teams"
  ],
  "used_by": [
   "Y Combinator",
   "Replit",
   "Stripe",
   "Notion"
  ],
  "docs": "https://docs.brev.dev/",
  "tutorial": "https://www.brev.dev/",
  "added": "2026-07-02",
  "dex": 37
 },
 {
  "id": "bytebase-database",
  "name": "Bytebase",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a version control system for your database, so you can track changes, review updates, and safely update your data like you do with code.",
  "description": "Bytebase is a database DevOps platform that brings GitOps principles to database management with schema versioning, change approval workflows, and multi-environment deployment. It integrates with CI/CD pipelines to automate and govern database changes.",
  "use_cases": [
   "Database schema version control and migration management",
   "Change approval workflows for database modifications",
   "Multi-environment database synchronization",
   "Compliance and audit trails for database changes"
  ],
  "used_by": [
   "Databricks",
   "HashiCorp",
   "Enterprise development teams"
  ],
  "docs": "https://www.bytebase.com/docs",
  "tutorial": "https://www.bytebase.com/docs/get-started",
  "added": "2026-06-23",
  "dex": 38
 },
 {
  "id": "chef",
  "name": "Chef",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A recipe book for servers — write instructions once and Chef cooks every server the same way.",
  "description": "Configuration management platform using Ruby-based DSL 'recipes' and 'cookbooks'. Agent-based: Chef client runs on each node and pulls config from Chef Server. Strong compliance and audit capabilities via InSpec. Acquired by Progress Software.",
  "use_cases": [
   "Server configuration management at scale with idempotent recipes",
   "Compliance as Code with InSpec: define and audit security policies",
   "Application deployment and lifecycle management on VM fleets",
   "Multi-platform config: Linux, Windows, AIX via same cookbook",
   "Chef Habitat for application packaging independent of infrastructure"
  ],
  "used_by": [
   "Facebook",
   "Nordstrom",
   "Etsy",
   "Bloomberg",
   "General Electric"
  ],
  "docs": "https://docs.chef.io/",
  "tutorial": "https://docs.chef.io/chef_overview/",
  "added": "2026-06-08",
  "dex": 39
 },
 {
  "id": "colcon",
  "name": "Colcon",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a smart organizer that builds all your LEGO projects in the right order so they don't break.",
  "description": "Colcon is a command line tool for building, testing, and publishing multiple software packages in a specified order. It's widely used in robotics and ROS 2 ecosystems to manage complex multi-package projects with dependency resolution.",
  "use_cases": [
   "Building ROS 2 robotics projects with multiple interdependent packages",
   "Managing large monorepos with complex build dependencies",
   "Running parallel builds and tests across packages",
   "Automating CI/CD pipelines for distributed software systems"
  ],
  "used_by": [
   "Boston Dynamics",
   "ABB Robotics",
   "Open Robotics",
   "Clearpath Robotics"
  ],
  "docs": "https://colcon.readthedocs.io/",
  "tutorial": "https://docs.ros.org/en/humble/Tutorials/Beginner-Client-Libraries/Colcon-Tutorial.html",
  "added": "2026-07-28",
  "dex": 40
 },
 {
  "id": "colima",
  "name": "Colima",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like Docker Desktop but lighter and faster, like choosing a scooter instead of a car to get around your neighborhood.",
  "description": "Colima is a container runtime for macOS and Linux that provides Docker and Kubernetes functionality without the overhead of Docker Desktop. It uses Lima to manage lightweight VMs and is optimized for developer workflows on resource-constrained machines.",
  "use_cases": [
   "Local development on MacBooks with limited resources",
   "Running Kubernetes clusters locally without Docker Desktop licensing",
   "Testing containerized applications in Linux environments on macOS",
   "CI/CD pipelines in resource-constrained environments"
  ],
  "used_by": [
   "Individual developers",
   "Startups",
   "Open source projects"
  ],
  "docs": "https://github.com/abiosoft/colima",
  "tutorial": "https://github.com/abiosoft/colima#quick-start",
  "added": "2026-06-25",
  "dex": 41
 },
 {
  "id": "consul-template",
  "name": "Consul Template",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a mail-merge program that automatically fills in a template with information that changes, so your configuration files always stay up-to-date.",
  "description": "Consul Template is a lightweight tool that uses Consul service registry data to dynamically render configuration files and restart services. It enables zero-downtime deployments and automatic service discovery integration without requiring application code changes.",
  "use_cases": [
   "Dynamic load balancer configuration management",
   "Certificate rotation and renewal automation",
   "Service discovery-driven application configuration",
   "Multi-datacenter deployment orchestration"
  ],
  "used_by": [
   "HashiCorp",
   "AWS",
   "Netflix",
   "Uber"
  ],
  "docs": "https://github.com/hashicorp/consul-template",
  "tutorial": "https://learn.hashicorp.com/tutorials/consul/consul-template",
  "added": "2026-07-07",
  "dex": 42
 },
 {
  "id": "copier-project-generator",
  "name": "Copier",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a template maker that creates a whole project structure for you with one command, so you don't have to build everything from scratch.",
  "description": "A robust project template generator and updater that allows teams to create, maintain, and distribute standardized project structures. It uses Jinja2 templating and can be updated dynamically as templates evolve.",
  "use_cases": [
   "Standardizing project structures across teams and organizations",
   "Onboarding new microservices with consistent configurations",
   "Maintaining cookiecutter templates with automatic updates",
   "Enforcing organizational coding standards and best practices"
  ],
  "used_by": [
   "Plone Foundation",
   "PyMongo",
   "FastAPI contributors",
   "Open source projects"
  ],
  "docs": "https://copier.readthedocs.io/",
  "tutorial": "https://copier.readthedocs.io/en/latest/tutorials/",
  "added": "2026-08-08",
  "dex": 43
 },
 {
  "id": "crossplane",
  "name": "Crossplane",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Terraform but living inside Kubernetes — declare cloud resources as YAML and the cluster manages them.",
  "description": "CNCF project that extends Kubernetes to manage cloud infrastructure. Define AWS, GCP, Azure resources as Kubernetes CRDs — the Crossplane controller reconciles real cloud state. Enables platform teams to build self-service internal developer platforms on top of Kubernetes.",
  "use_cases": [
   "Cloud infrastructure as Kubernetes resources (RDS, S3, GKE clusters as CRDs)",
   "Internal developer platforms: devs request databases via kubectl, platform team controls what's allowed",
   "Composition: bundle multiple cloud resources into a single reusable abstraction",
   "GitOps-native IaC — ArgoCD/Flux manages infrastructure the same way as apps",
   "Multi-cloud resource management from a single Kubernetes control plane"
  ],
  "used_by": [
   "Upbound",
   "Deutsche Telekom",
   "Grupo Boticário",
   "Nabla"
  ],
  "docs": "https://docs.crossplane.io/",
  "tutorial": "https://docs.crossplane.io/latest/getting-started/",
  "added": "2026-06-08",
  "dex": 44
 },
 {
  "id": "dagger",
  "name": "Dagger",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Dagger is like Docker Compose for your CI/CD pipelines — you write your build steps in code, and they run the same way on your laptop and in any CI system.",
  "description": "Portable CI/CD engine where pipelines are written as code (Go, Python, TypeScript). Runs locally and in any CI system via containers, eliminating YAML and environment drift.",
  "use_cases": [
   "Portable pipelines that run identically on CI and locally",
   "Eliminating YAML-based CI configuration",
   "Reusable build/test/deploy modules across projects",
   "Programmatic pipeline composition with real language features"
  ],
  "used_by": [
   "Grafana Labs",
   "Gitlab",
   "Shopify",
   "GitHub"
  ],
  "docs": "https://docs.dagger.io/",
  "tutorial": "https://docs.dagger.io/quickstart",
  "added": "2026-06-14",
  "dex": 45
 },
 {
  "id": "docker",
  "name": "Docker",
  "category": "Tool",
  "domain": "DevOps",
  "description": "Platform for building, shipping, and running applications in containers. Packages code and dependencies into a portable image that runs identically anywhere. Industry-standard container runtime and image format (OCI).",
  "use_cases": [
   "Packaging applications into reproducible, portable container images",
   "Local development environments with Docker Compose (multi-service stacks)",
   "Building images in CI pipelines for deployment to Kubernetes/ECS",
   "Running isolated microservices with controlled resource limits",
   "Multi-stage builds to produce minimal production images"
  ],
  "used_by": [
   "Spotify",
   "PayPal",
   "ADP",
   "ING",
   "eBay"
  ],
  "docs": "https://docs.docker.com/",
  "tutorial": "https://docs.docker.com/get-started/",
  "added": "2026-06-08",
  "eli5": "A lunchbox that packs your app with everything it needs to run anywhere.",
  "dex": 46
 },
 {
  "id": "docker-desktop",
  "name": "Docker Desktop",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "An app that lets your Mac or Windows computer run containers and a mini Kubernetes cluster with a simple point-and-click interface.",
  "description": "GUI application for Mac and Windows that bundles Docker Engine, Docker Compose, Kubernetes, and Dev Environments. Manages a Linux VM under the hood and integrates with Docker Hub, Extensions Marketplace, and WSL 2 on Windows.",
  "use_cases": [
   "Run Docker containers and Compose stacks on macOS and Windows without manual VM setup",
   "Enable a local single-node Kubernetes cluster with one toggle",
   "Build and push multi-arch images (amd64/arm64) from a single workstation",
   "Develop inside containers with Dev Environments for reproducible onboarding",
   "Extend functionality via Docker Extensions (Lens, Portainer, Snyk scanning)"
  ],
  "used_by": [
   "Individual developers",
   "Startup engineering teams",
   "Enterprise developer desktops"
  ],
  "docs": "https://docs.docker.com/desktop/",
  "tutorial": "https://docs.docker.com/get-started/",
  "added": "2026-06-09",
  "dex": 47
 },
 {
  "id": "driftctl",
  "name": "driftctl Infrastructure Drift Detection",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a security camera that watches your cloud infrastructure and alerts you if someone manually changed something that was supposed to be automated.",
  "description": "driftctl is an open-source CLI tool that detects and tracks infrastructure drift by comparing the actual state of cloud resources against their Infrastructure-as-Code (IaC) definitions. It supports Terraform, Cloudformation, and other IaC tools across AWS, GCP, and Azure.",
  "use_cases": [
   "Preventing configuration drift in production environments",
   "Automating cloud governance and compliance checking",
   "Reconciling manual changes with Infrastructure-as-Code definitions",
   "Cost optimization by identifying unused resources created outside IaC"
  ],
  "used_by": [
   "Scaleway",
   "Cloud providers",
   "Enterprise DevOps teams"
  ],
  "docs": "https://docs.driftctl.com/",
  "tutorial": "https://docs.driftctl.com/intro/getting-started/",
  "added": "2026-09-20",
  "dex": 48
 },
 {
  "id": "firefly",
  "name": "Firefly",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Firefly is like a detective that watches your cloud to find things you're paying for but not using, so you can stop wasting money.",
  "description": "Firefly is a cloud asset management and cost optimization platform that automatically discovers, tracks, and manages cloud resources across AWS, Azure, and GCP. It identifies unused resources and provides remediation recommendations to reduce cloud spending.",
  "use_cases": [
   "Cloud cost optimization and waste reduction",
   "Automated discovery of cloud infrastructure",
   "Compliance and security posture management",
   "FinOps and chargeback automation"
  ],
  "used_by": [
   "Cloudflare",
   "Notion",
   "SailPoint"
  ],
  "docs": "https://docs.firefly.ai/",
  "tutorial": "https://docs.firefly.ai/getting-started",
  "added": "2026-07-05",
  "dex": 49
 },
 {
  "id": "harbor",
  "name": "Harbor",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Your own private Docker Hub that you control — stores your container images safely on your servers.",
  "description": "Open-source cloud-native container registry by VMware. Extends Docker Registry with security scanning (Trivy/Clair), role-based access, image signing (Notary/Cosign), replication, and a web UI. CNCF graduated project.",
  "use_cases": [
   "Self-hosted private container registry with RBAC and audit logs",
   "Automatic vulnerability scanning of pushed images with Trivy",
   "Image signing and verification (Cosign) for supply chain security",
   "Cross-datacenter registry replication for multi-region deployments",
   "Proxy cache for Docker Hub / GCR to avoid rate limits and reduce egress"
  ],
  "used_by": [
   "VMware",
   "Alibaba Cloud",
   "JD.com",
   "Tencent",
   "Bytedance"
  ],
  "docs": "https://goharbor.io/docs/",
  "tutorial": "https://goharbor.io/docs/latest/install-config/",
  "added": "2026-06-08",
  "dex": 50
 },
 {
  "id": "helm",
  "name": "Helm",
  "category": "Tool",
  "domain": "DevOps",
  "description": "The package manager for Kubernetes. Bundles Kubernetes manifests into versioned, templatable charts. Manages installs, upgrades, and rollbacks of complex applications as a single unit. CNCF graduated project.",
  "use_cases": [
   "Packaging and distributing Kubernetes applications as reusable charts",
   "Templating manifests with values files for dev/staging/prod environments",
   "One-command install of complex stacks (Prometheus, Grafana, Cert-Manager)",
   "Atomic upgrades and rollbacks with helm upgrade / helm rollback",
   "Helm OCI: store and distribute charts in container registries"
  ],
  "used_by": [
   "Microsoft",
   "Bitnami",
   "DataDog",
   "Elastic",
   "HashiCorp"
  ],
  "docs": "https://helm.sh/docs/",
  "tutorial": "https://helm.sh/docs/intro/quickstart/",
  "added": "2026-06-08",
  "eli5": "An app store for Kubernetes — install big software with one command.",
  "dex": 51
 },
 {
  "id": "k3s",
  "name": "K3s",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "A very small version of Kubernetes that fits on a tiny computer like a Raspberry Pi and still does almost everything the big version can.",
  "description": "Lightweight, CNCF-certified Kubernetes distribution by Rancher/SUSE. Single binary under 100 MB, replaces etcd with SQLite by default, and strips cloud-provider APIs — purpose-built for edge, IoT, ARM, and resource-constrained environments.",
  "use_cases": [
   "Deploy Kubernetes on Raspberry Pi and ARM edge devices",
   "Run production workloads in resource-constrained or air-gapped environments",
   "Fast cluster bootstrap for CI pipelines (cluster up in under 30 seconds)",
   "Single-node or small HA clusters for branch offices and retail",
   "Foundation for Fleet and Rancher edge management"
  ],
  "used_by": [
   "Rancher/SUSE",
   "Walmart",
   "Siemens",
   "Edge and IoT teams globally"
  ],
  "docs": "https://docs.k3s.io/",
  "tutorial": "https://docs.k3s.io/quick-start",
  "added": "2026-06-09",
  "dex": 52
 },
 {
  "id": "k9s",
  "name": "k9s",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A cockpit dashboard for Kubernetes in your terminal — see everything happening in your cluster at a glance.",
  "description": "Terminal-based Kubernetes UI that provides a real-time dashboard of cluster resources. Navigate pods, deployments, services, logs, and exec into containers — all with keyboard shortcuts. Dramatically faster than running kubectl commands manually.",
  "use_cases": [
   "Real-time pod status, logs, and events without repetitive kubectl commands",
   "Exec into containers directly from the resource list",
   "Quickly delete, describe, edit, and port-forward resources",
   "Filter resources by namespace, label, or name with fuzzy search",
   "Custom aliases and hotkeys for team-specific cluster workflows"
  ],
  "used_by": [
   "Kubernetes operators everywhere — one of the most popular kubectl alternatives"
  ],
  "docs": "https://k9scli.io/topics/install/",
  "tutorial": "https://k9scli.io/",
  "added": "2026-06-08",
  "dex": 53
 },
 {
  "id": "kind",
  "name": "kind",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A way to run a fake Kubernetes cluster entirely inside Docker containers on your computer — like nesting boxes inside boxes.",
  "description": "Kubernetes IN Docker — spins up multi-node Kubernetes clusters using Docker containers as nodes. Purpose-built for testing Kubernetes itself and for fast, reproducible CI environments without VMs.",
  "use_cases": [
   "Run Kubernetes conformance tests in CI without cloud infrastructure",
   "Test multi-node cluster behavior locally with Docker",
   "Validate Kubernetes operators and admission webhooks",
   "Bootstrap ephemeral clusters in GitHub Actions or GitLab CI",
   "Develop and test Cluster API providers"
  ],
  "used_by": [
   "Kubernetes SIG Testing",
   "Cluster API teams",
   "Platform engineers"
  ],
  "docs": "https://kind.sigs.k8s.io/",
  "tutorial": "https://kind.sigs.k8s.io/docs/user/quick-start/",
  "added": "2026-06-09",
  "dex": 54
 },
 {
  "id": "kops",
  "name": "kops",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Like Terraform but specifically for creating full Kubernetes clusters on cloud providers — it sets up all the servers, networking, and Kubernetes pieces for you.",
  "description": "Kubernetes Operations — CLI tool for provisioning, upgrading, and managing production-grade Kubernetes clusters on cloud infrastructure (AWS primary, GCP/Azure in beta). Manages cloud resources (VPCs, ASGs, IAM) alongside the cluster lifecycle.",
  "use_cases": [
   "Provision HA Kubernetes clusters on AWS with full control over node groups and networking",
   "Declarative cluster configuration in YAML with git-ops-friendly state in S3",
   "Rolling cluster upgrades with zero-downtime node replacement",
   "Manage instance groups and node pools for mixed spot/on-demand fleets",
   "Validate cluster config and dry-run changes before applying to production"
  ],
  "used_by": [
   "AWS-heavy Kubernetes teams",
   "Startups self-hosting on AWS",
   "Platform engineers avoiding EKS overhead"
  ],
  "docs": "https://kops.sigs.k8s.io/",
  "tutorial": "https://kops.sigs.k8s.io/getting_started/aws/",
  "added": "2026-06-09",
  "dex": 55
 },
 {
  "id": "kubeadm",
  "name": "kubeadm",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A setup wizard that takes a bunch of blank computers and turns them into a working Kubernetes cluster with one command.",
  "description": "Official Kubernetes cluster bootstrapping tool. Handles control plane init, worker node join, certificate generation, and kubeconfig setup. The standard way to stand up a conformant cluster on bare metal or VMs.",
  "use_cases": [
   "Bootstrap a production-grade Kubernetes control plane on bare metal or VMs",
   "Join worker nodes to an existing cluster with a single join command",
   "Upgrade Kubernetes clusters in place with kubeadm upgrade",
   "Generate and rotate cluster certificates",
   "Serve as the foundation layer for higher-level tools like Kubespray and Cluster API"
  ],
  "used_by": [
   "Red Hat",
   "Canonical",
   "VMware",
   "Self-hosted Kubernetes operators"
  ],
  "docs": "https://kubernetes.io/docs/reference/setup-tools/kubeadm/",
  "tutorial": "https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/create-cluster-kubeadm/",
  "added": "2026-06-09",
  "dex": 56
 },
 {
  "id": "kubeops",
  "name": "KubeOps",
  "category": "Framework",
  "domain": "DevOps",
  "eli5": "A toolkit for .NET developers that lets them write Kubernetes operators — programs that automatically manage other programs running in a cluster — using C# instead of Go.",
  "description": "Kubernetes operator SDK for .NET/C#. Provides code-generation, reconciliation loops, CRD scaffolding, and webhook support so .NET teams can build production operators without leaving the C# ecosystem.",
  "use_cases": [
   "Build Kubernetes operators and controllers in C# for .NET-native teams",
   "Scaffold and register Custom Resource Definitions (CRDs) from C# classes",
   "Implement reconciliation loops with built-in retry and error handling",
   "Define validating and mutating admission webhooks in C#",
   "Automate lifecycle management of complex stateful .NET workloads on Kubernetes"
  ],
  "used_by": [
   "Microsoft ecosystem teams",
   ".NET Kubernetes adopters"
  ],
  "docs": "https://buehler.github.io/dotnet-operator-sdk/",
  "tutorial": "https://buehler.github.io/dotnet-operator-sdk/docs/getting-started",
  "added": "2026-06-09",
  "dex": 57
 },
 {
  "id": "kubernetes",
  "name": "Kubernetes",
  "category": "Platform",
  "domain": "DevOps",
  "description": "Open-source container orchestration system for automating deployment, scaling, and management of containerized applications. Originally designed by Google, now CNCF's flagship project. De facto standard for production container workloads.",
  "use_cases": [
   "Declarative workload management: Deployments, StatefulSets, DaemonSets",
   "Horizontal pod autoscaling based on CPU, memory, or custom metrics",
   "Service discovery and internal load balancing via kube-proxy/Services",
   "Secrets and ConfigMap management for app configuration",
   "Rolling updates and rollbacks with zero-downtime deployments"
  ],
  "used_by": [
   "Google",
   "Spotify",
   "Shopify",
   "Airbnb",
   "Goldman Sachs",
   "CERN"
  ],
  "docs": "https://kubernetes.io/docs/home/",
  "tutorial": "https://kubernetes.io/docs/tutorials/kubernetes-basics/",
  "added": "2026-06-08",
  "eli5": "A manager that keeps all your app boxes running and restarts them if they crash.",
  "dex": 58
 },
 {
  "id": "kubespray",
  "name": "Kubespray",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "An Ansible recipe book that automatically installs and configures Kubernetes across a whole fleet of servers for you.",
  "description": "Ansible-based Kubernetes cluster provisioner maintained under the Kubernetes SIGs umbrella. Supports bare metal, cloud VMs, and on-prem with pluggable CNI, CRI, and ingress options. Wraps kubeadm for production-ready multi-node deployments.",
  "use_cases": [
   "Provision multi-node Kubernetes clusters on bare metal or any cloud provider",
   "Declaratively configure CNI plugins (Calico, Flannel, Cilium) and container runtimes",
   "Rolling cluster upgrades via idempotent Ansible playbooks",
   "Air-gapped cluster installs for regulated environments",
   "Day-2 operations: add nodes, rotate certs, drain and decommission"
  ],
  "used_by": [
   "CERN",
   "Cloudflare",
   "Deutsche Telekom",
   "On-prem Kubernetes teams"
  ],
  "docs": "https://kubespray.io/",
  "tutorial": "https://github.com/kubernetes-sigs/kubespray/blob/master/docs/getting_started/setting-up-your-first-cluster.md",
  "added": "2026-06-09",
  "dex": 59
 },
 {
  "id": "kustomize",
  "name": "Kustomize",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A way to take the same Kubernetes YAML and patch it differently for dev, staging, and prod without copy-pasting.",
  "description": "Kubernetes-native configuration management tool built into `kubectl`. Uses overlays and patches to customize base YAML manifests for different environments — no templating language, just plain YAML. Part of kubectl since v1.14.",
  "use_cases": [
   "Environment overlays: same base manifests, different replicas/images per env",
   "Patching third-party manifests without forking (strategic merge patches)",
   "Generating ConfigMaps and Secrets from files or literals",
   "Name prefixing/suffixing and label injection across all resources",
   "ArgoCD and Flux integration as native rendering engine"
  ],
  "used_by": [
   "Kubernetes community",
   "ArgoCD users",
   "Flux users",
   "Google",
   "Red Hat"
  ],
  "docs": "https://kubectl.docs.kubernetes.io/references/kustomize/",
  "tutorial": "https://kubectl.docs.kubernetes.io/guides/introduction/kustomize/",
  "added": "2026-06-08",
  "dex": 60
 },
 {
  "id": "lefthook",
  "name": "Lefthook",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Lefthook is like a bouncer at a club door who checks if your code is ready before you submit it to the team.",
  "description": "Lefthook is a fast and language-agnostic Git hooks manager that runs tasks like linting and testing before commits are pushed. It's a modern replacement for husky with better performance and configuration flexibility.",
  "use_cases": [
   "Enforcing code quality standards before commits",
   "Running automated tests on staged files",
   "Preventing commits with security vulnerabilities",
   "Multi-language project hook management"
  ],
  "used_by": [
   "Shopify",
   "Netflix",
   "Figma"
  ],
  "docs": "https://github.com/evilmartians/lefthook",
  "tutorial": "https://evilmartians.com/chronicles/lefthook-new-generation-git-hooks",
  "added": "2026-08-30",
  "dex": 61
 },
 {
  "id": "lima",
  "name": "Lima",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like having a virtual Linux computer running on your Mac that works just like the real thing, making your development feel natural.",
  "description": "Lima launches Linux virtual machines with automatic file sharing and port forwarding, providing seamless Linux development environments on macOS and other platforms. It replaces Docker Desktop and Colima for many use cases with better performance and native integration.",
  "use_cases": [
   "Running Linux workloads natively on macOS",
   "Development environment consistency across teams",
   "Alternative to Docker Desktop for resource efficiency",
   "Kubernetes cluster development and testing"
  ],
  "used_by": [
   "Rancher",
   "GitLab",
   "CNCF projects",
   "Enterprise development teams"
  ],
  "docs": "https://github.com/lima-vm/lima",
  "tutorial": "https://github.com/lima-vm/lima#quick-start",
  "added": "2026-07-19",
  "dex": 62
 },
 {
  "id": "make",
  "name": "Make",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A shortcut sheet where you write long commands once and run them with a short nickname forever.",
  "description": "Classic Unix build automation tool using Makefiles. Defines targets and their dependencies — only rebuilds what changed. Ubiquitous as a project task runner even when not compiling C: `make test`, `make deploy`, `make docker-build` are common patterns across all languages.",
  "use_cases": [
   "Project task runner: standardize `make test`, `make build`, `make deploy` across repos",
   "Incremental builds: only recompile changed C/C++/Go files via dependency tracking",
   "CI/CD entrypoint: Makefile as the canonical interface between humans and pipelines",
   "Documentation generation, linting, and formatting as named targets",
   "Multi-step workflows with dependency ordering (target B requires target A)"
  ],
  "used_by": [
   "Linux kernel",
   "GNU project",
   "virtually every C/C++ project",
   "many DevOps teams"
  ],
  "docs": "https://www.gnu.org/software/make/manual/make.html",
  "tutorial": "https://makefiletutorial.com/",
  "added": "2026-06-08",
  "dex": 63
 },
 {
  "id": "meson-build",
  "name": "Meson Build System",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Meson is like a recipe card that tells your computer exactly what ingredients (code files) to mix together and in what order to bake a working program.",
  "description": "Meson is a modern build system written in Python that compiles code quickly and efficiently with native support for C, C++, Rust, and other languages. It emphasizes speed, simplicity, and correctness with clear error messages and dependency management.",
  "use_cases": [
   "Building large C/C++ projects with fast compilation times",
   "Cross-platform compilation for embedded systems and Linux distributions",
   "Dependency management and integration testing",
   "GNOME and system software development"
  ],
  "used_by": [
   "GNOME project",
   "Mesa graphics library",
   "systemd",
   "GStreamer"
  ],
  "docs": "https://mesonbuild.com/",
  "tutorial": "https://mesonbuild.com/Tutorial.html",
  "added": "2026-08-19",
  "dex": 64
 },
 {
  "id": "microk8s",
  "name": "MicroK8s",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "A small, self-contained Kubernetes that installs in one command and works like a full cluster — good for edge devices and laptops.",
  "description": "Lightweight, single-package Kubernetes distribution by Canonical. Delivered as a snap with built-in add-ons (Istio, Knative, GPU support, registry). Designed for edge, IoT, CI, and developer workstations with zero-ops cluster management.",
  "use_cases": [
   "Run production-grade Kubernetes on edge and IoT hardware",
   "Developer workstation cluster with snap-based lifecycle management",
   "Enable add-ons (Ingress, MetalLB, GPU, registry) with a single command",
   "Cluster multiple MicroK8s nodes for HA edge deployments",
   "CI/CD pipelines on Ubuntu runners without full cluster overhead"
  ],
  "used_by": [
   "Canonical",
   "Edge computing teams",
   "Ubuntu-based infrastructure operators"
  ],
  "docs": "https://microk8s.io/docs",
  "tutorial": "https://microk8s.io/docs/getting-started",
  "added": "2026-06-09",
  "dex": 65
 },
 {
  "id": "minikube",
  "name": "Minikube",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A tiny pretend Kubernetes cluster that runs on your laptop so you can practice without needing real servers.",
  "description": "Local Kubernetes cluster tool that spins up a single-node cluster inside a VM or container on your workstation. The canonical way for developers to run and test Kubernetes manifests locally before deploying to production.",
  "use_cases": [
   "Local development and testing of Kubernetes workloads",
   "Learning Kubernetes without cloud costs",
   "CI pipeline validation of manifests on ephemeral runners",
   "Testing Helm charts and operators locally",
   "Rapid iteration on containerized microservices"
  ],
  "used_by": [
   "Individual developers",
   "Platform engineering teams",
   "Kubernetes learners"
  ],
  "docs": "https://minikube.sigs.k8s.io/docs/",
  "tutorial": "https://minikube.sigs.k8s.io/docs/start/",
  "added": "2026-06-09",
  "dex": 66
 },
 {
  "id": "mise",
  "name": "Mise",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a magic box that automatically gives you the right version of each tool when you work on different projects, so you don't have to manually switch things.",
  "description": "Mise is a fast, polyglot runtime version manager that replaces asdf, nvm, and pyenv. It manages multiple language runtimes and tools with a single configuration file and automatic version switching per project.",
  "use_cases": [
   "Managing Node.js, Python, Ruby, and Go versions across projects",
   "Team development environment standardization",
   "CI/CD pipeline tool versioning",
   "Multi-language monorepo development"
  ],
  "used_by": [
   "Craft.do",
   "Various open-source projects",
   "Development teams"
  ],
  "docs": "https://mise.jdx.dev/",
  "tutorial": "https://mise.jdx.dev/getting-started.html",
  "added": "2026-06-30",
  "dex": 67
 },
 {
  "id": "mise-version-manager",
  "name": "mise (Tool Version Manager)",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a magic toolbox that automatically picks the right version of every tool you need based on what folder you're working in.",
  "description": "mise is a fast, simple tool version manager written in Rust that manages versions of programming languages, tools, and runtimes. It replaces tools like asdf, nvm, and pyenv with a unified, performant solution.",
  "use_cases": [
   "Managing multiple language versions (Python, Node.js, Go, Rust) across projects",
   "Enforcing consistent tool versions across development teams",
   "Automatic switching based on .mise.toml or .tool-versions files",
   "Reducing setup time for new developers onboarding to projects"
  ],
  "used_by": [
   "HashiCorp",
   "Vercel",
   "Linear",
   "Anduril Industries"
  ],
  "docs": "https://mise.jdx.dev/",
  "tutorial": "https://mise.jdx.dev/getting-started.html",
  "added": "2026-08-23",
  "dex": 68
 },
 {
  "id": "modyssey",
  "name": "Modyssey",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a smart map that shows you all the connections between your microservices and tells you when something breaks one of those connections.",
  "description": "Modyssey is a dependency mapping and API contract validation tool for microservices architectures that automatically discovers service relationships and detects breaking changes. It helps teams maintain governance and prevent integration failures across distributed systems.",
  "use_cases": [
   "Microservices dependency visualization and relationship mapping",
   "Detecting breaking API changes before they reach production",
   "Enforcing API contracts across teams",
   "Migration planning and refactoring complex service meshes"
  ],
  "used_by": [
   "Tech startups",
   "Microservices teams",
   "Enterprise API governance"
  ],
  "docs": "https://modyssey.io/docs",
  "tutorial": "https://modyssey.io/docs/quickstart",
  "added": "2026-07-25",
  "dex": 69
 },
 {
  "id": "nix",
  "name": "Nix",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a recipe book that guarantees your meal will taste exactly the same every time because it specifies every ingredient down to the exact version.",
  "description": "Nix is a declarative package manager and build system that enables reproducible, portable development environments and deployments across different machines. It uses a functional approach to ensure bit-for-bit reproducibility.",
  "use_cases": [
   "Creating reproducible development environments across teams",
   "Managing complex multi-language project dependencies",
   "Building immutable infrastructure and container images",
   "Enabling seamless collaboration with guaranteed environment consistency"
  ],
  "used_by": [
   "Target",
   "Tweag",
   "GitHub",
   "Shopify"
  ],
  "docs": "https://nixos.org/learn.html",
  "tutorial": "https://nix.dev/",
  "added": "2026-06-17",
  "dex": 70
 },
 {
  "id": "nix-package-manager",
  "name": "Nix Package Manager",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a recipe book that remembers exactly which ingredients and amounts you used to bake a cake, so you can always bake the same cake the same way.",
  "description": "Nix is a purely functional package manager and OS configuration tool that enables reproducible, declarative system configurations. It's increasingly adopted for development environments and infrastructure-as-code to eliminate 'works on my machine' problems.",
  "use_cases": [
   "Creating reproducible development environments",
   "Managing system configurations declaratively",
   "Building immutable deployment artifacts",
   "Cross-platform development environment consistency"
  ],
  "used_by": [
   "Tweag",
   "Replit",
   "Input Output Global",
   "Shopify"
  ],
  "docs": "https://nixos.org/guides/nix-pills/",
  "tutorial": "https://nix.dev/tutorials/first-steps",
  "added": "2026-06-21",
  "dex": 71
 },
 {
  "id": "earthly-container-build",
  "name": "Nixpack",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Nixpack is like a magic recipe that automatically figures out how to turn your code into a runnable container, without you having to write the instructions yourself.",
  "description": "Nixpack is a language-agnostic builder that automatically detects project structure and dependencies to create optimized OCI container images without requiring a Dockerfile. It's widely used in platforms like Railway and Render to simplify containerization.",
  "use_cases": [
   "Automatic Dockerfile generation for diverse tech stacks",
   "Zero-configuration deployment to container registries",
   "Building optimized multi-stage images without manual configuration",
   "CI/CD pipeline integration for rapid container builds"
  ],
  "used_by": [
   "Railway",
   "Render",
   "Fly.io",
   "Individual developers"
  ],
  "docs": "https://nixpacks.com/docs",
  "tutorial": "https://nixpacks.com/docs/getting-started",
  "added": "2026-09-23",
  "dex": 72
 },
 {
  "id": "nushell",
  "name": "Nushell",
  "category": "Language",
  "domain": "DevOps",
  "eli5": "It's a smarter way to talk to your computer that understands structured data like tables and lists, making it easier to find and organize things.",
  "description": "Nushell is a modern shell and scripting language that treats data as structured objects rather than plain text streams, making it more intuitive for system administration and automation. It combines the power of traditional shells with modern language features and better error handling.",
  "use_cases": [
   "System administration and automation with structured data",
   "Data processing pipelines without external tools like awk or sed",
   "Cross-platform scripting for Windows, Linux, and macOS",
   "Building maintainable DevOps automation workflows"
  ],
  "used_by": [
   "Rust community members",
   "Linux enthusiasts",
   "DevOps practitioners"
  ],
  "docs": "https://www.nushell.sh/book/",
  "tutorial": "https://www.nushell.sh/book/quick_tour.html",
  "added": "2026-08-12",
  "dex": 73
 },
 {
  "id": "opentofu",
  "name": "OpenTofu",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "OpenTofu is like Terraform's open-source twin — does all the same infrastructure tricks but is community-owned and free forever.",
  "description": "Open-source fork of Terraform under the Linux Foundation, maintained by the community after HashiCorp's BSL license change. Fully compatible with existing Terraform modules and providers.",
  "use_cases": [
   "Drop-in Terraform replacement under open license",
   "Infrastructure as code for AWS, GCP, Azure",
   "Managing Kubernetes, databases, and DNS",
   "Organizations avoiding BSL license restrictions"
  ],
  "used_by": [
   "Gruntwork",
   "Spacelift",
   "Scalr",
   "env0"
  ],
  "docs": "https://opentofu.org/docs/",
  "tutorial": "https://opentofu.org/docs/intro/",
  "added": "2026-06-11",
  "dex": 74
 },
 {
  "id": "oxide-helios",
  "name": "Oxide Helios",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "It's like having a smart power strip that manages all your computer's cables and makes sure everything is plugged in correctly and safely.",
  "description": "Oxide Helios is the control plane software that manages Oxide Computer's modular servers and provides unified infrastructure-as-code management across hardware and virtualization. It combines system firmware, hypervisor, and orchestration into a cohesive platform.",
  "use_cases": [
   "Building private cloud infrastructure with modern resource management",
   "Managing hybrid environments combining bare metal and virtualized workloads",
   "Providing infrastructure operators with unified control across hardware and software",
   "Running containerized and traditional workloads on the same physical platform"
  ],
  "used_by": [
   "Oxide Computer customers",
   "Fortune 500 enterprises",
   "Cloud infrastructure teams",
   "On-premises datacenters"
  ],
  "docs": "https://oxide.computer/docs/",
  "tutorial": "https://oxide.computer/blog/introducing-helios/",
  "added": "2026-09-09",
  "dex": 75
 },
 {
  "id": "packer",
  "name": "Packer",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A factory that bakes identical machine snapshots so every server starts from the same recipe.",
  "description": "HashiCorp tool for creating identical machine images (AMIs, Docker images, VMware templates) from a single config. Build once, deploy anywhere. Eliminates 'works on my machine' by baking all dependencies into the image before deployment.",
  "use_cases": [
   "Building AWS AMIs pre-baked with app dependencies for faster Auto Scaling Group launches",
   "Golden image pipelines: base OS → harden → install runtime → snapshot",
   "Multi-cloud image builds: same config produces AMI + GCP image + Azure VHD",
   "Immutable infrastructure: replace servers instead of patching in place",
   "CI/CD integration: Packer build triggered on dependency updates"
  ],
  "used_by": [
   "HashiCorp",
   "Netflix",
   "Airbnb",
   "Stripe",
   "Atlassian"
  ],
  "docs": "https://developer.hashicorp.com/packer/docs",
  "tutorial": "https://developer.hashicorp.com/packer/tutorials/aws-get-started",
  "added": "2026-06-08",
  "dex": 76
 },
 {
  "id": "par-parallel",
  "name": "Par (Parallel Execution)",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A helper that runs many tasks at the same time on your computer instead of one at a time, like cooking multiple dishes on different burners instead of using just one.",
  "description": "Par is a command-line tool for executing shell commands in parallel with dependency management, providing resource limits and organized output. It's used in build systems, data processing pipelines, and infrastructure automation to maximize throughput.",
  "use_cases": [
   "Parallel test execution across multiple workers",
   "Distributed build systems with task dependency graphs",
   "Batch processing with controlled concurrency limits",
   "Infrastructure provisioning with parallel resource creation"
  ],
  "used_by": [
   "Stripe",
   "HashiCorp",
   "CloudFlare",
   "Databricks"
  ],
  "docs": "https://github.com/google/par",
  "tutorial": "https://github.com/google/par/blob/master/README.md",
  "added": "2026-08-10",
  "dex": 77
 },
 {
  "id": "podman",
  "name": "Podman",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Docker but without a big daemon running as root — safer and works the same way.",
  "description": "Daemonless, rootless container engine by Red Hat — OCI-compatible Docker drop-in replacement. Runs containers without a privileged background daemon, improving security posture. Supports Docker Compose files via `podman-compose` and generates systemd units from containers.",
  "use_cases": [
   "Rootless container execution in security-sensitive or shared environments",
   "Drop-in Docker replacement: `alias docker=podman` works for most workflows",
   "Generating systemd unit files from containers for OS-native service management",
   "Pod concept (grouping containers) that mirrors Kubernetes Pod model locally",
   "CI environments where Docker daemon socket access is restricted"
  ],
  "used_by": [
   "Red Hat",
   "IBM",
   "CentOS/RHEL users",
   "OpenShift deployments"
  ],
  "docs": "https://docs.podman.io/en/latest/",
  "tutorial": "https://podman.io/getting-started/",
  "added": "2026-06-08",
  "dex": 78
 },
 {
  "id": "pulumi",
  "name": "Pulumi",
  "category": "Tool",
  "domain": "DevOps",
  "description": "Infrastructure as Code using general-purpose languages — Python, TypeScript, Go, C#, Java. Write cloud infrastructure with real loops, functions, and classes instead of HCL DSL. Same providers as Terraform via Pulumi's bridged provider ecosystem.",
  "use_cases": [
   "IaC in TypeScript/Python/Go for teams who find HCL limiting",
   "Dynamically generating resources with loops and conditionals in real code",
   "Component resources: reusable, testable infrastructure abstractions as classes",
   "Pulumi ESC for centralized secrets and environment configuration management",
   "Pulumi Cloud for state backend, CI/CD integration, and policy enforcement"
  ],
  "used_by": [
   "Mercedes-Benz",
   "Snowflake",
   "Cockroach Labs",
   "Nvidia",
   "Fauna"
  ],
  "docs": "https://www.pulumi.com/docs/",
  "tutorial": "https://www.pulumi.com/docs/get-started/",
  "added": "2026-06-08",
  "eli5": "Like Terraform but you write cloud blueprints in Python or Go instead of a special language.",
  "dex": 79
 },
 {
  "id": "puppet",
  "name": "Puppet",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A puppet master that tells all your servers exactly how to look and keeps them that way automatically.",
  "description": "Declarative configuration management tool. Define desired system state in Puppet DSL; the Puppet agent enforces and continuously corrects drift. Pull-based model with Puppet Server as the master. Strong at enforcing compliance across large, heterogeneous fleets.",
  "use_cases": [
   "Declarative server configuration: define desired state, Puppet enforces it",
   "Drift correction: auto-remediate unauthorized changes on managed nodes",
   "Cross-platform fleet management (Linux, Windows, network devices)",
   "Compliance reporting and enforcement at enterprise scale",
   "Puppet Forge module ecosystem for common infrastructure patterns"
  ],
  "used_by": [
   "Google",
   "Salesforce",
   "PayPal",
   "Stanford University",
   "Twitter"
  ],
  "docs": "https://www.puppet.com/docs/puppet/latest/puppet_index.html",
  "tutorial": "https://www.puppet.com/docs/puppet/latest/quick_start_essential_config.html",
  "added": "2026-06-08",
  "dex": 80
 },
 {
  "id": "qemu",
  "name": "QEMU",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "QEMU is like a computer inside your computer that can run any operating system, letting you test software on different systems without buying new hardware.",
  "description": "QEMU is a free and open-source machine emulator and virtualizer that enables running multiple operating systems and architectures on a single host system. It is widely used for development, testing, and production virtualization across diverse infrastructure environments.",
  "use_cases": [
   "Cross-architecture development and testing",
   "Lightweight virtualization for CI/CD pipelines",
   "Embedded systems development and debugging",
   "System-level software testing and simulation"
  ],
  "used_by": [
   "Red Hat",
   "Ubuntu",
   "SUSE",
   "Fedora Project"
  ],
  "docs": "https://www.qemu.org/docs/master/",
  "tutorial": "https://www.qemu.org/docs/master/system/quickstart.html",
  "added": "2026-06-20",
  "dex": 81
 },
 {
  "id": "scarf",
  "name": "Scarf",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a counter at a store that quietly tracks when people download your product so you know how many people are using it.",
  "description": "Scarf provides package analytics and insights for open source projects and commercial software by transparently tracking downloads and usage patterns. It helps maintainers understand adoption without compromising user privacy through its lightweight pixel-based tracking.",
  "use_cases": [
   "Measuring open source project adoption",
   "Tracking software download metrics",
   "Understanding user distribution by geography",
   "Monetizing open source projects ethically"
  ],
  "used_by": [
   "Docker",
   "HashiCorp",
   "JFrog"
  ],
  "docs": "https://docs.scarf.sh",
  "tutorial": "https://scarf.sh/",
  "added": "2026-06-28",
  "dex": 82
 },
 {
  "id": "semver",
  "name": "Semantic Versioning",
  "category": "Concept",
  "domain": "DevOps",
  "eli5": "A system that labels software versions like recipe versions—if you add an ingredient it's a small change, if you completely remake it that's a big change.",
  "description": "Semantic Versioning (SemVer) is a versioning scheme that uses MAJOR.MINOR.PATCH to communicate the nature and impact of code changes. It enables developers and automated systems to understand breaking changes, new features, and patches at a glance.",
  "use_cases": [
   "Dependency management in package managers like npm and pip",
   "API versioning to signal backward compatibility",
   "Release automation and CI/CD pipelines",
   "Library stability communication to downstream consumers"
  ],
  "used_by": [
   "GitHub",
   "npm Inc.",
   "Python Software Foundation",
   "Kubernetes"
  ],
  "docs": "https://semver.org/",
  "tutorial": "https://semver.org/",
  "added": "2026-09-10",
  "dex": 83
 },
 {
  "id": "serf",
  "name": "Serf",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Serf is like a group chat for your computers where they automatically tell each other when someone joins, leaves, or has a problem.",
  "description": "Serf is a decentralized solution for cluster membership, failure detection, and orchestration that uses gossip protocols to communicate between nodes. It's lighter than Consul and ideal for dynamic infrastructure environments.",
  "use_cases": [
   "Detecting node failures and triggering automatic remediation",
   "Coordinating service deployments across distributed clusters",
   "Building custom orchestration logic on top of cluster state",
   "Implementing dynamic service discovery in microservices"
  ],
  "used_by": [
   "HashiCorp",
   "SoundCloud",
   "Orbitz"
  ],
  "docs": "https://www.serf.io/docs/",
  "tutorial": "https://www.serf.io/intro/getting-started/",
  "added": "2026-07-06",
  "dex": 84
 },
 {
  "id": "skaffold",
  "name": "Skaffold",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Skaffold is like a live-reload button for Kubernetes — every time you save code, it automatically builds a container and deploys it to your cluster.",
  "description": "CLI tool for continuous development against Kubernetes. Handles build, push, and deploy automatically on file changes, with support for Helm, Kustomize, and kubectl.",
  "use_cases": [
   "Inner-loop dev with hot reload on Kubernetes",
   "CI pipeline with consistent build + deploy steps",
   "Multi-service local development with minikube/kind",
   "GitOps-friendly deploy pipelines"
  ],
  "used_by": [
   "Google",
   "Spotify",
   "Weaveworks",
   "JFrog"
  ],
  "docs": "https://skaffold.dev/docs/",
  "tutorial": "https://skaffold.dev/docs/quickstart/",
  "added": "2026-06-12",
  "dex": 85
 },
 {
  "id": "spacelift-iac",
  "name": "Spacelift",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "It's like a smart controller that safely manages changes to your entire infrastructure, checking everything before making updates.",
  "description": "Spacelift is a CI/CD platform purpose-built for infrastructure-as-code workflows supporting Terraform, CloudFormation, Pulumi, and other IaC tools. It provides drift detection, policy as code, cost estimation, and collaborative workflows for infrastructure management.",
  "use_cases": [
   "Terraform automation and drift detection across multiple environments",
   "Infrastructure change approvals and policy enforcement",
   "Multi-cloud infrastructure orchestration and cost estimation",
   "GitOps-based infrastructure provisioning and compliance"
  ],
  "used_by": [
   "Stripe",
   "Figma",
   "Vercel",
   "Wise"
  ],
  "docs": "https://docs.spacelift.io/",
  "tutorial": "https://docs.spacelift.io/getting-started",
  "added": "2026-07-15",
  "dex": 86
 },
 {
  "id": "speakeasy",
  "name": "Speakeasy",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Speakeasy is like a translator that automatically creates instruction manuals and example code for your API so other developers can use it easily.",
  "description": "Speakeasy is an SDK generation and API documentation platform that automatically creates production-quality SDKs and API docs from OpenAPI specifications. It enables teams to maintain consistent API experiences across multiple programming languages.",
  "use_cases": [
   "Automated SDK generation from OpenAPI schemas",
   "Multi-language API client library maintenance",
   "API documentation generation and versioning",
   "Developer experience and adoption acceleration"
  ],
  "used_by": [
   "Mercury Bank",
   "Stripe integration partners",
   "Various fintech startups"
  ],
  "docs": "https://docs.speakeasyapi.dev/",
  "tutorial": "https://docs.speakeasyapi.dev/docs/getting-started",
  "added": "2026-07-26",
  "dex": 87
 },
 {
  "id": "starship",
  "name": "Starship",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Starship is like a smart dashboard for your terminal that shows you useful information about what you're working on, instantly.",
  "description": "Starship is a minimal, blazing-fast, and customizable shell prompt written in Rust that displays relevant context like Git branch, language versions, and build status. It works across all major shells (Bash, Zsh, Fish, PowerShell) and significantly improves developer experience.",
  "use_cases": [
   "Accelerating developer workflows with fast, informative prompts",
   "Displaying project context (language, version, git status) at a glance",
   "Customizing terminal appearance across different machines and shells",
   "Integration with CI/CD pipelines and development container environments"
  ],
  "used_by": [
   "Vercel",
   "npm",
   "Netlify",
   "GitLab"
  ],
  "docs": "https://starship.rs/",
  "tutorial": "https://starship.rs/guide/",
  "added": "2026-07-31",
  "dex": 88
 },
 {
  "id": "terraform",
  "name": "Terraform",
  "category": "Tool",
  "domain": "DevOps",
  "description": "Infrastructure as Code tool by HashiCorp — declare cloud resources in HCL, plan changes, and apply. Provider ecosystem covers 3000+ services across AWS, GCP, Azure, and beyond. State file tracks real-world resource status.",
  "use_cases": [
   "Provisioning and managing cloud infrastructure declaratively (IaC)",
   "Multi-cloud and multi-provider resource management from one config",
   "Remote state in S3/GCS with DynamoDB locking for team collaboration",
   "Modules for reusable, versioned infrastructure components",
   "Drift detection and plan-before-apply safety for production changes"
  ],
  "used_by": [
   "Slack",
   "Twitch",
   "Starbucks",
   "Uber",
   "Barclays"
  ],
  "docs": "https://developer.hashicorp.com/terraform/docs",
  "tutorial": "https://developer.hashicorp.com/terraform/tutorials/aws-get-started",
  "added": "2026-06-08",
  "eli5": "A shopping list for cloud stuff — write what you want, it goes and builds it.",
  "dex": 89
 },
 {
  "id": "terraform-cloud",
  "name": "Terraform Cloud",
  "category": "Platform",
  "domain": "DevOps",
  "description": "Managed SaaS platform for Terraform by HashiCorp. Provides remote state storage, plan/apply execution, team access controls, policy enforcement (Sentinel/OPA), and a private module registry — turning Terraform into a collaborative, governed IaC platform.",
  "use_cases": [
   "Remote state backend with locking, versioning, and state history",
   "Remote plan/apply in ephemeral workers — no local credentials on developer machines",
   "Policy as Code with Sentinel or OPA to enforce cost/security guardrails before apply",
   "VCS-driven workflows: PR triggers speculative plan, merge triggers apply",
   "Private module registry for sharing versioned, reusable Terraform modules across teams"
  ],
  "used_by": [
   "HashiCorp customers",
   "Slack",
   "Twitch",
   "Deliveroo",
   "Spacelift"
  ],
  "docs": "https://developer.hashicorp.com/terraform/cloud-docs",
  "tutorial": "https://developer.hashicorp.com/terraform/tutorials/cloud-get-started",
  "added": "2026-06-08",
  "eli5": "A shared whiteboard where your whole team draws the same cloud blueprint together.",
  "dex": 90
 },
 {
  "id": "tilt",
  "name": "Tilt",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Tilt is like a smart live-reload for your entire microservices stack — change a file and only the affected service rebuilds and redeploys in seconds.",
  "description": "Local development environment orchestrator for Kubernetes microservices. Uses a Tiltfile (Python-like) to define smart rebuilds, dependency tracking, and a real-time dev dashboard.",
  "use_cases": [
   "Local multi-service Kubernetes development",
   "Smart incremental builds (only rebuild what changed)",
   "Unified dev dashboard for logs, builds, and health",
   "Consistent dev environments across teams"
  ],
  "used_by": [
   "Docker",
   "Grafana Labs",
   "Netlify",
   "Cockroach Labs"
  ],
  "docs": "https://docs.tilt.dev/",
  "tutorial": "https://docs.tilt.dev/tutorial/index.html",
  "added": "2026-06-12",
  "dex": 91
 },
 {
  "id": "turborepo",
  "name": "Turborepo",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Turborepo is like a smart build system that only rebuilds the parts of your monorepo that actually changed — skipping everything else to save time.",
  "description": "High-performance build system for JavaScript/TypeScript monorepos. Provides task caching, parallel execution, and remote cache sharing to dramatically speed up CI and local builds.",
  "use_cases": [
   "Monorepo build and test acceleration",
   "Remote caching to share build artifacts across CI runs",
   "Incremental builds (only rebuild changed packages)",
   "Task pipeline dependency management"
  ],
  "used_by": [
   "Vercel",
   "Microsoft",
   "HashiCorp",
   "Linear"
  ],
  "docs": "https://turbo.build/repo/docs",
  "tutorial": "https://turbo.build/repo/docs/getting-started/add-to-project",
  "added": "2026-06-14",
  "dex": 92
 },
 {
  "id": "unleash",
  "name": "Unleash",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a light switch that lets you turn features on or off for different users without redeploying your app.",
  "description": "Unleash is an open-source feature management platform that enables teams to control feature rollouts, A/B testing, and gradual deployments without code changes. It provides a centralized dashboard for managing feature flags across multiple applications.",
  "use_cases": [
   "Canary deployments and gradual feature rollouts",
   "A/B testing and experimentation",
   "Kill switches for emergency feature disabling",
   "Permission-based feature access control"
  ],
  "used_by": [
   "Spotify",
   "Yara",
   "Various enterprise teams"
  ],
  "docs": "https://docs.getunleash.io/",
  "tutorial": "https://docs.getunleash.io/getting-started",
  "added": "2026-07-13",
  "dex": 93
 },
 {
  "id": "vagrant",
  "name": "Vagrant",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "A magic button that spins up an identical local computer for every developer on the team.",
  "description": "HashiCorp tool for building and managing reproducible local development environments using VMs. A single Vagrantfile defines the OS, provisioning, and networking — every team member runs `vagrant up` and gets the same environment.",
  "use_cases": [
   "Reproducible local dev environments matching production OS and config",
   "Onboarding: new engineers run `vagrant up` instead of a 20-step setup doc",
   "Testing Ansible/Chef/Puppet playbooks locally before applying to production",
   "Isolated multi-VM topologies (app + DB + load balancer) on one laptop",
   "Legacy app environments that require specific OS versions"
  ],
  "used_by": [
   "HashiCorp",
   "GitHub",
   "Rackspace",
   "Engine Yard"
  ],
  "docs": "https://developer.hashicorp.com/vagrant/docs",
  "tutorial": "https://developer.hashicorp.com/vagrant/tutorials/getting-started",
  "added": "2026-06-08",
  "dex": 94
 },
 {
  "id": "velero",
  "name": "Velero",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Velero is like a backup and restore button for your entire Kubernetes cluster — it saves everything to cloud storage so you can recover or move it anywhere.",
  "description": "Open-source tool for backing up and restoring Kubernetes cluster resources and persistent volumes. Supports disaster recovery, cluster migration, and namespace-level snapshots.",
  "use_cases": [
   "Kubernetes cluster disaster recovery",
   "Migrating workloads between clusters or cloud providers",
   "Scheduled namespace/cluster backups to S3/GCS/Azure",
   "Pre-upgrade snapshots for rollback safety"
  ],
  "used_by": [
   "VMware",
   "Red Hat",
   "Replicated",
   "D2iQ"
  ],
  "docs": "https://velero.io/docs/",
  "tutorial": "https://velero.io/docs/main/contributions/ibm-ci/",
  "added": "2026-06-14",
  "dex": 95
 },
 {
  "id": "vscode-dev-containers",
  "name": "VS Code Dev Containers",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like having a portable computer setup inside a box that you can ship to anyone, and when they open it, their VS Code looks exactly like yours with all the tools already installed.",
  "description": "VS Code Dev Containers allows developers to use a Docker container as a complete development environment, ensuring consistent development setups across teams and reducing 'works on my machine' problems. The container configuration is version-controlled and shareable.",
  "use_cases": [
   "Onboarding new developers with zero setup time",
   "Ensuring identical development environments across teams",
   "Isolating project dependencies and avoiding conflicts",
   "Supporting multiple language stacks in a single organization"
  ],
  "used_by": [
   "Microsoft",
   "GitHub",
   "Google",
   "AWS"
  ],
  "docs": "https://code.visualstudio.com/docs/devcontainers/containers",
  "tutorial": "https://code.visualstudio.com/docs/devcontainers/tutorial",
  "added": "2026-09-13",
  "dex": 96
 },
 {
  "id": "vscode-remote",
  "name": "VS Code Remote Development",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "VS Code Remote is like having a magical tunnel that lets you edit files on a faraway computer as if they were right in front of you on your desk.",
  "description": "VS Code Remote Development enables developers to use VS Code to develop against code hosted on remote machines, containers, or WSL environments. It provides full IDE capabilities with extensions and debugging directly against remote systems.",
  "use_cases": [
   "Developing inside containerized environments without local setup complexity",
   "Collaborating on shared development servers with consistent environments",
   "Building and testing applications in production-like cloud environments",
   "Developing on Kubernetes clusters or remote VMs with full IDE support"
  ],
  "used_by": [
   "Microsoft",
   "Google",
   "GitHub",
   "Amazon"
  ],
  "docs": "https://code.visualstudio.com/docs/remote/remote-overview",
  "tutorial": "https://code.visualstudio.com/docs/remote/containers-tutorial",
  "added": "2026-07-29",
  "dex": 97
 },
 {
  "id": "walrus",
  "name": "Walrus",
  "category": "Platform",
  "domain": "DevOps",
  "eli5": "It's like a smart assistant that manages your infrastructure code and keeps everything running the same way every time, automatically.",
  "description": "Walrus is an open-source Infrastructure as Code (IaC) management platform that simplifies Terraform and other IaC workflows with a user-friendly interface and automation capabilities. It reduces complexity for teams managing cloud infrastructure at scale.",
  "use_cases": [
   "Self-service infrastructure provisioning for development teams",
   "Infrastructure state management and compliance tracking",
   "Multi-environment infrastructure deployment automation",
   "Cost optimization through centralized IaC governance"
  ],
  "used_by": [
   "Seal Software",
   "Cloud-native enterprises",
   "DevOps teams"
  ],
  "docs": "https://walrus.seal.io/docs",
  "tutorial": "https://walrus.seal.io/docs/guides/quickstart",
  "added": "2026-06-26",
  "dex": 98
 },
 {
  "id": "warp",
  "name": "Warp",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a super-powered command line terminal that remembers what you did, suggests commands, and explains errors to you automatically.",
  "description": "Warp is a modern terminal emulator built in Rust that replaces traditional shells like bash with AI-assisted command suggestions, command history search, and built-in collaboration features. It improves developer productivity through intelligent autocomplete and error explanation.",
  "use_cases": [
   "Faster command discovery and reduced manual lookup time",
   "Team collaboration on terminal sessions",
   "Automated workflows with command history and bookmarks",
   "Linux and macOS development environments"
  ],
  "used_by": [
   "Stripe",
   "Notion",
   "PagerDuty",
   "various Fortune 500 tech teams"
  ],
  "docs": "https://docs.warp.dev",
  "tutorial": "https://www.warp.dev/blog/getting-started-with-warp",
  "added": "2026-06-19",
  "dex": 99
 },
 {
  "id": "wunderbucket",
  "name": "Wunderbucket",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "It's like a smart assistant that watches your cloud storage and automatically organizes files based on rules you set up.",
  "description": "Wunderbucket is a cloud data management tool that automates data lifecycle policies, archival, and optimization across S3, Azure Blob, and GCS with intelligent tiering and cost management. It reduces cloud storage costs through automated policy enforcement and data movement.",
  "use_cases": [
   "Implementing automated data archival and retention policies",
   "Optimizing cloud storage costs through intelligent tiering",
   "Managing compliance-driven data lifecycle requirements",
   "Automating backup rotation and disaster recovery workflows"
  ],
  "used_by": [
   "Scale AI",
   "SambaNova",
   "Figma",
   "financial services firms"
  ],
  "docs": "https://docs.wunderbucket.com",
  "tutorial": "https://wunderbucket.com/docs/getting-started",
  "added": "2026-09-03",
  "dex": 100
 },
 {
  "id": "yaml",
  "name": "YAML",
  "category": "Language",
  "domain": "DevOps",
  "eli5": "A way to write settings and instructions so computers can read them, using indentation like an outline.",
  "description": "YAML Ain't Markup Language — human-readable data serialization format. The config language of cloud-native infrastructure. Used by Kubernetes manifests, Docker Compose, GitHub Actions, Ansible, and almost every modern DevOps tool. Indentation-sensitive.",
  "use_cases": [
   "Kubernetes resource definitions (Deployments, Services, ConfigMaps)",
   "GitHub Actions / GitLab CI pipeline definitions",
   "Docker Compose multi-service stack configuration",
   "Ansible playbooks and inventory files",
   "Helm chart values files and templates"
  ],
  "used_by": [
   "Every cloud-native engineering team"
  ],
  "docs": "https://yaml.org/spec/1.2.2/",
  "tutorial": "https://learnxinyminutes.com/docs/yaml/",
  "added": "2026-06-08",
  "dex": 101
 },
 {
  "id": "zarf",
  "name": "Zarf",
  "category": "Tool",
  "domain": "DevOps",
  "eli5": "Zarf is like a magic box that puts everything your application needs inside it so you can carry it anywhere—even places with no internet.",
  "description": "Zarf is an open-source tool by the U.S. Department of Defense that packages Kubernetes applications and dependencies into portable bundles for air-gapped and disconnected environments. It simplifies deploying complex systems in restricted networks without external package repositories.",
  "use_cases": [
   "Deploying applications in air-gapped/disconnected environments",
   "Packaging Kubernetes clusters with all dependencies included",
   "Defense and government infrastructure deployments",
   "Reducing deployment complexity in restricted network environments"
  ],
  "used_by": [
   "U.S. Department of Defense",
   "Enterprise security teams",
   "Air-gapped deployment specialists"
  ],
  "docs": "https://docs.zarf.dev",
  "tutorial": "https://docs.zarf.dev/getting-started",
  "added": "2026-07-14",
  "dex": 102
 },
 {
  "id": "astria",
  "name": "Astria",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Astria is like a shared kitchen where different restaurants can cook at the same time without getting in each other's way, making it cheaper and faster than each having their own kitchen.",
  "description": "Astria is a modular blockchain platform that separates consensus from execution, allowing applications to run on shared sequencers while maintaining sovereign rollups. It enables developers to launch custom blockchains with lower infrastructure costs and faster time-to-market.",
  "use_cases": [
   "Building sovereign rollups without maintaining full consensus infrastructure",
   "Running high-throughput applications on shared sequencers",
   "Reducing blockchain infrastructure costs for startups",
   "Creating interoperable rollup ecosystems"
  ],
  "used_by": [
   "Celestia ecosystem projects",
   "Various Cosmos developers",
   "Rollup startups"
  ],
  "docs": "https://docs.astria.org",
  "tutorial": "https://docs.astria.org/overview",
  "added": "2026-07-14",
  "dex": 103
 },
 {
  "id": "aws-ec2",
  "name": "AWS EC2",
  "category": "Platform",
  "domain": "Cloud",
  "description": "Amazon Elastic Compute Cloud — scalable virtual machines in AWS. Core compute primitive of AWS; choose instance type, OS, storage, and networking. Pay per second (Linux) or per hour (Windows).",
  "use_cases": [
   "Hosting web servers, APIs, and application backends",
   "Running batch jobs and HPC workloads on GPU/compute-optimized instances",
   "Autoscaling fleets behind a load balancer (Auto Scaling Groups)",
   "Running self-managed databases or Kubernetes nodes",
   "On-demand and Spot instances for cost-optimized workloads"
  ],
  "used_by": [
   "Netflix",
   "Airbnb",
   "Stripe",
   "NASA",
   "Samsung"
  ],
  "docs": "https://docs.aws.amazon.com/ec2/",
  "tutorial": "https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/EC2_GetStarted.html",
  "added": "2026-06-08",
  "eli5": "A rental computer in Amazon's giant warehouse you can turn on anytime.",
  "dex": 104
 },
 {
  "id": "aws-eks",
  "name": "AWS EKS",
  "category": "Platform",
  "domain": "Cloud",
  "description": "Amazon Elastic Kubernetes Service — managed Kubernetes control plane on AWS. AWS handles etcd, API server availability, and upgrades. Run worker nodes on EC2, Fargate, or EKS Anywhere (on-prem).",
  "use_cases": [
   "Running production Kubernetes workloads without managing the control plane",
   "Fargate profiles for serverless pod scheduling (no node management)",
   "IRSA (IAM Roles for Service Accounts) for pod-level AWS permissions",
   "Multi-tenant cluster isolation with namespaces and network policies",
   "Karpenter for intelligent, fast node autoscaling"
  ],
  "used_by": [
   "Snap",
   "Salesforce",
   "HSBC",
   "Intuit",
   "Delivery Hero"
  ],
  "docs": "https://docs.aws.amazon.com/eks/",
  "tutorial": "https://docs.aws.amazon.com/eks/latest/userguide/getting-started.html",
  "added": "2026-06-08",
  "eli5": "Amazon babysits Kubernetes so you only worry about your apps.",
  "dex": 105
 },
 {
  "id": "aws-lambda",
  "name": "AWS Lambda",
  "category": "Platform",
  "domain": "Cloud",
  "description": "Serverless compute service — run code without provisioning or managing servers. Pay only for compute time consumed (per-100ms billing). Scales automatically from zero to thousands of concurrent executions.",
  "use_cases": [
   "Event-driven processing: S3 uploads, DynamoDB streams, SQS/SNS triggers",
   "REST/GraphQL API backends via API Gateway",
   "Scheduled cron jobs and automation tasks",
   "Real-time data transformation in streaming pipelines (Kinesis)",
   "Lightweight microservices avoiding always-on server costs"
  ],
  "used_by": [
   "Coca-Cola",
   "iRobot",
   "Nordstrom",
   "Thomson Reuters",
   "Netflix"
  ],
  "docs": "https://docs.aws.amazon.com/lambda/",
  "tutorial": "https://docs.aws.amazon.com/lambda/latest/dg/getting-started.html",
  "added": "2026-06-08",
  "eli5": "A vending machine for code — drop in a coin (event), your code runs, then stops.",
  "dex": 106
 },
 {
  "id": "azure-functions",
  "name": "Azure Functions",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Microsoft's version of AWS Lambda — write a function, Azure runs it only when needed.",
  "description": "Azure's serverless compute service — event-driven functions that scale automatically and bill per execution. Supports C#, Python, Java, Node.js, PowerShell, and more. Durable Functions extension enables stateful, long-running workflows with fan-out/fan-in patterns.",
  "use_cases": [
   "Event-driven processing: HTTP, timer, Blob, Queue, Event Hub, Cosmos DB triggers",
   "Durable Functions for stateful orchestration (approval flows, fan-out/fan-in)",
   "API backends integrated with Azure API Management",
   "Real-time data transformation in Azure Event Hubs pipelines",
   "Scheduled automation and maintenance tasks via timer triggers"
  ],
  "used_by": [
   "KPMG",
   "Siemens",
   "HP",
   "Costco",
   "Carnival"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/azure-functions/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/azure-functions/functions-get-started",
  "added": "2026-06-08",
  "dex": 107
 },
 {
  "id": "aks",
  "name": "Azure Kubernetes Service",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Microsoft manages Kubernetes for you — just tell it what apps to run, it handles the cluster.",
  "description": "Azure's managed Kubernetes service. Free control plane; pay only for worker nodes. Deep Azure integration: Azure AD for RBAC, Azure Monitor for observability, Azure CNI for networking, and ACR for container images. Supports Windows node pools alongside Linux.",
  "use_cases": [
   "Production Kubernetes with Azure AD Workload Identity for pod-level RBAC",
   "Windows containers for .NET Framework apps alongside Linux workloads",
   "KEDA (Kubernetes Event-Driven Autoscaling) for queue/event-driven scaling",
   "Azure Service Mesh (Istio-based) for mTLS and traffic management",
   "GitOps with Flux CD as an AKS add-on"
  ],
  "used_by": [
   "ABB",
   "Siemens",
   "ING Bank",
   "KPMG",
   "Accenture"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/aks/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/aks/learn/quick-kubernetes-deploy-portal",
  "added": "2026-06-08",
  "dex": 108
 },
 {
  "id": "azure-vms",
  "name": "Azure Virtual Machines",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Microsoft's rental computers — pick a size, pay by the hour, run whatever you want on them.",
  "description": "Azure's IaaS virtual machine service supporting Linux and Windows workloads. Wide range of VM series optimized for compute, memory, storage, and GPU workloads. Spot VMs for fault-tolerant batch jobs at steep discounts. Deep integration with Azure AD and managed disks.",
  "use_cases": [
   "Lift-and-shift migration of on-prem Windows Server and SQL Server workloads",
   "GPU VMs (NC/ND series) for ML training and HPC",
   "VM Scale Sets for autoscaling application tiers behind Azure Load Balancer",
   "Spot VMs for cost-optimized batch processing and dev/test environments",
   "Azure Dedicated Hosts for compliance workloads needing physical isolation"
  ],
  "used_by": [
   "Heineken",
   "Rolls-Royce",
   "Toyota",
   "3M",
   "ASOS"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/virtual-machines/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/virtual-machines/linux/quick-create-portal",
  "added": "2026-06-08",
  "dex": 109
 },
 {
  "id": "bitnami",
  "name": "Bitnami",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "It's like pre-made LEGO kits for the cloud—software that's already assembled and ready to run instead of building from scratch.",
  "description": "Bitnami provides pre-built, production-ready application stacks and container images for deploying popular open-source software on cloud platforms. It simplifies deployment and reduces time-to-production for applications like WordPress, Kubernetes, and databases.",
  "use_cases": [
   "One-click deployment of web applications to cloud platforms",
   "Containerized application distribution via Docker and Kubernetes",
   "Enterprise software stack provisioning",
   "Multi-cloud application deployment consistency"
  ],
  "used_by": [
   "VMware",
   "AWS Marketplace",
   "Azure Marketplace",
   "Google Cloud"
  ],
  "docs": "https://bitnami.com/",
  "tutorial": "https://bitnami.com/get-started",
  "added": "2026-06-26",
  "dex": 110
 },
 {
  "id": "firecracker",
  "name": "Firecracker",
  "category": "Tool",
  "domain": "Cloud",
  "eli5": "It's like a super-fast toy factory that can spin up a brand new machine in milliseconds instead of seconds, so you can run lots of things at once.",
  "description": "Firecracker is a lightweight virtualization technology developed by AWS that enables microVMs to start in milliseconds with minimal overhead. It powers AWS Lambda and is used for serverless computing at massive scale with high density and security isolation.",
  "use_cases": [
   "Serverless function execution platforms",
   "Multi-tenant container isolation",
   "Edge computing and IoT deployments",
   "Function-as-a-Service (FaaS) infrastructure"
  ],
  "used_by": [
   "Amazon Web Services",
   "Fly.io",
   "Netlify"
  ],
  "docs": "https://github.com/firecracker-microvm/firecracker",
  "tutorial": "https://github.com/firecracker-microvm/firecracker/tree/main/docs",
  "added": "2026-06-28",
  "dex": 111
 },
 {
  "id": "gcp",
  "name": "Google Cloud Platform",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Google's version of Amazon's cloud — renting Google's computers, storage, and AI tools.",
  "description": "Google's public cloud platform offering 200+ services across compute, storage, data, AI/ML, and networking. Known for leading managed Kubernetes (GKE), BigQuery analytics, and Vertex AI. Strong in data engineering and ML workloads.",
  "use_cases": [
   "Running containerized workloads on GKE — the most mature managed Kubernetes service",
   "Large-scale analytics with BigQuery's serverless data warehouse",
   "ML training and serving with Vertex AI and TPU access",
   "Multi-cloud strategy alongside AWS or Azure",
   "Migrating from on-prem with Anthos for hybrid cloud management"
  ],
  "used_by": [
   "Spotify",
   "Twitter",
   "PayPal",
   "20th Century Fox",
   "Colgate-Palmolive"
  ],
  "docs": "https://cloud.google.com/docs",
  "tutorial": "https://cloud.google.com/docs/get-started",
  "added": "2026-06-08",
  "dex": 112
 },
 {
  "id": "cloud-run",
  "name": "Google Cloud Run",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Deploy any container and Google runs it for you — scales to zero when no one is using it.",
  "description": "GCP's fully managed serverless container platform. Deploy any OCI container; Cloud Run handles scaling from zero to thousands of instances, HTTPS, and traffic management. No Kubernetes to manage. Ideal for stateless services, APIs, and event-driven workloads.",
  "use_cases": [
   "Stateless HTTP APIs and microservices with zero infrastructure management",
   "Event-driven processing via Pub/Sub, Cloud Tasks, or Eventarc triggers",
   "Scale-to-zero services for cost-efficient low-traffic or batch workloads",
   "ML model serving: containerize a model server, Cloud Run handles traffic bursts",
   "Cloud Run Jobs for batch, cron, and data processing tasks"
  ],
  "used_by": [
   "Delivery Hero",
   "Ubisoft",
   "Meredith",
   "Lush",
   "Mercado Libre"
  ],
  "docs": "https://cloud.google.com/run/docs",
  "tutorial": "https://cloud.google.com/run/docs/quickstarts/build-and-deploy/deploy-python-service",
  "added": "2026-06-08",
  "dex": 113
 },
 {
  "id": "gce",
  "name": "Google Compute Engine",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Google's rental computers in the cloud — like AWS EC2 but on Google's network.",
  "description": "GCP's IaaS virtual machine service. Offers custom machine types, preemptible/Spot VMs, per-second billing, and live migration during host maintenance. Runs on Google's global fiber network with high-bandwidth, low-latency inter-region connectivity.",
  "use_cases": [
   "General-purpose VM workloads with custom vCPU/RAM ratios",
   "Preemptible VMs for batch jobs and fault-tolerant workloads at 60-90% discount",
   "GPU instances for ML training and rendering workloads",
   "Managed instance groups with autoscaling behind a load balancer",
   "Sole-tenant nodes for compliance workloads requiring physical isolation"
  ],
  "used_by": [
   "Spotify",
   "HSBC",
   "Etsy",
   "Best Buy",
   "Philips"
  ],
  "docs": "https://cloud.google.com/compute/docs",
  "tutorial": "https://cloud.google.com/compute/docs/quickstart-linux",
  "added": "2026-06-08",
  "dex": 114
 },
 {
  "id": "gke",
  "name": "Google Kubernetes Engine",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Google manages your Kubernetes cluster — they invented it, so they know it best.",
  "description": "GCP's managed Kubernetes service — the most mature and feature-rich managed Kubernetes offering. Autopilot mode fully manages nodes; Standard mode gives full control. Tight integration with GCP IAM, Cloud Logging, Cloud Monitoring, and Artifact Registry.",
  "use_cases": [
   "Production Kubernetes with Autopilot for fully managed, serverless node management",
   "Workload Identity for pod-level GCP IAM permissions without key files",
   "Binary Authorization for enforcing signed image policies at deploy time",
   "Multi-cluster deployments with GKE Fleet for unified management",
   "Config Connector to manage GCP resources as Kubernetes CRDs"
  ],
  "used_by": [
   "Spotify",
   "Shopify",
   "Airbus",
   "Niantic",
   "HSBC"
  ],
  "docs": "https://cloud.google.com/kubernetes-engine/docs",
  "tutorial": "https://cloud.google.com/kubernetes-engine/docs/quickstart",
  "added": "2026-06-08",
  "dex": 115
 },
 {
  "id": "localstack",
  "name": "LocalStack",
  "category": "Tool",
  "domain": "Cloud",
  "eli5": "LocalStack is like a fake AWS on your computer that lets you practice building things without spending real money.",
  "description": "LocalStack is a fully functional local AWS cloud stack that emulates AWS services on your machine for development and testing. It allows developers to build and test cloud applications locally without incurring AWS costs or needing internet connectivity.",
  "use_cases": [
   "Local development of AWS-dependent applications",
   "Integration testing of cloud infrastructure code",
   "CI/CD pipeline testing without cloud resource consumption",
   "Teaching and training on AWS services"
  ],
  "used_by": [
   "Atlassian",
   "Volkswagen",
   "Shopify",
   "HashiCorp"
  ],
  "docs": "https://docs.localstack.cloud/",
  "tutorial": "https://docs.localstack.cloud/getting-started/",
  "added": "2026-06-15",
  "dex": 116
 },
 {
  "id": "localstack-edge",
  "name": "LocalStack Edge",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "It's like having a tiny fake Amazon cloud on your computer that pretends to be the real thing, so you can practice without spending money.",
  "description": "LocalStack is a fully functional local AWS cloud stack that runs in Docker, allowing developers to develop and test AWS applications offline without AWS costs. LocalStack Edge extends this with real-time cloud connectivity and advanced mocking capabilities.",
  "use_cases": [
   "Local AWS service testing (S3, DynamoDB, Lambda, SQS)",
   "CI/CD pipeline testing without cloud provider costs",
   "Integration testing with multiple AWS services",
   "Offline development and rapid iteration on cloud applications"
  ],
  "used_by": [
   "Netflix",
   "Amazon",
   "HashiCorp",
   "Grab"
  ],
  "docs": "https://docs.localstack.cloud/",
  "tutorial": "https://docs.localstack.cloud/getting-started/installation/",
  "added": "2026-07-21",
  "dex": 117
 },
 {
  "id": "azure",
  "name": "Microsoft Azure",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "Microsoft's cloud — like AWS but Microsoft's, and very popular with companies already using Windows and Office.",
  "description": "Microsoft's public cloud platform with 200+ services. Dominant in enterprise due to deep Microsoft ecosystem integration (Active Directory, Office 365, Windows Server). Strong hybrid cloud story via Azure Arc. Second-largest cloud provider by market share.",
  "use_cases": [
   "Enterprise workloads already in the Microsoft ecosystem (Windows, SQL Server, .NET)",
   "Hybrid cloud with Azure Arc extending Azure management to on-prem and other clouds",
   "Azure OpenAI Service for GPT-4/Copilot integration in enterprise apps",
   "SAP, Oracle, and VMware workloads via certified Azure infrastructure",
   "Government and regulated industries via Azure Government and sovereign regions"
  ],
  "used_by": [
   "Boeing",
   "BMW",
   "Walmart",
   "FedEx",
   "ExxonMobil"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/guides/developer/azure-developer-guide",
  "added": "2026-06-08",
  "dex": 118
 },
 {
  "id": "openstack",
  "name": "OpenStack",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "It's like a recipe that lets you build your own private cloud computer, just like how you can make your own pizza instead of ordering from a restaurant.",
  "description": "OpenStack is an open-source cloud computing platform that allows organizations to build and manage large pools of computing, storage, and networking resources. It provides an alternative to public cloud providers by enabling on-premises or hybrid cloud infrastructure.",
  "use_cases": [
   "Building private cloud infrastructure for large enterprises",
   "Hybrid cloud deployments combining on-premises and public cloud",
   "Telecommunications and service provider cloud platforms",
   "High-performance computing and research computing clusters"
  ],
  "used_by": [
   "CERN",
   "PayPal",
   "Best Buy",
   "Walmart"
  ],
  "docs": "https://docs.openstack.org/",
  "tutorial": "https://docs.openstack.org/install/",
  "added": "2026-07-09",
  "dex": 119
 },
 {
  "id": "openyurt",
  "name": "OpenYurt",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "It's like extending your cloud's power to the edge of the network, so devices far away from the main cloud can still work smoothly.",
  "description": "OpenYurt is a Kubernetes-based platform for managing cloud-native applications across edge nodes, fog computing, and on-premises infrastructure. It extends Kubernetes to provide seamless orchestration at the edge with improved network resilience.",
  "use_cases": [
   "Kubernetes deployment and management at the network edge",
   "IoT device orchestration and cloud-edge integration",
   "Reducing latency for distributed applications",
   "Infrastructure spanning cloud, fog, and edge locations"
  ],
  "used_by": [
   "Alibaba Cloud",
   "Enterprise edge computing teams",
   "IoT operators"
  ],
  "docs": "https://openyurt.io/docs/",
  "tutorial": "https://openyurt.io/docs/tutorial/",
  "added": "2026-08-04",
  "dex": 120
 },
 {
  "id": "oxide-cloud-computer",
  "name": "Oxide Cloud Computer",
  "category": "Platform",
  "domain": "Cloud",
  "eli5": "It's like building your own mini cloud provider in a box—all the powerful cloud computing stuff, but you own the hardware.",
  "description": "Oxide Cloud Computer is a purpose-built rack computer that brings cloud-native infrastructure management to on-premises data centers. It provides seamless cloud-like experiences (orchestration, networking, storage) without requiring cloud vendor lock-in.",
  "use_cases": [
   "Regulated industries requiring on-premises infrastructure",
   "Hybrid cloud deployments with consistent management",
   "Organizations seeking cloud-native experience in private data centers",
   "Edge computing at scale"
  ],
  "used_by": [
   "Oxide Systems",
   "Various Fortune 500 enterprises in confidential deployments"
  ],
  "docs": "https://oxide.computer/docs",
  "tutorial": "https://oxide.computer/blog/introducing-oxide",
  "added": "2026-09-12",
  "dex": 121
 },
 {
  "id": "apache-cassandra",
  "name": "Apache Cassandra",
  "category": "Tool",
  "domain": "Database",
  "description": "Distributed wide-column NoSQL database designed for massive scale with no single point of failure. Optimized for write-heavy, globally distributed workloads. Tunable consistency (AP system per CAP theorem). Originally built at Facebook.",
  "use_cases": [
   "Time-series data at scale: IoT telemetry, metrics, event logs",
   "High-write-throughput workloads needing linear horizontal scalability",
   "Multi-region active-active deployments with zero downtime",
   "User activity feeds, recommendation data, messaging history",
   "Netflix-scale personalization data (hundreds of TB)"
  ],
  "used_by": [
   "Netflix",
   "Apple",
   "Discord",
   "Instagram",
   "Spotify",
   "Uber"
  ],
  "docs": "https://cassandra.apache.org/doc/latest/",
  "tutorial": "https://cassandra.apache.org/doc/latest/cassandra/getting-started/quickstart.html",
  "added": "2026-06-08",
  "eli5": "A filing cabinet spread across many rooms — still works if one room burns down.",
  "dex": 122
 },
 {
  "id": "astro-db",
  "name": "Astro DB",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a notebook that lives in the cloud where your website can quickly write down and look up information without having to run your own filing cabinet.",
  "description": "Astro DB is a fully managed SQL database built for the Astro framework, providing serverless PostgreSQL databases with built-in replication and backup. It's designed to seamlessly integrate with Astro applications for fast, reliable data persistence.",
  "use_cases": [
   "Building serverless web applications with Astro",
   "Storing user data and content for static site generators",
   "Managing application state without managing database infrastructure",
   "Rapid prototyping and MVP development"
  ],
  "used_by": [
   "Vercel ecosystem users",
   "Astro community projects",
   "Early-stage startups"
  ],
  "docs": "https://docs.astro.build/en/guides/astro-db/",
  "tutorial": "https://docs.astro.build/en/guides/astro-db/",
  "added": "2026-07-18",
  "dex": 123
 },
 {
  "id": "aws-dynamodb",
  "name": "AWS DynamoDB",
  "category": "Platform",
  "domain": "Database",
  "description": "AWS-managed serverless NoSQL key-value and document database. Single-digit millisecond latency at any scale. No servers to manage; capacity auto-scales. Pairs with DAX (in-memory cache) for microsecond reads.",
  "use_cases": [
   "Serverless app backends needing predictable low-latency at any scale",
   "Session stores, shopping carts, user preferences (key-value access patterns)",
   "DynamoDB Streams for event-driven triggers to Lambda/Kafka",
   "Global Tables for multi-region active-active replication",
   "Single-table design patterns for complex relational data with access-pattern-first modeling"
  ],
  "used_by": [
   "Amazon",
   "Lyft",
   "Redfin",
   "Samsung",
   "Capital One"
  ],
  "docs": "https://docs.aws.amazon.com/dynamodb/",
  "tutorial": "https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/GettingStartedDynamoDB.html",
  "added": "2026-06-08",
  "eli5": "Amazon's magic filing cabinet that grows automatically and is always fast.",
  "dex": 124
 },
 {
  "id": "axion",
  "name": "Axion",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Axion is like a special notebook that lets your database remember things even when the power goes out, and it does it super fast.",
  "description": "Axion is an embeddable SQL database engine written in Rust that provides ACID-compliant transactions with persistent storage. It's designed for applications that need reliable local data storage with SQL querying capabilities.",
  "use_cases": [
   "Embedded database for desktop and mobile applications",
   "Local data persistence in edge computing devices",
   "Testing and development environments requiring SQL compliance",
   "Applications needing offline-first capabilities with sync"
  ],
  "used_by": [
   "Various Rust-based startups",
   "Edge computing platforms",
   "Desktop application developers"
  ],
  "docs": "https://github.com/axion-db/axion",
  "tutorial": "https://github.com/axion-db/axion#getting-started",
  "added": "2026-07-26",
  "dex": 125
 },
 {
  "id": "cosmos-db",
  "name": "Azure Cosmos DB",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Microsoft's database that works in every country at once and is always fast, no matter where you are.",
  "description": "Microsoft's globally distributed, multi-model NoSQL database. Single-digit millisecond reads and writes anywhere in the world. Multiple APIs: Core SQL, MongoDB, Cassandra, Gremlin, and Table. Turnkey global distribution — add regions with a click.",
  "use_cases": [
   "Global apps needing low-latency reads/writes across multiple regions",
   "Multi-model flexibility: use SQL, MongoDB, or Cassandra API on the same data",
   "IoT telemetry ingestion with automatic partitioning at massive scale",
   "Gaming leaderboards, session state, and personalization at planetary scale",
   "Serverless mode for unpredictable or low-traffic workloads"
  ],
  "used_by": [
   "Microsoft",
   "Coca-Cola",
   "Jet.com",
   "MSN",
   "Nokia"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/cosmos-db/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/cosmos-db/nosql/quickstart-portal",
  "added": "2026-06-08",
  "dex": 126
 },
 {
  "id": "chromadb",
  "name": "ChromaDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "ChromaDB is like a sticky-note board for AI — you paste in documents, it remembers their meaning, and later the AI can find the most relevant ones instantly.",
  "description": "Open-source embedding database designed for LLM applications. Runs in-process or as a server, with a simple Python API for storing, querying, and managing embeddings.",
  "use_cases": [
   "Local RAG prototyping with LLMs",
   "Storing and querying document embeddings",
   "Long-term memory for AI agents",
   "Lightweight vector store for small to mid-scale apps"
  ],
  "used_by": [
   "LangChain (default)",
   "AutoGPT",
   "various LLM app builders"
  ],
  "docs": "https://docs.trychroma.com/",
  "tutorial": "https://docs.trychroma.com/getting-started",
  "added": "2026-06-12",
  "dex": 127
 },
 {
  "id": "clickhouse",
  "name": "ClickHouse",
  "category": "Tool",
  "domain": "Database",
  "description": "Open-source columnar OLAP database for real-time analytical queries. Blazing fast aggregations over billions of rows via vectorized execution and columnar storage. Used where Elasticsearch or Redshift is too slow or expensive.",
  "use_cases": [
   "Real-time analytics dashboards over event/log data (clickstreams, ad metrics)",
   "Log storage and search as a high-performance alternative to Elasticsearch",
   "Time-series aggregation: percentiles, histograms over billions of rows in milliseconds",
   "User-facing analytics (embedded analytics in SaaS products)",
   "ClickHouse Cloud as managed alternative to Snowflake for real-time workloads"
  ],
  "used_by": [
   "Cloudflare",
   "ByteDance",
   "Uber",
   "eBay",
   "Contentsquare",
   "Gitlab"
  ],
  "docs": "https://clickhouse.com/docs/en/intro",
  "tutorial": "https://clickhouse.com/docs/en/getting-started/quick-start",
  "added": "2026-06-08",
  "eli5": "A calculator that sums up billions of numbers in milliseconds.",
  "dex": 128
 },
 {
  "id": "clickhouse-cloud",
  "name": "ClickHouse Cloud",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like having a super-organized library that can search through millions of books in seconds, specifically designed to find and count things really, really fast.",
  "description": "A fully managed cloud analytics database optimized for OLAP (Online Analytical Processing) workloads with exceptional query performance on large datasets. ClickHouse excels at time-series data, log analysis, and real-time analytics at scale.",
  "use_cases": [
   "Real-time analytics dashboards and reporting",
   "Log aggregation and analysis at petabyte scale",
   "Time-series data storage and query",
   "Ad-tech and monitoring data warehousing"
  ],
  "used_by": [
   "Cloudflare",
   "Uber",
   "Yandex",
   "GitLab"
  ],
  "docs": "https://clickhouse.com/docs/en/cloud",
  "tutorial": "https://clickhouse.com/docs/en/getting-started/install",
  "added": "2026-08-06",
  "dex": 129
 },
 {
  "id": "firestore-realtime-database",
  "name": "Cloud Firestore",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a magical notebook that everyone can read and write to at the same time, and it automatically tells everyone when something changes.",
  "description": "Cloud Firestore is Google's fully managed, serverless NoSQL document database that syncs data in real-time across all connected clients. It scales automatically and integrates seamlessly with Firebase for mobile and web applications.",
  "use_cases": [
   "Real-time collaborative applications like Figma or Google Docs",
   "Mobile apps requiring offline-first synchronization",
   "Chat applications and messaging platforms",
   "IoT data collection and live dashboards"
  ],
  "used_by": [
   "Google",
   "Duolingo",
   "Slack",
   "Spotify"
  ],
  "docs": "https://cloud.google.com/firestore/docs",
  "tutorial": "https://firebase.google.com/docs/firestore/quickstart",
  "added": "2026-09-12",
  "dex": 130
 },
 {
  "id": "cockroachdb",
  "name": "CockroachDB",
  "category": "Tool",
  "domain": "Database",
  "description": "Cloud-native distributed SQL database combining PostgreSQL compatibility with horizontal scalability and multi-region active-active deployment. Survives node/datacenter failures with zero RPO. Built for global applications that outgrow single-node Postgres.",
  "use_cases": [
   "Global OLTP apps needing Postgres compatibility + horizontal scale",
   "Multi-region deployments with data residency controls (GDPR compliance)",
   "Replacing Postgres when write throughput or availability SLAs exceed single-node limits",
   "Lift-and-shift from Postgres with minimal query changes",
   "Financial systems needing serializable isolation at scale"
  ],
  "used_by": [
   "Bose",
   "Equifax",
   "Netflix",
   "Shipt",
   "SpaceX"
  ],
  "docs": "https://www.cockroachlabs.com/docs/",
  "tutorial": "https://www.cockroachlabs.com/docs/stable/start-a-local-cluster.html",
  "added": "2026-06-08",
  "eli5": "A filing cabinet that keeps working even if half of it is destroyed.",
  "dex": 131
 },
 {
  "id": "couchbase",
  "name": "Couchbase",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Couchbase is like a super-fast notebook that keeps copies in multiple places so you can always access your data even if one copy gets damaged.",
  "description": "Couchbase is a distributed NoSQL database that combines the flexibility of document storage with the performance of in-memory caching and built-in search capabilities. It provides high availability, horizontal scalability, and real-time sync across geographically distributed clusters.",
  "use_cases": [
   "High-performance mobile and edge computing with offline-first sync",
   "Real-time personalization and recommendation engines",
   "IoT data ingestion and analytics at scale",
   "Session management and user profile caching for high-traffic applications"
  ],
  "used_by": [
   "LinkedIn",
   "Expedia",
   "Cisco",
   "Marriott"
  ],
  "docs": "https://docs.couchbase.com/",
  "tutorial": "https://docs.couchbase.com/tutorials/",
  "added": "2026-07-31",
  "dex": 132
 },
 {
  "id": "dolt",
  "name": "Dolt",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Dolt is like Git for your database — you can save different versions, see what changed, and merge updates just like with code.",
  "description": "Dolt is a SQL database with Git-like version control built in, allowing teams to branch, merge, and diff their data just like they would with code. It's compatible with MySQL and enables collaborative data workflows with full audit trails.",
  "use_cases": [
   "Data versioning and lineage tracking",
   "Collaborative data workflows with branching and merging",
   "Reproducible analytics with data snapshots",
   "Compliance and audit requirements with change history"
  ],
  "used_by": [
   "Lattice",
   "DoltHub ecosystem",
   "Data-intensive startups"
  ],
  "docs": "https://docs.dolthub.com/",
  "tutorial": "https://docs.dolthub.com/getting-started/installation",
  "added": "2026-07-26",
  "dex": 133
 },
 {
  "id": "dragonfly-redis",
  "name": "Dragonfly",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a faster, stronger version of Redis that can handle way more customers in a store at the same time without getting tired.",
  "description": "A modern, high-performance in-memory data store designed as a drop-in replacement for Redis with significantly better throughput and lower latency. It supports Redis protocol compatibility while offering superior performance on modern hardware.",
  "use_cases": [
   "High-throughput caching for e-commerce platforms",
   "Real-time leaderboards and gaming applications",
   "Session management for large-scale web applications",
   "Message queuing and pub/sub systems"
  ],
  "used_by": [
   "Figma",
   "Typeform",
   "Discord",
   "Fortnite"
  ],
  "docs": "https://www.dragonflydb.io/docs",
  "tutorial": "https://www.dragonflydb.io/docs/getting-started",
  "added": "2026-08-08",
  "dex": 134
 },
 {
  "id": "duckdb",
  "name": "DuckDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a tiny, super-fast database that lives in your application instead of on a separate server, perfect for analyzing data quickly without setup.",
  "description": "DuckDB is an in-process SQL OLAP database engine optimized for analytical queries on large datasets with minimal overhead. It provides native support for Parquet, CSV, and other formats, making it ideal for data analysis workflows and embedding in applications.",
  "use_cases": [
   "Fast data analysis and querying on local files and Parquet datasets",
   "Building analytical dashboards and reports without a centralized database",
   "Embedded analytics in applications and Python/R notebooks",
   "Processing and transforming large datasets in data pipelines with minimal resource usage"
  ],
  "used_by": [
   "Evidence",
   "Ibis",
   "data scientists worldwide",
   "analytics companies"
  ],
  "docs": "https://duckdb.org/docs/",
  "tutorial": "https://duckdb.org/docs/guides/index",
  "added": "2026-06-14",
  "dex": 135
 },
 {
  "id": "elasticsearch",
  "name": "Elasticsearch",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Elasticsearch is like a super-fast librarian who can search every word in every book in seconds and tell you exactly which books match your question.",
  "description": "Distributed search and analytics engine built on Apache Lucene. Powers full-text search, log analytics, and observability at scale. Core of the ELK Stack.",
  "use_cases": [
   "Full-text search for applications",
   "Log and metric aggregation (ELK/EFK stack)",
   "E-commerce product search",
   "Security event analytics (SIEM)"
  ],
  "used_by": [
   "Netflix",
   "Uber",
   "GitHub",
   "Wikipedia"
  ],
  "docs": "https://www.elastic.co/guide/en/elasticsearch/reference/current/",
  "tutorial": "https://www.elastic.co/guide/en/elasticsearch/reference/current/getting-started.html",
  "added": "2026-06-12",
  "dex": 136
 },
 {
  "id": "etcd",
  "name": "etcd",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a tiny but super reliable address book that everyone in your house checks before doing anything, making sure everyone knows what's happening.",
  "description": "etcd is a distributed, strongly-consistent key-value store used for shared configuration and service discovery in distributed systems. It's the backbone of Kubernetes cluster state management and is critical for maintaining consistency across multiple machines.",
  "use_cases": [
   "Kubernetes cluster state and configuration management",
   "Service discovery in microservices architectures",
   "Distributed locks and leader election",
   "Configuration management for distributed applications"
  ],
  "used_by": [
   "Kubernetes",
   "CoreOS",
   "Red Hat",
   "Cloud Foundry"
  ],
  "docs": "https://etcd.io/docs/",
  "tutorial": "https://etcd.io/docs/v3.5/quickstart/",
  "added": "2026-06-25",
  "dex": 137
 },
 {
  "id": "go-migrate",
  "name": "golang-migrate",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a checklist that tracks every change you make to a database so you can undo or redo them in the right order.",
  "description": "golang-migrate is a database migration tool written in Go that supports multiple databases and provides CLI and library interfaces for version control of database schemas. It's widely used in production systems for managing database changes across environments.",
  "use_cases": [
   "Managing schema migrations in microservices architectures",
   "Automating database versioning in CI/CD pipelines",
   "Supporting multiple database backends (PostgreSQL, MySQL, SQLite, etc.) with single codebase",
   "Rolling back and replaying database changes safely in production"
  ],
  "used_by": [
   "Kubernetes",
   "Docker",
   "HashiCorp",
   "Atlassian"
  ],
  "docs": "https://github.com/golang-migrate/migrate",
  "tutorial": "https://github.com/golang-migrate/migrate/tree/master/cmd/migrate",
  "added": "2026-09-04",
  "dex": 138
 },
 {
  "id": "bigtable",
  "name": "Google Cloud Bigtable",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a giant spreadsheet that can handle millions of people writing to it at the same time without slowing down.",
  "description": "Google Cloud Bigtable is a fully managed, scalable NoSQL database service designed for massive analytical and operational workloads. It powers real-time big data applications requiring high throughput and low latency across petabytes of data.",
  "use_cases": [
   "Time-series data storage for IoT sensor streams",
   "Real-time analytics on clickstream and user behavior data",
   "Financial trading systems and market data storage",
   "Genomics research data processing and analysis"
  ],
  "used_by": [
   "Google",
   "Netflix",
   "Spotify",
   "Uber"
  ],
  "docs": "https://cloud.google.com/bigtable/docs",
  "tutorial": "https://cloud.google.com/bigtable/docs/quickstart-cbt",
  "added": "2026-08-16",
  "dex": 139
 },
 {
  "id": "bigtable-emulator",
  "name": "Google Cloud Bigtable Emulator",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a practice version of a giant filing cabinet that helps you organize and find billions of tiny pieces of data really fast.",
  "description": "The Google Cloud Bigtable Emulator is a local development tool that mimics Google Cloud Bigtable's behavior for testing and development without incurring cloud costs. It allows developers to build and test NoSQL applications locally before deploying to production.",
  "use_cases": [
   "Local development and testing of Bigtable applications",
   "CI/CD pipeline testing without cloud infrastructure costs",
   "Learning Bigtable API and schema design patterns",
   "Integration testing for time-series and analytical workloads"
  ],
  "used_by": [
   "Google",
   "Spotify",
   "Twitter",
   "Adobe"
  ],
  "docs": "https://cloud.google.com/bigtable/docs/emulator",
  "tutorial": "https://cloud.google.com/bigtable/docs/emulator-quickstart",
  "added": "2026-08-23",
  "dex": 140
 },
 {
  "id": "firestore",
  "name": "Google Cloud Firestore",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a magical notebook that automatically saves your data to the cloud and instantly syncs changes to all your devices.",
  "description": "A serverless, NoSQL document database that automatically syncs data in real-time across devices and scales seamlessly without infrastructure management. It's a core backend-as-a-service offering for mobile and web applications.",
  "use_cases": [
   "Real-time collaborative applications and shared documents",
   "Mobile app backend with offline support",
   "IoT data collection and management",
   "Multi-player gaming state synchronization"
  ],
  "used_by": [
   "Google",
   "Slack",
   "Spotify",
   "Duolingo"
  ],
  "docs": "https://firebase.google.com/docs/firestore",
  "tutorial": "https://firebase.google.com/docs/firestore/quickstart",
  "added": "2026-07-04",
  "dex": 141
 },
 {
  "id": "greptimedb",
  "name": "GreptimeDB",
  "category": "Platform",
  "domain": "Database",
  "eli5": "GreptimeDB is like a super-fast filing cabinet that's really good at storing measurements over time, like temperature readings or stock prices.",
  "description": "GreptimeDB is an open-source time-series database built for metrics and observability workloads, offering high performance ingestion, storage efficiency, and SQL/PromQL query compatibility. It's optimized for handling billions of time-series data points with minimal resource overhead.",
  "use_cases": [
   "Real-time metrics collection for infrastructure monitoring",
   "IoT sensor data aggregation and analysis",
   "Financial market data processing and analysis",
   "Application performance monitoring at scale"
  ],
  "used_by": [
   "Huawei Cloud",
   "Zhihu",
   "Various fintech companies"
  ],
  "docs": "https://docs.greptime.com/",
  "tutorial": "https://docs.greptime.com/getting-started/overview",
  "added": "2026-07-26",
  "dex": 142
 },
 {
  "id": "influxdb",
  "name": "InfluxDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "InfluxDB is like a fitness tracker for your infrastructure — it records millions of measurements per second and lets you query them by time instantly.",
  "description": "Purpose-built time-series database optimized for high-write workloads. Native support for metrics, events, and traces with a SQL-like query language (Flux/InfluxQL).",
  "use_cases": [
   "Infrastructure and application metrics storage",
   "IoT sensor data at high cardinality",
   "Real-time analytics dashboards with Grafana",
   "Financial tick data and market monitoring"
  ],
  "used_by": [
   "Tesla",
   "IBM",
   "Cisco",
   "Hulu"
  ],
  "docs": "https://docs.influxdata.com/influxdb/v2/",
  "tutorial": "https://docs.influxdata.com/influxdb/v2/get-started/",
  "added": "2026-06-14",
  "dex": 143
 },
 {
  "id": "litedb",
  "name": "LiteDB",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a SQLite database but designed for .NET applications, storing data in JSON-like documents instead of rigid tables.",
  "description": "LiteDB is a lightweight, serverless, self-contained .NET document database that runs embedded in applications without requiring a separate database server. It supports LINQ queries, transactions, and full-text search while maintaining minimal resource footprint.",
  "use_cases": [
   "Desktop and mobile applications requiring local data persistence",
   "Embedded analytics and reporting in standalone applications",
   "Development and testing environments with minimal infrastructure",
   "IoT and edge computing scenarios with limited resources"
  ],
  "used_by": [
   "Jetbrains",
   ".NET developers",
   "Enterprise desktop applications"
  ],
  "docs": "https://www.litedb.org/",
  "tutorial": "https://www.litedb.org/docs/getting-started/",
  "added": "2026-07-25",
  "dex": 144
 },
 {
  "id": "litestream",
  "name": "Litestream",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Litestream is like having a backup camera that constantly watches your SQLite database and sends copies to the cloud so you never lose data.",
  "description": "Litestream is a streaming replication tool for SQLite that continuously backs up your database to cloud storage (S3, Azure, GCS). It enables you to run SQLite in production with disaster recovery and high availability.",
  "use_cases": [
   "Replicating SQLite databases to cloud storage for disaster recovery",
   "Running stateful applications on serverless platforms with persistent storage",
   "Implementing read replicas of SQLite databases",
   "Migrating from traditional databases to SQLite with safety guarantees"
  ],
  "used_by": [
   "Fly.io",
   "Render",
   "Railway"
  ],
  "docs": "https://litestream.io/",
  "tutorial": "https://litestream.io/getting-started/",
  "added": "2026-07-06",
  "dex": 145
 },
 {
  "id": "meilisearch",
  "name": "Meilisearch",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a super-fast search box on a website that shows you results as you type, giving you instant answers without making you wait.",
  "description": "Meilisearch is a lightweight, open-source search engine optimized for relevant, typo-tolerant search experiences with minimal configuration. It's widely used in production for product search, documentation search, and real-time filtering.",
  "use_cases": [
   "E-commerce product search and filtering",
   "Documentation and knowledge base search",
   "Real-time search with typo tolerance",
   "Full-text search across application data"
  ],
  "used_by": [
   "Vercel",
   "Mailchimp",
   "Shopify",
   "YCombinator"
  ],
  "docs": "https://docs.meilisearch.com/",
  "tutorial": "https://docs.meilisearch.com/learn/getting_started/installation.html",
  "added": "2026-06-21",
  "dex": 146
 },
 {
  "id": "sqlserver",
  "name": "Microsoft SQL Server",
  "category": "Platform",
  "domain": "Database",
  "eli5": "SQL Server is like a giant organized filing cabinet that lets you ask questions about all your files and get answers super fast.",
  "description": "Microsoft SQL Server is a relational database management system that stores structured data and supports complex queries, transactions, and analytics at scale. It's widely used in enterprise environments for mission-critical applications and data warehousing.",
  "use_cases": [
   "Enterprise transactional systems and OLTP workloads",
   "Business intelligence and data warehouse analytics",
   "Financial services and banking applications",
   "Healthcare and medical records management"
  ],
  "used_by": [
   "Microsoft",
   "Bank of America",
   "Wells Fargo",
   "IBM"
  ],
  "docs": "https://learn.microsoft.com/en-us/sql/sql-server/",
  "tutorial": "https://learn.microsoft.com/en-us/sql/t-sql/tutorial-writing-transact-sql-statements",
  "added": "2026-09-26",
  "dex": 147
 },
 {
  "id": "milvus",
  "name": "Milvus",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's a super smart filing cabinet that remembers the meaning of things and can instantly find similar items, perfect for AI applications.",
  "description": "Milvus is an open-source vector database designed to store, index, and search vector embeddings at massive scale. It enables semantic search and similarity matching for AI/ML applications, supporting billions of vectors with millisecond latency.",
  "use_cases": [
   "Building semantic search systems for document retrieval",
   "Implementing RAG (Retrieval-Augmented Generation) pipelines for LLMs",
   "Content recommendation engines using embedding similarity",
   "Image and video similarity search at production scale"
  ],
  "used_by": [
   "Shopify",
   "Alibaba",
   "Hugging Face",
   "Alibaba Cloud"
  ],
  "docs": "https://milvus.io/docs",
  "tutorial": "https://milvus.io/docs/quickstart.md",
  "added": "2026-06-14",
  "dex": 148
 },
 {
  "id": "milvus-cloud-managed",
  "name": "Milvus Cloud (Managed)",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a library assistant that instantly finds books most similar to the one you're thinking of, but without needing to understand what the books are about.",
  "description": "Milvus Cloud is a managed vector database service that specializes in similarity search and semantic understanding at scale. It powers AI applications requiring vector embeddings, semantic search, and retrieval-augmented generation (RAG).",
  "use_cases": [
   "Retrieval-augmented generation (RAG) systems",
   "Semantic search across documents and images",
   "Recommendation systems based on embeddings",
   "AI-powered similarity matching for products or content"
  ],
  "used_by": [
   "Databricks",
   "Spotify",
   "Alibaba",
   "ByteDance"
  ],
  "docs": "https://docs.milvus.io/",
  "tutorial": "https://milvus.io/docs/quickstart.md",
  "added": "2026-08-24",
  "dex": 149
 },
 {
  "id": "milvus-standalone",
  "name": "Milvus Standalone",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Milvus Standalone is like a filing cabinet that's really good at remembering what things look like and quickly finding similar-looking items among millions of files.",
  "description": "Milvus Standalone is a lightweight, open-source vector database optimized for similarity search and AI applications, deployable as a single-node instance. It's increasingly used for powering semantic search and RAG systems in production.",
  "use_cases": [
   "Building semantic search engines for documentation",
   "Powering recommendation systems with vector embeddings",
   "Implementing retrieval-augmented generation (RAG) for LLM applications",
   "Similarity matching for images and media content"
  ],
  "used_by": [
   "Canva",
   "Shopify",
   "Hugging Face",
   "Alibaba"
  ],
  "docs": "https://milvus.io/docs/standalone_deploy.md",
  "tutorial": "https://milvus.io/docs/quickstart.md",
  "added": "2026-08-01",
  "dex": 150
 },
 {
  "id": "milvus-vector-db",
  "name": "Milvus Vector Database",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Milvus is like a super-smart filing system that finds similar pictures or ideas really fast, instead of looking at every single file one by one.",
  "description": "Milvus is an open-source vector database optimized for similarity search and AI applications, enabling scalable semantic search and retrieval-augmented generation (RAG) systems. It handles billions of vectors with millisecond latency across distributed deployments.",
  "use_cases": [
   "Building RAG (Retrieval-Augmented Generation) systems for LLMs",
   "Semantic search in recommendation systems",
   "Image and video similarity matching at scale",
   "Powering AI-powered search and personalization features"
  ],
  "used_by": [
   "Alibaba",
   "eBay",
   "Shopee",
   "TikTok"
  ],
  "docs": "https://milvus.io/docs",
  "tutorial": "https://milvus.io/docs/quickstart.md",
  "added": "2026-06-29",
  "dex": 151
 },
 {
  "id": "mongodb",
  "name": "MongoDB",
  "category": "Tool",
  "domain": "Database",
  "description": "Leading document-oriented NoSQL database. Stores data as flexible BSON documents (JSON-like). Schema-less by default, enabling rapid iteration. Horizontal sharding, replica sets, and rich aggregation pipeline built-in.",
  "use_cases": [
   "Flexible schema storage for rapidly evolving application data models",
   "Content management: catalogs, user profiles, event logs",
   "Real-time analytics with aggregation pipelines",
   "Geospatial queries with 2dsphere indexes",
   "Atlas Search (Lucene-backed) for full-text search without Elasticsearch",
   "Multi-document ACID transactions for complex business operations"
  ],
  "used_by": [
   "Uber",
   "eBay",
   "Forbes",
   "SEGA",
   "Adobe",
   "Verizon"
  ],
  "docs": "https://www.mongodb.com/docs/",
  "tutorial": "https://www.mongodb.com/docs/manual/tutorial/getting-started/",
  "added": "2026-06-08",
  "eli5": "A junk drawer where you shove data in any shape with no rules.",
  "dex": 152
 },
 {
  "id": "mysql",
  "name": "MySQL",
  "category": "Tool",
  "domain": "Database",
  "description": "World's most popular open-source relational database. Known for reliability, ease of use, and massive ecosystem. Powers much of the web (LAMP stack). InnoDB storage engine provides ACID transactions and row-level locking.",
  "use_cases": [
   "Web application backends (WordPress, Magento, countless SaaS apps)",
   "Read-heavy workloads with read replicas and connection pooling (ProxySQL)",
   "OLTP transactional workloads with InnoDB",
   "MySQL Group Replication / InnoDB Cluster for HA",
   "Managed via AWS RDS, GCP Cloud SQL, PlanetScale"
  ],
  "used_by": [
   "Facebook",
   "Twitter",
   "YouTube",
   "Netflix",
   "Shopify",
   "Booking.com"
  ],
  "docs": "https://dev.mysql.com/doc/",
  "tutorial": "https://dev.mysql.com/doc/mysql-getting-started/en/",
  "added": "2026-06-08",
  "eli5": "The world's most popular filing cabinet for websites.",
  "dex": 153
 },
 {
  "id": "neo4j",
  "name": "Neo4j",
  "category": "Tool",
  "domain": "Database",
  "description": "Leading graph database — stores data as nodes and relationships rather than tables or documents. Cypher query language for expressive graph traversals. Ideal when relationships between entities are as important as the entities themselves.",
  "use_cases": [
   "Fraud detection: identifying suspicious transaction rings and identity clusters",
   "Knowledge graphs and recommendation engines (collaborative filtering)",
   "Network and IT infrastructure dependency mapping",
   "Identity and access management graphs (who has access to what via what path)",
   "Social network analysis and community detection"
  ],
  "used_by": [
   "eBay",
   "Adobe",
   "Airbus",
   "NASA",
   "UBS",
   "Volvo"
  ],
  "docs": "https://neo4j.com/docs/",
  "tutorial": "https://neo4j.com/docs/getting-started/",
  "added": "2026-06-08",
  "eli5": "A map of who knows who — great for tracing connections between things.",
  "dex": 154
 },
 {
  "id": "neon",
  "name": "Neon",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Neon is like a PostgreSQL database that lives in the cloud and automatically grows or shrinks based on how much data you store, so you only pay for what you use.",
  "description": "Neon is a serverless PostgreSQL platform that provides automatic scaling, branching, and managed infrastructure. It offers a developer-friendly alternative to managing PostgreSQL instances with features like instant provisioning and connection pooling.",
  "use_cases": [
   "Building serverless web applications with PostgreSQL",
   "Development and staging database management",
   "Low-cost relational database hosting for startups",
   "Applications requiring automatic scaling without capacity planning"
  ],
  "used_by": [
   "Vercel",
   "Retool",
   "Cal.com",
   "Imploy"
  ],
  "docs": "https://neon.tech/docs/introduction",
  "tutorial": "https://neon.tech/docs/quickstart/console",
  "added": "2026-06-15",
  "dex": 155
 },
 {
  "id": "neon-autoscaling",
  "name": "Neon Autoscaling",
  "category": "Platform",
  "domain": "Database",
  "eli5": "A database that automatically grows and shrinks like an accordion depending on how busy it is, so you only pay for what you use.",
  "description": "Neon Autoscaling automatically adjusts compute resources based on demand, scaling database capacity up for traffic spikes and down during quiet periods. It enables cost-efficient PostgreSQL deployments with zero manual capacity planning.",
  "use_cases": [
   "SaaS applications with variable traffic patterns",
   "Development and staging environments with unpredictable usage",
   "Event-driven applications with bursty workloads",
   "Multi-tenant systems with per-customer resource isolation"
  ],
  "used_by": [
   "Vercel",
   "Together AI",
   "Retool",
   "Twelve Labs"
  ],
  "docs": "https://neon.tech/docs/guides/autoscaling",
  "tutorial": "https://neon.tech/docs/introduction",
  "added": "2026-08-10",
  "dex": 156
 },
 {
  "id": "neon-branching",
  "name": "Neon Branching",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like making instant copies of your entire database so you can safely test changes without touching the real one.",
  "description": "Neon provides serverless PostgreSQL with built-in branching capabilities, enabling developers to create isolated database copies for testing, staging, and feature development. Each branch is a full, independent database instance that can be created and destroyed instantly.",
  "use_cases": [
   "Preview environments for pull requests",
   "Running parallel integration tests",
   "Safe database schema migrations",
   "Feature development in isolated database environments"
  ],
  "used_by": [
   "Vercel",
   "Retool",
   "Raycast",
   "Linear"
  ],
  "docs": "https://neon.tech/docs/guides/branching",
  "tutorial": "https://neon.tech/docs/introduction",
  "added": "2026-07-12",
  "dex": 157
 },
 {
  "id": "neon-pooler",
  "name": "Neon Pooler",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a receptionist who groups visitors into elevators instead of sending each person up separately, saving everyone time.",
  "description": "Neon Pooler is a serverless PostgreSQL connection pooler that reduces connection overhead for high-concurrency workloads. It abstracts the complexity of managing database connections across serverless functions and microservices.",
  "use_cases": [
   "Handling thousands of concurrent serverless function connections to PostgreSQL",
   "Reducing latency and cost of serverless database access",
   "Auto-scaling applications without hitting connection limits",
   "Multi-tenant applications with isolated connection pools per tenant"
  ],
  "used_by": [
   "Vercel",
   "Replit",
   "Supabase partners",
   "Serverless startups"
  ],
  "docs": "https://neon.tech/docs/guides/connection-pooling/",
  "tutorial": "https://neon.tech/blog/how-to-use-pooling-with-serverless-functions/",
  "added": "2026-09-09",
  "dex": 158
 },
 {
  "id": "neon-postgres",
  "name": "Neon Postgres",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Neon Postgres is like a magic notebook that automatically saves copies of your work and can grow bigger whenever you need more pages, and you only pay for the pages you actually use.",
  "description": "Neon is a serverless PostgreSQL platform that offers instant provisioning, branching for development, and consumption-based pricing. It eliminates the need to manage database infrastructure while maintaining full PostgreSQL compatibility.",
  "use_cases": [
   "Building serverless applications without managing database capacity",
   "Creating isolated development and staging environments with database branching",
   "Reducing database infrastructure costs with auto-scaling and pay-per-use pricing",
   "Rapid prototyping and MVP development with instant PostgreSQL setup"
  ],
  "used_by": [
   "Vercel",
   "Supabase",
   "Stripe",
   "Hasura"
  ],
  "docs": "https://neon.tech/docs",
  "tutorial": "https://neon.tech/docs/get-started-with-neon",
  "added": "2026-07-17",
  "dex": 159
 },
 {
  "id": "neon-serverless",
  "name": "Neon Serverless Driver",
  "category": "Tool",
  "domain": "Database",
  "eli5": "The Neon Serverless Driver is like having a taxi waiting outside your house that only charges you when you actually use it, instead of paying for a car sitting in your garage all day.",
  "description": "Neon's serverless driver provides a low-latency, low-overhead JavaScript/TypeScript driver for PostgreSQL that's optimized for serverless and edge computing environments. It eliminates connection pooling overhead while maintaining compatibility with standard PostgreSQL connections.",
  "use_cases": [
   "Serverless function databases (AWS Lambda, Vercel, Cloudflare Workers)",
   "Edge computing applications requiring millisecond connection times",
   "Cost-optimized development and staging environments",
   "Ephemeral compute environments with unpredictable workloads"
  ],
  "used_by": [
   "Vercel",
   "Cloudflare",
   "Supabase",
   "Replicate"
  ],
  "docs": "https://neon.tech/docs/serverless/serverless-driver",
  "tutorial": "https://neon.tech/docs/serverless/serverless-driver-tutorial",
  "added": "2026-08-03",
  "dex": 160
 },
 {
  "id": "neon-serverless-postgres",
  "name": "Neon Serverless Postgres",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like having a PostgreSQL database that magically appears when you need it and disappears when you don't, so you only pay for what you actually use.",
  "description": "Neon is a serverless PostgreSQL platform that auto-scales and branches like Git, enabling developers to build with PostgreSQL without managing infrastructure. It provides instant provisioning, automated scaling, and database branching for development workflows.",
  "use_cases": [
   "Building serverless applications with PostgreSQL backend",
   "Creating database branches for feature development and testing",
   "Reducing database infrastructure costs with auto-scaling",
   "Enabling rapid prototyping without infrastructure setup"
  ],
  "used_by": [
   "Vercel",
   "Prisma",
   "Supabase developers"
  ],
  "docs": "https://neon.tech/docs",
  "tutorial": "https://neon.tech/docs/get-started-with-neon",
  "added": "2026-07-10",
  "dex": 161
 },
 {
  "id": "opensearch",
  "name": "OpenSearch",
  "category": "Tool",
  "domain": "Database",
  "eli5": "OpenSearch is like a free, open version of Elasticsearch — same powerful search engine, but community-owned and without licensing restrictions.",
  "description": "Community-driven fork of Elasticsearch and Kibana, maintained by AWS under the Apache 2.0 license. Provides full-text search, analytics, and observability dashboards.",
  "use_cases": [
   "Log analytics and SIEM (open-source ELK replacement)",
   "Full-text application search",
   "Security analytics and threat detection",
   "Observability dashboards with OpenSearch Dashboards"
  ],
  "used_by": [
   "AWS (Amazon OpenSearch Service)",
   "SAP",
   "Aiven"
  ],
  "docs": "https://opensearch.org/docs/latest/",
  "tutorial": "https://opensearch.org/docs/latest/getting-started/",
  "added": "2026-06-12",
  "dex": 162
 },
 {
  "id": "opensearch-neural",
  "name": "OpenSearch Neural Search",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like having a really smart librarian who understands what you mean, not just the exact words you use, to find the best books.",
  "description": "OpenSearch Neural Search enables semantic and vector search capabilities within OpenSearch, allowing similarity-based queries powered by machine learning models. It bridges traditional full-text search with modern AI-driven vector embeddings for more intelligent retrieval.",
  "use_cases": [
   "Building semantic search engines for e-commerce product discovery",
   "Implementing vector-based document retrieval for RAG systems",
   "Creating intelligent recommendation systems based on content similarity",
   "Enabling cross-lingual search without translation"
  ],
  "used_by": [
   "AWS",
   "Lyft",
   "eBay",
   "Salesforce"
  ],
  "docs": "https://opensearch.org/docs/latest/search-plugins/neural-search/",
  "tutorial": "https://opensearch.org/docs/latest/search-plugins/neural-search/neural-search-intro/",
  "added": "2026-09-01",
  "dex": 163
 },
 {
  "id": "opensearch-vector-db",
  "name": "OpenSearch Vector Engine",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a librarian who can find books not just by their title, but by understanding what they're about and finding similar ones.",
  "description": "OpenSearch's vector database capability enables semantic search and similarity matching for unstructured data like text and images. It integrates native vector indexing with full-text search, making it ideal for RAG applications and hybrid search workloads.",
  "use_cases": [
   "Building semantic search for documentation and knowledge bases",
   "Implementing retrieval-augmented generation (RAG) pipelines",
   "Hybrid search combining keyword and vector-based queries",
   "Product recommendation systems using embeddings"
  ],
  "used_by": [
   "DuckDuckGo",
   "Lyft",
   "Capital One"
  ],
  "docs": "https://opensearch.org/docs/latest/search-plugins/vector-search/",
  "tutorial": "https://opensearch.org/blog/semantic-search/",
  "added": "2026-07-27",
  "dex": 164
 },
 {
  "id": "opensearch-vector-search",
  "name": "OpenSearch Vector Search",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a librarian who doesn't just match exact words but understands the meaning of what you're looking for and finds similar books.",
  "description": "OpenSearch Vector Search enables semantic search and similarity matching using vector embeddings, allowing applications to find contextually relevant results rather than just keyword matches. It integrates vector databases with full-text search capabilities.",
  "use_cases": [
   "Semantic search in e-commerce and content discovery",
   "RAG (Retrieval-Augmented Generation) systems",
   "Recommendation engines and personalization",
   "Image and document similarity matching"
  ],
  "used_by": [
   "Netflix",
   "Amazon",
   "Expedia",
   "Stripe"
  ],
  "docs": "https://opensearch.org/docs/latest/search-plugins/vector-search/",
  "tutorial": "https://opensearch.org/blog/improving-search-with-vector-search/",
  "added": "2026-07-01",
  "dex": 165
 },
 {
  "id": "opensearch-ai-powered-search",
  "name": "OpenSearch with AI-Powered Search",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a super-smart search engine that understands what you mean, not just the exact words you type.",
  "description": "OpenSearch integrates vector search and neural search capabilities for semantic understanding of queries and documents. It combines traditional full-text search with machine learning models for more intelligent search results.",
  "use_cases": [
   "Implementing semantic search on documentation and knowledge bases",
   "Building recommendation systems using vector embeddings",
   "Enhancing e-commerce product search with AI understanding",
   "Log analysis and anomaly detection with neural search"
  ],
  "used_by": [
   "Amazon",
   "Netflix",
   "Workiva",
   "JPMorgan Chase"
  ],
  "docs": "https://opensearch.org/docs/latest/search-plugins/neural-search/",
  "tutorial": "https://opensearch.org/docs/latest/search-plugins/neural-search/",
  "added": "2026-09-22",
  "dex": 166
 },
 {
  "id": "pgadmin",
  "name": "pgAdmin",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like a remote control for your PostgreSQL database that lets you see and change everything from a web browser.",
  "description": "pgAdmin is a feature-rich open-source management and development platform for PostgreSQL. It provides a web-based GUI for administering PostgreSQL databases, running queries, and managing users across development and production environments.",
  "use_cases": [
   "Database administration and user management for PostgreSQL",
   "Visual query building and SQL script execution",
   "Backup and restoration of PostgreSQL databases",
   "Performance monitoring and optimization of database operations"
  ],
  "used_by": [
   "Zalando",
   "Booking.com",
   "Spotify",
   "IBM"
  ],
  "docs": "https://www.pgadmin.org/docs/",
  "tutorial": "https://www.pgadmin.org/docs/pgadmin4/latest/getting_started/index.html",
  "added": "2026-07-28",
  "dex": 167
 },
 {
  "id": "pglite",
  "name": "PGlite",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like having a tiny PostgreSQL database that lives inside your web browser, so your app can work even without internet.",
  "description": "PGlite is a lightweight, in-process PostgreSQL database compiled to WebAssembly that runs in browsers and edge environments. It provides full SQL compatibility without requiring a separate database server.",
  "use_cases": [
   "Building offline-first web applications with local persistence",
   "Edge computing and serverless database requirements",
   "Progressive web apps that need relational data without network dependency",
   "Development and testing environments without infrastructure overhead"
  ],
  "used_by": [
   "Vercel",
   "Expo",
   "Replit"
  ],
  "docs": "https://pglite.dev/",
  "tutorial": "https://pglite.dev/docs/getting-started",
  "added": "2026-08-04",
  "dex": 168
 },
 {
  "id": "postgres-ai-extension",
  "name": "pgvector",
  "category": "Tool",
  "domain": "Database",
  "eli5": "pgvector is like giving PostgreSQL the ability to understand pictures and text in a way that helps you find similar things really fast.",
  "description": "pgvector is a PostgreSQL extension enabling vector similarity search and embedding storage directly within the database. It allows AI/ML applications to perform semantic searches without external vector databases.",
  "use_cases": [
   "Semantic search in RAG applications",
   "Recommendation systems based on embeddings",
   "Image and document similarity matching",
   "Hybrid search combining SQL and vector queries"
  ],
  "used_by": [
   "OpenAI",
   "Vercel",
   "Perplexity AI",
   "Supabase"
  ],
  "docs": "https://github.com/pgvector/pgvector",
  "tutorial": "https://supabase.com/docs/guides/database/extensions/pgvector",
  "added": "2026-06-24",
  "dex": 169
 },
 {
  "id": "pinecone",
  "name": "Pinecone",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Pinecone is like a filing cabinet that stores meaning instead of words — you search by describing a concept and it finds everything similar, not just exact matches.",
  "description": "Managed vector database for storing and querying high-dimensional embeddings. Enables semantic search, recommendation engines, and RAG pipelines at scale.",
  "use_cases": [
   "Retrieval-augmented generation (RAG) for LLMs",
   "Semantic similarity search",
   "Recommendation and personalization systems",
   "Image and document similarity matching"
  ],
  "used_by": [
   "Hubspot",
   "Zapier",
   "monday.com",
   "Notion"
  ],
  "docs": "https://docs.pinecone.io/",
  "tutorial": "https://docs.pinecone.io/guides/get-started/quickstart",
  "added": "2026-06-12",
  "dex": 170
 },
 {
  "id": "postgresql",
  "name": "PostgreSQL",
  "category": "Tool",
  "domain": "Database",
  "description": "Advanced open-source relational database with strong ACID compliance, rich SQL support, and an extensible type system. Often called 'the world's most advanced open source database'. Supports JSON, full-text search, geospatial (PostGIS), and custom extensions.",
  "use_cases": [
   "Relational data storage with complex joins, transactions, and constraints",
   "JSONB columns for hybrid relational/document workloads",
   "Full-text search with tsvector/tsquery (no separate search engine needed)",
   "Geospatial queries with PostGIS extension",
   "Time-series data via TimescaleDB extension",
   "Logical replication for read replicas and CDC pipelines"
  ],
  "used_by": [
   "Instagram",
   "Spotify",
   "Reddit",
   "Twitch",
   "Apple",
   "Stripe"
  ],
  "docs": "https://www.postgresql.org/docs/",
  "tutorial": "https://www.postgresql.org/docs/current/tutorial.html",
  "added": "2026-06-08",
  "eli5": "A very organized filing cabinet with strict rules — everything in the right drawer.",
  "dex": 171
 },
 {
  "id": "qdrant",
  "name": "Qdrant",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Qdrant is like a super-smart library that organizes information by meaning instead of alphabetically, so it finds similar ideas really fast.",
  "description": "A vector database optimized for storing and searching high-dimensional embeddings with sub-millisecond latency at scale. It enables similarity search, recommendation systems, and semantic retrieval for AI/ML applications with built-in filtering and scalability.",
  "use_cases": [
   "Powering semantic search and RAG (Retrieval-Augmented Generation) systems",
   "Building recommendation engines based on content similarity",
   "Implementing vector similarity search for image and text matching",
   "Scaling embeddings-based applications from development to production"
  ],
  "used_by": [
   "Booking.com",
   "Activision",
   "Deutsche Bank"
  ],
  "docs": "https://qdrant.tech/documentation/",
  "tutorial": "https://qdrant.tech/documentation/quick-start/",
  "added": "2026-06-27",
  "dex": 172
 },
 {
  "id": "qdrant-vector-db",
  "name": "Qdrant Vector Database",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a library that's organized by meaning instead of alphabetical order, so you can find things that are similar to what you're looking for.",
  "description": "Qdrant is a vector database optimized for similarity search and AI applications, offering high-throughput nearest neighbor search with payload filtering and real-time updates. It's built in Rust and supports both in-memory and disk-based storage modes.",
  "use_cases": [
   "Semantic search and recommendation engines for AI applications",
   "Embedding storage and retrieval for LLM-powered chatbots",
   "Image and multimodal similarity search applications",
   "Real-time personalization and content discovery systems"
  ],
  "used_by": [
   "Y Combinator companies",
   "enterprise AI teams",
   "research organizations"
  ],
  "docs": "https://qdrant.tech/documentation/",
  "tutorial": "https://qdrant.tech/documentation/quick-start/",
  "added": "2026-09-17",
  "dex": 173
 },
 {
  "id": "redis",
  "name": "Redis",
  "category": "Tool",
  "domain": "Database",
  "description": "In-memory data structure store used as cache, message broker, and database. Sub-millisecond latency with rich data types (strings, hashes, lists, sets, sorted sets, streams). Persistence via RDB snapshots and AOF log.",
  "use_cases": [
   "Caching: session store, query result cache, CDN token cache",
   "Pub/Sub and lightweight message queuing (Redis Streams for durable queues)",
   "Leaderboards and real-time counters with sorted sets",
   "Distributed locks (Redlock algorithm) for cross-service coordination",
   "Rate limiting with sliding window counters",
   "Vector similarity search (Redis Stack) for AI/ML applications"
  ],
  "used_by": [
   "Twitter",
   "GitHub",
   "Snapchat",
   "Craigslist",
   "Stack Overflow",
   "Pinterest"
  ],
  "docs": "https://redis.io/docs/",
  "tutorial": "https://redis.io/docs/getting-started/",
  "added": "2026-06-08",
  "eli5": "A whiteboard next to your desk — super fast to read but wiped if unplugged.",
  "dex": 174
 },
 {
  "id": "redis-search",
  "name": "Redis Search",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like having a super-fast librarian who can instantly find books by any word in them, not just the title.",
  "description": "Redis Search is a queryable secondary index module for Redis that enables full-text search, vector search, and real-time analytics over data stored in Redis. It combines the speed of Redis with advanced search capabilities without requiring separate search infrastructure.",
  "use_cases": [
   "Real-time full-text search on user-generated content",
   "Vector similarity search for recommendation engines",
   "Fast autocomplete and search-as-you-type functionality",
   "Real-time analytics and aggregations on streaming data"
  ],
  "used_by": [
   "Databricks",
   "Okta",
   "Adobe"
  ],
  "docs": "https://redis.io/docs/stack/search/",
  "tutorial": "https://redis.io/docs/stack/search/tutorials/",
  "added": "2026-08-20",
  "dex": 175
 },
 {
  "id": "rocksdb",
  "name": "RocksDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "RocksDB is like a super-fast personal filing cabinet that stores and retrieves data at lightning speed, even when your computer has limited resources.",
  "description": "RocksDB is a high-performance embedded key-value storage library developed by Meta, optimized for fast access patterns and high throughput on modern hardware. It's widely used as the foundation for databases and storage systems that need low-latency access to large datasets.",
  "use_cases": [
   "Embedded storage engine for distributed databases",
   "High-performance caching layers in production systems",
   "Time-series database backends requiring fast writes",
   "State management in stream processing and blockchain systems"
  ],
  "used_by": [
   "Meta",
   "LinkedIn",
   "MongoDB",
   "CockroachDB"
  ],
  "docs": "https://rocksdb.org/docs/getting-started.html",
  "tutorial": "https://rocksdb.org/docs/basic-operations.html",
  "added": "2026-08-18",
  "dex": 176
 },
 {
  "id": "scylladb",
  "name": "ScyllaDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "ScyllaDB is like Cassandra on steroids — it does the same distributed database job but squeezes 10x more speed out of your hardware.",
  "description": "High-performance NoSQL database compatible with Apache Cassandra and DynamoDB APIs. Written in C++ using the Seastar framework for predictable low latency.",
  "use_cases": [
   "High-throughput time-series and IoT data",
   "Drop-in Cassandra replacement for latency-sensitive apps",
   "Ad tech and real-time personalization",
   "Gaming leaderboards and session stores"
  ],
  "used_by": [
   "Discord",
   "Comcast",
   "Palo Alto Networks",
   "FireEye"
  ],
  "docs": "https://docs.scylladb.com/",
  "tutorial": "https://docs.scylladb.com/stable/getting-started/",
  "added": "2026-06-12",
  "dex": 177
 },
 {
  "id": "sql",
  "name": "SQL",
  "category": "Language",
  "domain": "Database",
  "eli5": "A way to ask questions to a database, like 'show me all red toys sorted by size'.",
  "description": "Structured Query Language — the universal language for querying and manipulating relational databases. Declarative: you describe what you want, the database engine figures out how. Every major database (Postgres, MySQL, Snowflake, BigQuery) speaks SQL.",
  "use_cases": [
   "Querying, filtering, and aggregating data with SELECT, WHERE, GROUP BY",
   "Joining multiple tables to answer cross-entity questions",
   "Data transformation and reporting in warehouses (Snowflake, BigQuery, Redshift)",
   "Schema definition and migrations with DDL (CREATE, ALTER, DROP)",
   "Analytics engineering with dbt — models are just SQL SELECT statements"
  ],
  "used_by": [
   "Every company that stores data"
  ],
  "docs": "https://www.postgresql.org/docs/current/sql.html",
  "tutorial": "https://sqlzoo.net/wiki/SQL_Tutorial",
  "added": "2026-06-08",
  "dex": 178
 },
 {
  "id": "supabase",
  "name": "Supabase",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Supabase is like Firebase but built on Postgres — you get a real database, auto-generated APIs, auth, and file storage all wired up out of the box.",
  "description": "Open-source Firebase alternative built on PostgreSQL. Provides auto-generated REST and GraphQL APIs, realtime subscriptions, auth, edge functions, and S3-compatible storage.",
  "use_cases": [
   "Full-stack apps with instant REST/GraphQL APIs",
   "Realtime collaboration features via Postgres replication",
   "Auth with social login, MFA, and RLS policies",
   "Self-hostable Firebase replacement"
  ],
  "used_by": [
   "Mozilla",
   "PwC",
   "Pika",
   "Mobbin"
  ],
  "docs": "https://supabase.com/docs",
  "tutorial": "https://supabase.com/docs/guides/getting-started",
  "added": "2026-06-14",
  "dex": 179
 },
 {
  "id": "supabase-vector-search",
  "name": "Supabase Vector (pgvector)",
  "category": "Tool",
  "domain": "Database",
  "eli5": "It's like having a librarian who can find books not just by their title, but by how similar they are in meaning to what you're looking for.",
  "description": "Supabase's integration of pgvector extension for PostgreSQL enabling semantic search and vector similarity queries within a managed database. It powers AI-driven search features, recommendation engines, and similarity detection systems.",
  "use_cases": [
   "Semantic search for documents and content",
   "Recommendation systems based on embeddings",
   "AI-powered chatbot memory and retrieval",
   "Image and product similarity matching"
  ],
  "used_by": [
   "Supabase",
   "Vercel",
   "Hugging Face",
   "Replit"
  ],
  "docs": "https://supabase.com/docs/guides/database/extensions/pgvector",
  "tutorial": "https://supabase.com/docs/guides/database/vector-search",
  "added": "2026-07-04",
  "dex": 180
 },
 {
  "id": "surrealdb",
  "name": "SurrealDB",
  "category": "Platform",
  "domain": "Database",
  "eli5": "SurrealDB is like a super-smart filing cabinet that can store and find information using different languages and works everywhere from your phone to the cloud.",
  "description": "SurrealDB is a multi-model, edge-first database that supports SQL queries, graph relationships, and document storage all in one. It can run from the edge to cloud deployments with built-in authentication and real-time capabilities.",
  "use_cases": [
   "Building edge-first applications with offline-first sync",
   "Graph-based relationship queries on document data",
   "Real-time collaborative applications",
   "IoT and mobile backend with cloud synchronization"
  ],
  "used_by": [
   "Datadog",
   "Freshworks",
   "Roblox",
   "Twitch"
  ],
  "docs": "https://surrealdb.com/docs",
  "tutorial": "https://surrealdb.com/learn",
  "added": "2026-06-18",
  "dex": 181
 },
 {
  "id": "tidb",
  "name": "TiDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "TiDB is like MySQL that never runs out of space — it automatically splits your data across many machines as it grows, with no downtime.",
  "description": "Open-source distributed SQL database compatible with MySQL protocol. Combines OLTP and OLAP in one system (HTAP), with horizontal scaling and strong consistency.",
  "use_cases": [
   "MySQL-compatible workloads that outgrow a single node",
   "HTAP: mixed analytical + transactional queries",
   "Financial applications requiring strong consistency",
   "Replacing sharded MySQL with a single endpoint"
  ],
  "used_by": [
   "PingCAP",
   "Shopee",
   "Square",
   "ZaloPay"
  ],
  "docs": "https://docs.pingcap.com/tidb/stable",
  "tutorial": "https://docs.pingcap.com/tidb/stable/quick-start-with-tidb",
  "added": "2026-06-14",
  "dex": 182
 },
 {
  "id": "timescaledb",
  "name": "TimescaleDB",
  "category": "Tool",
  "domain": "Database",
  "eli5": "TimescaleDB is like a regular notebook that got a special time-travel index — it can flip to any date's entries instantly without flipping through every page.",
  "description": "PostgreSQL extension purpose-built for time-series data. Adds automatic partitioning (hypertables), continuous aggregates, and compression while staying fully SQL-compatible.",
  "use_cases": [
   "IoT sensor data storage and querying",
   "Financial tick data and OHLCV aggregation",
   "Infrastructure metrics retention beyond Prometheus",
   "Event logging with complex analytical queries"
  ],
  "used_by": [
   "Hubspot",
   "Outbrain",
   "Teleport",
   "Volvo"
  ],
  "docs": "https://docs.timescale.com/",
  "tutorial": "https://docs.timescale.com/getting-started/latest/",
  "added": "2026-06-11",
  "dex": 183
 },
 {
  "id": "typesense",
  "name": "Typesense",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Typesense is like a super-fast librarian who instantly finds books matching what you're searching for, even if you misspell the title.",
  "description": "Typesense is an open-source search engine optimized for fast, typo-tolerant search experiences with minimal setup. It provides sub-50ms search latency and is designed as a drop-in alternative to Elasticsearch for modern search applications.",
  "use_cases": [
   "Powering e-commerce product search with typo tolerance",
   "Building instant-as-you-type search interfaces",
   "Full-text search for SaaS platforms and content management",
   "Geographic and faceted search for real estate and classified listings"
  ],
  "used_by": [
   "Uber Eats",
   "Adidas",
   "Airbnb",
   "Microsoft"
  ],
  "docs": "https://typesense.org/docs/",
  "tutorial": "https://typesense.org/docs/guide/",
  "added": "2026-08-01",
  "dex": 184
 },
 {
  "id": "vald-distributed-vector-db",
  "name": "Vald",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a librarian who can instantly find the book most similar to the one you're reading, even when searching through millions of books spread across many buildings.",
  "description": "Vald is a distributed vector database optimized for large-scale similarity search with high throughput and low latency, designed for production AI applications. It provides scalable nearest neighbor search across distributed nodes with automatic sharding and replication.",
  "use_cases": [
   "Large-scale image and video retrieval systems",
   "Real-time recommendation engines serving millions of users",
   "Semantic search in enterprise knowledge management systems",
   "Face recognition and biometric matching in security applications"
  ],
  "used_by": [
   "LINE",
   "Yahoo Japan",
   "Twitter (X)"
  ],
  "docs": "https://vald.readthedocs.io/",
  "tutorial": "https://vald.readthedocs.io/en/latest/tutorial/",
  "added": "2026-09-18",
  "dex": 185
 },
 {
  "id": "valkey",
  "name": "Valkey",
  "category": "Platform",
  "domain": "Database",
  "eli5": "Valkey is like a really fast notebook that remembers things temporarily, helping your app get quick answers without asking the slow book library every time.",
  "description": "Valkey is an open-source, high-performance data structure server that maintains Redis protocol compatibility while adding community-driven enhancements. It's a production-ready alternative to Redis for caching, sessions, and real-time analytics.",
  "use_cases": [
   "High-speed session management in web applications",
   "Real-time leaderboards and counters",
   "Distributed caching across microservices",
   "Pub/sub messaging and event streaming"
  ],
  "used_by": [
   "AWS",
   "Snap",
   "Samsung"
  ],
  "docs": "https://valkey.io/",
  "tutorial": "https://github.com/valkey-io/valkey/wiki/Getting-Started",
  "added": "2026-08-30",
  "dex": 186
 },
 {
  "id": "vitess",
  "name": "Vitess",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Vitess is like a smart traffic director for MySQL — it splits one giant database across many servers and makes them look like one to your app.",
  "description": "Database clustering system for horizontal scaling of MySQL. Provides sharding, connection pooling, and query routing. Powers YouTube's database layer.",
  "use_cases": [
   "Horizontal sharding for MySQL at scale",
   "Connection pooling to reduce MySQL overhead",
   "Zero-downtime schema migrations",
   "Multi-region MySQL topologies"
  ],
  "used_by": [
   "YouTube",
   "Slack",
   "GitHub",
   "Square"
  ],
  "docs": "https://vitess.io/docs/",
  "tutorial": "https://vitess.io/docs/get-started/local/",
  "added": "2026-06-12",
  "dex": 187
 },
 {
  "id": "weaviate",
  "name": "Weaviate",
  "category": "Tool",
  "domain": "Database",
  "eli5": "Weaviate is a database that understands meaning — you can store objects and search by what they're about, not just by their exact words.",
  "description": "Open-source vector database with built-in ML model inference. Supports hybrid search (vector + keyword), GraphQL API, and multi-tenancy for AI-native applications.",
  "use_cases": [
   "Semantic search over documents and images",
   "RAG pipelines with built-in embedding models",
   "Hybrid vector + BM25 search",
   "Multi-tenant AI applications"
  ],
  "used_by": [
   "Stack Overflow",
   "Instabase",
   "AstraZeneca"
  ],
  "docs": "https://weaviate.io/developers/weaviate",
  "tutorial": "https://weaviate.io/developers/weaviate/quickstart",
  "added": "2026-06-12",
  "dex": 188
 },
 {
  "id": "xata",
  "name": "Xata",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a smart filing cabinet that organizes your data, lets you search it super fast, and automatically backs everything up for you.",
  "description": "Xata is a serverless PostgreSQL platform with built-in vector search, full-text search, and file attachments, designed for developers who want database functionality without managing infrastructure. It combines relational data with modern search capabilities in a single managed service.",
  "use_cases": [
   "Building AI applications with integrated vector embeddings",
   "Creating full-text searchable content platforms",
   "Developing serverless applications without database ops overhead",
   "Multi-tenant SaaS applications with instant global distribution"
  ],
  "used_by": [
   "Vercel ecosystem",
   "Netlify startups",
   "Gumroad",
   "Framer"
  ],
  "docs": "https://xata.io/docs",
  "tutorial": "https://xata.io/docs/getting-started",
  "added": "2026-08-14",
  "dex": 189
 },
 {
  "id": "milvus-cloud",
  "name": "Zilliz Cloud (Milvus Cloud)",
  "category": "Platform",
  "domain": "Database",
  "eli5": "It's like a giant library that organizes millions of pictures by how similar they look, so you can instantly find the ones most like your photo.",
  "description": "Zilliz Cloud is a managed vector database platform purpose-built for AI applications, offering fully-hosted Milvus with enterprise features. It enables semantic search and similarity matching at scale for LLM applications, RAG systems, and AI-powered recommendation engines.",
  "use_cases": [
   "Semantic search over documents and embeddings",
   "Recommendation systems powered by vector similarity",
   "LLM retrieval-augmented generation (RAG) backends",
   "Image and audio similarity search applications"
  ],
  "used_by": [
   "Alibaba",
   "Tencent",
   "Microsoft",
   "Various AI startups"
  ],
  "docs": "https://docs.zilliz.com/docs",
  "tutorial": "https://docs.zilliz.com/docs/quick-start",
  "added": "2026-07-20",
  "dex": 190
 },
 {
  "id": "anthropic-bedrock",
  "name": "Amazon Bedrock",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a playground where you can play with different AI toys without having to buy each one separately.",
  "description": "Amazon Bedrock is a fully managed service that provides access to foundation models from various providers including Anthropic, Meta, and Cohere through a single API. It enables developers to build generative AI applications without managing infrastructure.",
  "use_cases": [
   "Building chatbots and conversational AI applications",
   "Text generation and content creation workflows",
   "Document analysis and summarization at scale",
   "Retrieval-augmented generation (RAG) systems"
  ],
  "used_by": [
   "Amazon",
   "Accenture",
   "BMW",
   "GE"
  ],
  "docs": "https://docs.aws.amazon.com/bedrock/",
  "tutorial": "https://aws.amazon.com/bedrock/getting-started/",
  "added": "2026-07-15",
  "dex": 191
 },
 {
  "id": "anthropic-ai",
  "name": "Anthropic AI",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like having a super smart robot assistant that can read, write, and think through problems just like a person would.",
  "description": "Anthropic is an AI safety company that develops Claude, a family of large language models designed with constitutional AI principles to be helpful, harmless, and honest. Claude powers conversational AI applications and is widely integrated into enterprise software and developer tools.",
  "use_cases": [
   "Building AI-powered chatbots and customer support systems",
   "Code generation and software development assistance",
   "Content analysis and document processing",
   "Research and knowledge synthesis tasks"
  ],
  "used_by": [
   "Notion",
   "DuckDuckGo",
   "Perplexity",
   "Scale AI"
  ],
  "docs": "https://docs.anthropic.com/",
  "tutorial": "https://docs.anthropic.com/getting-started/overview",
  "added": "2026-08-31",
  "dex": 192
 },
 {
  "id": "anthropic-batch-api",
  "name": "Anthropic Batch API",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A way to send lots of questions to Claude all at once in the evening and get all the answers back the next morning for cheaper than asking one question at a time.",
  "description": "The Anthropic Batch API processes large volumes of API requests asynchronously at 50% lower cost than standard pricing, ideal for non-time-sensitive workloads. It accepts batches of up to 10,000 requests and returns results within 24 hours.",
  "use_cases": [
   "Large-scale document processing and analysis",
   "Batch content generation for marketing and product teams",
   "Research and data extraction projects with delayed deadlines",
   "Cost-optimized model fine-tuning and training data preparation"
  ],
  "used_by": [
   "Notion",
   "Canva",
   "DuckDuckGo",
   "Scale AI"
  ],
  "docs": "https://docs.anthropic.com/batch/overview",
  "tutorial": "https://docs.anthropic.com/batch/getting-started-with-batch-api",
  "added": "2026-07-24",
  "dex": 193
 },
 {
  "id": "anthropic-claude",
  "name": "Anthropic Claude",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Claude is like a very smart robot friend you can talk to through code to ask questions, write things, or solve problems.",
  "description": "Claude is a large language model API by Anthropic that provides state-of-the-art natural language understanding and generation capabilities. It's widely used for building AI-powered applications, content generation, code analysis, and conversational interfaces.",
  "use_cases": [
   "Building AI chatbots and conversational assistants",
   "Code generation and software development assistance",
   "Content creation and summarization",
   "Automated code review and analysis"
  ],
  "used_by": [
   "Notion",
   "DuckDuckGo",
   "Bridgewater Associates",
   "Canva"
  ],
  "docs": "https://docs.anthropic.com/",
  "tutorial": "https://docs.anthropic.com/quickstart",
  "added": "2026-06-15",
  "dex": 194
 },
 {
  "id": "anthropic-claude-tokens",
  "name": "Anthropic Claude Tokens",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Claude Tokens are like coins you spend to use Claude's brain to read and write things for you.",
  "description": "A token-based billing and usage system for Anthropic's Claude API, enabling developers to measure and optimize API consumption. Tokens represent units of text processed by Claude models, with distinct pricing for input and output tokens.",
  "use_cases": [
   "Cost tracking and optimization for large-scale Claude API deployments",
   "Implementing token budgets and rate limiting in multi-tenant applications",
   "Monitoring API usage patterns and forecasting costs",
   "Building cost-aware applications that adapt query complexity based on token budgets"
  ],
  "used_by": [
   "Notion",
   "DuckDuckGo",
   "Zapier"
  ],
  "docs": "https://docs.anthropic.com/en/docs/about-claude/models/overview",
  "tutorial": "https://docs.anthropic.com/en/docs/guides/tokens",
  "added": "2026-06-27",
  "dex": 195
 },
 {
  "id": "anthropic-compute",
  "name": "Anthropic Compute",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like renting a super-powerful brain in the cloud that can think through really hard problems for you.",
  "description": "Anthropic Compute is an API platform that provides access to Claude models with extended thinking capabilities for complex reasoning tasks. It enables developers to leverage advanced AI reasoning without managing their own infrastructure.",
  "use_cases": [
   "Research analysis and literature review automation",
   "Complex problem-solving and algorithm design",
   "Mathematical proofs and verification",
   "Strategic planning and decision-making support"
  ],
  "used_by": [
   "Anthropic",
   "Y Combinator startups",
   "Enterprise research teams"
  ],
  "docs": "https://docs.anthropic.com/",
  "tutorial": "https://docs.anthropic.com/quickstart",
  "added": "2026-07-13",
  "dex": 196
 },
 {
  "id": "anthropic-files-api",
  "name": "Anthropic Files API",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like giving Claude a folder of documents to read before answering your questions, instead of having to paste everything in the message each time.",
  "description": "The Anthropic Files API enables developers to upload files (documents, images, PDFs) that Claude can reference across multiple API calls without re-uploading. This reduces latency, costs, and simplifies handling of large documents in production systems.",
  "use_cases": [
   "Processing large document collections for legal or financial analysis",
   "Enabling multi-turn conversations with persistent file context",
   "Building document search and Q&A applications",
   "Reducing API costs for applications that reference the same files repeatedly"
  ],
  "used_by": [
   "Scale AI",
   "Anthropic customers",
   "Enterprise AI teams"
  ],
  "docs": "https://docs.anthropic.com/en/docs/build-a-classifier#files-api",
  "tutorial": "https://docs.anthropic.com/en/docs/build-a-classifier",
  "added": "2026-07-25",
  "dex": 197
 },
 {
  "id": "anthropic-files-batch-api",
  "name": "Anthropic Files API with Batch Processing",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like giving Claude a stack of documents to read all at once, and then asking questions about all of them together instead of one at a time.",
  "description": "Anthropic's Files API enables efficient handling of document uploads and batch processing with Claude, allowing developers to process large volumes of files at lower cost with asynchronous batch operations. It's essential for document-heavy AI applications requiring cost optimization and throughput.",
  "use_cases": [
   "Batch processing of contract documents for legal analysis",
   "Large-scale research paper summarization and analysis",
   "Bulk customer feedback classification and sentiment analysis",
   "Document extraction and structured data generation from thousands of PDFs"
  ],
  "used_by": [
   "Scale AI",
   "Anthropic customers",
   "Enterprise document processing firms"
  ],
  "docs": "https://docs.anthropic.com/en/docs/build-a-chatbot-with-claude",
  "tutorial": "https://docs.anthropic.com/en/docs/build-a-chatbot-with-claude",
  "added": "2026-08-02",
  "dex": 198
 },
 {
  "id": "anthropic-sdk-go",
  "name": "Anthropic Go SDK",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a translator that lets your Go program talk to Claude AI without learning a foreign language.",
  "description": "The official Go SDK for Anthropic's Claude API, providing native integration for building AI-powered applications in Go. It handles streaming, tool use, and vision capabilities with idiomatic Go patterns.",
  "use_cases": [
   "Building CLI tools powered by Claude",
   "Backend services that integrate AI reasoning",
   "Real-time streaming AI responses in Go servers",
   "Vision-based document processing applications"
  ],
  "used_by": [
   "Go ecosystem developers",
   "Backend engineers at scale",
   "Infrastructure teams",
   "DevOps tool builders"
  ],
  "docs": "https://pkg.go.dev/github.com/anthropics/anthropic-sdk-go",
  "tutorial": "https://github.com/anthropics/anthropic-sdk-go#readme",
  "added": "2026-09-07",
  "dex": 199
 },
 {
  "id": "anthropic-model-context-protocol",
  "name": "Anthropic Model Context Protocol",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a universal translator that lets Claude talk to any tool on your computer, the same way a telephone operator connects calls between different phone systems.",
  "description": "An open protocol that enables Claude and other AI models to connect with external tools, databases, and services through a standardized interface. It simplifies integration between AI applications and enterprise systems by defining how models request information from and act on external resources.",
  "use_cases": [
   "Connecting Claude to enterprise databases for real-time data retrieval",
   "Building AI agents that can interact with APIs and internal tools",
   "Enabling models to access file systems and knowledge bases dynamically",
   "Creating reproducible AI workflows across different organizations"
  ],
  "used_by": [
   "Anthropic",
   "Replit",
   "DuckDB"
  ],
  "docs": "https://modelcontextprotocol.io/docs",
  "tutorial": "https://modelcontextprotocol.io/quickstart",
  "added": "2026-08-05",
  "dex": 200
 },
 {
  "id": "anthropic-prompt-caching",
  "name": "Anthropic Prompt Caching",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like remembering the beginning of a story so you don't have to read it again every time you ask a question about it.",
  "description": "Anthropic's prompt caching feature reduces latency and costs by caching large context windows in Claude API calls, allowing reuse of repeated prompts without reprocessing. This significantly improves efficiency for applications with long documents, codebase analysis, or multi-turn conversations.",
  "use_cases": [
   "Reducing token costs for large document analysis workflows",
   "Accelerating multi-turn conversations with consistent context",
   "Improving performance for codebase analysis and refactoring tasks",
   "Enabling real-time processing of large datasets with shared context"
  ],
  "used_by": [
   "Scale AI",
   "Retool",
   "Cursor"
  ],
  "docs": "https://docs.anthropic.com/en/docs/build-a-bot/caching",
  "tutorial": "https://www.anthropic.com/news/prompt-caching",
  "added": "2026-08-20",
  "dex": 201
 },
 {
  "id": "anthropic-sdk-ai",
  "name": "Anthropic Python SDK",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a translator that helps your computer talk to Claude, an AI assistant, just like how you'd use a phone to call your friend.",
  "description": "The official Python SDK for interacting with Anthropic's Claude models, providing developers with easy-to-use APIs for text generation, vision processing, and advanced features like prompt caching and batch processing.",
  "use_cases": [
   "Building AI-powered chatbots and conversational interfaces",
   "Implementing document analysis and content extraction",
   "Creating code generation and debugging assistants",
   "Automating customer support and content moderation workflows"
  ],
  "used_by": [
   "Replit",
   "DuckDuckGo",
   "Bridgewater Associates",
   "Scale AI"
  ],
  "docs": "https://docs.anthropic.com/",
  "tutorial": "https://github.com/anthropics/anthropic-sdk-python",
  "added": "2026-08-22",
  "dex": 202
 },
 {
  "id": "anthropic-sdk",
  "name": "Anthropic SDK",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a translator that helps your code talk to Claude the AI robot in a structured way.",
  "description": "Official SDK for integrating Claude AI models into applications with support for multiple programming languages including Python, Node.js, and others. Provides convenient abstractions for API calls, streaming, and file handling.",
  "use_cases": [
   "Building AI-powered chatbots and customer support systems",
   "Integrating Claude into existing applications for content generation",
   "Creating autonomous agents and multi-step workflows",
   "Developing enterprise AI solutions with production-grade reliability"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Replit",
   "Quora"
  ],
  "docs": "https://github.com/anthropics/anthropic-sdk-python",
  "tutorial": "https://docs.anthropic.com/claude/reference/getting-started-with-the-api",
  "added": "2026-08-17",
  "dex": 203
 },
 {
  "id": "anthropic-tokens",
  "name": "Anthropic Tokens",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like buying game tokens at an arcade—you pay for how many words Claude reads and writes, not a monthly pass.",
  "description": "Anthropic's token-based pricing and usage system for Claude API, allowing granular billing based on input and output tokens consumed. It enables developers to optimize costs while managing model usage at scale.",
  "use_cases": [
   "Cost optimization for high-volume API applications",
   "Fine-tuned billing allocation across teams and projects",
   "Budgeting and forecasting for AI infrastructure",
   "Usage monitoring and quota management"
  ],
  "used_by": [
   "Scale AI",
   "Notion",
   "DuckDuckGo"
  ],
  "docs": "https://docs.anthropic.com/en/docs/resources/tokens",
  "tutorial": "https://docs.anthropic.com/en/docs/build-a-chatbot-with-claude",
  "added": "2026-06-26",
  "dex": 204
 },
 {
  "id": "anthropic-vision",
  "name": "Anthropic Vision API",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like giving Claude eyes so it can look at pictures and understand what's in them, just like you can recognize your mom in a photo.",
  "description": "Claude's multimodal capability enabling image analysis, document understanding, and visual reasoning. It processes images alongside text to extract information, answer questions about visual content, and perform document intelligence tasks.",
  "use_cases": [
   "Document extraction and OCR from PDFs and images",
   "Product catalog image tagging and classification",
   "Visual quality assurance and defect detection",
   "Chart and diagram interpretation for data analysis"
  ],
  "used_by": [
   "Notion",
   "DuckDuckGo",
   "Zapier"
  ],
  "docs": "https://docs.anthropic.com/en/docs/vision/vision-beta",
  "tutorial": "https://github.com/anthropics/cookbook/blob/main/vision/working_with_images.ipynb",
  "added": "2026-08-07",
  "dex": 205
 },
 {
  "id": "bentoml",
  "name": "BentoML",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "BentoML is like a shipping container for ML models — it packages your model with everything it needs so it runs the same way everywhere.",
  "description": "Framework for building, shipping, and scaling AI applications. Turns any ML model into a production-ready API with batching, runners, and cloud deployment built in.",
  "use_cases": [
   "Packaging models as REST/gRPC inference APIs",
   "Adaptive batching for throughput optimization",
   "Multi-model inference pipelines",
   "Deploying to Kubernetes, AWS, or GCP"
  ],
  "used_by": [
   "Baidu",
   "trivago",
   "Picsellia"
  ],
  "docs": "https://docs.bentoml.com/",
  "tutorial": "https://docs.bentoml.com/en/latest/get-started/quickstart.html",
  "added": "2026-06-12",
  "dex": 206
 },
 {
  "id": "anthropic-claude-opus",
  "name": "Claude 3 Opus",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Claude Opus is like having a super smart robot assistant that can read long books and answer really hard questions about them.",
  "description": "Claude 3 Opus is Anthropic's most capable large language model, designed for complex reasoning tasks and long-context analysis. It excels at handling multi-step problems, research tasks, and generating high-quality technical documentation.",
  "use_cases": [
   "Complex code generation and debugging",
   "Long-document analysis and summarization",
   "Multi-step reasoning and problem solving",
   "Technical writing and documentation"
  ],
  "used_by": [
   "Anthropic",
   "scale.com",
   "various Fortune 500 companies"
  ],
  "docs": "https://docs.anthropic.com/claude/reference/getting-started-with-the-api",
  "tutorial": "https://docs.anthropic.com/claude/docs/intro-to-claude",
  "added": "2026-07-05",
  "dex": 207
 },
 {
  "id": "anthropic-claude-3-5-haiku",
  "name": "Claude 3.5 Haiku",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A super-smart robot that understands words and pictures really quickly, like having a fast helpful friend who remembers everything you tell it.",
  "description": "Claude 3.5 Haiku is Anthropic's fastest and most compact large language model, designed for high-volume, latency-sensitive applications requiring real-time responses. It matches Claude 3 Opus performance while being significantly faster and cheaper.",
  "use_cases": [
   "Real-time chatbot responses for customer support",
   "Batch processing of large document collections",
   "Mobile and edge device AI inference",
   "High-frequency API endpoints with strict latency budgets"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Modal Labs",
   "Replit"
  ],
  "docs": "https://docs.anthropic.com/claude/reference/getting-started-with-the-api",
  "tutorial": "https://docs.anthropic.com/claude/tutorials/build-a-chatbot",
  "added": "2026-08-10",
  "dex": 208
 },
 {
  "id": "anthropic-claude-3-5-sonnet",
  "name": "Claude 3.5 Sonnet",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like having a super smart robot friend who can read, write, and answer almost any question you ask it.",
  "description": "Anthropic's advanced large language model that excels at reasoning, code generation, and complex analysis tasks. It's widely used for AI-powered development tools, content generation, and enterprise automation.",
  "use_cases": [
   "AI-assisted code generation and debugging",
   "Technical documentation and API reference generation",
   "Customer service chatbot implementations",
   "Data analysis and insight extraction"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Notion",
   "DuckDuckGo"
  ],
  "docs": "https://docs.anthropic.com/en/api/getting-started",
  "tutorial": "https://docs.anthropic.com/en/api/getting-started-with-the-api",
  "added": "2026-07-04",
  "dex": 209
 },
 {
  "id": "anthropic-claude-api",
  "name": "Claude API",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like having a very smart helper that can read and write things for you, similar to how a teacher can explain homework or write a summary.",
  "description": "Anthropic's Claude API provides access to advanced large language models through a REST API for building AI-powered applications. It's widely used for content generation, code assistance, and complex reasoning tasks in production systems.",
  "use_cases": [
   "Building AI chatbots and virtual assistants",
   "Code generation and technical documentation",
   "Content analysis and document summarization",
   "Complex reasoning and problem-solving automation"
  ],
  "used_by": [
   "DuckDuckGo",
   "Slack",
   "Notion",
   "Figma"
  ],
  "docs": "https://docs.anthropic.com/",
  "tutorial": "https://docs.anthropic.com/en/docs/quickstart",
  "added": "2026-06-21",
  "dex": 210
 },
 {
  "id": "anthropic-computer-vision",
  "name": "Claude Computer Vision",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Claude's ability to look at pictures and understand what's in them, like when you point at a photo and ask your smart friend what they see.",
  "description": "Claude's multimodal capabilities enable vision understanding for tasks like document analysis, image classification, and visual content extraction. It processes images alongside text in a single API call for comprehensive document and visual intelligence applications.",
  "use_cases": [
   "Document digitization and data extraction from PDFs and images",
   "Content moderation and visual compliance checking",
   "Product image analysis for e-commerce platforms",
   "Accessibility enhancement through image description generation"
  ],
  "used_by": [
   "Notion",
   "Figma",
   "Scale AI",
   "Anthropic"
  ],
  "docs": "https://docs.anthropic.com/vision/vision-overview",
  "tutorial": "https://docs.anthropic.com/vision/vision-overview",
  "added": "2026-07-24",
  "dex": 211
 },
 {
  "id": "anthropic-mcp-claude-integration",
  "name": "Claude MCP Integration",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like giving Claude a bunch of tools to use, just like how a chef has different utensils to cook with.",
  "description": "Model Context Protocol (MCP) integration that allows Claude to interact with external systems, databases, and APIs seamlessly. It enables developers to extend Claude's capabilities with custom tools and data sources while maintaining security.",
  "use_cases": [
   "Connecting Claude to private databases and knowledge bases",
   "Building AI agents that can perform complex multi-step operations",
   "Integrating Claude with internal company systems and APIs",
   "Creating context-aware AI assistants with real-time data access"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Notch",
   "Klu"
  ],
  "docs": "https://modelcontextprotocol.io/introduction",
  "tutorial": "https://modelcontextprotocol.io/quickstart",
  "added": "2026-08-08",
  "dex": 212
 },
 {
  "id": "anthropic-claude-sonnet",
  "name": "Claude Sonnet",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Claude Sonnet is like having a super-smart robot assistant that can read long documents and answer questions about them really quickly.",
  "description": "Claude Sonnet is Anthropic's fastest and most compact large language model, optimized for high-throughput tasks and extended context windows. It balances speed and capability for production AI applications requiring rapid inference and cost efficiency.",
  "use_cases": [
   "Real-time customer support chatbots",
   "Batch document analysis and summarization",
   "Code generation and technical documentation",
   "Content moderation at scale"
  ],
  "used_by": [
   "DuckDuckGo",
   "Notion",
   "Replit",
   "GitLab"
  ],
  "docs": "https://docs.anthropic.com/en/docs/about-claude/models/latest",
  "tutorial": "https://docs.anthropic.com/en/docs/quickstart",
  "added": "2026-06-24",
  "dex": 213
 },
 {
  "id": "anthropic-ai-claude-vision",
  "name": "Claude Vision API",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like showing a robot a picture and asking it questions about what's in the photo, just like you'd ask a friend.",
  "description": "Claude's multimodal API that processes images alongside text for analysis, extraction, and reasoning tasks. It enables applications to understand visual content and answer questions about images with high accuracy.",
  "use_cases": [
   "Document OCR and data extraction from PDFs and images",
   "Product image analysis for e-commerce platforms",
   "Medical imaging analysis and diagnostic support",
   "Visual content moderation and classification"
  ],
  "used_by": [
   "Notion",
   "DuckDuckGo",
   "Bridgewater Associates"
  ],
  "docs": "https://docs.anthropic.com/vision/vision-api",
  "tutorial": "https://docs.anthropic.com/en/docs/vision/vision-getting-started",
  "added": "2026-08-21",
  "dex": 214
 },
 {
  "id": "cline",
  "name": "Cline",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like having a robot assistant that can read your code and help you build software by understanding what you want and making changes automatically.",
  "description": "Cline is an AI-powered autonomous coding agent that integrates with VS Code to understand codebases and perform complex development tasks. It leverages large language models to write, test, and refactor code with minimal human intervention.",
  "use_cases": [
   "Automating routine code refactoring and modernization tasks",
   "Accelerating feature development and bug fixes in large codebases",
   "Generating boilerplate code and project scaffolding",
   "Cross-file code generation with context awareness"
  ],
  "used_by": [
   "Anthropic",
   "various startups",
   "enterprise development teams"
  ],
  "docs": "https://github.com/cline/cline",
  "tutorial": "https://github.com/cline/cline#quick-start",
  "added": "2026-09-17",
  "dex": 215
 },
 {
  "id": "cohere-api",
  "name": "Cohere API",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like having a smart library assistant who can read, understand, and generate text about any topic you ask.",
  "description": "Cohere provides a suite of large language models and APIs for natural language understanding and generation, designed for enterprise use cases with customization, fine-tuning, and production-grade reliability. It competes with OpenAI and Anthropic with focus on enterprise deployability.",
  "use_cases": [
   "Document summarization and information extraction",
   "Customer support chatbots and semantic search",
   "Content generation for marketing and email campaigns",
   "Named entity recognition and text classification tasks"
  ],
  "used_by": [
   "IBM",
   "Mailchimp",
   "ServiceTitan",
   "Typeform"
  ],
  "docs": "https://docs.cohere.com",
  "tutorial": "https://docs.cohere.com/docs/quickstart",
  "added": "2026-08-16",
  "dex": 216
 },
 {
  "id": "cadence-ai-code-generation",
  "name": "Continue",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Continue is like having a smart coding buddy sitting next to you who suggests what to type next, similar to autocomplete on your phone but way smarter for writing programs.",
  "description": "Continue is an open-source AI code completion and generation tool that integrates directly into IDEs like VS Code and JetBrains, allowing developers to use local or cloud-based LLMs for coding assistance. It enables seamless AI-powered autocomplete, code refactoring, and documentation generation without leaving the editor.",
  "use_cases": [
   "Real-time code autocompletion with local or remote LLMs",
   "AI-assisted code refactoring and debugging",
   "Automatic documentation and comment generation",
   "Context-aware code suggestions based on project structure"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Individual developers",
   "Open-source projects"
  ],
  "docs": "https://docs.continue.dev",
  "tutorial": "https://docs.continue.dev/getting-started/quick-start",
  "added": "2026-09-23",
  "dex": 217
 },
 {
  "id": "cortex",
  "name": "Cortex",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Cortex is like a lending library for AI models that lets you check out pre-trained models and run them on your own computer without needing to train them from scratch.",
  "description": "Cortex is an open-source platform that makes it easy to run any open-source language model locally with a simple CLI and API, providing GPU acceleration and model management without cloud dependencies. It abstracts away the complexity of managing dependencies, memory, and inference optimization.",
  "use_cases": [
   "Running open-source LLMs locally for privacy-sensitive applications",
   "Building offline-capable AI features for desktop and mobile apps",
   "Cost-effective inference without cloud provider lock-in",
   "Fine-tuning and customizing models for specific domain tasks"
  ],
  "used_by": [
   "Anthropic",
   "Hugging Face",
   "independent developers",
   "enterprises"
  ],
  "docs": "https://cortexcpp.com/docs",
  "tutorial": "https://cortexcpp.com/docs/getting-started",
  "added": "2026-07-31",
  "dex": 218
 },
 {
  "id": "cuda",
  "name": "CUDA",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Like a highway system that lets your graphics card do thousands of math problems at the same time instead of one at a time.",
  "description": "NVIDIA's parallel computing platform and programming model that enables GPU-accelerated computation. Underpins virtually all deep learning training and inference workloads by exposing thousands of GPU cores to general-purpose code.",
  "use_cases": [
   "Accelerate neural network training with GPU parallelism",
   "Run matrix multiplications and tensor operations orders of magnitude faster than CPU",
   "Custom GPU kernels for novel model architectures",
   "Inference acceleration for large language models",
   "Scientific simulations and HPC workloads"
  ],
  "used_by": [
   "OpenAI",
   "Google DeepMind",
   "Meta AI",
   "Hugging Face",
   "NVIDIA"
  ],
  "docs": "https://docs.nvidia.com/cuda/",
  "tutorial": "https://docs.nvidia.com/cuda/cuda-c-programming-guide/",
  "added": "2026-06-10",
  "dex": 219
 },
 {
  "id": "dria",
  "name": "Dria",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Dria is like having a personal assistant that reads through tons of documents and learns facts from them, so it can answer your questions accurately about what it learned.",
  "description": "Dria is a decentralized AI data platform that enables building, managing, and monetizing training datasets for large language models. It combines community contribution with blockchain verification to create high-quality, trustworthy ML datasets.",
  "use_cases": [
   "Creating labeled datasets for fine-tuning domain-specific language models",
   "Crowdsourcing and verifying training data for ML projects",
   "Building high-quality datasets while maintaining data privacy and ownership",
   "Monetizing proprietary data through verified dataset marketplace"
  ],
  "used_by": [
   "OpenAI",
   "Hugging Face",
   "Anthropic partners"
  ],
  "docs": "https://docs.dria.co/",
  "tutorial": "https://docs.dria.co/getting-started",
  "added": "2026-06-22",
  "dex": 220
 },
 {
  "id": "dspy",
  "name": "DSPy",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like teaching someone a recipe by showing them the ingredients and techniques, rather than just telling them the final dish—you optimize the way to use AI models instead of hardcoding prompts.",
  "description": "A framework by Stanford researchers that enables programmatic optimization of AI language model prompts and weights through automated techniques like in-context learning and finetuning. It treats prompt engineering as a compiler problem, allowing reproducible and maintainable AI applications.",
  "use_cases": [
   "Optimizing LLM prompts automatically rather than manual tuning",
   "Building modular AI pipelines with learnable components",
   "Creating RAG systems with automatic optimization of retrieval logic",
   "Testing and validating LLM behaviors across different models systematically"
  ],
  "used_by": [
   "Anthropic researchers",
   "Academic institutions",
   "AI-focused startups"
  ],
  "docs": "https://github.com/stanfordnlp/dspy",
  "tutorial": "https://github.com/stanfordnlp/dspy/tree/main/examples",
  "added": "2026-08-05",
  "dex": 221
 },
 {
  "id": "elyra-ml-orchestration",
  "name": "Elyra ML Orchestration",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a recipe card manager for machine learning experiments that lets you drag-and-drop ingredients instead of writing code.",
  "description": "An open-source extension for Jupyter that provides a visual pipeline editor for building, running, and monitoring ML workflows with support for multiple orchestration backends like Apache Airflow and Kubeflow. It bridges notebook-based development with production orchestration.",
  "use_cases": [
   "Visual ML pipeline design without deep orchestration knowledge",
   "Converting Jupyter notebooks to production workflows",
   "Multi-step model training and hyperparameter tuning pipelines",
   "Collaboration on ML experiments with non-technical stakeholders"
  ],
  "used_by": [
   "IBM",
   "Red Hat",
   "Kubernetes community",
   "enterprise ML teams"
  ],
  "docs": "https://elyra.readthedocs.io",
  "tutorial": "https://elyra.readthedocs.io/en/latest/getting-started/intro.html",
  "added": "2026-09-11",
  "dex": 222
 },
 {
  "id": "exareme",
  "name": "Exareme AI",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Exareme is like a smart calculator that can learn from medical data without ever seeing the actual patient records, keeping everything private.",
  "description": "Exareme AI is a federated learning platform that enables machine learning model training across decentralized, sensitive data sources without centralizing the raw data. It's particularly valuable in healthcare, finance, and regulated industries where data cannot leave its source.",
  "use_cases": [
   "Federated learning across hospital networks for drug discovery",
   "Training financial risk models across multiple institutions",
   "Privacy-preserving machine learning in regulated industries",
   "Collaborative AI model development without data centralization"
  ],
  "used_by": [
   "Novartis",
   "Philips Healthcare",
   "European Commission",
   "Banco Santander"
  ],
  "docs": "https://www.exareme.ai/docs/",
  "tutorial": "https://www.exareme.ai/getting-started/",
  "added": "2026-08-03",
  "dex": 223
 },
 {
  "id": "fal-serverless-gpu",
  "name": "Fal.ai",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Fal.ai is like renting a super-powerful computer with fancy AI brains just for a few seconds when you need them, instead of buying your own.",
  "description": "Fal.ai is a serverless GPU platform for running AI models and machine learning workloads without managing infrastructure. It provides on-demand access to GPUs for inference, fine-tuning, and batch processing of AI models.",
  "use_cases": [
   "Serverless inference for LLMs and generative models",
   "Image generation and computer vision tasks",
   "Real-time AI API endpoints for applications",
   "Batch processing and fine-tuning of AI models"
  ],
  "used_by": [
   "Replicate",
   "Hugging Face",
   "scale.com",
   "Runway"
  ],
  "docs": "https://fal.ai/docs/",
  "tutorial": "https://fal.ai/docs/quickstart/",
  "added": "2026-09-02",
  "dex": 224
 },
 {
  "id": "hugging-face",
  "name": "Hugging Face",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Like GitHub but for AI models — a place where people share, find, and use thousands of ready-made AI brains for free.",
  "description": "Central hub for open-source ML models, datasets, and Spaces (hosted demos). Hosts the Transformers library — the standard API for loading and running pretrained models — plus model cards, leaderboards, and hosted inference.",
  "use_cases": [
   "Browse and download 500k+ pretrained models for NLP, CV, audio",
   "Fine-tune models with the Trainer API and PEFT/LoRA",
   "Host and share ML demos with Gradio Spaces",
   "Dataset versioning and sharing with datasets library",
   "Inference endpoints for serverless model hosting"
  ],
  "used_by": [
   "Google",
   "Meta",
   "Amazon",
   "Microsoft",
   "Mistral AI"
  ],
  "docs": "https://huggingface.co/docs",
  "tutorial": "https://huggingface.co/learn",
  "added": "2026-06-10",
  "dex": 225
 },
 {
  "id": "huggingface-transformers",
  "name": "Hugging Face Transformers",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a toolbox that comes with pre-built AI brains you can use or teach to do things like understanding language or seeing pictures.",
  "description": "Hugging Face Transformers is an open-source library providing state-of-the-art transformer models and easy-to-use APIs for natural language processing, computer vision, and speech tasks. It abstracts complex model architectures with simple interfaces and hosts 500,000+ pre-trained models.",
  "use_cases": [
   "Fine-tuning pre-trained models for custom NLP tasks",
   "Building chatbots and question-answering systems",
   "Text classification, sentiment analysis, and named entity recognition",
   "Vision tasks like image classification and object detection"
  ],
  "used_by": [
   "Google",
   "Meta",
   "OpenAI",
   "Databricks"
  ],
  "docs": "https://huggingface.co/docs/transformers/",
  "tutorial": "https://huggingface.co/docs/transformers/quicktour",
  "added": "2026-07-20",
  "dex": 226
 },
 {
  "id": "hydra",
  "name": "Hydra",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a smart assistant for machine learning experiments that remembers all your settings and lets you change them without touching the code.",
  "description": "Hydra is a framework for configuring complex applications built by Facebook Research, particularly useful for machine learning. It simplifies managing configuration hierarchies, experiment tracking, and hyperparameter management through YAML-based configuration.",
  "use_cases": [
   "Managing hyperparameter configurations for ML experiments",
   "Running multiple experiment variations systematically",
   "Organizing and overriding nested configuration structures",
   "Reproducible machine learning pipelines"
  ],
  "used_by": [
   "Meta",
   "NVIDIA",
   "Microsoft Research",
   "DeepMind"
  ],
  "docs": "https://hydra.cc/",
  "tutorial": "https://hydra.cc/docs/tutorials/basic/running_your_app/",
  "added": "2026-07-02",
  "dex": 227
 },
 {
  "id": "kubeflow",
  "name": "Kubeflow",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Kubeflow is like a factory floor for machine learning on Kubernetes — it handles training jobs, experiment tracking, and model serving all in one place.",
  "description": "ML toolkit for Kubernetes that makes deploying scalable ML workflows simple. Includes Pipelines, Training Operator, KServe for model serving, and Katib for hyperparameter tuning.",
  "use_cases": [
   "End-to-end ML pipelines on Kubernetes",
   "Distributed training for PyTorch and TensorFlow",
   "Hyperparameter optimization with Katib",
   "Model serving with KServe"
  ],
  "used_by": [
   "Google",
   "Spotify",
   "Bloomberg",
   "Cisco"
  ],
  "docs": "https://www.kubeflow.org/docs/",
  "tutorial": "https://www.kubeflow.org/docs/started/introduction/",
  "added": "2026-06-12",
  "dex": 228
 },
 {
  "id": "langbase",
  "name": "Langbase",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a workshop where you can build and test recipes for AI assistants without needing to be a chef yourself.",
  "description": "Langbase is a platform for building, deploying, and managing LLM-powered applications with features like prompt management, fine-tuning, and API endpoints. It streamlines the workflow of turning LLM experiments into production applications.",
  "use_cases": [
   "Building and versioning AI chatbots and assistants",
   "Managing and testing multiple LLM prompts in production",
   "Fine-tuning open-source models with proprietary data",
   "Creating API endpoints for LLM inference at scale"
  ],
  "used_by": [
   "Vercel ecosystem",
   "Y Combinator startups",
   "AI-first SaaS companies",
   "Enterprise AI teams"
  ],
  "docs": "https://docs.langbase.com",
  "tutorial": "https://langbase.com/guides",
  "added": "2026-09-03",
  "dex": 229
 },
 {
  "id": "langchain",
  "name": "LangChain",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "A toolbox for connecting an AI brain to the real world — like giving it the ability to search the web, read files, or use a calculator.",
  "description": "Framework for building LLM-powered applications with composable chains, agents, and tool integrations. Provides abstractions for retrieval-augmented generation (RAG), memory, and multi-step agentic workflows across model providers.",
  "use_cases": [
   "Build RAG pipelines connecting LLMs to vector databases",
   "Create autonomous agents that use tools and APIs",
   "Chain multiple LLM calls into structured workflows",
   "Standardize prompt templates and output parsers",
   "Evaluate and trace LLM applications with LangSmith"
  ],
  "used_by": [
   "Airbnb",
   "Elastic",
   "Rakuten",
   "McKinsey",
   "Riot Games"
  ],
  "docs": "https://python.langchain.com/docs/",
  "tutorial": "https://python.langchain.com/docs/tutorials/",
  "added": "2026-06-10",
  "dex": 230
 },
 {
  "id": "langfuse",
  "name": "Langfuse",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a notebook that records everything your AI assistant does and says so you can see if it's working well and fix problems.",
  "description": "Langfuse is an open-source LLM engineering platform providing observability, analytics, and debugging tools for language model applications. It captures prompts, completions, latency, costs, and user feedback to help teams improve LLM application quality and performance.",
  "use_cases": [
   "Monitoring LLM application performance and token usage",
   "Debugging prompt chains and multi-step LLM workflows",
   "Collecting user feedback to improve model outputs",
   "Cost analysis and optimization for LLM API spending"
  ],
  "used_by": [
   "Hugging Face",
   "Together AI",
   "Vercel",
   "Amplitude"
  ],
  "docs": "https://langfuse.com/docs",
  "tutorial": "https://langfuse.com/docs/get-started",
  "added": "2026-07-21",
  "dex": 231
 },
 {
  "id": "langgraph",
  "name": "LangGraph",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "LangGraph is like a flowchart engine for AI — it lets your AI make decisions, use tools, loop back on itself, and hand off to other agents in a controlled way.",
  "description": "Framework for building stateful, multi-step AI agents as directed graphs. Built on LangChain, enables cycles, branching, human-in-the-loop, and multi-agent coordination.",
  "use_cases": [
   "Multi-step AI agents with tool use and memory",
   "Human-in-the-loop approval workflows",
   "Multi-agent systems with supervisor/subagent patterns",
   "Long-running agentic workflows with persistence"
  ],
  "used_by": [
   "LinkedIn",
   "Replit",
   "Elastic",
   "LangChain"
  ],
  "docs": "https://langchain-ai.github.io/langgraph/",
  "tutorial": "https://langchain-ai.github.io/langgraph/tutorials/introduction/",
  "added": "2026-06-14",
  "dex": 232
 },
 {
  "id": "langsmith",
  "name": "LangSmith",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "LangSmith is like a laboratory notebook for AI assistants, where you write down what they say, test if their answers are good, and make them smarter.",
  "description": "LangSmith is a platform by LangChain for monitoring, evaluating, and debugging large language model applications in production. It provides tracing, testing, and analytics tools to optimize AI application performance and reliability.",
  "use_cases": [
   "LLM application debugging and performance monitoring",
   "Evaluation and testing of AI model outputs",
   "Production monitoring and alerting for AI systems",
   "Cost tracking and token usage optimization"
  ],
  "used_by": [
   "Stripe",
   "Hubspot",
   "Zoom",
   "Anduril"
  ],
  "docs": "https://docs.smith.langchain.com/",
  "tutorial": "https://smith.langchain.com/",
  "added": "2026-07-03",
  "dex": 233
 },
 {
  "id": "litellm",
  "name": "LiteLLM",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "LiteLLM is like a universal translator that lets you use any AI language model with the same language, switching between them without changing your code.",
  "description": "LiteLLM is a lightweight library that standardizes API calls across major LLM providers (OpenAI, Anthropic, Gemini, Cohere, etc.). It reduces vendor lock-in and simplifies switching between language models.",
  "use_cases": [
   "Building AI applications agnostic to underlying LLM provider",
   "Cost optimization by comparing different model providers",
   "Fallback mechanisms when one provider is unavailable",
   "Load balancing across multiple LLM APIs"
  ],
  "used_by": [
   "Vercel",
   "Mint AI",
   "Composio",
   "Voiceflow"
  ],
  "docs": "https://docs.litellm.ai/",
  "tutorial": "https://docs.litellm.ai/docs/getting_started/quick_start",
  "added": "2026-06-16",
  "dex": 234
 },
 {
  "id": "llama-cpp",
  "name": "llama.cpp",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a translator that lets you run really smart AI models on your personal computer instead of needing a giant server.",
  "description": "llama.cpp is a C++ implementation that enables efficient inference of large language models on consumer hardware with minimal memory requirements. It's widely used for running quantized models locally with superior performance compared to Python-based alternatives.",
  "use_cases": [
   "Running open-source LLMs locally on laptops and edge devices",
   "Building privacy-preserving AI applications without cloud dependencies",
   "Deploying language models in resource-constrained environments",
   "Creating low-latency inference servers for production use"
  ],
  "used_by": [
   "Ollama",
   "LM Studio",
   "Hugging Face ecosystem",
   "Individual developers and startups"
  ],
  "docs": "https://github.com/ggerganov/llama.cpp",
  "tutorial": "https://github.com/ggerganov/llama.cpp#usage",
  "added": "2026-08-02",
  "dex": 235
 },
 {
  "id": "llamafile",
  "name": "LlamaFile",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like putting a whole library into one magic book you can carry anywhere and read without an internet connection.",
  "description": "LlamaFile is a single-file executable that packages open-source large language models with an embedded HTTP server, allowing you to run inference locally without external dependencies. Created by Jart and Mozilla, it simplifies deployment of LLMs to any machine.",
  "use_cases": [
   "Running private LLMs on laptops and edge devices without cloud dependencies",
   "Deploying models to air-gapped or offline environments",
   "Creating self-contained AI-powered applications for distribution",
   "Testing and prototyping LLM applications with zero infrastructure"
  ],
  "used_by": [
   "Mozilla",
   "Anthropic community",
   "LocalAI community",
   "Individual developers"
  ],
  "docs": "https://github.com/Mozilla-Ocho/llamafile",
  "tutorial": "https://github.com/Mozilla-Ocho/llamafile/blob/main/README.md",
  "added": "2026-09-09",
  "dex": 236
 },
 {
  "id": "llamaindex",
  "name": "LlamaIndex",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "LlamaIndex is like a filing system that helps AI read and remember your documents — it indexes them so the AI can find the right info before answering.",
  "description": "Data framework for connecting LLMs to external data sources. Provides tools for ingestion, indexing, and retrieval to build RAG pipelines and AI agents.",
  "use_cases": [
   "Retrieval-augmented generation (RAG) over private docs",
   "Building knowledge assistants over internal wikis",
   "Structured data querying with natural language",
   "Multi-step AI agents with tool use"
  ],
  "used_by": [
   "Replit",
   "MongoDB",
   "Figma",
   "Uber"
  ],
  "docs": "https://docs.llamaindex.ai/",
  "tutorial": "https://docs.llamaindex.ai/en/stable/getting_started/starter_example/",
  "added": "2026-06-12",
  "dex": 237
 },
 {
  "id": "mcp-client",
  "name": "MCP Client",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a universal remote control that lets AI assistants use any tool or program you connect to it.",
  "description": "Model Context Protocol (MCP) Client enables AI models and applications to securely interact with external tools, data sources, and services through a standardized interface. It's used in Claude, various IDEs, and agent frameworks.",
  "use_cases": [
   "Connecting AI assistants to internal company tools",
   "Enabling code editors with AI-powered capabilities",
   "Building multi-tool AI agents",
   "Secure API access for language models"
  ],
  "used_by": [
   "Anthropic",
   "Cline",
   "Continue",
   "Jetbrains"
  ],
  "docs": "https://modelcontextprotocol.io/introduction",
  "tutorial": "https://modelcontextprotocol.io/quickstart/server",
  "added": "2026-07-12",
  "dex": 238
 },
 {
  "id": "milvus-lite",
  "name": "Milvus Lite",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a smart filing cabinet that remembers not just the exact words you wrote, but the meaning and spirit of what you said.",
  "description": "Milvus Lite is a lightweight, serverless vector database perfect for embedding search and AI applications, deployable in a single Python file without external dependencies. It powers semantic search, RAG systems, and recommendation engines with high performance and ease of use.",
  "use_cases": [
   "Building RAG (Retrieval-Augmented Generation) pipelines",
   "Semantic search and similarity matching",
   "AI chatbot knowledge base management",
   "Recommendation systems with vector embeddings"
  ],
  "used_by": [
   "Alibaba",
   "Microsoft",
   "Shopify",
   "DuckDuckGo"
  ],
  "docs": "https://milvus.io/docs/milvus_lite.md",
  "tutorial": "https://milvus.io/docs/quickstart.md",
  "added": "2026-07-19",
  "dex": 239
 },
 {
  "id": "mistral-ai",
  "name": "Mistral AI",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Mistral AI is like a smart robot that can read and write, similar to ChatGPT, but it's made by a different company and runs really fast.",
  "description": "Mistral AI is a French AI company providing open-source and commercial large language models optimized for performance and efficiency. Their models power enterprise applications with lower latency and cost compared to larger alternatives.",
  "use_cases": [
   "Building conversational AI chatbots and customer support systems",
   "Code generation and software development assistance",
   "Document analysis and summarization for enterprises",
   "Real-time inference in production environments with resource constraints"
  ],
  "used_by": [
   "Mistral AI",
   "Hugging Face",
   "Together AI",
   "Scaleway"
  ],
  "docs": "https://docs.mistral.ai/",
  "tutorial": "https://docs.mistral.ai/getting-started/quickstart/",
  "added": "2026-09-06",
  "dex": 240
 },
 {
  "id": "mlflow",
  "name": "MLflow",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A lab notebook for AI experiments — it automatically records what ingredients you used, what settings you tried, and how well each experiment worked.",
  "description": "Open-source MLOps platform for tracking experiments, packaging models, and managing the ML lifecycle. Provides a UI for comparing runs, a model registry for versioning, and deployment tools across frameworks.",
  "use_cases": [
   "Track hyperparameters, metrics, and artifacts across training runs",
   "Register and version models for staging and production promotion",
   "Reproduce any experiment from stored parameters and code",
   "Serve models with mlflow models serve or to Kubernetes",
   "Integrate with PyTorch, TensorFlow, sklearn, and XGBoost"
  ],
  "used_by": [
   "Databricks",
   "Microsoft",
   "LinkedIn",
   "Accenture",
   "Zillow"
  ],
  "docs": "https://mlflow.org/docs/latest/",
  "tutorial": "https://mlflow.org/docs/latest/getting-started/",
  "added": "2026-06-10",
  "dex": 241
 },
 {
  "id": "mcp",
  "name": "Model Context Protocol",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a universal translator that lets AI assistants talk to any tool or database, like giving them access to every book in a library instead of just one.",
  "description": "Model Context Protocol (MCP) is an open standard developed by Anthropic that enables AI models to securely interact with external data sources and tools through a standardized interface. It allows seamless integration of context and capabilities into LLM applications.",
  "use_cases": [
   "Connecting AI assistants to enterprise databases and knowledge bases",
   "Building AI-powered code generation tools with access to version control",
   "Creating AI agents that interact with APIs and business systems",
   "Enabling retrieval-augmented generation (RAG) with custom data sources"
  ],
  "used_by": [
   "Anthropic",
   "Integration partners",
   "Enterprise AI teams"
  ],
  "docs": "https://modelcontextprotocol.io/",
  "tutorial": "https://modelcontextprotocol.io/quickstart",
  "added": "2026-06-25",
  "dex": 242
 },
 {
  "id": "mcp-servers",
  "name": "Model Context Protocol (MCP)",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "MCP is like a universal translator that lets AI assistants talk to any tool or system on your computer, the same way USB lets devices talk to your computer.",
  "description": "The Model Context Protocol is an open standard that enables AI models and applications to seamlessly interact with external tools, databases, and services. It provides a standardized interface for connecting LLMs to real-world data sources and capabilities.",
  "use_cases": [
   "Enabling Claude and other AI models to access local file systems and databases",
   "Building AI agents that interact with enterprise systems and APIs",
   "Creating IDE integrations for AI-assisted code generation and analysis",
   "Connecting language models to custom business logic and tools"
  ],
  "used_by": [
   "Anthropic",
   "Cline AI",
   "Continue",
   "Various Enterprise AI Teams"
  ],
  "docs": "https://modelcontextprotocol.io/",
  "tutorial": "https://modelcontextprotocol.io/quickstart/server",
  "added": "2026-09-15",
  "dex": 243
 },
 {
  "id": "mcp-server",
  "name": "Model Context Protocol Server",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "MCP Server is like a translator that lets AI assistants talk to your tools and files just like you do, but in their own language.",
  "description": "The Model Context Protocol (MCP) is an open standard for connecting AI models to external data sources and tools through a unified interface. It enables Claude and other LLMs to safely access databases, APIs, and file systems with standardized communication.",
  "use_cases": [
   "Integrating AI assistants with enterprise databases and knowledge systems",
   "Building secure bridges between LLMs and internal tools without exposing credentials",
   "Creating agentic workflows where AI can read and modify application state",
   "Standardizing how multiple AI services access company data and APIs"
  ],
  "used_by": [
   "Anthropic",
   "Figma",
   "Raycast"
  ],
  "docs": "https://modelcontextprotocol.io/introduction",
  "tutorial": "https://modelcontextprotocol.io/quickstart",
  "added": "2026-06-27",
  "dex": 244
 },
 {
  "id": "mojo",
  "name": "Mojo",
  "category": "Language",
  "domain": "AI/ML",
  "eli5": "It's like Python's faster, stronger older sibling that still feels easy to write but runs super quick.",
  "description": "Mojo is a compiled programming language that combines Python's ease of use with C/C++-level performance, designed specifically for AI and systems programming. It allows developers to write high-performance AI workloads without abandoning Python's simplicity.",
  "use_cases": [
   "High-performance machine learning model inference",
   "Systems programming with Python-like syntax",
   "Numerical computing and scientific simulations",
   "Edge AI and embedded systems optimization"
  ],
  "used_by": [
   "Modular",
   "various AI research labs",
   "performance-critical ML teams"
  ],
  "docs": "https://docs.modular.com/mojo",
  "tutorial": "https://docs.modular.com/mojo/manual/get-started",
  "added": "2026-06-19",
  "dex": 245
 },
 {
  "id": "ollama",
  "name": "Ollama",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Like Docker but for AI models — type one command and a full AI brain runs on your own laptop without sending data to anyone.",
  "description": "CLI tool and local server for running open-source LLMs entirely on your own hardware. Handles model downloads, quantization, and provides an OpenAI-compatible API, making local inference approachable for developers.",
  "use_cases": [
   "Run Llama, Mistral, Gemma locally with one command",
   "Privacy-first AI without data leaving your machine",
   "Offline development and testing of LLM applications",
   "Rapid model switching and comparison",
   "Self-hosted AI for enterprise air-gapped environments"
  ],
  "used_by": [
   "Individual developers",
   "Privacy-focused enterprises",
   "AI hobbyists"
  ],
  "docs": "https://ollama.com/",
  "tutorial": "https://github.com/ollama/ollama",
  "added": "2026-06-10",
  "dex": 246
 },
 {
  "id": "ollama-embed",
  "name": "Ollama Embeddings",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a translator that turns words into special number patterns that computers understand, so they can find similar ideas quickly.",
  "description": "Ollama provides a simple way to run and embed large language models locally without needing expensive cloud APIs. It enables developers to create semantic search and RAG applications with models like Mistral, Llama, and others running on their own hardware.",
  "use_cases": [
   "Building local semantic search engines",
   "Creating retrieval-augmented generation (RAG) pipelines",
   "Privacy-first chatbot applications",
   "Offline text embedding and similarity matching"
  ],
  "used_by": [
   "Anthropic",
   "Scale AI",
   "Hugging Face"
  ],
  "docs": "https://github.com/ollama/ollama",
  "tutorial": "https://ollama.ai/library",
  "added": "2026-06-28",
  "dex": 247
 },
 {
  "id": "ollama-llm-runner",
  "name": "Ollama LLM Runner",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Ollama is like a recipe card that lets you run powerful AI brains right on your computer without needing to send your data to the internet.",
  "description": "Ollama is a lightweight tool for running large language models locally on consumer hardware, making it easy to run models like Llama 2, Mistral, and others without cloud dependencies. It provides a simple CLI and API for local LLM inference.",
  "use_cases": [
   "Local LLM inference without cloud API costs",
   "Development and testing of AI applications offline",
   "Privacy-focused applications requiring on-device AI",
   "Edge deployment of AI models on limited hardware"
  ],
  "used_by": [
   "OpenUI",
   "Continue IDE",
   "Anythingllm",
   "LocalAI"
  ],
  "docs": "https://github.com/ollama/ollama",
  "tutorial": "https://ollama.ai/blog/getting-started-with-ollama",
  "added": "2026-07-03",
  "dex": 248
 },
 {
  "id": "ollama-quantized-models",
  "name": "Ollama Quantized Models",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like running a smart brain on your own computer by using a compressed version that's faster and uses less memory than the full version.",
  "description": "Ollama provides pre-quantized large language models that can run locally on consumer hardware without requiring cloud APIs or GPU servers. It enables private, offline AI inference with models like Llama, Mistral, and others.",
  "use_cases": [
   "Local LLM inference for privacy-sensitive applications",
   "Running AI models offline without internet connectivity",
   "Development and testing of AI applications before cloud deployment",
   "Edge AI deployment on resource-constrained devices"
  ],
  "used_by": [
   "Individual developers",
   "Privacy-focused startups",
   "Enterprise AI teams",
   "Research institutions"
  ],
  "docs": "https://ollama.ai/",
  "tutorial": "https://github.com/ollama/ollama#readme",
  "added": "2026-08-24",
  "dex": 249
 },
 {
  "id": "ollama-vision",
  "name": "Ollama Vision",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like giving AI the ability to see pictures, so you can show it images and ask questions about what's in them, all running on your own computer.",
  "description": "Ollama Vision extends the Ollama platform with multimodal capabilities, allowing vision language models to process and understand images locally without cloud dependencies. It enables private, on-device image analysis and visual reasoning.",
  "use_cases": [
   "Building document processing and OCR applications locally",
   "Creating image analysis features without cloud API costs",
   "Enabling privacy-preserving visual search applications",
   "Developing computer vision features with open-source models"
  ],
  "used_by": [
   "Anthropic partners",
   "Open-source communities",
   "Enterprise AI teams"
  ],
  "docs": "https://github.com/ollama/ollama",
  "tutorial": "https://github.com/ollama/ollama/blob/main/README.md",
  "added": "2026-07-10",
  "dex": 250
 },
 {
  "id": "ollama-vision-model",
  "name": "Ollama Vision Models",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Ollama Vision is like having a smart robot you can run on your own computer that can look at pictures and describe what it sees, without sending the pictures to the internet.",
  "description": "Ollama now supports multimodal vision models (like LLaVA) that can process both text and images locally on consumer hardware. This enables private, offline image understanding without cloud APIs.",
  "use_cases": [
   "Document processing and OCR for confidential business documents",
   "Local image analysis for manufacturing quality control",
   "Privacy-preserving visual content moderation",
   "Building AI applications that don't require cloud connectivity"
  ],
  "used_by": [
   "Local-first startups, Enterprise security teams, Healthcare providers"
  ],
  "docs": "https://github.com/ollama/ollama/blob/main/docs/README.md",
  "tutorial": "https://ollama.com/blog/vision-models",
  "added": "2026-07-11",
  "dex": 251
 },
 {
  "id": "onnx",
  "name": "ONNX",
  "category": "Concept",
  "domain": "AI/ML",
  "eli5": "A universal translator for AI models — lets a model trained in one AI toolkit run in a completely different one without starting over.",
  "description": "Open Neural Network Exchange — an open standard for representing ML models. Enables interoperability between frameworks (PyTorch, TensorFlow, sklearn) and runtimes (ONNX Runtime, TensorRT, CoreML) for portable, optimized inference.",
  "use_cases": [
   "Export PyTorch models and run them with ONNX Runtime for faster CPU inference",
   "Deploy models to edge and mobile (CoreML, TFLite via conversion)",
   "Hardware-agnostic model optimization with ORT execution providers",
   "Cross-framework model sharing and reproducibility",
   "Visualize model graphs with Netron"
  ],
  "used_by": [
   "Microsoft",
   "NVIDIA",
   "Intel",
   "Qualcomm",
   "AMD"
  ],
  "docs": "https://onnx.ai/",
  "tutorial": "https://onnxruntime.ai/docs/get-started/",
  "added": "2026-06-10",
  "dex": 252
 },
 {
  "id": "open-webui",
  "name": "Open WebUI",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a friendly chat interface where you can talk to different AI assistants, kind of like having multiple tutors you can switch between.",
  "description": "Open WebUI is a self-hosted web interface for interacting with large language models like Ollama, OpenAI, and other LLM providers. It provides a ChatGPT-like experience with features like conversation management, model switching, and local deployment capabilities.",
  "use_cases": [
   "Running private AI chatbots without cloud dependencies",
   "Multi-model LLM experimentation and comparison",
   "Enterprise AI deployments with data privacy requirements",
   "Custom knowledge base integration with document upload"
  ],
  "used_by": [
   "Hugging Face",
   "Replicate",
   "Anyscale",
   "individual ML engineers"
  ],
  "docs": "https://docs.openwebui.com",
  "tutorial": "https://github.com/open-webui/open-webui",
  "added": "2026-06-19",
  "dex": 253
 },
 {
  "id": "openai-api",
  "name": "OpenAI API",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "The OpenAI API is like renting a super-smart brain — your app sends questions or tasks, and it sends back intelligent answers, code, or images.",
  "description": "Cloud API providing access to GPT-4, o1, embeddings, image generation (DALL-E), speech, and fine-tuning. The most widely adopted LLM API in production applications.",
  "use_cases": [
   "Chat and assistant features in applications",
   "Code generation and review automation",
   "Embeddings for semantic search and RAG",
   "Structured data extraction from unstructured text"
  ],
  "used_by": [
   "Microsoft",
   "Snap",
   "Salesforce",
   "Duolingo"
  ],
  "docs": "https://platform.openai.com/docs/",
  "tutorial": "https://platform.openai.com/docs/quickstart",
  "added": "2026-06-14",
  "dex": 254
 },
 {
  "id": "opendevin",
  "name": "OpenDevin",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like having a helpful robot assistant that can read and write code, run programs, and fix things when you tell it what to do.",
  "description": "OpenDevin is an open-source autonomous AI agent for software development that can write code, run tests, browse files, and complete programming tasks end-to-end. It integrates with popular IDEs and development workflows to provide AI-assisted development.",
  "use_cases": [
   "Automated bug fixing and code debugging",
   "Scaffolding new features and boilerplate generation",
   "Test generation and coverage improvement",
   "Code refactoring and technical debt reduction"
  ],
  "used_by": [
   "OpenDevin contributors",
   "AI-first development teams",
   "Research institutions"
  ],
  "docs": "https://github.com/All-Hands-AI/OpenDevin",
  "tutorial": "https://github.com/All-Hands-AI/OpenDevin/blob/main/README.md",
  "added": "2026-06-30",
  "dex": 255
 },
 {
  "id": "perplexity-api",
  "name": "Perplexity API",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like having a smart librarian who can read the entire internet and answer your questions with current information in real-time.",
  "description": "Perplexity API provides access to real-time search and reasoning capabilities with citations, allowing developers to build AI applications with current web information. It combines large language models with live internet search to provide accurate, up-to-date answers.",
  "use_cases": [
   "Building AI chatbots that need current information and news",
   "Research assistants that verify facts against live web data",
   "Real-time market analysis and trend monitoring",
   "Knowledge retrieval systems with source attribution"
  ],
  "used_by": [
   "Scale AI",
   "Replit",
   "Notion AI",
   "Y Combinator startups"
  ],
  "docs": "https://docs.perplexity.ai/",
  "tutorial": "https://docs.perplexity.ai/guides/getting-started",
  "added": "2026-08-14",
  "dex": 256
 },
 {
  "id": "pipecat",
  "name": "Pipecat",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "Pipecat is like a conveyor belt in a factory that moves different types of stuff (voice, video, AI) down the line in the right order to build voice and video applications.",
  "description": "Pipecat is an open-source framework for building real-time, multi-modal AI applications with low-latency voice, video, and text processing. It abstracts away the complexity of coordinating multiple AI/ML models and managing real-time media streams.",
  "use_cases": [
   "Building conversational AI voice agents",
   "Creating real-time video conferencing applications with AI features",
   "Developing intelligent meeting assistants and transcription services",
   "Building interactive voice response (IVR) systems with natural language understanding"
  ],
  "used_by": [
   "Daily",
   "Anthropic",
   "Various AI Startups",
   "Enterprise Contact Centers"
  ],
  "docs": "https://docs.pipecat.ai/",
  "tutorial": "https://docs.pipecat.ai/get-started",
  "added": "2026-09-15",
  "dex": 257
 },
 {
  "id": "pydantic-ai",
  "name": "Pydantic AI",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "Pydantic AI is like a smart clipboard that helps you ask AI assistants questions and makes sure their answers are in exactly the format you need.",
  "description": "Pydantic AI is a Python framework for building production-ready applications powered by large language models with strong validation and structured outputs. It provides type-safe interactions with various LLM providers and reduces overhead compared to traditional LLM frameworks.",
  "use_cases": [
   "Building AI agents with structured tool use and reasoning",
   "Ensuring validated outputs from language models",
   "Multi-provider LLM applications with provider switching",
   "Enterprise AI application development with type safety"
  ],
  "used_by": [
   "Pydantic maintainers",
   "enterprises building AI applications"
  ],
  "docs": "https://ai.pydantic.dev/",
  "tutorial": "https://ai.pydantic.dev/latest/getting-started/",
  "added": "2026-08-19",
  "dex": 258
 },
 {
  "id": "pytorch",
  "name": "PyTorch",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "A Lego kit for building and training AI brains — you snap pieces together and it figures out how to teach itself from examples.",
  "description": "Open-source deep learning framework by Meta AI with dynamic computation graphs and Pythonic ergonomics. The dominant choice for ML research and increasingly for production, powering most frontier model development.",
  "use_cases": [
   "Train large language models and transformers from scratch",
   "Fine-tune pretrained models on custom datasets",
   "Research new architectures with dynamic graphs and eager execution",
   "Multi-GPU and distributed training with FSDP and DDP",
   "Export models to TorchScript or ONNX for deployment"
  ],
  "used_by": [
   "Meta",
   "OpenAI",
   "Hugging Face",
   "Tesla",
   "Microsoft"
  ],
  "docs": "https://pytorch.org/docs/",
  "tutorial": "https://pytorch.org/tutorials/",
  "added": "2026-06-10",
  "dex": 259
 },
 {
  "id": "ray",
  "name": "Ray",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "Like a factory manager for AI work — it splits a huge job across many computers and makes sure everything gets done in the right order.",
  "description": "Open-source distributed computing framework for scaling Python and ML workloads from a laptop to a cluster. Core of the Ray AI Runtime (AIR) — includes Ray Train, Tune, Serve, and Data for end-to-end ML pipelines.",
  "use_cases": [
   "Distributed LLM training across hundreds of GPUs",
   "Hyperparameter tuning at scale with Ray Tune",
   "Production model serving with Ray Serve and autoscaling",
   "Parallel data preprocessing pipelines with Ray Data",
   "Reinforcement learning with RLlib"
  ],
  "used_by": [
   "Shopify",
   "Uber",
   "OpenAI",
   "Cohere",
   "Instacart"
  ],
  "docs": "https://docs.ray.io/",
  "tutorial": "https://docs.ray.io/en/latest/ray-overview/getting-started.html",
  "added": "2026-06-10",
  "dex": 260
 },
 {
  "id": "rocm",
  "name": "ROCm",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "AMD's answer to NVIDIA's CUDA — lets you use AMD graphics cards to train and run AI models, like using a different brand of engine in the same race car.",
  "description": "AMD's open-source GPU computing platform, providing a full software stack (HIP, MIOpen, rocBLAS) for running ML workloads on AMD GPUs. HIP allows near-direct CUDA code porting to AMD hardware.",
  "use_cases": [
   "Train PyTorch and TensorFlow models on AMD Instinct GPUs",
   "Port CUDA codebases to AMD hardware with HIP",
   "Run vLLM and Hugging Face models on AMD MI300X clusters",
   "Cost-competitive alternative to NVIDIA for HPC and ML",
   "Open-source ML infrastructure without CUDA vendor lock-in"
  ],
  "used_by": [
   "AMD",
   "Microsoft Azure",
   "Oracle Cloud",
   "HPE",
   "Meta"
  ],
  "docs": "https://rocm.docs.amd.com/",
  "tutorial": "https://rocm.docs.amd.com/en/latest/tutorial/quick-start.html",
  "added": "2026-06-10",
  "dex": 261
 },
 {
  "id": "runwayml-gen2",
  "name": "Runway Gen-2",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Runway Gen-2 is like having a magic artist who can watch a video you show them and then create new videos that continue the story or change what happens in the video.",
  "description": "Runway Gen-2 is an AI-powered video generation and editing tool that can create, extend, and manipulate videos using natural language prompts. It's built for creators, filmmakers, and content producers who need fast, accessible AI-driven video production.",
  "use_cases": [
   "Generating short-form video content for social media from text descriptions",
   "Extending or editing video sequences without manual frame-by-frame work",
   "Creating visual effects and motion graphics through natural language commands",
   "Prototyping video ideas and storyboards at speed"
  ],
  "used_by": [
   "Adobe",
   "TikTok creators",
   "Independent filmmakers",
   "Marketing agencies"
  ],
  "docs": "https://runwayml.com/research/gen-2",
  "tutorial": "https://runwayml.com/guides",
  "added": "2026-07-17",
  "dex": 262
 },
 {
  "id": "runwayml",
  "name": "Runway ML",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Runway ML is like a magic tool that lets artists and creators use powerful AI to generate videos and images without needing to be computer scientists.",
  "description": "Runway ML is a generative AI platform offering video and image generation, editing, and AI-powered creative tools accessible through an intuitive interface. It enables content creators and developers to leverage cutting-edge machine learning models without deep technical expertise.",
  "use_cases": [
   "AI video generation and editing",
   "Image synthesis and manipulation",
   "Content creation automation",
   "Motion tracking and visual effects"
  ],
  "used_by": [
   "Adobe",
   "Getty Images",
   "major production studios"
  ],
  "docs": "https://docs.runwayml.com/",
  "tutorial": "https://learn.runwayml.com/",
  "added": "2026-07-05",
  "dex": 263
 },
 {
  "id": "semantic-kernel",
  "name": "Semantic Kernel",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like a translator that helps your regular computer programs talk to AI models so they can work together.",
  "description": "Microsoft's open-source SDK that enables developers to integrate large language models and AI services into their applications through a plugin-based architecture. It abstracts complexity of prompt engineering and function calling across multiple AI providers.",
  "use_cases": [
   "Building AI-augmented enterprise applications with LLM orchestration",
   "Creating autonomous agents that combine multiple AI capabilities",
   "Integrating OpenAI, Azure OpenAI, and other LLMs into existing codebases",
   "Implementing retrieval-augmented generation (RAG) patterns"
  ],
  "used_by": [
   "Microsoft",
   "GitHub",
   "Enterprise software teams"
  ],
  "docs": "https://learn.microsoft.com/en-us/semantic-kernel/overview/",
  "tutorial": "https://github.com/microsoft/semantic-kernel/tree/main/samples",
  "added": "2026-08-25",
  "dex": 264
 },
 {
  "id": "sparrow",
  "name": "Sparrow",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Sparrow is like a smart robot that reads documents and tables and can answer questions about them by finding and understanding the information inside.",
  "description": "Sparrow is Google's research tool for document understanding and table question-answering powered by foundation models. It demonstrates advanced capabilities in extracting information from complex layouts and answering questions about document content.",
  "use_cases": [
   "Automated extraction of information from unstructured documents",
   "Question-answering over tables and semi-structured data",
   "Document classification and information retrieval",
   "Processing invoices, forms, and business documents at scale"
  ],
  "used_by": [
   "Google Cloud",
   "Enterprise AI platforms",
   "Document processing startups"
  ],
  "docs": "https://github.com/google-research/sparrow",
  "tutorial": "https://github.com/google-research/sparrow/blob/main/README.md",
  "added": "2026-07-06",
  "dex": 265
 },
 {
  "id": "tensorrt",
  "name": "TensorRT",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A turbocharger for AI models on NVIDIA GPUs — it restructures and compresses the AI so it runs much faster without losing accuracy.",
  "description": "NVIDIA SDK for high-performance deep learning inference. Optimizes trained models via layer fusion, precision calibration (FP16/INT8), and kernel auto-tuning to minimize latency and maximize GPU throughput at inference time.",
  "use_cases": [
   "Reduce LLM and vision model latency for real-time applications",
   "INT8/FP16 quantization with accuracy calibration",
   "Deploy optimized models to Jetson edge devices",
   "Integrate with Triton for production inference pipelines",
   "Accelerate NLP, CV, and recommendation models"
  ],
  "used_by": [
   "NVIDIA",
   "Tesla",
   "Mercedes-Benz",
   "Salesforce",
   "Adobe"
  ],
  "docs": "https://docs.nvidia.com/deeplearning/tensorrt/",
  "tutorial": "https://docs.nvidia.com/deeplearning/tensorrt/quick-start-guide/",
  "added": "2026-06-10",
  "dex": 266
 },
 {
  "id": "triton-inference-server",
  "name": "Triton Inference Server",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A professional delivery system for AI models — it takes requests from many users and routes them to the right AI model as fast as possible.",
  "description": "NVIDIA's open-source inference serving platform supporting multiple ML frameworks (TensorRT, PyTorch, ONNX, TensorFlow) with dynamic batching, model ensembles, and GPU/CPU scheduling optimized for production deployments.",
  "use_cases": [
   "Serve multiple model frameworks from a single server",
   "Dynamic batching to maximize GPU throughput",
   "Model ensemble pipelines for multi-stage inference",
   "gRPC and HTTP endpoints with metrics for Prometheus",
   "Model versioning and A/B testing in production"
  ],
  "used_by": [
   "NVIDIA",
   "AWS",
   "Microsoft",
   "Waymo",
   "Comcast"
  ],
  "docs": "https://docs.nvidia.com/deeplearning/triton-inference-server/",
  "tutorial": "https://github.com/triton-inference-server/server/blob/main/docs/getting_started/quickstart.md",
  "added": "2026-06-10",
  "dex": 267
 },
 {
  "id": "ultralytics-yolo",
  "name": "Ultralytics YOLOv8",
  "category": "Framework",
  "domain": "AI/ML",
  "eli5": "It's like giving a computer the ability to instantly point out and identify every object in a photo, like finding all the cats in a room full of animals.",
  "description": "YOLOv8 is a state-of-the-art object detection framework that can identify, locate, and classify objects in images and videos with real-time performance. It's widely adopted for computer vision tasks and provides a user-friendly Python API with pre-trained models.",
  "use_cases": [
   "Real-time object detection in video surveillance systems",
   "Automated quality inspection in manufacturing",
   "Autonomous vehicle perception systems",
   "Retail shelf monitoring and inventory tracking"
  ],
  "used_by": [
   "Tesla",
   "Meta",
   "Amazon",
   "Microsoft"
  ],
  "docs": "https://docs.ultralytics.com/",
  "tutorial": "https://docs.ultralytics.com/quickstart/",
  "added": "2026-09-16",
  "dex": 268
 },
 {
  "id": "vapi-voice-ai",
  "name": "Vapi Voice AI",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a robot receptionist you can hire instantly that answers phone calls and talks to customers using artificial intelligence, no training required.",
  "description": "Vapi is a platform for building, deploying, and managing voice AI agents that can handle phone calls, handle customer conversations, and integrate with business systems. It abstracts away the complexity of speech recognition, LLM processing, and text-to-speech synthesis.",
  "use_cases": [
   "Building AI customer support phone agents",
   "Automating appointment scheduling and confirmations",
   "Creating voice-based lead qualification systems",
   "Building multilingual voice assistants for sales teams"
  ],
  "used_by": [
   "Redfin",
   "Blank",
   "Helix",
   "Wellness Coach"
  ],
  "docs": "https://docs.vapi.ai/",
  "tutorial": "https://docs.vapi.ai/introduction",
  "added": "2026-09-16",
  "dex": 269
 },
 {
  "id": "vertex-ai",
  "name": "Vertex AI",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Google's AI workshop — a place where you train, test, and deploy machine learning models without managing servers.",
  "description": "GCP's unified ML platform — consolidates AutoML and custom training into one surface. Covers the full ML lifecycle: data prep, training, evaluation, deployment, and monitoring. Integrates with Gemini models, TPUs, and the Vertex AI Model Garden for 100+ foundation models.",
  "use_cases": [
   "Training custom ML models with managed compute (GPUs, TPUs) at scale",
   "Deploying models to managed endpoints with autoscaling and A/B traffic splitting",
   "Vertex AI Pipelines for reproducible MLOps workflows (Kubeflow-based)",
   "Gemini API access and fine-tuning for enterprise LLM applications",
   "Feature Store for sharing, serving, and monitoring ML features across teams"
  ],
  "used_by": [
   "Wayfair",
   "Levi Strauss",
   "Ford",
   "Carrefour",
   "McLaren"
  ],
  "docs": "https://cloud.google.com/vertex-ai/docs",
  "tutorial": "https://cloud.google.com/vertex-ai/docs/start/introduction-unified-platform",
  "added": "2026-06-08",
  "dex": 270
 },
 {
  "id": "vllm",
  "name": "vLLM",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "A super-efficient waiter for AI models — it batches everyone's requests together so the AI can answer many people at once instead of one at a time.",
  "description": "High-throughput and memory-efficient LLM inference and serving engine. Uses PagedAttention to manage GPU KV-cache like virtual memory, dramatically increasing serving throughput vs. naive implementations.",
  "use_cases": [
   "Serve open-source LLMs (Llama, Mistral, Qwen) at production scale",
   "OpenAI-compatible API endpoint for self-hosted models",
   "Continuous batching to maximize GPU utilization",
   "Speculative decoding for faster token generation",
   "Multi-LoRA serving to host many fine-tuned model variants on one GPU"
  ],
  "used_by": [
   "Mistral AI",
   "Together AI",
   "Anyscale",
   "Replicate",
   "Nvidia"
  ],
  "docs": "https://docs.vllm.ai/",
  "tutorial": "https://docs.vllm.ai/en/latest/getting_started/quickstart.html",
  "added": "2026-06-10",
  "dex": 271
 },
 {
  "id": "voxel51",
  "name": "Voxel51 FiftyOne",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a quality inspector for AI training data—you can see what pictures your AI learned from and spot mistakes before they cause problems.",
  "description": "FiftyOne is an open-source platform for curating, analyzing, and improving computer vision datasets and models. It provides interactive tools to detect label errors, visualize model predictions, and manage data at scale for machine learning workflows.",
  "use_cases": [
   "Detecting and correcting labeling errors in vision datasets before training",
   "Analyzing model failure modes and finding edge cases",
   "Managing and versioning large-scale image and video datasets",
   "Validating autonomous vehicle perception datasets"
  ],
  "used_by": [
   "Google",
   "Tesla",
   "Cruise",
   "Roboflow"
  ],
  "docs": "https://docs.voxel51.com/",
  "tutorial": "https://docs.voxel51.com/getting_started/index.html",
  "added": "2026-09-01",
  "dex": 272
 },
 {
  "id": "weave",
  "name": "Weave",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "It's like a notebook that watches what your AI does and remembers every step so you can see if it made any mistakes.",
  "description": "Weave is an AI observability and evaluation platform that tracks, evaluates, and debugs AI applications including LLM chains and agentic systems. It provides production monitoring and testing tools for generative AI systems.",
  "use_cases": [
   "LLM application monitoring and debugging",
   "Prompt engineering and optimization",
   "AI model evaluation and quality assurance",
   "Production observability for agentic AI systems"
  ],
  "used_by": [
   "OpenAI partners",
   "Anthropic ecosystem",
   "Scale AI",
   "Hugging Face"
  ],
  "docs": "https://docs.wandb.ai/guides/weave",
  "tutorial": "https://github.com/wandb/weave",
  "added": "2026-07-01",
  "dex": 273
 },
 {
  "id": "weaviate-search",
  "name": "Weaviate Vector Search",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "It's like a super-smart librarian who organizes books not just by title, but by meaning, so you can find what you're looking for even if you describe it in different words.",
  "description": "Weaviate is an open-source vector database and AI-native search engine that enables semantic search, similarity matching, and retrieval-augmented generation (RAG) applications. It's designed to scale vector embeddings with built-in support for large language models and real-time indexing.",
  "use_cases": [
   "Semantic search and recommendation systems",
   "Retrieval-augmented generation (RAG) for LLM applications",
   "Similarity-based product and content discovery",
   "Vector-based image and document search"
  ],
  "used_by": [
   "Neon",
   "Canva",
   "GetApp",
   "Siemens"
  ],
  "docs": "https://weaviate.io/developers/weaviate",
  "tutorial": "https://weaviate.io/learn",
  "added": "2026-07-07",
  "dex": 274
 },
 {
  "id": "weights-and-biases",
  "name": "Weights & Biases",
  "category": "Tool",
  "domain": "AI/ML",
  "eli5": "Weights & Biases is like a detailed science notebook that automatically records every experiment you run so you can compare what worked and what didn't.",
  "description": "MLOps platform for experiment tracking, model versioning, dataset management, and hyperparameter optimization. Provides dashboards for comparing runs across teams.",
  "use_cases": [
   "Tracking loss curves and metrics across training runs",
   "Hyperparameter sweep automation",
   "Model artifact versioning and registry",
   "Collaborative ML experiment sharing"
  ],
  "used_by": [
   "OpenAI",
   "NVIDIA",
   "Toyota Research",
   "Hugging Face"
  ],
  "docs": "https://docs.wandb.ai/",
  "tutorial": "https://docs.wandb.ai/quickstart",
  "added": "2026-06-11",
  "dex": 275
 },
 {
  "id": "zilliz-cloud",
  "name": "Zilliz Cloud",
  "category": "Platform",
  "domain": "AI/ML",
  "eli5": "Zilliz Cloud is like a super-smart memory for AI that instantly finds the most similar things you're looking for from billions of possibilities.",
  "description": "Zilliz Cloud is a managed vector database service providing scalable and high-performance vector search capabilities for AI/ML applications. It's the commercial offering built on Milvus, optimized for semantic search, RAG, and similarity matching workloads.",
  "use_cases": [
   "Building semantic search systems for retrieval-augmented generation (RAG)",
   "Implementing AI recommendation engines with embeddings",
   "Powering AI chatbots with long-term memory and context retrieval",
   "Duplicate detection and anomaly detection in large datasets"
  ],
  "used_by": [
   "Capital One",
   "Citi",
   "Thomson Reuters",
   "ZTE"
  ],
  "docs": "https://docs.zilliz.com/",
  "tutorial": "https://docs.zilliz.com/docs/quickstart",
  "added": "2026-07-23",
  "dex": 276
 },
 {
  "id": "alertmanager",
  "name": "Alertmanager",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Alertmanager is like the on-call coordinator for your monitoring system — it groups noisy alerts, silences duplicates, and routes them to the right team on Slack or PagerDuty.",
  "description": "Alert routing and management component of the Prometheus ecosystem. Handles deduplication, grouping, silencing, and routing of alerts to receivers like PagerDuty, Slack, and email.",
  "use_cases": [
   "Routing Prometheus alerts to Slack/PagerDuty/email",
   "Deduplicating and grouping related alerts",
   "Silencing alerts during maintenance windows",
   "Inhibiting child alerts when parent is already firing"
  ],
  "used_by": [
   "Anyone running Prometheus",
   "SoundCloud",
   "DigitalOcean"
  ],
  "docs": "https://prometheus.io/docs/alerting/latest/alertmanager/",
  "tutorial": "https://prometheus.io/docs/alerting/latest/configuration/",
  "added": "2026-06-14",
  "dex": 277
 },
 {
  "id": "jmeter-load-testing",
  "name": "Apache JMeter",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "JMeter is like a stress-test machine that pretends to be thousands of people using your website at the same time to see if it breaks.",
  "description": "Apache JMeter is an open-source Java application designed for load testing and performance measurement of web applications, APIs, and databases. It simulates heavy user loads to identify performance bottlenecks and verify system capacity.",
  "use_cases": [
   "Load testing and stress testing web applications",
   "API performance and reliability testing",
   "Database and backend performance benchmarking",
   "Capacity planning and scalability validation"
  ],
  "used_by": [
   "Netflix",
   "Twitter",
   "Spotify",
   "Adobe"
  ],
  "docs": "https://jmeter.apache.org/usermanual/index.html",
  "tutorial": "https://jmeter.apache.org/usermanual/get-started.html",
  "added": "2026-09-26",
  "dex": 278
 },
 {
  "id": "aws-cloudwatch",
  "name": "AWS CloudWatch",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "AWS CloudWatch — unified observability service for logs, metrics, traces, and alarms across AWS infrastructure. Native integration with every AWS service; single pane of glass for AWS-native monitoring.",
  "use_cases": [
   "Metrics collection and dashboards for EC2, Lambda, RDS, EKS",
   "Log aggregation via CloudWatch Logs and Logs Insights queries",
   "Alarms and auto-scaling triggers on metric thresholds",
   "CloudWatch Container Insights for EKS/ECS workload visibility",
   "Synthetics canaries for endpoint availability monitoring"
  ],
  "used_by": [
   "All AWS customers"
  ],
  "docs": "https://docs.aws.amazon.com/cloudwatch/",
  "tutorial": "https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/GettingStarted.html",
  "added": "2026-06-08",
  "eli5": "A security camera and alarm system for everything in your AWS house.",
  "dex": 279
 },
 {
  "id": "axiom",
  "name": "Axiom",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Axiom is like a massive event recorder that watches everything happening in your app and lets you search through it all super fast to find problems.",
  "description": "Axiom is a serverless data analytics platform purpose-built for log and event data, offering high-volume ingestion with sub-second query performance and cost efficiency. It eliminates the need for complex log aggregation infrastructure.",
  "use_cases": [
   "Real-time log aggregation and analysis",
   "Event streaming and debugging for distributed systems",
   "Security and compliance audit logging",
   "Cost-effective high-volume metric and trace storage"
  ],
  "used_by": [
   "Vercel",
   "Supabase",
   "Webflow",
   "Retool"
  ],
  "docs": "https://axiom.co/docs",
  "tutorial": "https://axiom.co/docs/getting-started",
  "added": "2026-07-03",
  "dex": 280
 },
 {
  "id": "axiom-cloud-analytics",
  "name": "Axiom Cloud Analytics",
  "category": "Platform",
  "domain": "SRE/Observability",
  "eli5": "Axiom is like a search engine for all the log messages and events from your application, letting you find problems super fast without needing huge expensive computers.",
  "description": "Axiom is a serverless log analytics and monitoring platform that ingests, stores, and analyzes vast amounts of structured data with sub-second query performance. It eliminates the need for managing infrastructure like Elasticsearch while providing cost-effective observability at scale.",
  "use_cases": [
   "Real-time log aggregation and analysis from distributed systems",
   "Cost-effective alternative to traditional centralized logging platforms",
   "Event-driven debugging and root cause analysis",
   "Compliance and audit logging at scale"
  ],
  "used_by": [
   "B2B SaaS companies",
   "DevOps teams managing microservices",
   "enterprises using event-driven architectures"
  ],
  "docs": "https://axiom.co/docs",
  "tutorial": "https://axiom.co/docs/getting-started",
  "added": "2026-08-19",
  "dex": 281
 },
 {
  "id": "axiom-data",
  "name": "Axiom Data",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Axiom is like a super-fast record keeper that remembers everything that happens in your computer and lets you search through it instantly to find problems.",
  "description": "Axiom is a serverless log management and analytics platform designed for modern cloud applications with unlimited cardinality and sub-second query performance. It provides log ingestion, storage, and analysis without the complexity of traditional observability stacks.",
  "use_cases": [
   "Ingesting and searching high-volume application logs at scale",
   "Debugging production issues with full-text search across all log data",
   "Building custom dashboards and alerts for operational metrics",
   "Streaming logs from serverless functions and microservices"
  ],
  "used_by": [
   "Vercel",
   "Railway",
   "Cloud providers",
   "SaaS platforms"
  ],
  "docs": "https://axiom.co/docs",
  "tutorial": "https://axiom.co/docs/getting-started",
  "added": "2026-07-17",
  "dex": 282
 },
 {
  "id": "datadog",
  "name": "Datadog",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "Cloud-scale monitoring and observability SaaS platform. Unified dashboards for metrics, traces, logs, and synthetics across infrastructure, applications, and third-party services. 600+ integrations out of the box.",
  "use_cases": [
   "Infrastructure monitoring: hosts, containers, Kubernetes, serverless",
   "APM with distributed tracing, flame graphs, and service maps",
   "Log Management with live tail, parsing pipelines, and archive to S3",
   "Synthetic monitoring: browser tests and API checks for uptime and performance",
   "Cloud cost monitoring and anomaly detection with Watchdog AI"
  ],
  "used_by": [
   "Peloton",
   "Samsung",
   "Airbnb",
   "Whole Foods",
   "Atlassian"
  ],
  "docs": "https://docs.datadoghq.com/",
  "tutorial": "https://docs.datadoghq.com/getting_started/",
  "added": "2026-06-08",
  "eli5": "A dashboard showing every heartbeat of your apps and servers, all in one place.",
  "dex": 283
 },
 {
  "id": "dynatrace",
  "name": "Dynatrace",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "AI-powered full-stack observability platform. OneAgent auto-instruments hosts, containers, and processes with zero config. Davis AI engine automatically detects anomalies, determines root cause, and pinpoints blast radius — reducing MTTR significantly.",
  "use_cases": [
   "Auto-instrumented APM across microservices, serverless, and mainframes via OneAgent",
   "Davis AI: automated root cause analysis without manual alert correlation",
   "Real user monitoring (RUM) and session replay for web and mobile apps",
   "Kubernetes observability with automatic workload and dependency discovery",
   "Dynatrace Grail for unified metrics, logs, traces, and events in one lakehouse"
  ],
  "used_by": [
   "SAP",
   "Delta Airlines",
   "Lenovo",
   "Lufthansa",
   "BNP Paribas"
  ],
  "docs": "https://docs.dynatrace.com/docs",
  "tutorial": "https://docs.dynatrace.com/docs/get-started",
  "added": "2026-06-08",
  "eli5": "A smart robot that watches your whole app and spots problems before you do.",
  "dex": 284
 },
 {
  "id": "elk-stack",
  "name": "ELK Stack",
  "category": "Tool",
  "domain": "SRE/Observability",
  "description": "Elasticsearch + Logstash + Kibana — the classic open-source log management trio. Elasticsearch indexes and searches logs at scale, Logstash ingests and transforms, Kibana visualizes. Often extended with Beats agents (now Elastic Stack).",
  "use_cases": [
   "Centralized log aggregation from servers, containers, and applications",
   "Full-text search and analysis across billions of log events",
   "Security analytics and SIEM (Elastic Security)",
   "APM (Application Performance Monitoring) via Elastic APM agents",
   "Kibana dashboards for log-driven operational intelligence"
  ],
  "used_by": [
   "LinkedIn",
   "Netflix",
   "Adobe",
   "Microsoft",
   "Walmart"
  ],
  "docs": "https://www.elastic.co/guide/index.html",
  "tutorial": "https://www.elastic.co/guide/en/elasticsearch/reference/current/getting-started.html",
  "added": "2026-06-08",
  "eli5": "A giant diary plus search engine for all your app's log messages.",
  "dex": 285
 },
 {
  "id": "grafana",
  "name": "Grafana",
  "category": "Tool",
  "domain": "SRE/Observability",
  "description": "Open-source analytics and visualization platform. Connects to 150+ data sources (Prometheus, Loki, Tempo, Elasticsearch, CloudWatch) and renders time-series dashboards, logs, and traces in a unified UI.",
  "use_cases": [
   "Building operational dashboards for infrastructure and application metrics",
   "Unified observability: correlate metrics (Prometheus), logs (Loki), traces (Tempo)",
   "Alerting with multi-dimensional rules and on-call routing (Grafana OnCall)",
   "SLO/SLA dashboards with error budget burn rate tracking",
   "Grafana Cloud as managed observability stack (LGTM: Loki, Grafana, Tempo, Mimir)"
  ],
  "used_by": [
   "eBay",
   "PayPal",
   "Bloomberg",
   "Grafana Labs",
   "Salesforce"
  ],
  "docs": "https://grafana.com/docs/grafana/latest/",
  "tutorial": "https://grafana.com/docs/grafana/latest/getting-started/build-first-dashboard/",
  "added": "2026-06-08",
  "eli5": "Pretty charts built from Prometheus's notebook so humans can understand things.",
  "dex": 286
 },
 {
  "id": "grafana-alloy",
  "name": "Grafana Alloy",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a universal data collector that gathers information from everywhere in your systems and organizes it so you can understand what's happening.",
  "description": "Grafana Alloy (formerly Grafana Agent Flow) is a declarative observability pipeline distribution that collects, processes, and exports metrics, logs, and traces. It unifies observability data collection with flexible configuration and no performance penalties.",
  "use_cases": [
   "Collecting metrics, logs, and traces from hybrid infrastructure",
   "Processing and transforming observability data before ingestion",
   "Multi-backend observability pipeline (Prometheus, Loki, Tempo, others)",
   "Reducing vendor lock-in with flexible data routing"
  ],
  "used_by": [
   "Grafana Labs",
   "Canonical",
   "DigitalOcean"
  ],
  "docs": "https://grafana.com/docs/alloy/latest/",
  "tutorial": "https://grafana.com/docs/alloy/latest/get-started/install/",
  "added": "2026-07-27",
  "dex": 287
 },
 {
  "id": "grafana-loki",
  "name": "Grafana Loki",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Loki is like a filing cabinet for all your app logs — it only reads the label on the folder to find them fast, not every single word inside.",
  "description": "Horizontally scalable log aggregation system inspired by Prometheus. Indexes only metadata (labels), not log content, making it far cheaper than full-text search solutions.",
  "use_cases": [
   "Kubernetes pod log aggregation",
   "Correlated log + metric + trace exploration in Grafana",
   "Cost-efficient alternative to Elasticsearch for logs",
   "Long-term log retention with object storage backend"
  ],
  "used_by": [
   "Grafana Labs",
   "Digital Ocean",
   "Gojek",
   "Adidas"
  ],
  "docs": "https://grafana.com/docs/loki/latest/",
  "tutorial": "https://grafana.com/docs/loki/latest/get-started/",
  "added": "2026-06-11",
  "dex": 288
 },
 {
  "id": "grafana-tempo",
  "name": "Grafana Tempo",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Tempo is like a GPS trip recorder for your requests — it saves every route a request took through your services so you can replay it later.",
  "description": "Scalable, cost-efficient distributed tracing backend that stores traces in object storage and integrates natively with Grafana, Loki, and Prometheus for correlated observability.",
  "use_cases": [
   "Distributed trace storage at scale",
   "Correlating traces with logs and metrics in Grafana",
   "Root cause analysis for latency spikes",
   "OpenTelemetry and Jaeger trace ingestion"
  ],
  "used_by": [
   "Grafana Labs",
   "Shopify",
   "Weaveworks"
  ],
  "docs": "https://grafana.com/docs/tempo/latest/",
  "tutorial": "https://grafana.com/docs/tempo/latest/getting-started/",
  "added": "2026-06-11",
  "dex": 289
 },
 {
  "id": "jaeger",
  "name": "Jaeger",
  "category": "Tool",
  "domain": "SRE/Observability",
  "description": "Open-source distributed tracing platform originally built at Uber. Traces requests across microservices to pinpoint latency bottlenecks and failures. Implements OpenTracing/OpenTelemetry standards. CNCF graduated project.",
  "use_cases": [
   "End-to-end request tracing across microservices to find latency root causes",
   "Service dependency graph visualization from live trace data",
   "Comparing trace samples between deployments for performance regression detection",
   "Identifying slow DB queries and external API calls in distributed flows",
   "Ingesting OTel spans via OTLP protocol"
  ],
  "used_by": [
   "Uber",
   "Red Hat",
   "Ticketmaster",
   "Grafana Labs",
   "Weaveworks"
  ],
  "docs": "https://www.jaegertracing.io/docs/latest/",
  "tutorial": "https://www.jaegertracing.io/docs/latest/getting-started/",
  "added": "2026-06-08",
  "eli5": "A detective that follows a request through all your services to find where it slowed down.",
  "dex": 290
 },
 {
  "id": "k6-performance-testing",
  "name": "k6",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a stress test machine that pretends to be thousands of users visiting your website at once to make sure it doesn't break.",
  "description": "Developer-friendly load testing and performance testing platform that uses JavaScript for test scripting and provides cloud-based distributed load generation. It enables teams to catch performance bottlenecks early and validate system reliability under load.",
  "use_cases": [
   "Load testing APIs and web applications before production deployment",
   "Continuous performance testing in CI/CD pipelines",
   "Spike testing and stress testing to find breaking points",
   "Monitoring production performance with synthetic transactions"
  ],
  "used_by": [
   "Grafana Labs",
   "Microsoft",
   "GitHub",
   "Stripe"
  ],
  "docs": "https://grafana.com/docs/k6/latest/",
  "tutorial": "https://grafana.com/docs/k6/latest/get-started/",
  "added": "2026-08-25",
  "dex": 291
 },
 {
  "id": "new-relic",
  "name": "New Relic",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "Full-stack observability platform with APM, infrastructure monitoring, browser/mobile, logs, and synthetics. NRQL (New Relic Query Language) for ad-hoc telemetry analysis. Offers a generous free tier with 100GB/month data ingest.",
  "use_cases": [
   "APM: transaction traces, error rates, slow query detection across all major languages",
   "Infrastructure monitoring for hosts, Kubernetes, and cloud services",
   "Browser and mobile real-user monitoring (RUM) with Core Web Vitals",
   "NRQL dashboards for custom metric exploration and alerting",
   "OTel-native ingest: send spans and metrics via OTLP without proprietary agents"
  ],
  "used_by": [
   "GitHub",
   "Epic Games",
   "DoorDash",
   "Ticketmaster",
   "Ford"
  ],
  "docs": "https://docs.newrelic.com/",
  "tutorial": "https://docs.newrelic.com/docs/new-relic-solutions/get-started/intro-new-relic/",
  "added": "2026-06-08",
  "eli5": "A doctor for your app — checks its health and explains when and why it got sick.",
  "dex": 292
 },
 {
  "id": "open-source-observability",
  "name": "OpenObservability Alliance",
  "category": "Concept",
  "domain": "SRE/Observability",
  "eli5": "It's like having multiple doctors (logging, metrics, and traces) working together to understand what your app is doing, instead of each one working separately.",
  "description": "The OpenObservability Alliance standardizes open-source observability tools and practices through specifications like OpenTelemetry. It enables unified metrics, logs, and traces collection across distributed systems without vendor lock-in.",
  "use_cases": [
   "Unified observability across microservices architectures",
   "Multi-vendor observability platform integration",
   "Standardized instrumentation across polyglot codebases",
   "Cost-effective observability with open-source tools"
  ],
  "used_by": [
   "Datadog",
   "New Relic",
   "Splunk",
   "Grafana Labs"
  ],
  "docs": "https://openobservability.io/",
  "tutorial": "https://opentelemetry.io/docs/getting-started/",
  "added": "2026-06-26",
  "dex": 293
 },
 {
  "id": "openobserve",
  "name": "OpenObserve",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a giant notebook that records everything happening in your computer system so you can look back and figure out what went wrong.",
  "description": "OpenObserve is a cloud-native observability platform designed as a lightweight alternative to Elasticsearch for logs, metrics, and traces. It provides high-performance data ingestion and search capabilities with a significantly lower resource footprint.",
  "use_cases": [
   "Centralized log aggregation and analysis across microservices",
   "Real-time monitoring of application performance and infrastructure metrics",
   "Distributed tracing for debugging complex request flows",
   "Cost-effective observability for resource-constrained environments"
  ],
  "used_by": [
   "Cisco",
   "SAP",
   "Broadcom",
   "OpenObserve Community"
  ],
  "docs": "https://openobserve.ai/docs/",
  "tutorial": "https://openobserve.ai/docs/quickstart/",
  "added": "2026-06-17",
  "dex": 294
 },
 {
  "id": "opensearch-dashboards",
  "name": "OpenSearch Dashboards",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "OpenSearch Dashboards is like a control panel with colorful displays that shows you everything happening inside your computer system in real time.",
  "description": "OpenSearch Dashboards is the visualization and exploration platform for OpenSearch, providing real-time insights into logs, metrics, and security data. It enables rapid problem diagnosis and operational intelligence.",
  "use_cases": [
   "Log analysis and troubleshooting",
   "Real-time security monitoring and threat detection",
   "Application performance monitoring and debugging",
   "Business analytics and metrics visualization"
  ],
  "used_by": [
   "Capital One",
   "Cisco",
   "Samsung",
   "Expedia"
  ],
  "docs": "https://opensearch.org/docs/latest/dashboards/",
  "tutorial": "https://opensearch.org/docs/latest/dashboards/get-started/",
  "added": "2026-06-16",
  "dex": 295
 },
 {
  "id": "opentelemetry",
  "name": "OpenTelemetry",
  "category": "Framework",
  "domain": "SRE/Observability",
  "description": "Vendor-neutral observability framework and CNCF standard for generating, collecting, and exporting telemetry (traces, metrics, logs). Single set of APIs and SDKs replaces vendor-specific agents. The future of cloud-native instrumentation.",
  "use_cases": [
   "Instrument apps once with OTel SDK; route to any backend (Jaeger, Tempo, Datadog, Honeycomb)",
   "Auto-instrumentation for popular frameworks (no code changes for basic traces)",
   "OTel Collector as vendor-agnostic telemetry pipeline with processors and exporters",
   "Correlating traces, metrics, and logs via shared trace/span context",
   "Replacing proprietary agents (Datadog, New Relic) to avoid vendor lock-in"
  ],
  "used_by": [
   "Google",
   "Microsoft",
   "Splunk",
   "Datadog",
   "Honeycomb",
   "Grafana Labs"
  ],
  "docs": "https://opentelemetry.io/docs/",
  "tutorial": "https://opentelemetry.io/docs/getting-started/",
  "added": "2026-06-08",
  "eli5": "A universal plug that sends your app's health data to any monitoring tool you like.",
  "dex": 296
 },
 {
  "id": "pagerduty",
  "name": "PagerDuty",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "Incident management and on-call alerting platform. Routes alerts from monitoring tools to the right on-call engineer via phone, SMS, Slack, or push. Manages escalation policies, runbooks, and postmortem workflows.",
  "use_cases": [
   "On-call scheduling with escalation policies and override management",
   "Alert routing from Prometheus, Datadog, CloudWatch, and 700+ integrations",
   "Intelligent alert grouping and noise reduction (Event Intelligence)",
   "Incident timelines, stakeholder communication, and status page updates",
   "Postmortem workflows and SLA/SLO reporting for reliability metrics"
  ],
  "used_by": [
   "GE",
   "Lululemon",
   "Comcast",
   "IBM",
   "Zoom",
   "Slack"
  ],
  "docs": "https://support.pagerduty.com/docs",
  "tutorial": "https://support.pagerduty.com/docs/introduction",
  "added": "2026-06-08",
  "eli5": "An alarm clock that wakes up the right engineer when something breaks at 3am.",
  "dex": 297
 },
 {
  "id": "pagerduty-event-intelligence",
  "name": "PagerDuty Event Intelligence",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like having a smart alarm system that listens to all your house sensors and groups similar warnings together instead of waking you up for every little thing.",
  "description": "PagerDuty Event Intelligence uses machine learning to intelligently group, correlate, and deduplicate alerts from multiple monitoring systems, reducing alert noise and helping teams respond faster to real incidents. It learns from incident history to improve noise reduction over time.",
  "use_cases": [
   "Reducing alert fatigue and false positives in large-scale systems",
   "Automating incident correlation across multiple monitoring tools",
   "Improving mean time to resolution (MTTR) through intelligent routing",
   "Learning incident patterns to predict and prevent outages"
  ],
  "used_by": [
   "Uber",
   "Twitch",
   "GitHub",
   "Adobe"
  ],
  "docs": "https://support.pagerduty.com/docs/event-intelligence",
  "tutorial": "https://support.pagerduty.com/docs/event-intelligence-getting-started",
  "added": "2026-09-13",
  "dex": 298
 },
 {
  "id": "parseable",
  "name": "Parseable",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's a lightweight log storage and search tool that works like a notebook where you can quickly write down and find all the things that happen in your applications.",
  "description": "Parseable is a lightweight, cost-effective log storage and analysis platform designed as an alternative to Elasticsearch, offering better performance and lower resource consumption. It provides full-text search, data retention policies, and integrations with popular observability stacks.",
  "use_cases": [
   "Cost-effective centralized logging for cloud applications",
   "Real-time log aggregation and analysis from distributed systems",
   "Compliance and audit logging with retention policies",
   "Integration with existing monitoring stacks (Prometheus, Grafana)"
  ],
  "used_by": [
   "Startups",
   "Cloud-native teams",
   "Organizations seeking Elasticsearch alternatives"
  ],
  "docs": "https://www.parseable.io/docs/",
  "tutorial": "https://www.parseable.io/docs/getting-started",
  "added": "2026-08-12",
  "dex": 299
 },
 {
  "id": "prometheus",
  "name": "Prometheus",
  "category": "Tool",
  "domain": "SRE/Observability",
  "description": "Open-source metrics monitoring and alerting system. Scrapes time-series metrics from instrumented targets via HTTP pull model. PromQL for powerful dimensional queries. CNCF graduated project; de facto standard for Kubernetes monitoring.",
  "use_cases": [
   "Scraping metrics from apps, Kubernetes nodes, and exporters (node, blackbox, etc.)",
   "PromQL queries for real-time dashboards and alerting rules",
   "Kubernetes cluster monitoring via kube-state-metrics and cAdvisor",
   "Alertmanager integration for routing alerts to PagerDuty, Slack, etc.",
   "Long-term storage via remote_write to Thanos, Cortex, or Mimir"
  ],
  "used_by": [
   "SoundCloud",
   "DigitalOcean",
   "GitLab",
   "Robinhood",
   "Cloudflare"
  ],
  "docs": "https://prometheus.io/docs/introduction/overview/",
  "tutorial": "https://prometheus.io/docs/prometheus/latest/getting_started/",
  "added": "2026-06-08",
  "eli5": "A notebook that writes down how your app is doing every few seconds.",
  "dex": 300
 },
 {
  "id": "quickwit",
  "name": "Quickwit",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a super-fast library card catalog that can instantly find millions of log entries you're looking for, much faster than searching through boxes of paper.",
  "description": "Quickwit is a cloud-native search and analytics engine optimized for log and trace data with sub-second latency and cost efficiency. It's designed as a lighter alternative to Elasticsearch for organizations seeking lower infrastructure costs.",
  "use_cases": [
   "Searching and analyzing application logs at scale",
   "Real-time distributed tracing and debugging",
   "Cost-effective log aggregation and retention",
   "Security and compliance log analysis"
  ],
  "used_by": [
   "Datadog",
   "Stripe",
   "Segment",
   "Grafana Labs"
  ],
  "docs": "https://quickwit.io/docs/",
  "tutorial": "https://quickwit.io/docs/get-started/",
  "added": "2026-06-21",
  "dex": 301
 },
 {
  "id": "ryot",
  "name": "Ryot",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a personal diary that automatically remembers everything you watched, read, or listened to, so you can look back anytime.",
  "description": "Ryot is an open-source media tracking application that helps users document and organize their consumption of movies, TV shows, books, music, and games. It provides insights and statistics about media habits with social features.",
  "use_cases": [
   "Personal media library management and discovery",
   "Tracking viewing and reading habits over time",
   "Social media integration to share watched content",
   "Generating insights and statistics about consumption patterns"
  ],
  "used_by": [
   "Media enthusiasts",
   "Self-hosted communities",
   "Individual users"
  ],
  "docs": "https://ryot.io/docs",
  "tutorial": "https://ryot.io/docs/getting-started",
  "added": "2026-08-04",
  "dex": 302
 },
 {
  "id": "sentry",
  "name": "Sentry",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "Application monitoring platform focused on error tracking and performance. Captures exceptions with full stack traces, breadcrumbs, and user context. Helps developers find, triage, and fix production bugs fast. Open-source core, SaaS or self-hosted.",
  "use_cases": [
   "Real-time error and exception tracking with full stack trace and release attribution",
   "Performance monitoring: transaction traces, slow DB queries, N+1 detection",
   "Source maps for minified JS — see original code in production errors",
   "Release health: crash-free session rate per deploy version",
   "Alerts routed to Slack/PagerDuty with issue deduplication and ownership rules"
  ],
  "used_by": [
   "Cloudflare",
   "Disney",
   "GitHub",
   "Stripe",
   "Atlassian",
   "Microsoft"
  ],
  "docs": "https://docs.sentry.io/",
  "tutorial": "https://docs.sentry.io/platforms/",
  "added": "2026-06-08",
  "eli5": "A bug catcher that tells you exactly when, where, and why your app crashed.",
  "dex": 303
 },
 {
  "id": "sentry-ai-powered-debugging",
  "name": "Sentry AI-Powered Debugging",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like having a detective who automatically finds and reports every time something breaks in your app, then explains why it happened.",
  "description": "Sentry is an error tracking platform that captures, monitors, and analyzes exceptions and performance issues in real-time across web and mobile applications. It uses AI to automatically group related errors, suggest fixes, and predict where problems will occur.",
  "use_cases": [
   "Real-time crash detection and alerting for production applications",
   "Performance monitoring and bottleneck identification across services",
   "Session replay and user impact analysis for debugging issues",
   "AI-powered root cause analysis and automatic error grouping"
  ],
  "used_by": [
   "Airbnb",
   "Uber",
   "Spotify",
   "Microsoft"
  ],
  "docs": "https://docs.sentry.io/",
  "tutorial": "https://docs.sentry.io/product/getting-started/",
  "added": "2026-08-11",
  "dex": 304
 },
 {
  "id": "sentry-relay",
  "name": "Sentry Relay",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a security guard that checks and filters error reports before they reach the main office, stopping sensitive data from leaking out.",
  "description": "Sentry Relay is a service that acts as a proxy between your applications and Sentry's error tracking platform, allowing you to filter, sample, and redact sensitive data before it's sent. It enables on-premise deployment of Sentry's data processing pipeline with improved security and compliance.",
  "use_cases": [
   "Filtering personally identifiable information (PII) before sending error data to cloud services",
   "Rate limiting and sampling error events to reduce data ingestion costs",
   "Running Sentry infrastructure in air-gapped or regulated environments",
   "Processing and transforming error data with custom rules before storage"
  ],
  "used_by": [
   "Shopify",
   "Uber",
   "Twilio",
   "Segment"
  ],
  "docs": "https://develop.sentry.dev/relay/",
  "tutorial": "https://docs.sentry.io/product/relay/",
  "added": "2026-09-04",
  "dex": 305
 },
 {
  "id": "sentry-replays",
  "name": "Sentry Session Replays",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like a security camera for your website that records exactly what users did right before something broke, so you can rewatch and see what went wrong.",
  "description": "Sentry Session Replays captures user interactions and browser state to provide video-like playback of sessions leading up to errors. This enables developers to see exactly what users were doing when issues occurred, dramatically reducing debugging time.",
  "use_cases": [
   "Investigating frontend errors by replaying user sessions",
   "Understanding user experience issues and pain points",
   "Reproducing hard-to-diagnose bugs in production environments",
   "Analyzing performance issues in real user scenarios"
  ],
  "used_by": [
   "Shopify",
   "Airbnb",
   "Figma",
   "Discord"
  ],
  "docs": "https://docs.sentry.io/product/session-replay/",
  "tutorial": "https://docs.sentry.io/product/session-replay/getting-started/",
  "added": "2026-09-16",
  "dex": 306
 },
 {
  "id": "splunk",
  "name": "Splunk",
  "category": "Platform",
  "domain": "SRE/Observability",
  "description": "Enterprise platform for searching, monitoring, and analyzing machine-generated data — logs, metrics, events, and traces. SPL (Search Processing Language) for ad-hoc investigation. Dominant in enterprise SIEM and SOC workflows.",
  "use_cases": [
   "Centralized log aggregation and SPL-powered investigation at petabyte scale",
   "SIEM: security event correlation, threat detection, and compliance reporting",
   "IT operations: real-time dashboards and alerting on infrastructure log patterns",
   "Splunk SOAR for automated security incident response playbooks",
   "Observability Cloud (formerly SignalFx) for metrics and APM"
  ],
  "used_by": [
   "Domino's",
   "Carnival",
   "Booz Allen",
   "ING",
   "United States DoD"
  ],
  "docs": "https://docs.splunk.com/Documentation/Splunk",
  "tutorial": "https://docs.splunk.com/Documentation/Splunk/latest/SearchTutorial/WelcometotheSearchTutorial",
  "added": "2026-06-08",
  "eli5": "Google for your logs — find anything in billions of log lines instantly.",
  "dex": 307
 },
 {
  "id": "tailon",
  "name": "Tailon",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "It's like having a window into your computer's brain that shows you everything it's thinking in real-time.",
  "description": "Tailon is a web UI for tailing and analyzing log files in real-time across multiple servers and sources. It provides a unified interface for log aggregation without requiring external infrastructure like ELK or Loki.",
  "use_cases": [
   "Development environment log monitoring",
   "On-premise log aggregation without external services",
   "Real-time log filtering and searching",
   "Multiple server log streaming in one interface"
  ],
  "used_by": [
   "DevOps teams",
   "System administrators",
   "Smaller startups",
   "Internal tools teams"
  ],
  "docs": "https://github.com/ggreer/tail-f",
  "tutorial": "https://github.com/ggreer/tail-f#readme",
  "added": "2026-09-07",
  "dex": 308
 },
 {
  "id": "thanos",
  "name": "Thanos",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Thanos is like a backup brain for Prometheus — it copies all the metrics to cheap cloud storage and lets multiple Prometheus servers act as one.",
  "description": "Highly available Prometheus setup with long-term storage. Extends Prometheus with global query view, unlimited retention via object storage, and multi-cluster deduplication.",
  "use_cases": [
   "Long-term Prometheus metrics retention (S3/GCS/Azure Blob)",
   "Global query view across multiple Prometheus instances",
   "HA Prometheus with deduplication",
   "Cross-cluster metric federation"
  ],
  "used_by": [
   "Cloudflare",
   "Improbable",
   "Banzai Cloud",
   "Shopify"
  ],
  "docs": "https://thanos.io/tip/thanos/getting-started.md/",
  "tutorial": "https://thanos.io/tip/tutorials/quick-tutorial.md/",
  "added": "2026-06-12",
  "dex": 309
 },
 {
  "id": "vector-io",
  "name": "Vector",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "Vector is like a smart mail sorting machine that takes logs and metrics from everywhere in your house and delivers them to exactly where they need to go.",
  "description": "Vector is a lightweight, ultra-fast tool for building observability pipelines that collect, transform, and route logs, metrics, and traces from any source to any destination. It replaces multiple specialized tools with a single, composable runtime written in Rust.",
  "use_cases": [
   "Collecting and normalizing logs from Kubernetes clusters to multiple backends",
   "Building cost-effective observability pipelines by filtering and sampling high-volume metrics",
   "Transforming and enriching application telemetry before sending to SaaS platforms",
   "Aggregating traces and metrics from distributed systems with minimal performance overhead"
  ],
  "used_by": [
   "Datadog",
   "New Relic",
   "Fastly",
   "Segment"
  ],
  "docs": "https://vector.dev/docs/",
  "tutorial": "https://vector.dev/guides/getting-started/",
  "added": "2026-07-30",
  "dex": 310
 },
 {
  "id": "victoriametrics",
  "name": "VictoriaMetrics",
  "category": "Tool",
  "domain": "SRE/Observability",
  "eli5": "VictoriaMetrics is like a turbocharged Prometheus that stores way more metrics for way less disk space and doesn't break a sweat under load.",
  "description": "High-performance, cost-efficient time-series database and monitoring solution. Drop-in Prometheus-compatible with 10x better compression and horizontal scalability.",
  "use_cases": [
   "Long-term Prometheus metrics storage",
   "High-cardinality metrics from large Kubernetes clusters",
   "Replacing Thanos/Cortex for simpler HA Prometheus",
   "Multi-tenant monitoring platforms"
  ],
  "used_by": [
   "Grammarly",
   "Adidas",
   "Wix",
   "Roblox"
  ],
  "docs": "https://docs.victoriametrics.com/",
  "tutorial": "https://docs.victoriametrics.com/quick-start/",
  "added": "2026-06-12",
  "dex": 311
 },
 {
  "id": "attestation",
  "name": "Attestation",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a tamper-proof badge that proves your software came from a trusted source and wasn't modified.",
  "description": "Software attestation uses cryptographic signatures to prove the provenance and integrity of builds, enabling secure supply chain verification. Tools like Sigstore make attestation accessible for verifying that binaries were built as claimed.",
  "use_cases": [
   "Verifying software supply chain security",
   "Proving build artifact authenticity",
   "Compliance with SLSA framework requirements",
   "Detecting and preventing compromised dependencies"
  ],
  "used_by": [
   "Google",
   "Red Hat",
   "Chainguard",
   "Linux Foundation"
  ],
  "docs": "https://slsa.dev/",
  "tutorial": "https://docs.sigstore.dev/",
  "added": "2026-07-12",
  "dex": 312
 },
 {
  "id": "aws-kms",
  "name": "AWS KMS",
  "category": "Platform",
  "domain": "Security",
  "description": "AWS Key Management Service — managed service for creating and controlling encryption keys. Integrates with nearly every AWS service for envelope encryption. Keys never leave HSM hardware unencrypted.",
  "use_cases": [
   "Envelope encryption for S3, EBS, RDS, DynamoDB, Secrets Manager",
   "Customer-managed keys (CMKs) for compliance and auditability",
   "Signing and verification (asymmetric keys) for JWT, code signing",
   "Cross-account and cross-region key policies",
   "CloudTrail audit trail for every key usage event"
  ],
  "used_by": [
   "All regulated-industry AWS customers",
   "Financial services",
   "Healthcare companies"
  ],
  "docs": "https://docs.aws.amazon.com/kms/",
  "tutorial": "https://docs.aws.amazon.com/kms/latest/developerguide/getting-started.html",
  "added": "2026-06-08",
  "eli5": "A bank vault for your secret keys — lets you use them but never hands them over.",
  "dex": 313
 },
 {
  "id": "bitnami-sealed-secrets",
  "name": "Bitnami Sealed Secrets",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a locked treasure chest where only your Kubernetes cluster has the key to open and read your secrets.",
  "description": "Sealed Secrets is a Kubernetes controller that encrypts secrets so they can be safely stored in Git repositories. It uses asymmetric cryptography to ensure only the intended Kubernetes cluster can decrypt the secrets.",
  "use_cases": [
   "GitOps workflows with sensitive configuration management",
   "Storing encrypted secrets in version control systems",
   "Securing credentials across multiple Kubernetes clusters",
   "Automating secret rotation in CI/CD pipelines"
  ],
  "used_by": [
   "Red Hat",
   "Bitnami",
   "Cloud Native Computing Foundation members",
   "Enterprise Kubernetes users"
  ],
  "docs": "https://github.com/bitnami-labs/sealed-secrets",
  "tutorial": "https://github.com/bitnami-labs/sealed-secrets#installation",
  "added": "2026-08-24",
  "dex": 314
 },
 {
  "id": "bitwarden",
  "name": "Bitwarden",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Bitwarden is like a super-secure locker where you keep all your passwords locked up, and only you have the key to open it.",
  "description": "Bitwarden is an open-source password manager and secrets vault that securely stores and manages credentials, API keys, and sensitive data across teams and organizations. It provides end-to-end encryption and can be self-hosted or cloud-managed.",
  "use_cases": [
   "Team credential management and password sharing",
   "Secure storage of API keys and database credentials",
   "Self-hosted secrets management for enterprises",
   "Integration with CI/CD pipelines for secret injection"
  ],
  "used_by": [
   "Mozilla",
   "DuckDuckGo",
   "Basecamp",
   "Stack Overflow"
  ],
  "docs": "https://bitwarden.com/help/",
  "tutorial": "https://bitwarden.com/blog/getting-started-with-bitwarden/",
  "added": "2026-07-03",
  "dex": 315
 },
 {
  "id": "cerbos",
  "name": "Cerbos",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard at a theme park who checks your ticket and decides which rides you're allowed to go on based on the rules.",
  "description": "Cerbos is an open-source, language-agnostic authorization engine that separates access control logic from application code. It enables fine-grained, policy-based authorization across microservices and cloud applications.",
  "use_cases": [
   "Implementing role-based and attribute-based access control",
   "Centralizing authorization policies across microservices",
   "Managing complex permission hierarchies and relationships",
   "Enforcing compliance and audit requirements"
  ],
  "used_by": [
   "Cerbos Enterprise",
   "Cerbos Cloud",
   "GitBook",
   "AppDirect"
  ],
  "docs": "https://docs.cerbos.dev/",
  "tutorial": "https://docs.cerbos.dev/cerbos/latest/get-started",
  "added": "2026-06-21",
  "dex": 316
 },
 {
  "id": "cert-manager",
  "name": "cert-manager",
  "category": "Tool",
  "domain": "Security",
  "description": "Kubernetes-native certificate management controller. Automatically provisions, rotates, and renews TLS certificates from ACME (Let's Encrypt), Vault, Venafi, or self-signed CAs. Eliminates manual cert rotation toil in Kubernetes clusters.",
  "use_cases": [
   "Automatic Let's Encrypt TLS certificates for Ingress resources (HTTP-01/DNS-01 challenge)",
   "Vault PKI integration for internal mTLS certificates across services",
   "Automatic cert rotation before expiry — no more expired cert incidents",
   "Issuing certificates for Istio/SPIFFE workload identity via external issuers",
   "Wildcard and SAN certificates for multi-domain Kubernetes workloads"
  ],
  "used_by": [
   "Widely adopted across Kubernetes-based platforms and cloud-native stacks"
  ],
  "docs": "https://cert-manager.io/docs/",
  "tutorial": "https://cert-manager.io/docs/tutorials/acme/nginx-ingress/",
  "added": "2026-06-08",
  "eli5": "A robot that renews your HTTPS padlock automatically before it ever expires.",
  "dex": 317
 },
 {
  "id": "chainguard-images",
  "name": "Chainguard Images",
  "category": "Platform",
  "domain": "Security",
  "eli5": "It's like getting pre-made, ultra-safe containers that have been thoroughly checked so hackers can't hide inside them.",
  "description": "Chainguard Images provides minimal, distroless container images that are continuously scanned and updated for vulnerabilities. They significantly reduce attack surface by removing unnecessary packages while maintaining compatibility with existing applications.",
  "use_cases": [
   "Building secure supply chains with SLSA-compliant artifacts",
   "Reducing container image size and vulnerability footprint",
   "Running production workloads with minimal attack surface",
   "Meeting compliance requirements with verified, auditable container images"
  ],
  "used_by": [
   "Kubernetes",
   "CNCF",
   "Google Cloud",
   "GitHub"
  ],
  "docs": "https://images.chainguard.dev/",
  "tutorial": "https://edu.chainguard.dev/chainguard-images/getting-started/",
  "added": "2026-09-01",
  "dex": 318
 },
 {
  "id": "checkov",
  "name": "Checkov",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Checkov is like a building code inspector for your infrastructure files — it reads your Terraform and Kubernetes configs before you deploy and flags anything unsafe.",
  "description": "Static analysis tool for IaC security and compliance. Scans Terraform, CloudFormation, Kubernetes, Helm, and Dockerfiles against 1000+ built-in security and compliance checks.",
  "use_cases": [
   "IaC security scanning in CI/CD",
   "CIS benchmark compliance for cloud configs",
   "Detecting misconfigurations before deployment",
   "HIPAA, SOC2, PCI policy enforcement on infra code"
  ],
  "used_by": [
   "Bridgecrew (Palo Alto)",
   "Wix",
   "Etsy",
   "Monday.com"
  ],
  "docs": "https://www.checkov.io/1.Welcome/What%20is%20Checkov.html",
  "tutorial": "https://www.checkov.io/1.Welcome/Quick%20Start.html",
  "added": "2026-06-14",
  "dex": 319
 },
 {
  "id": "coraza",
  "name": "Coraza",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a smart security guard who reads rule books and stops bad people from entering your house before they can cause trouble.",
  "description": "A Go-based Web Application Firewall (WAF) engine that implements OWASP ModSecurity rules and can be embedded in any application or proxy. It provides protection against common web exploits with minimal performance overhead.",
  "use_cases": [
   "Embedded WAF protection in Go applications",
   "API gateway security enforcement",
   "Reverse proxy protection against OWASP Top 10 attacks",
   "Serverless function input validation and filtering"
  ],
  "used_by": [
   "Cloudflare",
   "Caddy",
   "Various enterprise users"
  ],
  "docs": "https://coraza.io/",
  "tutorial": "https://coraza.io/docs/tutorials/",
  "added": "2026-08-21",
  "dex": 320
 },
 {
  "id": "corellium",
  "name": "Corellium",
  "category": "Platform",
  "domain": "Security",
  "eli5": "Corellium is like a virtual phone or iPad that runs on computers, so security researchers can test apps without needing real devices.",
  "description": "Corellium provides cloud-hosted virtualization of iOS and Android devices for security research, mobile app testing, and threat analysis. It enables organizations to run authentic mobile operating systems in the cloud for dynamic analysis and penetration testing.",
  "use_cases": [
   "Mobile security vulnerability research and penetration testing",
   "Sandbox environment for analyzing malware on iOS and Android",
   "Automated security testing for mobile application development",
   "Compliance and forensic analysis of mobile platforms"
  ],
  "used_by": [
   "Apple",
   "Google",
   "Microsoft",
   "Fortune 500 Security Teams"
  ],
  "docs": "https://corellium.com/documentation",
  "tutorial": "https://corellium.com/tutorials",
  "added": "2026-09-15",
  "dex": 321
 },
 {
  "id": "cosign",
  "name": "Cosign",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Cosign is like a digital wax seal for container images — it proves the image you're running is exactly what the publisher shipped and nobody tampered with it.",
  "description": "Container image signing and verification tool from the Sigstore project. Enables keyless signing via OIDC, attestation storage in OCI registries, and supply chain security.",
  "use_cases": [
   "Signing container images in CI/CD pipelines",
   "Verifying image provenance before deployment",
   "Keyless signing with GitHub Actions OIDC",
   "SLSA attestation and SBOM attachment"
  ],
  "used_by": [
   "Chainguard",
   "Google",
   "Red Hat",
   "GitHub"
  ],
  "docs": "https://docs.sigstore.dev/cosign/overview/",
  "tutorial": "https://docs.sigstore.dev/cosign/signing/quickstart/",
  "added": "2026-06-12",
  "dex": 322
 },
 {
  "id": "crowdsec",
  "name": "CrowdSec",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a neighborhood watch system where everyone shares information about bad actors trying to break into houses, so everyone stays safer together.",
  "description": "CrowdSec is an open-source, collaborative security engine that detects and remedies cyber threats through crowdsourced threat intelligence and behavior analysis. It provides real-time protection against attacks by analyzing logs and sharing threat data across a global community.",
  "use_cases": [
   "Detecting and blocking brute force attacks and credential stuffing",
   "Collaborative threat intelligence sharing across organizations",
   "Automated response to suspicious behavior and DDoS patterns",
   "Log analysis and intrusion detection for web applications and servers"
  ],
  "used_by": [
   "OVH",
   "AWS partners",
   "Cloudflare customers",
   "Enterprise security teams"
  ],
  "docs": "https://docs.crowdsec.net/",
  "tutorial": "https://docs.crowdsec.net/docs/getting-started",
  "added": "2026-08-14",
  "dex": 323
 },
 {
  "id": "falco",
  "name": "Falco",
  "category": "Tool",
  "domain": "Security",
  "description": "Cloud-native runtime security tool by Sysdig. Uses eBPF/kernel module to detect anomalous behavior in containers and Kubernetes at the syscall level — privilege escalation, unexpected network connections, file writes to sensitive paths. CNCF graduated.",
  "use_cases": [
   "Detecting container escape attempts and privilege escalation at runtime",
   "Alerting on unexpected shell spawns inside production containers",
   "Kubernetes audit log monitoring for suspicious API server activity",
   "Compliance: detecting access to sensitive files (SSH keys, /etc/passwd)",
   "Integration with SIEM/SOAR via Falcosidekick for alert fanout"
  ],
  "used_by": [
   "Shopify",
   "Booz Allen Hamilton",
   "GitLab",
   "Randstad",
   "Sysdig"
  ],
  "docs": "https://falco.org/docs/",
  "tutorial": "https://falco.org/docs/getting-started/",
  "added": "2026-06-08",
  "eli5": "A smoke detector for containers — screams if something suspicious happens inside.",
  "dex": 324
 },
 {
  "id": "falco-security",
  "name": "Falco Security Alerts",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a guard dog that watches everything happening in your computer and barks loudly if something suspicious or dangerous starts to happen.",
  "description": "A real-time container and system security monitoring tool that detects abnormal behavior and threats at runtime using dynamic instrumentation. Falco is widely used for detecting unauthorized process execution, privilege escalation, and suspicious network activity in cloud-native environments.",
  "use_cases": [
   "Runtime threat detection in Kubernetes clusters",
   "Detecting unauthorized shell access and privilege escalation",
   "Monitoring suspicious network connections and file access",
   "Compliance auditing and forensic analysis"
  ],
  "used_by": [
   "Google Cloud",
   "Amazon Web Services",
   "Sysdig",
   "CloudBees"
  ],
  "docs": "https://falco.org/docs/",
  "tutorial": "https://falco.org/docs/getting-started/",
  "added": "2026-08-06",
  "dex": 325
 },
 {
  "id": "frida",
  "name": "Frida",
  "category": "Tool",
  "domain": "Security",
  "eli5": "A magic wand that lets you peek inside and control running programs without stopping them, like watching a video game and changing what happens while it's playing.",
  "description": "Frida is a dynamic instrumentation framework that allows you to inject code into running processes on Windows, macOS, iOS, Android, and Linux for debugging, reverse engineering, and security research. It enables real-time inspection and modification of application behavior without source code access.",
  "use_cases": [
   "Reverse engineering and analyzing closed-source applications",
   "Runtime code injection and patching for debugging and testing",
   "Security research and vulnerability discovery in mobile apps",
   "API hooking and monitoring of system calls in native applications"
  ],
  "used_by": [
   "Google Security Team",
   "Cybersecurity researchers",
   "Mobile app developers",
   "Penetration testing firms"
  ],
  "docs": "https://frida.re/docs/home/",
  "tutorial": "https://frida.re/docs/tutorials/",
  "added": "2026-08-13",
  "dex": 326
 },
 {
  "id": "sigstore-fulcio",
  "name": "Fulcio (Sigstore PKI)",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a digital notary that instantly creates unforgeable certificates for software to prove who created it, without anyone needing to manage secret keys.",
  "description": "Fulcio is the root certification authority in Sigstore that issues short-lived certificates using OIDC-based identity verification, eliminating the need for traditional key management in supply chain security. It's the foundation enabling practical zero-trust software signing at scale.",
  "use_cases": [
   "Signing container images and artifacts in CI/CD pipelines without key management",
   "Verifying provenance of open-source packages to prevent supply chain attacks",
   "Securing software supply chains for regulated industries with automatic audit trails",
   "Enabling developers to cryptographically sign commits without managing long-lived keys"
  ],
  "used_by": [
   "GitHub",
   "Google Cloud",
   "Red Hat",
   "CNCF projects"
  ],
  "docs": "https://docs.sigstore.dev/fulcio/overview/",
  "tutorial": "https://docs.sigstore.dev/fulcio/certificate-issuing-flow/",
  "added": "2026-08-02",
  "dex": 327
 },
 {
  "id": "gitleaks",
  "name": "GitLeaks",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard that checks your code before you upload it to make sure you didn't accidentally leave your house keys in plain sight.",
  "description": "GitLeaks is a SAST tool that scans git repositories to detect secrets and sensitive information like API keys, passwords, and tokens before they're committed. It prevents credential leaks in source control.",
  "use_cases": [
   "Pre-commit hooks to prevent secret leaks",
   "CI/CD pipeline scanning for credentials",
   "Repository auditing and compliance",
   "Developer security training and awareness"
  ],
  "used_by": [
   "Snyk",
   "GitHub",
   "GitLab",
   "Uber"
  ],
  "docs": "https://github.com/gitleaks/gitleaks",
  "tutorial": "https://gitleaks.io/",
  "added": "2026-07-01",
  "dex": 328
 },
 {
  "id": "grype",
  "name": "Grype",
  "category": "Tool",
  "domain": "Security",
  "eli5": "A tool that scans your software like a security guard checking IDs to catch known bad software packages before they get installed.",
  "description": "Grype is a vulnerability scanner that detects known security vulnerabilities in container images, filesystems, and software artifacts by analyzing SBOMs. It integrates seamlessly into CI/CD pipelines to prevent vulnerable dependencies from reaching production.",
  "use_cases": [
   "Container image scanning in CI/CD pipelines",
   "Supply chain security scanning and compliance",
   "Software composition analysis (SCA) for vulnerability detection",
   "Automated SBOM analysis and reporting"
  ],
  "used_by": [
   "Anchore",
   "GitHub",
   "VMware",
   "Red Hat"
  ],
  "docs": "https://github.com/anchore/grype",
  "tutorial": "https://github.com/anchore/grype#getting-started",
  "added": "2026-09-10",
  "dex": 329
 },
 {
  "id": "boundary",
  "name": "HashiCorp Boundary",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Boundary is like a security guard at the entrance of your office building who checks that you're allowed to go in and keeps track of everyone coming and going.",
  "description": "HashiCorp Boundary is a zero-trust access management platform that provides secure, auditable access to critical systems without requiring VPNs. It enables organizations to enforce fine-grained access controls and session recording for compliance.",
  "use_cases": [
   "Providing secure remote access to databases without VPN",
   "Implementing zero-trust access for cloud infrastructure",
   "Session recording and audit logging for compliance requirements",
   "Managing access to on-premise resources from remote workers"
  ],
  "used_by": [
   "HashiCorp customers",
   "enterprise infrastructure teams",
   "government agencies",
   "healthcare systems"
  ],
  "docs": "https://www.boundaryproject.io/docs",
  "tutorial": "https://www.boundaryproject.io/docs/getting-started",
  "added": "2026-08-01",
  "dex": 330
 },
 {
  "id": "hashicorp-vault",
  "name": "HashiCorp Vault",
  "category": "Tool",
  "domain": "Security",
  "description": "Secrets management and data protection platform. Centrally store, access, and distribute secrets (API keys, passwords, certs) with fine-grained policies and full audit logging. Dynamic secrets eliminate long-lived credentials.",
  "use_cases": [
   "Dynamic secrets: on-demand, short-lived DB credentials and cloud IAM tokens",
   "PKI as a service: issue and rotate TLS certificates automatically",
   "Kubernetes auth: pods authenticate with ServiceAccount tokens to get secrets",
   "Encryption as a service (Transit engine) without exposing keys to apps",
   "Secret leasing and renewal with automatic revocation on expiry"
  ],
  "used_by": [
   "Adobe",
   "Capital One",
   "Twitch",
   "Barclays",
   "Cisco"
  ],
  "docs": "https://developer.hashicorp.com/vault/docs",
  "tutorial": "https://developer.hashicorp.com/vault/tutorials/getting-started",
  "added": "2026-06-08",
  "eli5": "A really secure safe for all your passwords — only gives them to people it trusts.",
  "dex": 331
 },
 {
  "id": "infisical",
  "name": "Infisical",
  "category": "Platform",
  "domain": "Security",
  "eli5": "It's like a locked safe for all your passwords and secret codes that your applications can grab whenever they need them, but only the apps you trust.",
  "description": "Infisical is an open-source secrets management platform that stores, encrypts, and syncs environment variables and secrets across development, staging, and production environments. It provides a centralized alternative to HashiCorp Vault with simpler setup and team collaboration features.",
  "use_cases": [
   "Managing API keys and database passwords across CI/CD pipelines",
   "Syncing secrets to Kubernetes clusters and serverless functions",
   "Preventing secrets from being committed to version control",
   "Auditing and rotating credentials across multiple environments"
  ],
  "used_by": [
   "Shopify",
   "Figma",
   "Mercury",
   "Zapier"
  ],
  "docs": "https://infisical.com/docs",
  "tutorial": "https://infisical.com/docs/getting-started/introduction",
  "added": "2026-06-14",
  "dex": 332
 },
 {
  "id": "kyverno",
  "name": "Kyverno",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Kyverno is like a strict school hall monitor for Kubernetes — it checks every deployment against the rules and turns away anything that breaks them.",
  "description": "Kubernetes-native policy engine that validates, mutates, and generates resources using policies written as Kubernetes resources — no Rego required.",
  "use_cases": [
   "Enforcing pod security standards",
   "Auto-injecting labels and sidecars",
   "Blocking privileged containers in CI/CD",
   "Generating default NetworkPolicies"
  ],
  "used_by": [
   "Fidelity",
   "CNCF projects",
   "Rabobank",
   "Vodafone"
  ],
  "docs": "https://kyverno.io/docs/",
  "tutorial": "https://kyverno.io/docs/installation/",
  "added": "2026-06-11",
  "dex": 333
 },
 {
  "id": "mitmproxy",
  "name": "mitmproxy",
  "category": "Tool",
  "domain": "Security",
  "eli5": "mitmproxy is like a mail inspector that can read and modify letters going between two people before delivering them, for testing and debugging.",
  "description": "mitmproxy is an interactive HTTPS/HTTP proxy that allows developers to inspect, modify, and replay network traffic for debugging, testing, and security analysis. It's essential for API testing and understanding application behavior.",
  "use_cases": [
   "API endpoint debugging and testing",
   "Mobile app network traffic inspection",
   "Security testing and vulnerability scanning",
   "Request/response modification for testing edge cases"
  ],
  "used_by": [
   "Uber",
   "Airbnb",
   "Slack",
   "Square"
  ],
  "docs": "https://docs.mitmproxy.org/",
  "tutorial": "https://docs.mitmproxy.org/stable/overview-getting-started/",
  "added": "2026-06-16",
  "dex": 334
 },
 {
  "id": "mitmproxy-mitmdump",
  "name": "mitmproxy (mitmdump)",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard who watches all the messages sent between your computer and the internet, and can show you exactly what they say.",
  "description": "mitmproxy is an interactive HTTP and HTTPS proxy that allows developers and security professionals to inspect, modify, and replay web traffic. It's widely used for API testing, debugging, and security research.",
  "use_cases": [
   "Debugging mobile and web application network traffic",
   "API integration testing and validation",
   "Security vulnerability assessment and penetration testing",
   "Request/response modification for testing edge cases"
  ],
  "used_by": [
   "Uber",
   "Slack",
   "Square",
   "Mozilla"
  ],
  "docs": "https://docs.mitmproxy.org/",
  "tutorial": "https://docs.mitmproxy.org/stable/overview-getting-started/",
  "added": "2026-08-23",
  "dex": 335
 },
 {
  "id": "opa-gatekeeper",
  "name": "OPA Gatekeeper",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard at the gate who checks every rule before letting anyone in or doing anything.",
  "description": "Gatekeeper is a policy engine for Kubernetes that uses Open Policy Agent (OPA) to enforce custom policies through admission webhooks. It enables organizations to implement fine-grained access control and compliance requirements across their clusters.",
  "use_cases": [
   "Enforcing security policies in Kubernetes deployments",
   "Compliance validation for regulatory requirements",
   "Container image scanning and vulnerability prevention",
   "Resource quotas and cost optimization policies"
  ],
  "used_by": [
   "Google Cloud",
   "Microsoft Azure",
   "Styra",
   "Capital One"
  ],
  "docs": "https://open-policy-agent.github.io/gatekeeper/",
  "tutorial": "https://open-policy-agent.github.io/gatekeeper/website/docs/getting-started",
  "added": "2026-09-12",
  "dex": 336
 },
 {
  "id": "opa",
  "name": "Open Policy Agent (OPA)",
  "category": "Tool",
  "domain": "Security",
  "description": "General-purpose policy engine that decouples policy decisions from application logic. Policies written in Rego language. Used as Kubernetes admission controller, API gateway authorizer, and Terraform plan enforcer. CNCF graduated.",
  "use_cases": [
   "Kubernetes admission control: block non-compliant resources before they land (via Gatekeeper)",
   "API authorization: fine-grained, attribute-based access control for microservices",
   "Terraform plan policy enforcement (Conftest) before infrastructure changes",
   "Data masking and row-level filtering in data access layers",
   "Centralized policy management across heterogeneous systems"
  ],
  "used_by": [
   "Netflix",
   "Pinterest",
   "Goldman Sachs",
   "Capital One",
   "Atlassian"
  ],
  "docs": "https://www.openpolicyagent.org/docs/latest/",
  "tutorial": "https://www.openpolicyagent.org/docs/latest/#running-opa",
  "added": "2026-06-08",
  "eli5": "A rulebook apps must check before doing anything: 'Am I allowed to do this?'",
  "dex": 337
 },
 {
  "id": "osquery",
  "name": "osquery",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like having a detective that can instantly answer questions about everything happening inside your computer.",
  "description": "osquery is an operating system instrumentation framework that exposes system data as SQL tables, enabling real-time visibility and monitoring of host security. It's widely used for endpoint detection, compliance monitoring, and incident response across Linux, macOS, and Windows systems.",
  "use_cases": [
   "Real-time endpoint threat detection and hunting",
   "Compliance monitoring and audit logging",
   "Incident response and forensics investigation",
   "Inventory management and configuration auditing"
  ],
  "used_by": [
   "Meta",
   "Apple",
   "Google",
   "Uber"
  ],
  "docs": "https://osquery.io/docs/",
  "tutorial": "https://osquery.io/docs/installation/install-linux/",
  "added": "2026-08-20",
  "dex": 338
 },
 {
  "id": "rustls",
  "name": "Rustls",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Rustls is like a safer lock for internet messages that's written in a language that prevents accidentally leaving doors open.",
  "description": "Rustls is a modern TLS library written in Rust that provides cryptographic security for network communications without the memory safety vulnerabilities of traditional implementations like OpenSSL. It's becoming the default choice for projects prioritizing both security and performance.",
  "use_cases": [
   "Securing HTTPS connections in web servers and reverse proxies",
   "Protecting gRPC and service-to-service communication",
   "Encrypting database connections and APIs",
   "Building secure networking layers in cloud-native applications"
  ],
  "used_by": [
   "Cloudflare",
   "Mozilla",
   "Brave",
   "Tokio"
  ],
  "docs": "https://docs.rs/rustls/",
  "tutorial": "https://rustls.dev/",
  "added": "2026-06-29",
  "dex": 339
 },
 {
  "id": "semgrep",
  "name": "Semgrep",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Semgrep is like grep but it understands code structure — it finds security bugs and bad patterns that look different on the surface but mean the same dangerous thing.",
  "description": "Fast, open-source static analysis tool for finding bugs and security vulnerabilities. Uses pattern matching on ASTs across 30+ languages, with a large community rule registry.",
  "use_cases": [
   "SAST in CI/CD pipelines",
   "Detecting OWASP Top 10 vulnerabilities",
   "Enforcing custom code standards at scale",
   "Secrets detection and dependency scanning"
  ],
  "used_by": [
   "Dropbox",
   "Figma",
   "Trail of Bits",
   "GitLab"
  ],
  "docs": "https://semgrep.dev/docs/",
  "tutorial": "https://semgrep.dev/docs/getting-started/",
  "added": "2026-06-14",
  "dex": 340
 },
 {
  "id": "semgrep-pro",
  "name": "Semgrep Pro",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a teacher that reads all your code and catches mistakes before you turn it in, finding bugs and security problems automatically.",
  "description": "Semgrep is a fast, static analysis tool that finds bugs, security vulnerabilities, and anti-patterns in code across multiple languages. The Pro version adds enterprise features like triage, deduplication, and advanced policy management.",
  "use_cases": [
   "Finding security vulnerabilities in pull requests before merge",
   "Enforcing code quality standards and best practices",
   "Detecting OWASP Top 10 vulnerabilities automatically",
   "Compliance scanning for regulated industries"
  ],
  "used_by": [
   "Stripe",
   "Databricks",
   "Auth0",
   "Figma"
  ],
  "docs": "https://semgrep.dev/docs/",
  "tutorial": "https://semgrep.dev/r/",
  "added": "2026-08-09",
  "dex": 341
 },
 {
  "id": "sigstore",
  "name": "Sigstore",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Sigstore is like a tamper-evident seal on a package that proves nobody opened it and that it really came from who it says it came from.",
  "description": "Sigstore is a Linux Foundation project that provides free, easy-to-use signing, verification, and transparency for software artifacts. It enables keyless signing using OIDC identity providers, removing the need to manage long-lived cryptographic keys.",
  "use_cases": [
   "Signing and verifying container images and software artifacts",
   "Establishing supply chain security for open-source projects",
   "Auditable software provenance tracking",
   "Keyless signing without managing private keys"
  ],
  "used_by": [
   "Google",
   "Red Hat",
   "GitHub"
  ],
  "docs": "https://docs.sigstore.dev/",
  "tutorial": "https://docs.sigstore.dev/signing/quickstart/",
  "added": "2026-06-15",
  "dex": 342
 },
 {
  "id": "snyk",
  "name": "Snyk",
  "category": "Platform",
  "domain": "Security",
  "description": "Developer-first security platform for finding and fixing vulnerabilities in code, dependencies, containers, and IaC. Integrates into IDEs, CI/CD, and SCM. Prioritizes by exploitability, not just CVSS score.",
  "use_cases": [
   "SCA (Software Composition Analysis): detect vulnerable open-source dependencies",
   "SAST: static code analysis for security issues in first-party code",
   "Container image scanning integrated into Docker build and registry workflows",
   "IaC scanning for Terraform, CloudFormation, and Kubernetes misconfigurations",
   "License compliance checks across the dependency graph"
  ],
  "used_by": [
   "Google",
   "Salesforce",
   "Intuit",
   "MongoDB",
   "Atlassian"
  ],
  "docs": "https://docs.snyk.io/",
  "tutorial": "https://docs.snyk.io/getting-started",
  "added": "2026-06-08",
  "eli5": "A spell-checker for security — finds dangerous ingredients in your code's recipe.",
  "dex": 343
 },
 {
  "id": "sonarqube",
  "name": "SonarQube",
  "category": "Tool",
  "domain": "Security",
  "eli5": "SonarQube is like a spell-checker for your code but also checks for security holes, bad practices, and code smells before you ship anything.",
  "description": "Platform for continuous code quality and security inspection. Detects bugs, vulnerabilities, and code smells across 30+ languages. Integrates into CI/CD pipelines.",
  "use_cases": [
   "Static application security testing (SAST)",
   "Code quality gates in CI/CD",
   "Technical debt tracking across large codebases",
   "Compliance enforcement (OWASP, CERT)"
  ],
  "used_by": [
   "Microsoft",
   "NASA",
   "Bosch",
   "T-Mobile"
  ],
  "docs": "https://docs.sonarsource.com/sonarqube/",
  "tutorial": "https://docs.sonarsource.com/sonarqube/latest/try-out-sonarqube/",
  "added": "2026-06-12",
  "dex": 344
 },
 {
  "id": "step-ca",
  "name": "Step CA",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Step CA is like a trusted teacher who gives out digital ID cards that prove who you are online and makes sure only the right people can talk to each other.",
  "description": "Step Certificates (Step CA) is an open-source online certificate authority that simplifies certificate management and automation for internal infrastructure and applications. It provides easy certificate provisioning, renewal, and revocation without operational complexity.",
  "use_cases": [
   "Internal PKI and certificate management",
   "Mutual TLS (mTLS) authentication",
   "HTTPS certificate automation",
   "Zero-trust security implementations"
  ],
  "used_by": [
   "Google",
   "GitHub",
   "HashiCorp",
   "Smallstep Labs"
  ],
  "docs": "https://smallstep.com/docs/step-ca/",
  "tutorial": "https://smallstep.com/docs/step-ca/getting-started/",
  "added": "2026-06-20",
  "dex": 345
 },
 {
  "id": "syntaxe",
  "name": "Syntaxe",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Syntaxe is like a security guard for your code that checks every line to find hidden problems before hackers can exploit them.",
  "description": "Syntaxe is a static application security testing (SAST) platform that scans source code to identify vulnerabilities, misconfigurations, and security patterns using AI-enhanced analysis. It integrates into CI/CD pipelines to catch security issues early in the development lifecycle.",
  "use_cases": [
   "Continuous security scanning in CI/CD pipelines",
   "Identifying OWASP Top 10 vulnerabilities in code",
   "Detecting infrastructure-as-code misconfigurations",
   "Compliance and security posture management"
  ],
  "used_by": [
   "Enterprise software companies",
   "Financial institutions",
   "Healthcare technology providers"
  ],
  "docs": "https://syntaxe.io/docs",
  "tutorial": "https://syntaxe.io/docs/getting-started",
  "added": "2026-07-14",
  "dex": 346
 },
 {
  "id": "tailscale-ssh",
  "name": "Tailscale SSH",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like having a magic door in your bedroom that lets you walk directly into any of your friend's rooms without needing to know the house address or climb through windows.",
  "description": "Tailscale SSH is a native SSH implementation built on Tailscale's mesh VPN that provides automatic key management, device-based access control, and zero-trust connections without managing SSH keys or jump hosts. It simplifies remote access while maintaining enterprise security standards.",
  "use_cases": [
   "Secure remote server access for distributed DevOps teams",
   "Zero-trust access to production infrastructure without VPN",
   "Automated SSH authentication across multiple devices and environments",
   "Compliance-friendly access logging and audit trails"
  ],
  "used_by": [
   "DuckDuckGo",
   "Vercel",
   "Segment",
   "Figma"
  ],
  "docs": "https://tailscale.com/kb/1193/tailscale-ssh/",
  "tutorial": "https://tailscale.com/kb/1282/ssh-setup/",
  "added": "2026-09-18",
  "dex": 347
 },
 {
  "id": "teleport",
  "name": "Teleport",
  "category": "Platform",
  "domain": "Security",
  "eli5": "Teleport is like a bouncer at a club who checks IDs and keeps a record of who went where, making sure only the right people access sensitive computers and servers.",
  "description": "Teleport is an identity-native infrastructure access platform that consolidates SSH, Kubernetes, database, and application access under a unified zero-trust framework. It eliminates shared credentials and provides detailed audit logs for compliance and security investigations.",
  "use_cases": [
   "Zero-trust SSH and database access management across infrastructure",
   "Kubernetes cluster access control with audit logging",
   "Just-in-time privilege escalation and access requests",
   "Unified identity and access management for hybrid cloud environments"
  ],
  "used_by": [
   "Stripe",
   "Cloudflare",
   "Figma"
  ],
  "docs": "https://goteleport.com/docs/",
  "tutorial": "https://goteleport.com/docs/getting-started/",
  "added": "2026-06-22",
  "dex": 348
 },
 {
  "id": "torq",
  "name": "Torq",
  "category": "Platform",
  "domain": "Security",
  "eli5": "Torq is like a smart traffic controller for security alerts that automatically decides what to do with them so security teams don't have to handle every single one manually.",
  "description": "Torq is a no-code security automation and orchestration platform that connects security tools and automates incident response workflows. It enables security teams to respond faster to threats by automating repetitive tasks and integrating disparate security systems.",
  "use_cases": [
   "Automating security incident response workflows",
   "Integrating multiple security tools into unified workflows",
   "Enriching and correlating security alerts automatically",
   "Reducing mean time to respond (MTTR) for security incidents"
  ],
  "used_by": [
   "Palo Alto Networks",
   "Accenture",
   "Shopify",
   "Barclays"
  ],
  "docs": "https://docs.torq.io/",
  "tutorial": "https://torq.io/platform-tour/",
  "added": "2026-06-18",
  "dex": 349
 },
 {
  "id": "trivy",
  "name": "Trivy",
  "category": "Tool",
  "domain": "Security",
  "description": "Comprehensive, fast open-source vulnerability and misconfiguration scanner by Aqua Security. Scans container images, filesystems, Git repos, Kubernetes clusters, and IaC files. Single binary, zero config needed.",
  "use_cases": [
   "Container image scanning for OS and language-level CVEs in CI pipelines",
   "IaC misconfiguration scanning for Terraform, Helm, Dockerfile, and Kubernetes manifests",
   "Kubernetes cluster scanning for vulnerabilities in running workloads",
   "SBOM generation (CycloneDX, SPDX) for supply chain compliance",
   "Secret detection in source code and container layers"
  ],
  "used_by": [
   "Aqua Security",
   "AWS",
   "GitLab",
   "GitHub Actions ecosystem"
  ],
  "docs": "https://aquasecurity.github.io/trivy/",
  "tutorial": "https://aquasecurity.github.io/trivy/latest/getting-started/installation/",
  "added": "2026-06-08",
  "eli5": "An X-ray machine for containers that finds hidden diseases (security holes) inside.",
  "dex": 350
 },
 {
  "id": "typos",
  "name": "Typos",
  "category": "Tool",
  "domain": "Security",
  "eli5": "Typos is like a spell-checker for your code that catches common spelling mistakes in variable names and comments that could be security problems.",
  "description": "Typos is a fast, configurable source code spell checker written in Rust that catches misspellings in code, comments, and documentation. It's designed to prevent typos from becoming security vulnerabilities and runs efficiently across large codebases.",
  "use_cases": [
   "CI/CD pipeline integration to catch typos before code merge",
   "Pre-commit hooks for preventing misspellings in source code",
   "Security scanning for common misspellings that could enable vulnerabilities",
   "Documentation quality assurance across multiple repositories"
  ],
  "used_by": [
   "Servo",
   "Cryptography.io",
   "Microsoft"
  ],
  "docs": "https://github.com/crate-ci/typos",
  "tutorial": "https://github.com/crate-ci/typos#getting-started",
  "added": "2026-09-25",
  "dex": 351
 },
 {
  "id": "vault-secrets",
  "name": "Vault (Secrets Management)",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a super-secure safe that only opens when you prove who you are, and it remembers to change the combination automatically.",
  "description": "HashiCorp Vault is an identity-based secrets and encryption management system that centralizes and secures access to secrets, certificates, and encryption keys across distributed infrastructure. It provides dynamic secret generation, encryption-as-a-service, and detailed audit logging.",
  "use_cases": [
   "Managing database credentials and API tokens in production",
   "Generating dynamic database passwords that rotate automatically",
   "Encrypting sensitive data at rest and in transit",
   "Auditing all access to secrets and compliance reporting"
  ],
  "used_by": [
   "HashiCorp",
   "Google",
   "GitHub",
   "Uber"
  ],
  "docs": "https://www.vaultproject.io/docs",
  "tutorial": "https://learn.hashicorp.com/tutorials/vault/getting-started",
  "added": "2026-06-25",
  "dex": 352
 },
 {
  "id": "vault-cli",
  "name": "Vault CLI (Python)",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a secure lockbox for your passwords and secrets that you can open from the command line without ever showing the secrets on your screen.",
  "description": "Vault CLI is a Python-based command-line interface for interacting with secret management backends, enabling developers to securely retrieve and manage credentials in automated workflows. It abstracts the complexity of multiple secret backends into a simple, unified interface.",
  "use_cases": [
   "Retrieving database credentials in CI/CD pipelines without exposing them",
   "Managing API keys and secrets across development, staging, and production",
   "Automating secret rotation and lifecycle management",
   "Integrating secret management into application startup scripts"
  ],
  "used_by": [
   "GitLab",
   "Databricks",
   "Okta",
   "HashiCorp"
  ],
  "docs": "https://python-hvac.readthedocs.io/",
  "tutorial": "https://python-hvac.readthedocs.io/en/stable/usage/auth_methods/index.html",
  "added": "2026-07-28",
  "dex": 353
 },
 {
  "id": "vault-secrets-auth",
  "name": "Vault Secrets Auth",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a super-secure safe that keeps your passwords and secret keys locked up, and only lets the right people or programs access them when needed.",
  "description": "HashiCorp Vault Secrets Auth is an identity and access management solution that provides dynamic secret generation, encryption, and audit logging for credentials across infrastructure. It eliminates hardcoded secrets and enables fine-grained access control for applications and teams.",
  "use_cases": [
   "Centralized secrets management for microservices",
   "Dynamic database credential generation",
   "PKI and certificate lifecycle management",
   "Audit logging and compliance reporting"
  ],
  "used_by": [
   "Citrix",
   "Expedia",
   "DuckDuckGo",
   "Deloitte"
  ],
  "docs": "https://www.vaultproject.io/docs",
  "tutorial": "https://learn.hashicorp.com/collections/vault/getting-started",
  "added": "2026-07-07",
  "dex": 354
 },
 {
  "id": "vault-secrets-operator",
  "name": "Vault Secrets Operator",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard that automatically fetches secret passwords from a safe and gives them to your applications without anyone seeing them.",
  "description": "Kubernetes operator that integrates HashiCorp Vault with Kubernetes to automatically inject and rotate secrets into applications through native Kubernetes resources. It eliminates manual secret management and reduces exposure of sensitive data in cluster configurations.",
  "use_cases": [
   "Automatic secret injection and rotation in Kubernetes deployments",
   "Enforcing zero-trust security practices in containerized environments",
   "Compliance with secrets management and audit requirements",
   "Multi-environment secret management across cloud and on-premise"
  ],
  "used_by": [
   "HashiCorp",
   "Enterprise Kubernetes users",
   "Fortune 500 companies"
  ],
  "docs": "https://developer.hashicorp.com/vault/docs/platform/k8s/vso",
  "tutorial": "https://developer.hashicorp.com/vault/docs/platform/k8s/vso/installation",
  "added": "2026-08-25",
  "dex": 355
 },
 {
  "id": "attestation-powered-security",
  "name": "Witness",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a detailed receipt that proves who did what and when in your software factory, so you can trace back exactly how each piece was made.",
  "description": "Witness is an open-source framework for creating cryptographic attestations of software artifact provenance and supply chain security. It integrates with CI/CD pipelines to record and verify every step of the build and deployment process.",
  "use_cases": [
   "Software supply chain security and provenance tracking",
   "CI/CD pipeline integrity verification",
   "Compliance and audit trail requirements",
   "Container image and artifact attestation"
  ],
  "used_by": [
   "Enterprise security teams",
   "Cloud-native projects",
   "SLSA-focused organizations"
  ],
  "docs": "https://github.com/testifysec/witness",
  "tutorial": "https://github.com/testifysec/witness/blob/main/README.md",
  "added": "2026-06-30",
  "dex": 356
 },
 {
  "id": "wiz",
  "name": "Wiz",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard for your cloud who walks around and checks every corner to make sure nothing dangerous was left behind or forgotten.",
  "description": "Wiz is a cloud security platform that provides runtime threat detection, vulnerability management, and compliance monitoring across multi-cloud environments. It uses runtime intelligence to identify and prioritize security risks in cloud workloads and infrastructure.",
  "use_cases": [
   "Detecting and remediating cloud misconfigurations and vulnerabilities in real-time",
   "Identifying runtime threats and anomalous behavior in containerized workloads",
   "Meeting compliance requirements across AWS, Azure, and GCP environments",
   "Prioritizing security issues based on actual runtime context and exploitability"
  ],
  "used_by": [
   "SailPoint",
   "Figma",
   "Canva",
   "Scale AI"
  ],
  "docs": "https://docs.wiz.io/",
  "tutorial": "https://docs.wiz.io/wiz-docs/docs/getting-started",
  "added": "2026-06-14",
  "dex": 357
 },
 {
  "id": "wiz-cloud-security",
  "name": "Wiz Cloud Security",
  "category": "Platform",
  "domain": "Security",
  "eli5": "It's like a security guard that walks around your entire cloud building and tells you which doors are unlocked and which rooms have problems.",
  "description": "Wiz is a cloud security platform that provides runtime threat detection, vulnerability management, and misconfiguration remediation across multi-cloud environments. It correlates data from multiple sources to provide context-aware security insights.",
  "use_cases": [
   "Identifying and remediating misconfigurations in AWS, Azure, and GCP",
   "Runtime threat detection and response in containerized workloads",
   "Supply chain security and software bill of materials (SBOM) analysis",
   "Compliance monitoring and audit trail management"
  ],
  "used_by": [
   "Stripe",
   "Databricks",
   "Figma",
   "SailPoint"
  ],
  "docs": "https://docs.wiz.io",
  "tutorial": "https://www.wiz.io/lp/quick-start",
  "added": "2026-09-17",
  "dex": 358
 },
 {
  "id": "zap",
  "name": "Zap",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a security guard that checks your website for hidden weak spots and tells you where someone could sneak in.",
  "description": "OWASP ZAP (Zed Attack Proxy) is a free and open-source web application security scanner used to find vulnerabilities in web applications during development and testing. It supports both manual and automated testing, making it essential for secure development pipelines.",
  "use_cases": [
   "Automated security scanning in CI/CD pipelines",
   "Dynamic application security testing (DAST) for web applications",
   "Manual penetration testing and vulnerability assessment",
   "Compliance scanning for security standards and regulations"
  ],
  "used_by": [
   "OWASP Foundation",
   "Enterprise security teams",
   "DevSecOps practitioners"
  ],
  "docs": "https://www.zaproxy.org/docs/",
  "tutorial": "https://www.zaproxy.org/getting-started/",
  "added": "2026-08-12",
  "dex": 359
 },
 {
  "id": "zerossl",
  "name": "ZeroSSL",
  "category": "Tool",
  "domain": "Security",
  "eli5": "It's like a digital bouncer that checks if you're really who you say you are when you visit a website, and it doesn't charge you money for the checking service.",
  "description": "ZeroSSL is an automated SSL/TLS certificate provider and management platform offering free and paid certificates with easy integration for ACME protocol support. It simplifies certificate lifecycle management for websites and APIs at scale.",
  "use_cases": [
   "Obtaining free SSL/TLS certificates for websites and APIs",
   "Automating certificate renewal and deployment pipelines",
   "Managing multi-domain and wildcard certificates with flexibility",
   "Compliance with HTTPS requirements for e-commerce and sensitive applications"
  ],
  "used_by": [
   "Automattic",
   "Yext",
   "Protocol Labs",
   "Namecheap"
  ],
  "docs": "https://zerossl.com/documentation/",
  "tutorial": "https://zerossl.com/documentation/getting-started/",
  "added": "2026-07-08",
  "dex": 360
 },
 {
  "id": "airbyte",
  "name": "Airbyte",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "Airbyte is like a smart mail carrier that automatically moves data packages from one building to another without you having to carry them yourself.",
  "description": "Airbyte is an open-source data integration platform that provides pre-built connectors to move data between hundreds of sources and destinations with minimal configuration. It simplifies ETL/ELT workflows for data warehousing and analytics pipelines.",
  "use_cases": [
   "Syncing data from SaaS applications to data warehouses",
   "Real-time CDC (Change Data Capture) replication",
   "Building modular data pipelines without coding",
   "Data lake ingestion from multiple sources"
  ],
  "used_by": [
   "Notion",
   "Figma",
   "Dashbird"
  ],
  "docs": "https://docs.airbyte.com/",
  "tutorial": "https://docs.airbyte.com/using-airbyte/getting-started",
  "added": "2026-08-30",
  "dex": 361
 },
 {
  "id": "airbyte-connector",
  "name": "Airbyte ELT Platform",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Airbyte is like a postal service for data that automatically picks up information from one place and delivers it to another place, and you don't have to manually carry each piece yourself.",
  "description": "Airbyte is an open-source ELT (Extract, Load, Transform) platform that simplifies data integration by providing pre-built connectors for hundreds of data sources and destinations. It enables non-technical users and engineers to build and manage data pipelines without writing custom code.",
  "use_cases": [
   "Syncing data from SaaS applications like Salesforce, HubSpot, and Stripe to data warehouses",
   "Building real-time data pipelines between operational databases and analytics platforms",
   "Creating backup and disaster recovery pipelines across multiple cloud storage systems",
   "Consolidating data from multiple sources into a central repository for analytics and ML"
  ],
  "used_by": [
   "Coursera",
   "Typeform",
   "Notion",
   "Figma"
  ],
  "docs": "https://docs.airbyte.com/",
  "tutorial": "https://docs.airbyte.com/connector-development/",
  "added": "2026-09-24",
  "dex": 362
 },
 {
  "id": "apache-airflow",
  "name": "Apache Airflow",
  "category": "Tool",
  "domain": "Data Engineering",
  "description": "Open-source workflow orchestration platform. Pipelines defined as DAGs in Python — giving full programmatic control over scheduling, branching, retries, and dependencies. De facto standard for data pipeline orchestration.",
  "use_cases": [
   "Orchestrating multi-step ETL/ELT pipelines with dependency-aware scheduling",
   "Triggering Spark jobs, dbt runs, SQL queries, and API calls in sequence",
   "Backfilling historical data runs with date-parameterized DAGs",
   "Dynamic DAG generation for multi-tenant or config-driven pipelines",
   "Managed via MWAA (AWS), Cloud Composer (GCP), or Astronomer"
  ],
  "used_by": [
   "Airbnb",
   "Twitter",
   "Lyft",
   "Robinhood",
   "Adobe",
   "PayPal"
  ],
  "docs": "https://airflow.apache.org/docs/",
  "tutorial": "https://airflow.apache.org/docs/apache-airflow/stable/tutorial/index.html",
  "added": "2026-06-08",
  "eli5": "A chore chart that runs your data jobs in the right order at the right time.",
  "dex": 363
 },
 {
  "id": "apache-beam",
  "name": "Apache Beam",
  "category": "Framework",
  "domain": "Data Engineering",
  "eli5": "Beam is like writing one recipe that works whether you cook on a gas stove, electric, or campfire — your data pipeline runs on Spark, Flink, or Dataflow unchanged.",
  "description": "Unified programming model for batch and streaming data pipelines. Write once, run on any execution engine (Spark, Flink, Google Dataflow) via pluggable runners.",
  "use_cases": [
   "Portable ETL pipelines across multiple runners",
   "Streaming data processing with event-time semantics",
   "ML inference pipelines at scale",
   "Google Dataflow-native batch and stream jobs"
  ],
  "used_by": [
   "Google",
   "LinkedIn",
   "PayPal",
   "Lyft"
  ],
  "docs": "https://beam.apache.org/documentation/",
  "tutorial": "https://beam.apache.org/get-started/wordcount-example/",
  "added": "2026-06-12",
  "dex": 364
 },
 {
  "id": "apache-druid",
  "name": "Apache Druid",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "It's like having a super-fast encyclopedia that can answer billions of questions about your data in just a blink.",
  "description": "Apache Druid is a real-time OLAP datastore designed for fast analytical queries on large datasets with sub-second latencies. It's optimized for time-series data and powers interactive dashboards and exploratory analytics.",
  "use_cases": [
   "Real-time analytics dashboards and monitoring",
   "Ad-tech and marketing analytics platforms",
   "Network and security analytics",
   "Financial market and trading data analysis"
  ],
  "used_by": [
   "Airbnb",
   "Lyft",
   "Cisco",
   "Alibaba"
  ],
  "docs": "https://druid.apache.org/docs/latest/",
  "tutorial": "https://druid.apache.org/docs/latest/tutorials/index.html",
  "added": "2026-09-12",
  "dex": 365
 },
 {
  "id": "apache-flink",
  "name": "Apache Flink",
  "category": "Framework",
  "domain": "Data Engineering",
  "eli5": "Flink is like a factory assembly line that processes items the moment they arrive, rather than waiting to collect a big pile before starting.",
  "description": "Distributed stream and batch processing engine with exactly-once semantics, stateful computations, and event-time processing at massive scale.",
  "use_cases": [
   "Real-time fraud detection",
   "Streaming ETL pipelines",
   "Event-driven applications",
   "CDC (change data capture) processing"
  ],
  "used_by": [
   "Alibaba",
   "Netflix",
   "Uber",
   "LinkedIn",
   "AWS"
  ],
  "docs": "https://nightlies.apache.org/flink/flink-docs-stable/",
  "tutorial": "https://nightlies.apache.org/flink/flink-docs-stable/docs/try-flink/local_installation/",
  "added": "2026-06-11",
  "dex": 366
 },
 {
  "id": "apache-iceberg",
  "name": "Apache Iceberg",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Iceberg is like a version-controlled spreadsheet for petabytes of data — you can time-travel to yesterday's data, roll back mistakes, and add columns without breaking anything.",
  "description": "Open table format for huge analytic datasets. Brings ACID transactions, schema evolution, and time-travel queries to data lakes stored in S3, HDFS, or GCS.",
  "use_cases": [
   "ACID transactions on data lake tables",
   "Schema evolution without breaking queries",
   "Time-travel and incremental reads for CDC",
   "Multi-engine interoperability (Spark, Flink, Trino)"
  ],
  "used_by": [
   "Netflix",
   "Apple",
   "LinkedIn",
   "Adobe"
  ],
  "docs": "https://iceberg.apache.org/docs/latest/",
  "tutorial": "https://iceberg.apache.org/spark-quickstart/",
  "added": "2026-06-12",
  "dex": 367
 },
 {
  "id": "apache-iceberg-catalog",
  "name": "Apache Iceberg Catalog",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a super-organized filing cabinet for your data that remembers every version of every file, so you can go back in time to see what things looked like before.",
  "description": "Apache Iceberg is an open-table format designed for large-scale, analytic datasets that provides ACID transactions, time travel, and hidden partitioning. It solves problems with traditional data lake formats by offering schema evolution and reliable concurrent writes.",
  "use_cases": [
   "Data lakehouse implementations with ACID guarantees",
   "Time-travel queries to audit data changes and rollback corruptions",
   "Schema evolution without breaking existing queries",
   "Multi-engine analytics with consistent data views across Spark, Trino, and Flink"
  ],
  "used_by": [
   "Netflix",
   "Adobe",
   "Apple",
   "Booking.com"
  ],
  "docs": "https://iceberg.apache.org/",
  "tutorial": "https://iceberg.apache.org/docs/latest/getting-started/",
  "added": "2026-07-09",
  "dex": 368
 },
 {
  "id": "parquet",
  "name": "Apache Parquet",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like organizing your toy box by color and size so you can find exactly what you need without dumping everything out on the floor.",
  "description": "Apache Parquet is a columnar storage format optimized for analytical queries on large datasets. It provides efficient compression, encoding, and nested data support, making it the standard format for data lakes and analytical workloads.",
  "use_cases": [
   "Storing large datasets in cloud data lakes (S3, GCS, Azure)",
   "Improving query performance in analytical databases like BigQuery and Snowflake",
   "Data pipeline processing with Apache Spark and Dask",
   "Cost reduction through better compression and selective column access"
  ],
  "used_by": [
   "Netflix",
   "Uber",
   "Meta",
   "Twitter"
  ],
  "docs": "https://parquet.apache.org/docs/overview/",
  "tutorial": "https://parquet.apache.org/docs/file-format/",
  "added": "2026-06-25",
  "dex": 369
 },
 {
  "id": "apache-spark",
  "name": "Apache Spark",
  "category": "Framework",
  "domain": "Data Engineering",
  "description": "Unified analytics engine for large-scale data processing. In-memory computation makes it 10-100x faster than MapReduce. Supports batch, streaming (Structured Streaming), SQL, ML (MLlib), and graph (GraphX) workloads in one engine.",
  "use_cases": [
   "Large-scale ETL and data transformation across petabytes of data",
   "Structured Streaming for real-time event processing from Kafka",
   "Distributed ML training with MLlib on massive datasets",
   "Interactive SQL analytics via Spark SQL on data lakes (Parquet, Delta, Iceberg)",
   "Graph analytics and iterative algorithms with GraphX"
  ],
  "used_by": [
   "Netflix",
   "Uber",
   "Airbnb",
   "Apple",
   "Microsoft",
   "Databricks"
  ],
  "docs": "https://spark.apache.org/docs/latest/",
  "tutorial": "https://spark.apache.org/docs/latest/quick-start.html",
  "added": "2026-06-08",
  "eli5": "A team of workers that splits a huge data pile and processes all of it at once.",
  "dex": 370
 },
 {
  "id": "apache-superset",
  "name": "Apache Superset",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a magic coloring book where you tell it what questions to ask your data, and it automatically draws pretty pictures to show you the answers.",
  "description": "Apache Superset is an open-source data visualization and business intelligence platform that allows users to create interactive dashboards and explore datasets through a web interface. It supports multiple database backends and provides SQL query capabilities alongside drag-and-drop dashboard creation.",
  "use_cases": [
   "Building interactive business intelligence dashboards for stakeholders",
   "Exploring and querying data without writing complex SQL",
   "Creating real-time monitoring dashboards for metrics and KPIs",
   "Sharing data insights across departments with role-based access control"
  ],
  "used_by": [
   "Airbnb",
   "Lyft",
   "Cisco",
   "Twitter"
  ],
  "docs": "https://superset.apache.org/",
  "tutorial": "https://superset.apache.org/docs/intro",
  "added": "2026-07-08",
  "dex": 371
 },
 {
  "id": "axle",
  "name": "Axle",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Axle is like a smart librarian that automatically organizes and transforms messy data into neat, usable formats without you having to write complicated instructions.",
  "description": "Axle is a data integration and transformation platform that uses AI to automatically generate data pipelines and mappings. It reduces the time needed to build ETL workflows by intelligently understanding data schemas and relationships.",
  "use_cases": [
   "Automated data pipeline generation between disparate data sources",
   "Schema mapping and data transformation without manual coding",
   "Data quality monitoring and anomaly detection",
   "Cross-platform data integration for enterprise systems"
  ],
  "used_by": [
   "Databricks",
   "Stripe",
   "Notion"
  ],
  "docs": "https://docs.axle.so/",
  "tutorial": "https://axle.so/docs/getting-started",
  "added": "2026-06-22",
  "dex": 372
 },
 {
  "id": "benthos",
  "name": "Benthos Stream Processing",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a Swiss Army knife for moving data around—it can read from anywhere, transform what it reads, and send it anywhere else, all in one tool.",
  "description": "Benthos is a high-performance stream processing tool for building data pipelines with a simple YAML configuration. It connects multiple sources, applies transformations, and outputs to various destinations without requiring code compilation.",
  "use_cases": [
   "Real-time data pipeline orchestration",
   "Multi-source data integration and normalization",
   "Log and metric transformation before storage",
   "Event stream filtering and enrichment"
  ],
  "used_by": [
   "Meltwater",
   "BentoML",
   "Various fintech firms"
  ],
  "docs": "https://www.benthos.dev/docs/about",
  "tutorial": "https://www.benthos.dev/docs/guides/getting_started",
  "added": "2026-08-07",
  "dex": 373
 },
 {
  "id": "bigeye",
  "name": "Bigeye",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like having a quality inspector at a restaurant who watches the kitchen and tells you immediately if something looks wrong with the food being prepared.",
  "description": "Bigeye is a data observability platform that monitors data quality, schema changes, and anomalies in real-time across data warehouses and pipelines. It helps teams detect and resolve data issues before they impact downstream analytics and business decisions.",
  "use_cases": [
   "Monitoring data quality metrics and detecting anomalies automatically",
   "Alerting on schema changes and breaking data contracts",
   "Tracing data lineage to identify root causes of quality issues",
   "Building data trust scorecards for business stakeholders"
  ],
  "used_by": [
   "SiriusXM",
   "Toast",
   "Yelp",
   "DoorDash"
  ],
  "docs": "https://docs.bigeye.com/",
  "tutorial": "https://docs.bigeye.com/docs/getting-started",
  "added": "2026-07-08",
  "dex": 374
 },
 {
  "id": "bigquery",
  "name": "BigQuery",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "A Google spreadsheet that holds billions of rows and answers questions about them in seconds.",
  "description": "GCP's serverless, highly scalable data warehouse. Separates compute from storage — pay per query or flat-rate slots. Columnar storage and Dremel execution engine enable sub-second queries over petabyte datasets. Native ML (BigQuery ML), geospatial, and streaming support.",
  "use_cases": [
   "Ad-hoc analytics over petabyte datasets without managing infrastructure",
   "Real-time streaming inserts via BigQuery Storage Write API",
   "BigQuery ML: train and serve ML models with SQL — no Python required",
   "BI dashboards via Looker Studio, Tableau, or Power BI direct connection",
   "Data sharing across organizations via BigQuery Analytics Hub"
  ],
  "used_by": [
   "Twitter",
   "Spotify",
   "The New York Times",
   "Dow Jones",
   "UPS"
  ],
  "docs": "https://cloud.google.com/bigquery/docs",
  "tutorial": "https://cloud.google.com/bigquery/docs/quickstarts/load-data-console",
  "added": "2026-06-08",
  "dex": 375
 },
 {
  "id": "buf-schema",
  "name": "Buf Schema Registry",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a shared rulebook that tells all your applications exactly how to talk to each other using special message formats, and it remembers every version.",
  "description": "Buf is a platform for managing Protocol Buffers and gRPC services with schema versioning, linting, and breaking change detection. It provides centralized schema management with CLI tools and a managed registry to ensure API compatibility across distributed teams.",
  "use_cases": [
   "Protocol Buffers and gRPC API schema management",
   "API versioning and breaking change detection",
   "Code generation automation for multiple languages",
   "Microservices contract enforcement"
  ],
  "used_by": [
   "Stripe",
   "Lyft",
   "Square",
   "Confluence"
  ],
  "docs": "https://buf.build/docs",
  "tutorial": "https://buf.build/docs/tour/getting-started-with-buf-cli",
  "added": "2026-07-20",
  "dex": 376
 },
 {
  "id": "clickhouse-keeper",
  "name": "ClickHouse Keeper",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a referee who keeps track of who should talk to whom in a big network of computers that all store massive amounts of data together.",
  "description": "ClickHouse Keeper is a coordination service that replaces ZooKeeper for ClickHouse clusters, providing consensus and metadata management with improved performance and reduced operational complexity. It enables ClickHouse deployments to manage distributed replicas and cluster state without external dependencies.",
  "use_cases": [
   "Distributed ClickHouse cluster coordination",
   "Metadata management for replicated tables",
   "Failover and high-availability orchestration",
   "Reducing operational overhead vs. ZooKeeper deployments"
  ],
  "used_by": [
   "Yandex",
   "Cloudflare",
   "Bing",
   "Alibaba"
  ],
  "docs": "https://clickhouse.com/docs/en/operations/clickhouse-keeper",
  "tutorial": "https://clickhouse.com/docs/en/operations/clickhouse-keeper/quickstart",
  "added": "2026-08-23",
  "dex": 377
 },
 {
  "id": "databricks",
  "name": "Databricks",
  "category": "Platform",
  "domain": "Data Engineering",
  "description": "Unified analytics platform built on Apache Spark — combines data engineering, ML training, and SQL analytics in one lakehouse. Runs on AWS, Azure, or GCP. Founded by the creators of Apache Spark.",
  "use_cases": [
   "Large-scale ETL and data transformation pipelines with Delta Lake",
   "Distributed ML model training with MLflow experiment tracking",
   "SQL analytics on data lake via Databricks SQL warehouses",
   "Streaming pipelines with Structured Streaming on Kafka/Kinesis",
   "Unity Catalog for governed, multi-cloud data mesh"
  ],
  "used_by": [
   "Comcast",
   "Regeneron",
   "Shell",
   "Conde Nast",
   "Block"
  ],
  "docs": "https://docs.databricks.com/",
  "tutorial": "https://docs.databricks.com/en/getting-started/index.html",
  "added": "2026-06-08",
  "eli5": "A giant playground where data people crunch huge piles of data together.",
  "dex": 378
 },
 {
  "id": "dbt",
  "name": "dbt",
  "category": "Tool",
  "domain": "Data Engineering",
  "description": "Data Build Tool — SQL-first transformation framework that brings software engineering practices (version control, testing, documentation, modularity) to analytics engineering. Runs inside your data warehouse; no data moves outside.",
  "use_cases": [
   "Transforming raw warehouse data into clean, tested, documented data models",
   "Defining data lineage DAGs with ref() for dependency-aware execution",
   "Testing data quality: not-null, uniqueness, referential integrity, custom assertions",
   "Auto-generating data documentation and lineage graphs from model definitions",
   "dbt Cloud for managed scheduling, CI checks on PRs, and IDE"
  ],
  "used_by": [
   "Spotify",
   "Shopify",
   "HubSpot",
   "GitLab",
   "Conde Nast"
  ],
  "docs": "https://docs.getdbt.com/",
  "tutorial": "https://docs.getdbt.com/guides",
  "added": "2026-06-08",
  "eli5": "A chef that turns raw data ingredients into clean, tested, ready-to-serve dishes.",
  "dex": 379
 },
 {
  "id": "dbt-core-metrics",
  "name": "dbt Metrics",
  "category": "Framework",
  "domain": "Data Engineering",
  "eli5": "It's like a recipe book where you write down how to calculate important business numbers so everyone uses the same recipe.",
  "description": "dbt Metrics is a framework for defining business metrics in dbt projects that ensures consistency across analytics and reporting. It allows data teams to centrally define metrics once and reference them across multiple tools and dashboards.",
  "use_cases": [
   "Defining and maintaining consistent KPIs across organization dashboards",
   "Enabling seamless metric sharing between dbt, BI tools, and downstream applications",
   "Creating a single source of truth for business metrics definitions",
   "Automating metric calculations and dependencies in the transformation workflow"
  ],
  "used_by": [
   "Gitlab",
   "Atlan",
   "Fivetran",
   "Mode Analytics"
  ],
  "docs": "https://docs.getdbt.com/docs/build/metrics",
  "tutorial": "https://docs.getdbt.com/guides/use-metricflow",
  "added": "2026-09-04",
  "dex": 380
 },
 {
  "id": "dbt-semantic-layer",
  "name": "dbt Semantic Layer",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like creating a standardized translator between your data warehouse and all your business tools, so everyone gets the same answer when they ask about your metrics.",
  "description": "dbt Semantic Layer is a centralized metrics platform that defines business metrics once and makes them available across analytics tools and applications. It ensures consistency in metric definitions and enables self-service analytics while maintaining data governance and quality standards.",
  "use_cases": [
   "Centralized business metrics definition and governance",
   "Self-service analytics for non-technical users",
   "Multi-tool metric consistency and reconciliation",
   "Data lineage and impact analysis for metrics"
  ],
  "used_by": [
   "Shopify",
   "Figma",
   "DoorDash",
   "Glossier"
  ],
  "docs": "https://docs.getdbt.com/docs/use-cases/semantic-layer",
  "tutorial": "https://docs.getdbt.com/guides/dbt-semantic-layer",
  "added": "2026-07-07",
  "dex": 381
 },
 {
  "id": "delta-lake",
  "name": "Delta Lake",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Delta Lake is like adding a save history to your data lake — you can undo mistakes, read yesterday's data, and write reliably without corrupting anything.",
  "description": "Open-source storage layer bringing ACID transactions to Apache Spark and big data workloads. Provides time travel, schema enforcement, and unified batch/streaming.",
  "use_cases": [
   "ACID transactions on Spark data lakes",
   "Upserts and deletes on Parquet tables",
   "CDC pipeline targets with merge operations",
   "Data versioning and audit trails"
  ],
  "used_by": [
   "Databricks",
   "Microsoft",
   "Comcast",
   "AT&T"
  ],
  "docs": "https://docs.delta.io/latest/",
  "tutorial": "https://docs.delta.io/latest/quick-start.html",
  "added": "2026-06-12",
  "dex": 382
 },
 {
  "id": "duckdb-iceberg",
  "name": "DuckDB with Apache Iceberg",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like having a super-smart filing cabinet that organizes your data really efficiently and lets you query it incredibly fast without needing a whole team of database administrators.",
  "description": "DuckDB's integration with Apache Iceberg provides a modern analytical database experience with versioning, time-travel, and reliable ACID transactions. This combination is transforming data lakehouses by enabling efficient SQL analytics directly on cloud object storage.",
  "use_cases": [
   "Building cost-effective data lakehouses on S3/GCS without Spark",
   "Time-travel analytics and data debugging with Iceberg table versioning",
   "Real-time OLAP workloads with sub-second query latency",
   "Data governance and compliance with Iceberg's schema evolution and retention policies"
  ],
  "used_by": [
   "Databricks customers",
   "Netflix (Iceberg creators)",
   "Stripe",
   "Apple"
  ],
  "docs": "https://duckdb.org/docs/extensions/iceberg",
  "tutorial": "https://duckdb.org/docs/extensions/iceberg#creating-iceberg-tables",
  "added": "2026-08-02",
  "dex": 383
 },
 {
  "id": "elyra",
  "name": "Elyra",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a visual puzzle builder where you can drag-and-drop pieces to create data pipelines without writing code.",
  "description": "Elyra is an open-source JupyterLab extension that provides visual tools for building and running data science and machine learning pipelines. It simplifies workflow orchestration by enabling both code-based and visual pipeline creation.",
  "use_cases": [
   "Rapid prototyping of ML workflows",
   "Visual data pipeline orchestration",
   "Kubeflow and Apache Airflow integration",
   "Notebook-to-production pipeline conversion"
  ],
  "used_by": [
   "IBM",
   "Red Hat",
   "Open Data Foundation"
  ],
  "docs": "https://elyra.readthedocs.io/",
  "tutorial": "https://elyra.readthedocs.io/en/latest/getting_started/overview.html",
  "added": "2026-07-13",
  "dex": 384
 },
 {
  "id": "evidence-data-analytics",
  "name": "Evidence",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Evidence is like turning your SQL questions into beautiful reports automatically, so you ask a question about your data and it creates a nice dashboard you can share.",
  "description": "Evidence is an open-source framework that transforms SQL queries and Python scripts into interactive, version-controlled analytics reports using markdown-like syntax. It enables data teams to build and deploy analytics without learning BI tool interfaces, integrating with SQL and Python for dynamic dashboards.",
  "use_cases": [
   "Creating self-service analytics dashboards from SQL queries",
   "Building data-driven documents version-controlled in Git",
   "Automating report generation and distribution",
   "Embedding analytics into documentation and wikis"
  ],
  "used_by": [
   "Databricks",
   "Seed Analytics",
   "Data-driven startups",
   "Enterprise analytics teams"
  ],
  "docs": "https://docs.evidence.dev",
  "tutorial": "https://docs.evidence.dev/getting-started/start-here",
  "added": "2026-09-23",
  "dex": 385
 },
 {
  "id": "fivetran",
  "name": "Fivetran",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Fivetran is like an automatic conveyor belt that moves data from many different stores into one big warehouse without you having to manually carry each box.",
  "description": "Fivetran is a cloud-native data integration platform that automates ETL/ELT pipelines, connecting hundreds of data sources to cloud data warehouses. It reduces manual data pipeline work and ensures data quality through pre-built connectors.",
  "use_cases": [
   "Consolidating data from SaaS applications into data warehouses",
   "Real-time data synchronization across platforms",
   "Building self-service analytics infrastructure",
   "Enabling DataOps with automated data quality checks"
  ],
  "used_by": [
   "SurveyMonkey",
   "Nextdoor",
   "Notion",
   "Retool"
  ],
  "docs": "https://fivetran.com/docs",
  "tutorial": "https://fivetran.com/docs/getting-started",
  "added": "2026-06-16",
  "dex": 386
 },
 {
  "id": "frugal",
  "name": "Frugal",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "A smart tool that makes it super easy for different computer programs to talk to each other using a shared language they all understand.",
  "description": "Frugal is a code generation tool that creates multi-language RPC clients and servers with message serialization, enabling efficient service-to-service communication across polyglot architectures. It extends Thrift with better code generation and modern language support.",
  "use_cases": [
   "Microservice communication in polyglot systems",
   "Building consistent APIs across Python, Go, Java, and Rust services",
   "Event-driven architecture message definitions",
   "gRPC alternative with simpler code generation for legacy systems"
  ],
  "used_by": [
   "Walmart Labs",
   "Groupon",
   "SeatGeek",
   "DigitalOcean"
  ],
  "docs": "https://frugal.io/",
  "tutorial": "https://frugal.io/docs/tutorial/",
  "added": "2026-08-10",
  "dex": 387
 },
 {
  "id": "materialized",
  "name": "Materialize",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "It's like having a smart worker who continuously updates a whiteboard with the latest information from your data streams so you always see what's happening right now.",
  "description": "Materialize is a SQL-powered streaming database that enables real-time analytics by computing incremental views over streaming data. It uses computational differential dataflow to provide low-latency, consistent results for complex analytical queries on continuously changing data.",
  "use_cases": [
   "Real-time analytics dashboards and monitoring systems",
   "Streaming data transformation and enrichment pipelines",
   "Complex event processing and anomaly detection",
   "Building operational intelligence systems from event streams"
  ],
  "used_by": [
   "Figma",
   "Pagerduty",
   "Jamio",
   "Various fintech firms"
  ],
  "docs": "https://materialize.com/docs/",
  "tutorial": "https://materialize.com/docs/get-started/",
  "added": "2026-06-17",
  "dex": 388
 },
 {
  "id": "opensearch-ingestion",
  "name": "OpenSearch Ingestion",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "OpenSearch Ingestion is like a mail sorting center that takes data from lots of different places and organizes it into a searchable library.",
  "description": "OpenSearch Ingestion is a fully managed data collection and processing service that enables users to ingest, parse, and enrich data from multiple sources before indexing into OpenSearch clusters. It simplifies log aggregation and data pipeline construction without managing infrastructure.",
  "use_cases": [
   "Log aggregation from distributed systems",
   "Real-time data enrichment pipelines",
   "Metrics collection from multiple sources",
   "Security event correlation and analysis"
  ],
  "used_by": [
   "Amazon",
   "BMW",
   "Equinix",
   "Overstock"
  ],
  "docs": "https://opensearch.org/docs/latest/data-prepper/",
  "tutorial": "https://opensearch.org/docs/latest/data-prepper/getting-started/",
  "added": "2026-06-20",
  "dex": 389
 },
 {
  "id": "oryx-data-plane",
  "name": "Oryx Data Plane",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a conveyor belt system that moves data from one place to another really fast, and can transform it along the way without needing to stop.",
  "description": "Oryx is a real-time machine learning platform that enables continuous feature computation and model serving with millisecond latency. It unifies data pipelines, feature engineering, and model deployment in a single scalable system.",
  "use_cases": [
   "Real-time fraud detection and anomaly detection",
   "Online machine learning model serving",
   "Continuous feature engineering for recommendation systems",
   "Low-latency prediction serving in financial trading applications"
  ],
  "used_by": [
   "Yahoo",
   "Alibaba",
   "LinkedIn"
  ],
  "docs": "https://oryxonline.github.io/",
  "tutorial": "https://oryxonline.github.io/docs/quickstart/",
  "added": "2026-09-20",
  "dex": 390
 },
 {
  "id": "par",
  "name": "Par",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a smart librarian that organizes massive amounts of data into neat shelves so computers can find what they need super fast.",
  "description": "Par is a columnar data format and query engine designed for analytical workloads at massive scale with superior compression and query performance. It optimizes storage efficiency and read performance for OLAP operations.",
  "use_cases": [
   "Data warehousing and analytical queries on large datasets",
   "Time-series data analysis and metrics storage",
   "Scientific data processing and research computing",
   "Cost optimization for cloud-based data analytics"
  ],
  "used_by": [
   "DuckDB community",
   "Analytics engineers",
   "Data scientists"
  ],
  "docs": "https://duckdb.org/docs/guides/file_formats/parquet",
  "tutorial": "https://duckdb.org/docs/guides/file_formats/parquet",
  "added": "2026-08-04",
  "dex": 391
 },
 {
  "id": "prefect",
  "name": "Prefect",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Prefect is like a smart to-do list manager for your data pipelines — it schedules tasks, retries failures, and tells you exactly what went wrong.",
  "description": "Modern workflow orchestration platform for data pipelines. Python-native, with automatic retries, observability, and a hosted UI. Positioned as a simpler Airflow alternative.",
  "use_cases": [
   "ETL/ELT pipeline orchestration",
   "ML training pipeline scheduling",
   "Data quality checks and alerting",
   "Event-triggered and cron-based data workflows"
  ],
  "used_by": [
   "Doordash",
   "Accenture",
   "Labcorp",
   "Raft"
  ],
  "docs": "https://docs.prefect.io/",
  "tutorial": "https://docs.prefect.io/v3/get-started/quickstart",
  "added": "2026-06-12",
  "dex": 392
 },
 {
  "id": "risingwave",
  "name": "RisingWave",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "It's like a waiter who watches a stream of orders coming in and automatically tells you when patterns happen, without waiting for everything to stop.",
  "description": "A distributed SQL streaming database built in Rust that enables real-time analytics and stream processing without the complexity of traditional stream processing frameworks. It uses PostgreSQL-compatible SQL syntax for data transformations.",
  "use_cases": [
   "Real-time event stream processing and analytics",
   "Continuous ETL pipelines with incremental computing",
   "Live dashboards and metrics aggregation",
   "Anomaly detection and alerting systems"
  ],
  "used_by": [
   "Uber",
   "Airbnb",
   "Shopify"
  ],
  "docs": "https://docs.risingwave.com/",
  "tutorial": "https://docs.risingwave.com/docs/current/get-started/",
  "added": "2026-08-21",
  "dex": 393
 },
 {
  "id": "snowflake",
  "name": "Snowflake",
  "category": "Platform",
  "domain": "Data Engineering",
  "description": "Cloud data warehouse built for the cloud with separate compute and storage. Scales compute instantly per query, enabling concurrent workloads without contention. Supports structured and semi-structured (JSON, Avro, Parquet) data natively.",
  "use_cases": [
   "Enterprise data warehouse replacing on-prem Teradata/Redshift",
   "Data sharing across organizations without copying data (Snowflake Marketplace)",
   "ELT pipelines with dbt transformations on top of raw ingested data",
   "Data lake queries over external S3/GCS/Azure storage via External Tables",
   "Snowpark for Python/Java/Scala data engineering inside Snowflake"
  ],
  "used_by": [
   "Capital One",
   "JetBlue",
   "Instacart",
   "DoorDash",
   "Zoom",
   "Adobe"
  ],
  "docs": "https://docs.snowflake.com/",
  "tutorial": "https://docs.snowflake.com/en/user-guide/tutorials/tasty-bytes-introduction",
  "added": "2026-06-08",
  "eli5": "A shared spreadsheet in the cloud that gets more powerful when you need it.",
  "dex": 394
 },
 {
  "id": "sqlglot",
  "name": "sqlglot",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "It's like a translator that converts recipes written in one restaurant's style into another restaurant's style so both kitchens can cook the same meal.",
  "description": "sqlglot is a SQL parser, transpiler, and optimizer that can parse and generate SQL across different databases (PostgreSQL, BigQuery, Snowflake, etc.). It enables data engineers to write database-agnostic SQL and automatically translate queries between different SQL dialects.",
  "use_cases": [
   "Cross-database query translation and migration",
   "SQL validation and formatting",
   "Query optimization across multiple platforms",
   "Building data pipeline orchestration tools"
  ],
  "used_by": [
   "Databricks",
   "Stripe",
   "dbt Labs"
  ],
  "docs": "https://sqlglot.com/",
  "tutorial": "https://github.com/tobymao/sqlglot",
  "added": "2026-06-28",
  "dex": 395
 },
 {
  "id": "teable",
  "name": "Teable",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "It's like a super-powered spreadsheet that acts like a database and lets multiple people collaborate in real-time.",
  "description": "Open-source no-code database platform that combines spreadsheet UX with relational database power, offering real-time collaboration and API-first architecture. It serves as a modern alternative to traditional databases for teams that want database capabilities without complex SQL.",
  "use_cases": [
   "Managing project data and workflows without writing code",
   "Building collaborative business applications with real-time synchronization",
   "Creating internal tools and dashboards for non-technical teams",
   "Rapid prototyping of data-driven applications"
  ],
  "used_by": [
   "Startups",
   "Enterprise teams",
   "No-code advocates"
  ],
  "docs": "https://docs.teable.io/",
  "tutorial": "https://github.com/teableio/teable#quick-start",
  "added": "2026-08-25",
  "dex": 396
 },
 {
  "id": "shadowing",
  "name": "Tinybird",
  "category": "Platform",
  "domain": "Data Engineering",
  "eli5": "Tinybird is like a magic notebook that instantly transforms messy notes into beautiful charts and answers questions about what happened, all without waiting.",
  "description": "Tinybird is a serverless real-time analytics platform that ingests, processes, and serves analytics data at scale without infrastructure management. It enables teams to build real-time dashboards and APIs on streaming data with sub-second latency.",
  "use_cases": [
   "Real-time product analytics and dashboards",
   "Building real-time APIs that serve analytics queries",
   "Event streaming analysis and monitoring",
   "Time-series data aggregation without managing databases"
  ],
  "used_by": [
   "Auth0",
   "Vercel",
   "Notion"
  ],
  "docs": "https://www.tinybird.co/docs/",
  "tutorial": "https://www.tinybird.co/docs/concepts/data-sources/",
  "added": "2026-06-15",
  "dex": 397
 },
 {
  "id": "trino",
  "name": "Trino",
  "category": "Tool",
  "domain": "Data Engineering",
  "eli5": "Trino is like a universal translator that lets you ask questions across many different databases at once without moving all the data to one place.",
  "description": "Distributed SQL query engine for big data analytics across heterogeneous data sources including S3, HDFS, Kafka, and relational databases — formerly known as PrestoSQL.",
  "use_cases": [
   "Federated queries across data lakes and warehouses",
   "Ad-hoc analytics on S3/Hive tables",
   "Replacing ETL with in-place querying",
   "Interactive data exploration at petabyte scale"
  ],
  "used_by": [
   "Meta",
   "Netflix",
   "LinkedIn",
   "Lyft",
   "Airbnb"
  ],
  "docs": "https://trino.io/docs/current/",
  "tutorial": "https://trino.io/docs/current/installation/deployment.html",
  "added": "2026-06-11",
  "dex": 398
 },
 {
  "id": "angular",
  "name": "Angular",
  "category": "Framework",
  "domain": "Frontend",
  "description": "Full-featured TypeScript-first frontend framework by Google. Opinionated, batteries-included: routing, forms, HTTP client, dependency injection, and CLI all built in. Suits large enterprise teams needing strong conventions and long-term stability.",
  "use_cases": [
   "Large-scale enterprise SPAs with structured module and component architecture",
   "Reactive forms with built-in validation and two-way data binding",
   "RxJS-powered async patterns for complex event and data stream handling",
   "Server-side rendering via Angular Universal for SEO and performance",
   "Micro-frontends with Module Federation and Angular standalone components"
  ],
  "used_by": [
   "Google",
   "Microsoft",
   "Forbes",
   "UPS",
   "Samsung",
   "Deutsche Bank"
  ],
  "docs": "https://angular.dev/overview",
  "tutorial": "https://angular.dev/tutorials/learn-angular",
  "added": "2026-06-08",
  "eli5": "A complete workshop for building big websites — Google made it with strict rules.",
  "dex": 399
 },
 {
  "id": "astro",
  "name": "Astro",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Astro is like a website builder that sends zero JavaScript to your users by default — pages load super fast because there's nothing extra to download.",
  "description": "Content-focused web framework that ships zero JS by default using an 'islands' architecture. Supports React, Vue, Svelte, and Solid components side-by-side.",
  "use_cases": [
   "Marketing sites and blogs with minimal JS",
   "Documentation sites with fast page loads",
   "E-commerce storefronts with selective hydration",
   "Mixing multiple UI frameworks in one project"
  ],
  "used_by": [
   "Google",
   "Microsoft",
   "Netlify",
   "The Guardian"
  ],
  "docs": "https://docs.astro.build/",
  "tutorial": "https://docs.astro.build/en/tutorial/0-introduction/",
  "added": "2026-06-12",
  "dex": 400
 },
 {
  "id": "astro-content-collections",
  "name": "Astro Content Collections",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "It's like having a filing cabinet that automatically organizes all your documents and makes sure they're formatted correctly before you use them.",
  "description": "Astro Content Collections is a feature for managing structured content with type-safe frontmatter validation and querying capabilities. It integrates with Astro's build system to provide compile-time validation of content schemas.",
  "use_cases": [
   "Building type-safe documentation sites with validated markdown frontmatter",
   "Creating content-driven websites with runtime guarantees on content structure",
   "Managing blog posts and collections with automatic schema validation",
   "Building multi-language content platforms with consistent structure"
  ],
  "used_by": [
   "Astro documentation",
   "Parity.io",
   "Librarians",
   "Technical bloggers"
  ],
  "docs": "https://docs.astro.build/en/guides/content-collections/",
  "tutorial": "https://docs.astro.build/en/tutorials/add-content-collections/",
  "added": "2026-09-09",
  "dex": 401
 },
 {
  "id": "biome",
  "name": "Biome",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Biome is like a super-smart teacher that checks your JavaScript and fixes mistakes in your code automatically, all in one place.",
  "description": "Biome is a fast, all-in-one toolchain for JavaScript and TypeScript that combines linting, formatting, and code analysis. It replaces tools like ESLint, Prettier, and other formatters with a single, unified solution written in Rust for speed.",
  "use_cases": [
   "Enforcing consistent code style across large JavaScript/TypeScript projects",
   "Catching bugs and security issues before code review",
   "Reducing CI/CD time with fast, parallelized linting and formatting",
   "Migrating away from fragmented tool chains like ESLint + Prettier"
  ],
  "used_by": [
   "Rome (now Biome), Astro, Shopify"
  ],
  "docs": "https://biomejs.dev/",
  "tutorial": "https://biomejs.dev/guides/getting-started/",
  "added": "2026-07-11",
  "dex": 402
 },
 {
  "id": "bun",
  "name": "Bun",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Bun is like a turbocharged Node.js — it runs JavaScript much faster and also replaces npm, webpack, and Jest all in one tool.",
  "description": "Fast all-in-one JavaScript runtime, bundler, test runner, and package manager built with Zig. Drop-in Node.js compatible with 3x faster startup and built-in TypeScript support.",
  "use_cases": [
   "Drop-in Node.js replacement for faster backends",
   "Faster CI pipelines (install + test + build)",
   "Edge and serverless JS workloads",
   "Monorepo tooling with built-in workspace support"
  ],
  "used_by": [
   "Vercel",
   "Cloudflare",
   "various JS toolchain teams"
  ],
  "docs": "https://bun.sh/docs",
  "tutorial": "https://bun.sh/docs/quickstart",
  "added": "2026-06-12",
  "dex": 403
 },
 {
  "id": "cursor-ai-editor",
  "name": "Cursor",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "A code editor where you can chat with an AI helper to write and fix code for you, like having a really smart programming partner sitting next to you.",
  "description": "Cursor is an AI-powered code editor built on VS Code that integrates Claude and GPT-4 for inline code generation, refactoring, and debugging. It provides context-aware code suggestions and allows developers to iterate on code through natural language commands.",
  "use_cases": [
   "Accelerated development with AI-assisted code generation",
   "Rapid prototyping and boilerplate code scaffolding",
   "Bug detection and code refactoring assistance",
   "Learning and understanding unfamiliar codebases"
  ],
  "used_by": [
   "Vercel",
   "Stack Overflow",
   "Y Combinator startups",
   "Individual developers"
  ],
  "docs": "https://docs.cursor.sh/",
  "tutorial": "https://docs.cursor.sh/getting-started/quick-start",
  "added": "2026-07-24",
  "dex": 404
 },
 {
  "id": "esbuild",
  "name": "esbuild",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like a super-fast photocopier that takes your JavaScript code and makes it smaller and faster, kind of like shrinking your toys to fit in a smaller box.",
  "description": "An extremely fast JavaScript bundler and minifier written in Go that significantly reduces build times compared to traditional bundlers. It's widely adopted for its performance advantages and is integrated into popular frameworks like Vite and Next.js.",
  "use_cases": [
   "JavaScript and TypeScript application bundling",
   "CSS and asset minification for production builds",
   "Development server with hot module replacement",
   "Building libraries and npm packages"
  ],
  "used_by": [
   "Vercel",
   "Shopify",
   "Figma",
   "Stripe"
  ],
  "docs": "https://esbuild.github.io/",
  "tutorial": "https://esbuild.github.io/getting-started/",
  "added": "2026-08-06",
  "dex": 405
 },
 {
  "id": "framer-motion",
  "name": "Framer Motion",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "It's like a magic wand for making objects on a web page move and dance smoothly without you having to do all the difficult math.",
  "description": "Framer Motion is a React animation library that simplifies creating fluid, declarative animations and gestures with GPU-accelerated rendering. It provides an intuitive API for complex motion design without requiring advanced animation knowledge.",
  "use_cases": [
   "Building smooth page transitions and component animations",
   "Creating gesture-driven interfaces with drag, pan, and tap interactions",
   "Implementing advanced scroll-triggered animations",
   "Designing interactive data visualizations and micro-interactions"
  ],
  "used_by": [
   "Vercel",
   "Stripe",
   "Figma",
   "Twitter/X"
  ],
  "docs": "https://www.framer.com/motion/",
  "tutorial": "https://www.framer.com/motion/tutorials/",
  "added": "2026-09-03",
  "dex": 406
 },
 {
  "id": "vscode-copilot",
  "name": "GitHub Copilot",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like having a helpful coding partner sitting next to you who suggests the next lines of code as you type.",
  "description": "GitHub Copilot is an AI-powered code completion tool that uses OpenAI's Codex model to suggest code snippets, functions, and entire blocks based on context and comments. It integrates with VS Code and other editors to accelerate development workflows.",
  "use_cases": [
   "Accelerating boilerplate code generation and routine implementations",
   "Suggesting test cases and documentation based on function signatures",
   "Learning new languages and frameworks through intelligent code suggestions",
   "Reducing time-to-productivity for developers working with unfamiliar codebases"
  ],
  "used_by": [
   "Microsoft",
   "Spotify",
   "AWS",
   "Adobe"
  ],
  "docs": "https://github.com/features/copilot",
  "tutorial": "https://docs.github.com/en/copilot/quickstart",
  "added": "2026-09-04",
  "dex": 407
 },
 {
  "id": "marimo",
  "name": "Marimo",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Marimo is like a smart notebook that lets you write Python code that automatically updates everything when you change something, kind of like a spreadsheet but more powerful.",
  "description": "Marimo is a reactive Python notebook that automatically re-runs cells when dependencies change, providing an Excel-like reactive programming experience. It's designed to be reproducible, version-controllable, and deployable as web applications.",
  "use_cases": [
   "Building interactive data science dashboards and reports",
   "Creating reproducible computational notebooks for research",
   "Deploying Python analyses as standalone web applications",
   "Teaching programming with immediate visual feedback"
  ],
  "used_by": [
   "Google Research",
   "Meta AI",
   "Stanford University"
  ],
  "docs": "https://docs.marooml.io/",
  "tutorial": "https://marooml.io/#getting-started",
  "added": "2026-07-06",
  "dex": 408
 },
 {
  "id": "nextjs",
  "name": "Next.js",
  "category": "Framework",
  "domain": "Frontend",
  "description": "React framework for production — handles routing, SSR, SSG, ISR, API routes, and image/font optimization out of the box. App Router (Next.js 13+) enables React Server Components and streaming. Developed and maintained by Vercel.",
  "use_cases": [
   "Hybrid rendering: SSR for dynamic pages, SSG for static content, ISR for revalidation",
   "Full-stack apps with API routes or Server Actions colocated with UI",
   "React Server Components for zero-JS server-rendered UI",
   "Image optimization, font loading, and script management via built-in components",
   "Edge middleware for auth, redirects, and A/B testing at the CDN layer"
  ],
  "used_by": [
   "Vercel",
   "TikTok",
   "Twitch",
   "Notion",
   "Nike",
   "GitHub"
  ],
  "docs": "https://nextjs.org/docs",
  "tutorial": "https://nextjs.org/learn",
  "added": "2026-06-08",
  "eli5": "React with superpowers — handles the boring stuff so you just build cool features.",
  "dex": 409
 },
 {
  "id": "opengl",
  "name": "OpenGL",
  "category": "Platform",
  "domain": "Frontend",
  "eli5": "OpenGL is like a universal instruction manual that tells your computer's graphics card how to draw 3D pictures and games on your screen.",
  "description": "OpenGL is a cross-platform graphics API that enables developers to create 2D and 3D graphics applications by communicating directly with GPU hardware. It's the industry standard for rendering in games, scientific visualization, and graphics-intensive applications.",
  "use_cases": [
   "Video game rendering and graphics pipelines",
   "CAD software and 3D modeling applications",
   "Scientific visualization and computational graphics",
   "VR/AR content creation and real-time rendering"
  ],
  "used_by": [
   "Blender",
   "Unity Engine",
   "Unreal Engine",
   "Autodesk"
  ],
  "docs": "https://www.opengl.org/",
  "tutorial": "https://learnopengl.com/",
  "added": "2026-09-02",
  "dex": 410
 },
 {
  "id": "oxc",
  "name": "OXC",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like a super-fast spell checker for JavaScript code that catches mistakes before you run it.",
  "description": "OXC is a high-performance JavaScript linter and parser written in Rust, designed as a faster alternative to ESLint. It provides comprehensive code quality checks with significantly improved speed for large codebases.",
  "use_cases": [
   "Linting monorepos with thousands of files",
   "Pre-commit hooks requiring sub-second feedback",
   "IDE integration for real-time code analysis",
   "CI/CD pipelines with strict performance requirements"
  ],
  "used_by": [
   "Alibaba",
   "Shopify",
   "Vercel",
   "Datadog"
  ],
  "docs": "https://oxc.rs/",
  "tutorial": "https://oxc.rs/docs/guide/",
  "added": "2026-07-12",
  "dex": 411
 },
 {
  "id": "pagefind",
  "name": "Pagefind",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Pagefind is like a tiny librarian built into your website that can search through all the words on every page instantly, without needing to ask a server for help.",
  "description": "Pagefind is a fully static search library for static sites that requires zero server infrastructure, built with Rust and compiled to WebAssembly. It automatically indexes your entire site and provides instant full-text search with filtering capabilities entirely in the browser.",
  "use_cases": [
   "Adding search functionality to static documentation sites without backend servers",
   "Building searchable blog archives and knowledge bases for static sites",
   "Improving user experience on Astro, Hugo, Jekyll, or other static site generators",
   "Creating offline-capable search for progressive web applications"
  ],
  "used_by": [
   "Astro documentation",
   "Pocketbase docs",
   "Meilisearch docs",
   "Nuxt Labs"
  ],
  "docs": "https://pagefind.app/docs/",
  "tutorial": "https://pagefind.app/docs/installation/",
  "added": "2026-09-14",
  "dex": 412
 },
 {
  "id": "partytown",
  "name": "Partytown",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "It's like sending your website's annoying chores to a worker in another room so they don't slow down your main page.",
  "description": "Partytown is a JavaScript library that runs third-party scripts (analytics, ads, trackers) in a web worker to prevent them from blocking the main thread. This significantly improves Core Web Vitals and user experience without removing required scripts.",
  "use_cases": [
   "Improving website performance with analytics",
   "Running ads without blocking page interactions",
   "Serving tracking pixels and marketing scripts",
   "Enhancing Core Web Vitals scores"
  ],
  "used_by": [
   "Shopify",
   "Zapier",
   "BuilderIO",
   "Various e-commerce sites"
  ],
  "docs": "https://partytown.builder.io/docs",
  "tutorial": "https://partytown.builder.io/docs/getting-started",
  "added": "2026-07-22",
  "dex": 413
 },
 {
  "id": "penpot",
  "name": "Penpot",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Penpot is like Figma but free and open-source, so teams can design websites and apps together without paying a subscription.",
  "description": "Penpot is an open-source design and prototyping tool providing vector graphics, wireframing, and interactive prototypes with real-time collaboration. It offers a Figma alternative with no vendor lock-in and self-hosting options.",
  "use_cases": [
   "Collaborative UI/UX design and prototyping",
   "Design system documentation and governance",
   "Wireframing and user journey mapping",
   "Self-hosted design infrastructure for enterprises"
  ],
  "used_by": [
   "Mozilla",
   "Telefonica",
   "Liferay",
   "Several EU government agencies"
  ],
  "docs": "https://penpot.app/docs",
  "tutorial": "https://penpot.app/view/getting-started",
  "added": "2026-06-24",
  "dex": 414
 },
 {
  "id": "playwright",
  "name": "Playwright",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like a robot that can click buttons and fill out forms on websites automatically, so you can test if your website works correctly.",
  "description": "Playwright is a cross-browser automation framework that enables testing and automation of web applications across Chromium, Firefox, and WebKit. It's widely used for end-to-end testing, web scraping, and performance testing in modern development pipelines.",
  "use_cases": [
   "End-to-end testing of web applications",
   "Cross-browser compatibility testing",
   "Web scraping and data extraction",
   "Automated visual regression testing"
  ],
  "used_by": [
   "Microsoft",
   "Google",
   "Meta",
   "Stripe"
  ],
  "docs": "https://playwright.dev/",
  "tutorial": "https://playwright.dev/docs/intro",
  "added": "2026-07-07",
  "dex": 415
 },
 {
  "id": "react",
  "name": "React",
  "category": "Framework",
  "domain": "Frontend",
  "description": "JavaScript library for building user interfaces via a declarative, component-based model. Introduced the virtual DOM and unidirectional data flow. Maintained by Meta. The most widely adopted frontend library — vast ecosystem of tooling and components.",
  "use_cases": [
   "Building interactive SPAs with component-driven UI architecture",
   "Server components (React 19) for streaming HTML with zero client JS",
   "State management with hooks (useState, useReducer) and Context API",
   "Universal rendering via Next.js (SSR/SSG) or Remix",
   "Mobile apps via React Native sharing business logic with web"
  ],
  "used_by": [
   "Meta",
   "Airbnb",
   "Netflix",
   "Atlassian",
   "Twitter",
   "Uber"
  ],
  "docs": "https://react.dev/reference/react",
  "tutorial": "https://react.dev/learn",
  "added": "2026-06-08",
  "eli5": "Lego blocks for websites — build pieces and snap them together into a full page.",
  "dex": 416
 },
 {
  "id": "remix",
  "name": "Remix",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Remix is like Next.js but designed around web fundamentals — it handles loading and submitting data the way browsers always did, making apps fast and resilient.",
  "description": "Full-stack React framework built on web standards (fetch, forms, HTTP). Nested routing, parallel data loading, and progressive enhancement for fast, resilient user experiences.",
  "use_cases": [
   "Full-stack React apps with server-side data loading",
   "Progressive enhancement for form-heavy apps",
   "Nested layout routing with parallel data fetching",
   "Deploying to edge runtimes and traditional Node"
  ],
  "used_by": [
   "Shopify",
   "NASA",
   "Adobe",
   "Chance the Rapper"
  ],
  "docs": "https://remix.run/docs",
  "tutorial": "https://remix.run/docs/en/main/start/tutorial",
  "added": "2026-06-14",
  "dex": 417
 },
 {
  "id": "retool",
  "name": "Retool",
  "category": "Platform",
  "domain": "Frontend",
  "eli5": "It's like a LEGO builder for dashboards and admin panels where you snap together pre-made blocks to create apps without writing code.",
  "description": "Retool is a low-code platform for building internal tools, dashboards, and applications by visually connecting UI components to databases and APIs. It significantly reduces development time for business applications that previously required weeks of custom coding.",
  "use_cases": [
   "Building internal admin panels and CRUD applications",
   "Creating data dashboards for operations teams",
   "Automating business processes with form-based workflows",
   "Building customer-facing applications with fast iteration"
  ],
  "used_by": [
   "Amazon",
   "Slack",
   "Microsoft",
   "Vimeo"
  ],
  "docs": "https://docs.retool.com/",
  "tutorial": "https://docs.retool.com/docs/tutorial",
  "added": "2026-09-16",
  "dex": 418
 },
 {
  "id": "shadcn-ui",
  "name": "shadcn/ui",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "shadcn/ui is like a box of beautiful, pre-built UI components that you copy into your own project and own completely — no library to update or fight with.",
  "description": "Collection of accessible, composable React components built on Radix UI and Tailwind CSS. Components are copied into your codebase rather than installed as a dependency.",
  "use_cases": [
   "Accessible component primitives for design systems",
   "Admin dashboards and SaaS UIs",
   "Rapid UI development with full customization",
   "Consistent UI across Next.js and Vite projects"
  ],
  "used_by": [
   "Vercel",
   "Clerk",
   "Cal.com",
   "Liveblocks"
  ],
  "docs": "https://ui.shadcn.com/docs",
  "tutorial": "https://ui.shadcn.com/docs/installation",
  "added": "2026-06-14",
  "dex": 419
 },
 {
  "id": "storybook",
  "name": "Storybook",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Storybook is like a showroom for your UI components — you can see every button, card, and form in isolation without running the whole app.",
  "description": "Frontend workshop for building, documenting, and testing UI components in isolation. Supports React, Vue, Angular, Svelte, and more with addons for accessibility and visual testing.",
  "use_cases": [
   "Component-driven development and documentation",
   "Visual regression testing with Chromatic",
   "Design system documentation and sharing",
   "Isolated component debugging without app context"
  ],
  "used_by": [
   "Shopify",
   "Microsoft",
   "BBC",
   "Airbnb"
  ],
  "docs": "https://storybook.js.org/docs",
  "tutorial": "https://storybook.js.org/tutorials/intro-to-storybook/",
  "added": "2026-06-12",
  "dex": 420
 },
 {
  "id": "svelte",
  "name": "Svelte",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Svelte is like a magic compiler that turns your code into tiny, fast instructions the browser understands directly — no middleman needed.",
  "description": "Compile-time UI framework that shifts work to the build step, producing vanilla JS with no runtime overhead. SvelteKit provides full-stack capabilities.",
  "use_cases": [
   "Highly interactive UIs with minimal bundle size",
   "Full-stack apps with SvelteKit",
   "Embedded widgets with zero runtime dependency",
   "Animations and transitions with built-in directives"
  ],
  "used_by": [
   "Apple",
   "Spotify",
   "The New York Times",
   "Razorpay"
  ],
  "docs": "https://svelte.dev/docs",
  "tutorial": "https://learn.svelte.dev/",
  "added": "2026-06-11",
  "dex": 421
 },
 {
  "id": "tailwind-css",
  "name": "Tailwind CSS",
  "category": "Framework",
  "domain": "Frontend",
  "description": "Utility-first CSS framework — compose designs directly in markup using atomic utility classes instead of writing custom CSS. JIT compiler generates only the CSS classes used, keeping bundle sizes minimal. Pairs with headless UI libraries for accessible components.",
  "use_cases": [
   "Rapid UI development without leaving HTML/JSX — no context switching to CSS files",
   "Design system enforcement via tailwind.config.js tokens (colors, spacing, fonts)",
   "Dark mode, responsive breakpoints, and state variants via class modifiers",
   "Component libraries built on Tailwind: shadcn/ui, Headless UI, Radix + Tailwind",
   "Tailwind UI for pre-built, production-ready component templates"
  ],
  "used_by": [
   "Vercel",
   "GitHub",
   "Shopify",
   "OpenAI",
   "Stripe",
   "NASA"
  ],
  "docs": "https://tailwindcss.com/docs",
  "tutorial": "https://tailwindcss.com/docs/installation",
  "added": "2026-06-08",
  "eli5": "Pre-mixed paint colors for your website — pick from a palette instead of mixing yourself.",
  "dex": 422
 },
 {
  "id": "tailwindui",
  "name": "Tailwind UI",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Pre-made beautiful website building blocks that you can drop into your project, like using LEGO pieces instead of painting your own.",
  "description": "A comprehensive library of professionally designed, fully responsive UI components built with Tailwind CSS. It accelerates frontend development by providing production-ready component patterns with copy-paste code examples.",
  "use_cases": [
   "Building marketing websites and landing pages quickly",
   "Creating consistent UI patterns across SaaS applications",
   "Prototyping dashboard layouts and admin interfaces",
   "Accelerating design-to-code handoff with pre-built components"
  ],
  "used_by": [
   "Vercel",
   "Stripe",
   "GitHub",
   "Figma"
  ],
  "docs": "https://tailwindui.com/documentation",
  "tutorial": "https://tailwindui.com/components",
  "added": "2026-09-19",
  "dex": 423
 },
 {
  "id": "typescript",
  "name": "TypeScript",
  "category": "Language",
  "domain": "Frontend",
  "eli5": "JavaScript but with a spell-checker that catches your mistakes before the code runs.",
  "description": "Strongly-typed superset of JavaScript developed by Microsoft. Adds static types, interfaces, and generics to JS — compiles down to plain JavaScript. Catches bugs at compile time that would silently fail at runtime. Now the default choice for large JS codebases.",
  "use_cases": [
   "Large-scale frontend apps where type safety prevents runtime bugs",
   "Shared type contracts between frontend and backend in monorepos",
   "Node.js backends with full type safety end-to-end",
   "API response typing to catch schema mismatches at compile time",
   "Library authoring with generated .d.ts declaration files for consumers"
  ],
  "used_by": [
   "Microsoft",
   "Slack",
   "Airbnb",
   "Asana",
   "Lyft",
   "Vercel"
  ],
  "docs": "https://www.typescriptlang.org/docs/",
  "tutorial": "https://www.typescriptlang.org/docs/handbook/intro.html",
  "added": "2026-06-08",
  "dex": 424
 },
 {
  "id": "unocss",
  "name": "UnoCSS",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "It's like a smart shortcut maker for decorating web pages — instead of writing long style rules, you use tiny magic words that instantly make things look good.",
  "description": "UnoCSS is an instant on-demand atomic CSS engine that provides a highly customizable and performant utility-first CSS framework. It offers better performance and flexibility compared to traditional approaches like Tailwind CSS through its instant mode and plugin system.",
  "use_cases": [
   "Building responsive web applications with minimal CSS bundle size",
   "Creating design systems with customizable utility tokens",
   "Rapid prototyping with pre-built component utilities",
   "Integrating CSS-in-JS solutions with zero runtime overhead"
  ],
  "used_by": [
   "Nuxt",
   "VueUse",
   "Slidev",
   "Windicss"
  ],
  "docs": "https://unocss.dev/",
  "tutorial": "https://unocss.dev/guide/",
  "added": "2026-06-14",
  "dex": 425
 },
 {
  "id": "valibot",
  "name": "Valibot",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Valibot is like a very careful bouncer at a club who checks that everyone coming in meets the exact requirements before letting them in.",
  "description": "Valibot is a modular and lightweight schema validation library for TypeScript that provides runtime validation with minimal bundle size overhead. It's designed as a faster, more tree-shakeable alternative to Zod for validating data structures in JavaScript applications.",
  "use_cases": [
   "Form validation in web applications",
   "API request and response validation",
   "Configuration file parsing and validation",
   "Type-safe data transformation pipelines"
  ],
  "used_by": [
   "Vercel ecosystem",
   "TypeScript-first startups",
   "Frontend-focused teams"
  ],
  "docs": "https://valibot.dev/",
  "tutorial": "https://valibot.dev/guides/introduction/",
  "added": "2026-07-26",
  "dex": 426
 },
 {
  "id": "vite",
  "name": "Vite",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "Vite is like a chef who preps only the dish you ordered right now instead of cooking the entire menu in advance.",
  "description": "Next-generation frontend build tool that uses native ES modules for near-instant dev server startup and Rollup for optimized production builds.",
  "use_cases": [
   "Development server with sub-millisecond HMR",
   "Building React, Vue, and Svelte apps",
   "Library bundling with ESM/CJS output",
   "Replacing Create React App and webpack"
  ],
  "used_by": [
   "Vue core team",
   "Shopify",
   "Astro",
   "SvelteKit"
  ],
  "docs": "https://vite.dev/guide/",
  "tutorial": "https://vite.dev/guide/",
  "added": "2026-06-11",
  "dex": 427
 },
 {
  "id": "vitest",
  "name": "Vitest",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like a teacher who checks your homework instantly while you're writing it, making sure everything is correct right away.",
  "description": "Vitest is a blazing fast unit test framework powered by Vite, designed for modern JavaScript and TypeScript projects. It offers native ESM support, instant feedback, and compatibility with Jest, making it ideal for testing in the Vite ecosystem.",
  "use_cases": [
   "Unit testing Vue.js and React components",
   "Testing TypeScript applications with instant HMR feedback",
   "Running tests in parallel with minimal configuration",
   "Migrating from Jest while maintaining compatibility"
  ],
  "used_by": [
   "Nuxt",
   "Vue.js",
   "Vite",
   "Element Plus"
  ],
  "docs": "https://vitest.dev/",
  "tutorial": "https://vitest.dev/guide/",
  "added": "2026-07-19",
  "dex": 428
 },
 {
  "id": "vue",
  "name": "Vue.js",
  "category": "Framework",
  "domain": "Frontend",
  "description": "Progressive JavaScript framework for building UIs. Approachable for beginners yet scales to complex SPAs. Composition API (Vue 3) brings React-like composability while retaining an intuitive single-file component (SFC) model with template, script, and style.",
  "use_cases": [
   "SPAs and component-driven UIs with reactive data binding",
   "Progressive enhancement: drop Vue into existing pages without full rewrite",
   "Nuxt.js for SSR, SSG, and full-stack Vue applications",
   "Pinia for lightweight, type-safe state management",
   "Mobile apps via Ionic Vue or NativeScript-Vue"
  ],
  "used_by": [
   "Alibaba",
   "GitLab",
   "Adobe",
   "Grammarly",
   "Nintendo",
   "BMW"
  ],
  "docs": "https://vuejs.org/guide/introduction.html",
  "tutorial": "https://vuejs.org/tutorial/",
  "added": "2026-06-08",
  "eli5": "A friendlier version of React — easier to pick up, still very powerful.",
  "dex": 429
 },
 {
  "id": "wasm-bindgen",
  "name": "wasm-bindgen",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "wasm-bindgen is like a translator that lets you write super-fast code in Rust and use it directly in web browsers like JavaScript.",
  "description": "wasm-bindgen is a Rust library that facilitates high-level interactions between WebAssembly modules and JavaScript. It enables developers to write performance-critical frontend code in Rust and seamlessly integrate it with JavaScript applications.",
  "use_cases": [
   "Building high-performance frontend components in Rust",
   "Creating browser-based graphics and game engines",
   "Implementing cryptographic operations in WebAssembly",
   "Porting existing Rust libraries to run in browsers"
  ],
  "used_by": [
   "Figma",
   "Cloudflare Workers",
   "Mozilla",
   "Discord"
  ],
  "docs": "https://rustwasm.org/docs/wasm-bindgen/",
  "tutorial": "https://rustwasm.org/docs/wasm-bindgen/examples/index.html",
  "added": "2026-06-18",
  "dex": 430
 },
 {
  "id": "wix-velo",
  "name": "Wix Velo",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Wix Velo is like having a magical toolbox that lets you add smart behavior to a website builder without needing to code everything from scratch.",
  "description": "Wix Velo is a full-stack development platform that allows developers to build web applications with JavaScript/TypeScript on the Wix platform, providing backend APIs, databases, and frontend components. It enables rapid application development with built-in hosting, authentication, and CMS capabilities.",
  "use_cases": [
   "Building custom business applications and dashboards on the Wix platform",
   "Creating e-commerce sites with advanced personalization and custom workflows",
   "Developing appointment booking and scheduling systems with real-time updates",
   "Building CRM and data management applications with built-in user authentication"
  ],
  "used_by": [
   "Wix customers",
   "Small business owners",
   "Agencies",
   "Freelancers"
  ],
  "docs": "https://www.wix.com/velo/reference/",
  "tutorial": "https://www.wix.com/velo/learn",
  "added": "2026-09-24",
  "dex": 431
 },
 {
  "id": "wrangler",
  "name": "Wrangler",
  "category": "Tool",
  "domain": "Frontend",
  "eli5": "It's like a helper tool that lets you write and run code on Cloudflare's global servers instead of just your own computer, making your apps super fast everywhere.",
  "description": "Wrangler is a CLI tool for building, testing, and deploying Cloudflare Workers applications. It enables developers to write serverless functions that run on Cloudflare's edge network globally with minimal latency.",
  "use_cases": [
   "Building edge functions and APIs that run globally",
   "Creating serverless middleware for content transformation",
   "Deploying applications closer to users for reduced latency",
   "Testing and debugging Workers locally before deployment"
  ],
  "used_by": [
   "Cloudflare",
   "Discord",
   "Notion",
   "SoundCloud"
  ],
  "docs": "https://developers.cloudflare.com/workers/wrangler/",
  "tutorial": "https://developers.cloudflare.com/workers/get-started/guide/",
  "added": "2026-07-02",
  "dex": 432
 },
 {
  "id": "zod",
  "name": "Zod",
  "category": "Framework",
  "domain": "Frontend",
  "eli5": "Zod is like a bouncer at a club who checks that everyone's ID is real before they can enter, making sure your data is exactly what you expect.",
  "description": "Zod is a TypeScript-first schema validation library that provides runtime type checking and data validation with an intuitive API. It's essential for ensuring data integrity in JavaScript/TypeScript applications without external dependencies.",
  "use_cases": [
   "Validating API request and response payloads",
   "Form validation in web applications",
   "TypeScript type inference from runtime schemas",
   "Building type-safe API clients and servers"
  ],
  "used_by": [
   "Vercel",
   "Shopify",
   "tRPC",
   "Prisma"
  ],
  "docs": "https://zod.dev/",
  "tutorial": "https://zod.dev/?id=basic-usage",
  "added": "2026-06-15",
  "dex": 433
 },
 {
  "id": "axiom-serverless",
  "name": "Axiom Serverless Queryable State",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a notebook that serverless functions can write notes to and read from each other, so they can remember things between runs without needing a database.",
  "description": "Axiom provides queryable serverless state storage that allows functions to persist and query data without managing infrastructure or external databases. It's designed for event-driven workloads and offers real-time data ingestion with SQL query capabilities.",
  "use_cases": [
   "Tracking application state in serverless/FaaS architectures",
   "Real-time event aggregation and analytics",
   "Building stateful workflows on serverless platforms",
   "Debugging and observability for distributed functions"
  ],
  "used_by": [
   "Vercel partners",
   "Serverless startups",
   "Event-driven platforms"
  ],
  "docs": "https://axiom.co/docs",
  "tutorial": "https://axiom.co/docs/getting-started",
  "added": "2026-07-25",
  "dex": 434
 },
 {
  "id": "axon-framework",
  "name": "Axon Framework",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a detailed diary that records every decision you make, so you can always replay what happened and fix mistakes.",
  "description": "Axon Framework is a Java framework for building event-driven microservices using Command Query Responsibility Segregation (CQRS) and event sourcing patterns. It simplifies distributed system development by treating events as the source of truth.",
  "use_cases": [
   "Financial transaction systems requiring audit trails",
   "Order management and e-commerce platforms",
   "Event-driven microservices architectures",
   "Systems requiring temporal data reconstruction"
  ],
  "used_by": [
   "ABN AMRO Bank",
   "ING",
   "PUMA",
   "Commerzbank"
  ],
  "docs": "https://docs.axoniq.io/reference-guide/",
  "tutorial": "https://docs.axoniq.io/getting-started/",
  "added": "2026-09-12",
  "dex": 435
 },
 {
  "id": "axum",
  "name": "Axum",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Axum is like a Lego set for building super-fast web applications in Rust, where each block snaps together easily.",
  "description": "Axum is a modular and composable web framework built on top of Tokio that prioritizes ergonomics and modularity for building high-performance async web services in Rust. It's developed and maintained by the Tokio team and has become the de facto standard for Rust web APIs.",
  "use_cases": [
   "Building high-performance REST APIs and microservices",
   "Real-time WebSocket applications with low latency requirements",
   "Cloud-native applications requiring minimal resource overhead",
   "Async-first backend services with complex middleware requirements"
  ],
  "used_by": [
   "Fly.io",
   "Figma",
   "Discord",
   "1Password"
  ],
  "docs": "https://docs.rs/axum/",
  "tutorial": "https://github.com/tokio-rs/axum/tree/main/examples",
  "added": "2026-08-03",
  "dex": 436
 },
 {
  "id": "budibase",
  "name": "Budibase",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like a magic LEGO set where you snap together blocks to build a working app without needing to write any code.",
  "description": "An open-source low-code platform for rapidly building internal tools, dashboards, and CRUD applications with a visual builder and integrated database connectivity. It reduces development time for business applications by up to 10x compared to traditional coding.",
  "use_cases": [
   "Building internal admin panels and dashboards",
   "Creating CRUD applications without coding",
   "Database management interfaces",
   "Rapid prototyping of business applications"
  ],
  "used_by": [
   "Netflix",
   "Deloitte",
   "Accenture",
   "Bupa"
  ],
  "docs": "https://docs.budibase.com/",
  "tutorial": "https://docs.budibase.com/getting-started",
  "added": "2026-08-06",
  "dex": 437
 },
 {
  "id": "buf",
  "name": "Buf",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's a quality checker and organizer for the language that computers use to talk to each other over the internet.",
  "description": "Buf is a CLI tool for working with Protocol Buffers (protobuf) that provides linting, code generation, and breaking change detection. It simplifies protobuf workflows with modern tooling and manages dependency versioning for proto files.",
  "use_cases": [
   "Detecting breaking changes in gRPC service definitions before deployment",
   "Enforcing API design standards across proto files",
   "Managing protobuf dependencies and versions in large monorepos",
   "Generating idiomatic code in multiple languages from proto definitions"
  ],
  "used_by": [
   "Google",
   "Uber",
   "Lyft",
   "Netflix"
  ],
  "docs": "https://buf.build/docs",
  "tutorial": "https://buf.build/docs/tour/getting-started",
  "added": "2026-06-14",
  "dex": 438
 },
 {
  "id": "buf-connect",
  "name": "Buf Connect",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like having a universal translator that lets different programs talk to each other easily, no matter what language they use.",
  "description": "Buf Connect is a library for building better services with Protocol Buffers and gRPC, providing simplified APIs for request/response patterns across multiple languages. It reduces boilerplate code and makes it easier to build connected microservices.",
  "use_cases": [
   "Building gRPC services with less ceremony and configuration",
   "Creating language-agnostic microservice frameworks",
   "Implementing request-response patterns with schema validation",
   "Modernizing REST APIs with performance benefits of gRPC"
  ],
  "used_by": [
   "Stripe",
   "Square",
   "Google"
  ],
  "docs": "https://buf.build/docs/connect/overview",
  "tutorial": "https://buf.build/docs/tutorials/getting-started-with-connect",
  "added": "2026-07-27",
  "dex": 439
 },
 {
  "id": "builtins",
  "name": "Builtins",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a toolbox that comes with your programming language so you don't have to buy tools separately.",
  "description": "Builtins are the core functions and objects that come pre-loaded in programming languages like Python, JavaScript, and Go. They provide essential functionality without requiring external dependencies.",
  "use_cases": [
   "Data type operations and conversions",
   "String and collection manipulation",
   "File I/O and system interactions",
   "Math and type checking operations"
  ],
  "used_by": [
   "Google",
   "Facebook",
   "Spotify",
   "Netflix"
  ],
  "docs": "https://docs.python.org/3/library/functions.html",
  "tutorial": "https://www.w3schools.com/python/python_ref_functions.asp",
  "added": "2026-07-01",
  "dex": 440
 },
 {
  "id": "bun-javascript-runtime",
  "name": "Bun JavaScript Runtime",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Bun is like a super-fast engine for running JavaScript code that starts up quicker and runs much speedier than older engines, like upgrading from a bicycle to a sports car.",
  "description": "Bun is a fast, all-in-one JavaScript runtime and toolkit built in Zig that serves as a drop-in replacement for Node.js. It includes a bundler, transpiler, package manager, and test runner, significantly improving development experience and application performance.",
  "use_cases": [
   "Building fast backend servers and APIs with significantly lower latency than Node.js",
   "Running existing Node.js applications and npm packages with improved performance",
   "Creating CLI tools and scripts with rapid startup times and minimal resource usage",
   "Development workflow acceleration with built-in bundling, transpilation, and testing capabilities"
  ],
  "used_by": [
   "Figma",
   "Stripe",
   "Vercel",
   "Discord"
  ],
  "docs": "https://bun.sh/docs",
  "tutorial": "https://bun.sh/docs/quickstart",
  "added": "2026-09-24",
  "dex": 441
 },
 {
  "id": "cadence",
  "name": "Cadence",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Cadence is like a smart planner that remembers what your app was doing even if the computer crashes, so it can pick up where it left off.",
  "description": "Cadence is a distributed workflow orchestration platform that enables reliable execution of asynchronous, long-running business logic. It provides fault-tolerant workflow execution with automatic retries, timeouts, and state persistence across service failures.",
  "use_cases": [
   "Orchestrating complex payment processing workflows with retry logic",
   "Managing multi-step onboarding processes that span multiple services",
   "Coordinating batch jobs and data pipelines with automatic recovery",
   "Building reliable order fulfillment systems across microservices"
  ],
  "used_by": [
   "Uber",
   "Box",
   "DoorDash",
   "Stripe"
  ],
  "docs": "https://cadenceworkflow.io/docs/",
  "tutorial": "https://cadenceworkflow.io/docs/01-starting/",
  "added": "2026-07-29",
  "dex": 442
 },
 {
  "id": "cadence-workflow",
  "name": "Cadence Workflow Engine",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Cadence is like a reliable assistant that remembers exactly where you left off in a task, even if the office catches fire and restarts.",
  "description": "Cadence is a distributed workflow orchestration engine created by Uber that ensures long-running, asynchronous business processes execute reliably with automatic retries and state recovery. It powers mission-critical workflows in finance, e-commerce, and infrastructure automation.",
  "use_cases": [
   "Orchestrating multi-step financial transaction processes",
   "Managing complex order processing and fulfillment workflows",
   "Coordinating microservice interactions with guaranteed delivery",
   "Implementing saga patterns for distributed transactions"
  ],
  "used_by": [
   "Uber",
   "Box",
   "Square",
   "DoorDash"
  ],
  "docs": "https://cadenceworkflow.io/docs/",
  "tutorial": "https://cadenceworkflow.io/docs/quick-start/",
  "added": "2026-06-29",
  "dex": 443
 },
 {
  "id": "cloudflare-workers",
  "name": "Cloudflare Workers",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Tiny computers spread all over the world that instantly process your website requests without needing a big server.",
  "description": "Cloudflare Workers is a serverless computing platform that runs JavaScript/TypeScript code on Cloudflare's global edge network with sub-50ms latency worldwide. It enables developers to build full-stack applications without managing infrastructure.",
  "use_cases": [
   "API gateways and middleware",
   "Real-time data processing at the edge",
   "Personalized content delivery",
   "Request routing and authentication"
  ],
  "used_by": [
   "Discord",
   "Shopify",
   "Figma",
   "Patreon"
  ],
  "docs": "https://developers.cloudflare.com/workers/",
  "tutorial": "https://developers.cloudflare.com/workers/get-started/guide/",
  "added": "2026-07-16",
  "dex": 444
 },
 {
  "id": "dapr",
  "name": "Dapr (Distributed Application Runtime)",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "A universal translator that helps different parts of your application talk to each other without needing to learn each other's language, like a translator at the United Nations.",
  "description": "Dapr is an open-source runtime that provides building blocks for distributed applications, including state management, pub/sub messaging, service invocation, and actor patterns. It abstracts away the complexity of microservices infrastructure and works with any language or framework, reducing boilerplate code significantly.",
  "use_cases": [
   "Microservices communication without tight coupling to specific messaging systems",
   "Distributed state management across multiple services and languages",
   "Building serverless and event-driven applications with consistent APIs",
   "Multi-cloud and hybrid deployments with cloud-agnostic abstractions"
  ],
  "used_by": [
   "Microsoft",
   "Apache Foundation",
   "Alibaba",
   "BMW"
  ],
  "docs": "https://docs.dapr.io/",
  "tutorial": "https://docs.dapr.io/getting-started/",
  "added": "2026-08-13",
  "dex": 445
 },
 {
  "id": "deno",
  "name": "Deno",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "Deno is like Node.js fixed — it runs JavaScript and TypeScript securely by default, with built-in tools so you don't need npm for most things.",
  "description": "Secure JavaScript/TypeScript runtime built on V8 and Rust. Permissions-first security model, native TypeScript support, built-in formatter/linter/test runner, and Web API compatibility.",
  "use_cases": [
   "Secure scripting with explicit permission grants",
   "TypeScript backend servers without build steps",
   "Edge functions on Deno Deploy",
   "CLI tools with single-binary compilation"
  ],
  "used_by": [
   "Deno Deploy",
   "Netlify Edge Functions",
   "Supabase Edge Functions"
  ],
  "docs": "https://docs.deno.com/",
  "tutorial": "https://docs.deno.com/runtime/",
  "added": "2026-06-14",
  "dex": 446
 },
 {
  "id": "django",
  "name": "Django",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Django is like a fully-stocked kitchen for building websites in Python — everything from database to login pages to admin panels comes included.",
  "description": "High-level Python web framework that follows the 'batteries included' philosophy. Includes ORM, auth, admin, and templating out of the box.",
  "use_cases": [
   "Full-stack web applications with server-side rendering",
   "REST APIs (with Django REST Framework)",
   "Admin dashboards and CMS backends",
   "Rapid prototyping for data-heavy apps"
  ],
  "used_by": [
   "Instagram",
   "Mozilla",
   "Disqus",
   "Pinterest"
  ],
  "docs": "https://docs.djangoproject.com/",
  "tutorial": "https://docs.djangoproject.com/en/stable/intro/tutorial01/",
  "added": "2026-06-12",
  "dex": 447
 },
 {
  "id": "encore-backend-framework",
  "name": "Encore Backend Framework",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a magic blueprint that automatically builds the invisible infrastructure your app needs, so you only have to write the important business logic.",
  "description": "Encore is an open-source backend framework that automatically provisions cloud infrastructure and handles DevOps concerns like API gateways, databases, and authentication. Developers define their backend using Go with minimal configuration while Encore handles deployment, scaling, and observability.",
  "use_cases": [
   "Rapid API backend development without infrastructure setup",
   "Automatic OpenAPI and GraphQL schema generation",
   "Built-in tracing and monitoring for all services",
   "Multi-cloud deployment with automatic infrastructure provisioning"
  ],
  "used_by": [
   "Stripe",
   "Vercel ecosystem partners",
   "Y Combinator startups"
  ],
  "docs": "https://encore.dev/docs",
  "tutorial": "https://encore.dev/docs/tutorial",
  "added": "2026-09-20",
  "dex": 448
 },
 {
  "id": "express",
  "name": "Express.js",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Express is like a thin layer of helpful tools on top of Node.js — it makes building web servers and APIs way less tedious without getting in your way.",
  "description": "Minimal, unopinionated Node.js web framework. The de-facto standard for building HTTP servers and REST APIs in JavaScript/TypeScript with a huge middleware ecosystem.",
  "use_cases": [
   "REST API servers",
   "Middleware pipelines for auth, logging, rate limiting",
   "Server-side rendering backends",
   "BFF (backend for frontend) layers"
  ],
  "used_by": [
   "IBM",
   "Accenture",
   "Fox Sports",
   "Uber"
  ],
  "docs": "https://expressjs.com/",
  "tutorial": "https://expressjs.com/en/starter/hello-world.html",
  "added": "2026-06-12",
  "dex": 449
 },
 {
  "id": "fastapi",
  "name": "FastAPI",
  "category": "Framework",
  "domain": "Backend",
  "description": "Modern, high-performance Python web framework for building APIs. Built on Starlette and Pydantic — async by default, auto-generates OpenAPI/Swagger docs, and validates request/response schemas via type hints. One of the fastest Python frameworks.",
  "use_cases": [
   "REST and async API backends with automatic OpenAPI docs generation",
   "ML model serving: wrap a model inference function as an HTTP endpoint",
   "Data validation and serialization with Pydantic models",
   "Background tasks and WebSocket support for real-time APIs",
   "Dependency injection system for auth, DB sessions, and shared resources"
  ],
  "used_by": [
   "Microsoft",
   "Uber",
   "Netflix",
   "Explosion AI",
   "Hugging Face"
  ],
  "docs": "https://fastapi.tiangolo.com/",
  "tutorial": "https://fastapi.tiangolo.com/tutorial/",
  "added": "2026-06-08",
  "eli5": "A super-fast waiter that takes requests from users and brings back exactly what they asked.",
  "dex": 450
 },
 {
  "id": "gin",
  "name": "Gin",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Gin is like a Formula 1 car for building web APIs in Go — extremely fast, very light, and gets out of your way.",
  "description": "High-performance HTTP web framework for Go. Features a martini-like API with up to 40x better performance, middleware support, and JSON validation.",
  "use_cases": [
   "High-throughput REST APIs in Go",
   "Microservices with minimal memory footprint",
   "Middleware chains for auth, logging, rate limiting",
   "JSON API servers with request binding and validation"
  ],
  "used_by": [
   "ByteDance",
   "Docker",
   "Kubernetes ecosystem tools"
  ],
  "docs": "https://gin-gonic.com/docs/",
  "tutorial": "https://gin-gonic.com/docs/quickstart/",
  "added": "2026-06-12",
  "dex": 451
 },
 {
  "id": "gleam-programming-language",
  "name": "Gleam",
  "category": "Language",
  "domain": "Backend",
  "eli5": "Gleam is a programming language that catches your mistakes before you run your code, like a spellchecker that stops you from sending a message with typos.",
  "description": "Gleam is a statically-typed, garbage-collected language that compiles to both JavaScript and Erlang/OTP, combining the safety of strong typing with the performance and reliability of the BEAM VM. It's designed for building scalable, fault-tolerant backend systems with minimal runtime errors.",
  "use_cases": [
   "Building concurrent backend services with strong guarantees",
   "Full-stack applications using JavaScript target for frontend",
   "Telecom and real-time systems leveraging BEAM runtime",
   "Type-safe distributed systems and microservices"
  ],
  "used_by": [
   "Mojeek search engine",
   "Fly.io infrastructure",
   "PagerDuty infrastructure teams",
   "Open-source projects"
  ],
  "docs": "https://gleam.run/book",
  "tutorial": "https://gleam.run/start",
  "added": "2026-09-23",
  "dex": 452
 },
 {
  "id": "go",
  "name": "Go",
  "category": "Language",
  "domain": "Backend",
  "eli5": "A language designed so a big team can write the same code without making a mess.",
  "description": "Statically typed, compiled language designed at Google for simplicity, concurrency, and fast build times. Goroutines and channels make concurrent programming straightforward. Single binary output, minimal runtime — ideal for cloud-native tools and services.",
  "use_cases": [
   "High-throughput microservices and APIs with built-in concurrency via goroutines",
   "CLI tools and DevOps tooling (Docker, Kubernetes, Terraform are all written in Go)",
   "Network servers and proxies with low memory footprint",
   "Cloud-native infrastructure: most CNCF projects are written in Go",
   "gRPC services with generated Go stubs from protobuf definitions"
  ],
  "used_by": [
   "Google",
   "Uber",
   "Dropbox",
   "Cloudflare",
   "Docker",
   "HashiCorp"
  ],
  "docs": "https://go.dev/doc/",
  "tutorial": "https://go.dev/tour/welcome/1",
  "added": "2026-06-08",
  "dex": 453
 },
 {
  "id": "gotenberg",
  "name": "Gotenberg",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a printing wizard that magically turns web pages into PDF files or images without needing a real printer.",
  "description": "Gotenberg is a stateless API for converting HTML, Markdown, and Office documents to PDF and images using headless browsers. It simplifies document generation for applications without managing browser instances or complex dependencies.",
  "use_cases": [
   "Invoice and receipt PDF generation",
   "Report generation from dynamic content",
   "HTML to PDF conversion at scale",
   "Screenshot generation from web content"
  ],
  "used_by": [
   "Invoice platforms",
   "SaaS document generators",
   "Report automation systems",
   "Healthcare document platforms"
  ],
  "docs": "https://gotenberg.dev/docs/about",
  "tutorial": "https://gotenberg.dev/docs/getting-started/installation",
  "added": "2026-09-07",
  "dex": 454
 },
 {
  "id": "graphql",
  "name": "GraphQL",
  "category": "Concept",
  "domain": "Backend",
  "eli5": "GraphQL is like ordering exactly what you want from a menu instead of getting a fixed combo — you ask for the specific fields you need and nothing more.",
  "description": "Query language and runtime for APIs that lets clients request exactly the data they need. Developed by Facebook, replaces multiple REST endpoints with a single typed schema.",
  "use_cases": [
   "Flexible APIs for mobile and web clients",
   "Aggregating data from multiple microservices (schema federation)",
   "Reducing over-fetching and under-fetching",
   "Real-time data with GraphQL subscriptions"
  ],
  "used_by": [
   "GitHub",
   "Shopify",
   "Twitter",
   "Airbnb"
  ],
  "docs": "https://graphql.org/learn/",
  "tutorial": "https://graphql.org/learn/",
  "added": "2026-06-12",
  "dex": 455
 },
 {
  "id": "grpc",
  "name": "gRPC",
  "category": "Framework",
  "domain": "Backend",
  "description": "High-performance, open-source RPC framework from Google. Uses HTTP/2 for transport and Protocol Buffers (protobuf) as the IDL and wire format. Enables strongly-typed, code-generated service contracts across polyglot microservices.",
  "use_cases": [
   "Low-latency, high-throughput inter-service communication (vs REST+JSON)",
   "Bidirectional streaming for real-time data (chat, live feeds, telemetry)",
   "Strongly-typed API contracts via .proto files — catch mismatches at compile time",
   "Polyglot microservices: generate client/server stubs in 11+ languages",
   "gRPC-Web for browser clients; gRPC-Gateway to expose REST alongside gRPC"
  ],
  "used_by": [
   "Google",
   "Netflix",
   "Square",
   "CoreOS",
   "Cockroach Labs",
   "Cisco"
  ],
  "docs": "https://grpc.io/docs/",
  "tutorial": "https://grpc.io/docs/languages/go/quickstart/",
  "added": "2026-06-08",
  "eli5": "Two apps talking in a secret, super-fast language only they understand.",
  "dex": 456
 },
 {
  "id": "grpcurl",
  "name": "grpcurl",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "grpcurl is like a telephone for talking to backend services using a special modern language they understand, instead of the old-fashioned HTTP way.",
  "description": "grpcurl is a command-line tool for interacting with gRPC services, similar to curl for HTTP APIs. It allows developers to call gRPC methods, inspect service definitions, and debug gRPC servers without writing client code.",
  "use_cases": [
   "Testing and debugging gRPC service endpoints",
   "Inspecting gRPC service definitions and capabilities",
   "Scripting gRPC calls in CI/CD pipelines",
   "Troubleshooting gRPC communication issues"
  ],
  "used_by": [
   "Google Cloud",
   "Netflix",
   "Uber",
   "Square"
  ],
  "docs": "https://github.com/fullstorydev/grpcurl",
  "tutorial": "https://github.com/fullstorydev/grpcurl/blob/master/README.md",
  "added": "2026-09-02",
  "dex": 457
 },
 {
  "id": "hono",
  "name": "Hono",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Hono is like an ultra-light Express for the edge — it runs anywhere JavaScript runs, including Cloudflare Workers, Deno, and Bun, with almost no overhead.",
  "description": "Ultra-fast, lightweight web framework for edge runtimes. Works on Cloudflare Workers, Deno, Bun, and Node.js with a familiar Express-like API and zero dependencies.",
  "use_cases": [
   "Edge API routes on Cloudflare Workers",
   "Lightweight REST APIs with Bun or Deno",
   "TypeScript-first backend with end-to-end types",
   "Serverless functions with minimal cold start"
  ],
  "used_by": [
   "Cloudflare",
   "Deno",
   "various edge-native startups"
  ],
  "docs": "https://hono.dev/docs/",
  "tutorial": "https://hono.dev/docs/getting-started/basic",
  "added": "2026-06-14",
  "dex": 458
 },
 {
  "id": "httpie",
  "name": "HTTPie",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a friendly way to talk to web servers, easier than using curl because it shows you colorful readable messages.",
  "description": "HTTPie is a command-line HTTP client that makes testing APIs and debugging HTTP requests simple and intuitive. It's designed as a user-friendly alternative to curl with better defaults, colorized output, and a more readable syntax.",
  "use_cases": [
   "Testing RESTful APIs during development and debugging",
   "Making ad-hoc HTTP requests without complex curl syntax",
   "Automating API testing in scripts and CI/CD pipelines",
   "Learning HTTP concepts through interactive request/response inspection"
  ],
  "used_by": [
   "Stripe",
   "GitHub",
   "Netflix",
   "Mozilla"
  ],
  "docs": "https://httpie.io/docs",
  "tutorial": "https://httpie.io/cli",
  "added": "2026-07-28",
  "dex": 459
 },
 {
  "id": "hurl",
  "name": "Hurl",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "Hurl is like a testing checklist for websites that lets you verify your web services are working correctly by running through a series of requests.",
  "description": "Hurl is a command-line tool for running and testing HTTP requests defined in plain text files, supporting assertions, captures, and dynamic value injection. It streamlines API testing, integration testing, and debugging of web services.",
  "use_cases": [
   "API endpoint testing and validation",
   "Integration testing in CI/CD pipelines",
   "Load testing and performance benchmarking",
   "API documentation and examples"
  ],
  "used_by": [
   "Mozilla",
   "Datadog",
   "GitHub",
   "GitLab"
  ],
  "docs": "https://hurl.dev/docs/",
  "tutorial": "https://hurl.dev/docs/tutorial/",
  "added": "2026-06-20",
  "dex": 460
 },
 {
  "id": "hydra-config-framework",
  "name": "Hydra Configuration Framework",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a recipe card system that lets you organize and switch between different configurations for your programs without rewriting anything.",
  "description": "Hydra is a Python framework for configuring complex applications through composable configuration files and command-line overrides. It simplifies managing hyperparameters, environment-specific settings, and application lifecycle in data science and ML workflows.",
  "use_cases": [
   "Managing machine learning experiment configurations and hyperparameters",
   "Multi-environment application deployment with environment-specific configs",
   "Complex data pipeline orchestration with composable config structures",
   "Parameter optimization and grid search automation"
  ],
  "used_by": [
   "Meta",
   "research institutions",
   "ML engineering teams"
  ],
  "docs": "https://hydra.cc/",
  "tutorial": "https://hydra.cc/docs/tutorials/basic/your_first_hydra_app/",
  "added": "2026-09-17",
  "dex": 461
 },
 {
  "id": "java",
  "name": "Java",
  "category": "Language",
  "domain": "Backend",
  "eli5": "A very reliable, strict language that runs the same everywhere — like a rule-follower who never breaks character.",
  "description": "Statically typed, object-oriented language running on the JVM. Write once, run anywhere. Massive enterprise ecosystem (Spring, Jakarta EE). Verbose but predictable — dominates banking, telecoms, and large enterprise backends. Long-term support releases make it stable for decades.",
  "use_cases": [
   "Enterprise backend services and REST APIs with Spring Boot",
   "Android app development (alongside Kotlin)",
   "Big data processing: Hadoop, Spark, Kafka are all JVM-based",
   "Microservices with Quarkus or Micronaut for low-footprint JVM services",
   "High-frequency trading and financial systems needing JVM tuning"
  ],
  "used_by": [
   "LinkedIn",
   "Amazon",
   "Google",
   "Netflix",
   "Uber",
   "Goldman Sachs"
  ],
  "docs": "https://docs.oracle.com/en/java/",
  "tutorial": "https://dev.java/learn/",
  "added": "2026-06-08",
  "dex": 462
 },
 {
  "id": "kotlin",
  "name": "Kotlin",
  "category": "Language",
  "domain": "Backend",
  "eli5": "Java but way less wordy — like texting instead of writing a formal letter, but just as powerful.",
  "description": "Statically typed JVM language by JetBrains — fully interoperable with Java but far more concise, null-safe, and expressive. Official language for Android development. Coroutines make async code as readable as synchronous code.",
  "use_cases": [
   "Android app development — Google's preferred language since 2019",
   "Backend services with Ktor or Spring Boot (full Java interop)",
   "Kotlin Multiplatform for sharing business logic across Android, iOS, and web",
   "Coroutines for structured concurrency without callback hell",
   "Replacing Java in existing JVM codebases incrementally"
  ],
  "used_by": [
   "Google",
   "Pinterest",
   "Trello",
   "Evernote",
   "JetBrains",
   "Gradle"
  ],
  "docs": "https://kotlinlang.org/docs/home.html",
  "tutorial": "https://kotlinlang.org/docs/getting-started.html",
  "added": "2026-06-08",
  "dex": 463
 },
 {
  "id": "liteflow",
  "name": "LiteFlow",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "LiteFlow is like a recipe that tells your computer exactly what steps to follow in order, making sure complex tasks happen one after another correctly.",
  "description": "LiteFlow is a Java-based workflow orchestration framework that simplifies building complex business logic through declarative configuration. It's designed for rule engines and workflow automation with minimal boilerplate code.",
  "use_cases": [
   "Building business rule engines for financial systems",
   "Orchestrating multi-step approval workflows",
   "Implementing complex conditional logic in microservices",
   "Creating reusable workflow patterns for enterprise applications"
  ],
  "used_by": [
   "Alibaba",
   "Baidu",
   "NetEase",
   "ByteDance"
  ],
  "docs": "https://liteflow.yomahub.com/",
  "tutorial": "https://liteflow.yomahub.com/pages/detail?id=4e81cfc3b3dd44f19f6e27fe2f7c5c37",
  "added": "2026-08-01",
  "dex": 464
 },
 {
  "id": "litestar",
  "name": "Litestar",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a super-fast recipe for building web applications that writes boring parts for you while staying out of your way.",
  "description": "Litestar (formerly Starlite) is a modern, fast ASGI framework for building scalable APIs and web applications in Python with exceptional developer experience. It provides automatic OpenAPI documentation, dependency injection, validation, and serialization out of the box.",
  "use_cases": [
   "Building high-performance REST and GraphQL APIs",
   "Microservices architecture with automatic documentation",
   "Real-time applications with WebSocket support",
   "Data validation and serialization with Pydantic integration"
  ],
  "used_by": [
   "TikTok",
   "Alibaba",
   "Bytes",
   "Various fintech startups"
  ],
  "docs": "https://docs.litestar.dev/",
  "tutorial": "https://docs.litestar.dev/latest/topics/getting-started.html",
  "added": "2026-07-19",
  "dex": 465
 },
 {
  "id": "mcp-server-stdlib",
  "name": "MCP Server Stdlib",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a universal adapter that lets AI assistants safely use your computer's built-in tools like reading files, running code, and sending emails.",
  "description": "MCP Server Stdlib provides official Model Context Protocol server implementations for common system operations, enabling safe and standardized integration between AI models and local system resources. It's part of the broader MCP ecosystem that provides interoperability standards for AI tool use.",
  "use_cases": [
   "Enabling Claude to safely read and write local files",
   "Running shell commands through AI assistants with sandboxing",
   "Creating standardized integrations between AI tools and applications",
   "Building autonomous agents with controlled system access"
  ],
  "used_by": [
   "Anthropic",
   "Figma",
   "Slack",
   "Replit"
  ],
  "docs": "https://modelcontextprotocol.io/",
  "tutorial": "https://github.com/modelcontextprotocol/servers/tree/main/src/stdlib",
  "added": "2026-08-14",
  "dex": 466
 },
 {
  "id": "medplum",
  "name": "Medplum",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like a special filing cabinet built just for doctors and hospitals to store and share patient information safely and legally.",
  "description": "Medplum is an open-source platform for building healthcare applications with built-in compliance for HIPAA, HL7 FHIR standards, and healthcare workflows. It provides APIs, hosting, and tools specifically designed for healthcare software development.",
  "use_cases": [
   "Building HIPAA-compliant patient management systems",
   "Implementing electronic health record (EHR) integrations",
   "Creating telemedicine platforms with secure messaging",
   "Developing healthcare interoperability solutions using FHIR standards"
  ],
  "used_by": [
   "Ro",
   "Devoted Health",
   "Amazon Healthcare Innovation",
   "Google Health"
  ],
  "docs": "https://www.medplum.com/docs",
  "tutorial": "https://www.medplum.com/docs/tutorials",
  "added": "2026-09-01",
  "dex": 467
 },
 {
  "id": "medusa",
  "name": "Medusa",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "A ready-made shopping store engine for the internet, like getting a fully stocked retail shop that you just need to customize and connect to your website.",
  "description": "An open-source headless commerce platform built with Node.js that provides APIs for building scalable e-commerce applications. It decouples the commerce logic from the frontend, enabling flexible storefront implementations across web, mobile, and other channels.",
  "use_cases": [
   "Building custom e-commerce storefronts with modern tech stacks",
   "Creating multi-vendor marketplaces with complex fulfillment logic",
   "Implementing B2B commerce platforms with subscription and bulk ordering",
   "Developing mobile-first shopping experiences with native apps"
  ],
  "used_by": [
   "ComfortDelGro",
   "Tekion",
   "Rask",
   "LanceDB"
  ],
  "docs": "https://docs.medusajs.com",
  "tutorial": "https://docs.medusajs.com/quickstart/quick-start",
  "added": "2026-09-19",
  "dex": 468
 },
 {
  "id": "mux-video",
  "name": "Mux Video API",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like having a video production studio in the cloud that automatically makes your videos play perfectly on any device.",
  "description": "Mux Video is a video infrastructure platform providing transcoding, streaming, and analytics APIs that abstract away video complexity. It handles encoding, delivery, and real-time quality monitoring so developers can focus on building video experiences without managing video servers.",
  "use_cases": [
   "Live and on-demand video streaming with adaptive bitrate",
   "Real-time video quality monitoring and metrics",
   "Automated video transcoding and format conversion",
   "Video player analytics and user experience optimization"
  ],
  "used_by": [
   "Vercel",
   "Figma",
   "Discord"
  ],
  "docs": "https://docs.mux.com/",
  "tutorial": "https://docs.mux.com/guides/video/get-started-with-mux-video",
  "added": "2026-08-20",
  "dex": 469
 },
 {
  "id": "nestjs",
  "name": "NestJS",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "NestJS is like LEGO for building Node.js servers — each piece snaps together in a structured way so your code stays organized as the project grows.",
  "description": "Progressive Node.js framework for building scalable server-side applications. Uses TypeScript and Angular-inspired decorators to enforce modularity and testability.",
  "use_cases": [
   "REST and GraphQL API servers",
   "Microservices with built-in transport adapters (Kafka, NATS, Redis)",
   "Enterprise-grade Node.js backends",
   "CLI tools and background workers"
  ],
  "used_by": [
   "Adidas",
   "Roche",
   "Capgemini",
   "Autodesk"
  ],
  "docs": "https://docs.nestjs.com/",
  "tutorial": "https://docs.nestjs.com/first-steps",
  "added": "2026-06-11",
  "dex": 470
 },
 {
  "id": "pglast",
  "name": "Pglast",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "Pglast is like a tool that helps you understand and transform SQL queries by breaking them into pieces you can work with.",
  "description": "Pglast is a Python library that provides a parser and object model for PostgreSQL SQL statements using libpg_query. It allows developers to inspect, analyze, and manipulate SQL queries programmatically with full AST (Abstract Syntax Tree) support.",
  "use_cases": [
   "Query analysis and optimization tools for PostgreSQL databases",
   "SQL linting and validation in development pipelines",
   "Building database access control layers that inspect queries",
   "Generating documentation from SQL schema and query patterns"
  ],
  "used_by": [
   "Hasura",
   "pgAdmin",
   "Supabase"
  ],
  "docs": "https://docs.pglast.org",
  "tutorial": "https://docs.pglast.org/en/latest/quickstart.html",
  "added": "2026-09-25",
  "dex": 471
 },
 {
  "id": "pocketbase",
  "name": "PocketBase",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "PocketBase is like having a complete backend in a single file that you can carry in your pocket and run anywhere.",
  "description": "A lightweight self-hosted backend-as-a-service written in Go that provides a REST/realtime API, built-in database (SQLite), user authentication, and admin dashboard in a single executable. It eliminates the complexity of deploying traditional backends for small to medium projects.",
  "use_cases": [
   "Building full-stack applications without separate backend infrastructure",
   "Rapid prototyping and MVPs with instant API and admin panel",
   "Self-hosted alternative to Firebase or Supabase with minimal dependencies",
   "Embedded backend for desktop and mobile applications"
  ],
  "used_by": [
   "Indie developers",
   "Startups",
   "Educational institutions"
  ],
  "docs": "https://pocketbase.io/docs/",
  "tutorial": "https://pocketbase.io/docs/getting-started/",
  "added": "2026-06-27",
  "dex": 472
 },
 {
  "id": "postman-api-platform",
  "name": "Postman",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a postal worker who helps you send and receive messages to buildings (servers) and shows you exactly what they reply with, so you can test if they're working right.",
  "description": "A comprehensive API development and collaboration platform that simplifies testing, documenting, and monitoring APIs throughout their lifecycle. Postman is essential for backend developers and QA teams to streamline API development workflows and ensure API reliability.",
  "use_cases": [
   "Manual and automated API testing and validation",
   "API documentation generation and sharing with team members",
   "Integration testing and end-to-end testing workflows",
   "API monitoring and performance testing in production"
  ],
  "used_by": [
   "Microsoft",
   "Shopify",
   "Slack",
   "Twitter"
  ],
  "docs": "https://learning.postman.com/docs/getting-started/overview/",
  "tutorial": "https://learning.postman.com/docs/getting-started/sending-the-first-request/",
  "added": "2026-08-28",
  "dex": 473
 },
 {
  "id": "pydantic",
  "name": "Pydantic",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Pydantic is like a strict teacher that checks if your homework has all the right answers in the right format before accepting it.",
  "description": "Pydantic is a Python library for data validation and settings management using Python type annotations. It ensures data is correct, well-typed, and serializable, becoming essential infrastructure for FastAPI applications and data pipelines.",
  "use_cases": [
   "API request/response validation in FastAPI applications",
   "Configuration management with type safety",
   "Data pipeline validation and transformation",
   "JSON schema generation and documentation"
  ],
  "used_by": [
   "Uber",
   "Microsoft",
   "Amazon"
  ],
  "docs": "https://docs.pydantic.dev/",
  "tutorial": "https://docs.pydantic.dev/latest/getting-started/",
  "added": "2026-06-15",
  "dex": 474
 },
 {
  "id": "pydantic-core",
  "name": "Pydantic Core",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a bouncer at a club who checks that everyone coming in matches the dress code before letting them through the door.",
  "description": "Pydantic Core is a high-performance data validation library written in Rust that powers Pydantic v2, providing type checking and serialization for Python applications. It's essential for building robust APIs and data processing pipelines that need strong guarantees about data shape and types.",
  "use_cases": [
   "FastAPI request/response validation",
   "Data ETL pipeline schema enforcement",
   "ML model input validation before inference",
   "Configuration file parsing and validation"
  ],
  "used_by": [
   "OpenAI",
   "Shopify",
   "Uber",
   "Stripe"
  ],
  "docs": "https://docs.pydantic.dev/latest/",
  "tutorial": "https://docs.pydantic.dev/latest/usage/getting-started/",
  "added": "2026-09-07",
  "dex": 475
 },
 {
  "id": "pydantic-settings",
  "name": "Pydantic Settings",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a checklist that makes sure all the settings your application needs are correct before it starts running.",
  "description": "Pydantic Settings is a library for managing application configuration with automatic validation and type checking. It simplifies loading and validating settings from environment variables, .env files, and other sources.",
  "use_cases": [
   "Loading environment-specific configurations in FastAPI applications",
   "Validating database connection strings and credentials",
   "Managing feature flags and service configuration",
   "Multi-environment deployments (dev, staging, production)"
  ],
  "used_by": [
   "Uber",
   "Netflix",
   "Microsoft",
   "Spotify"
  ],
  "docs": "https://docs.pydantic.dev/latest/concepts/pydantic_settings/",
  "tutorial": "https://docs.pydantic.dev/latest/concepts/pydantic_settings/",
  "added": "2026-09-22",
  "dex": 476
 },
 {
  "id": "pydantic-v2",
  "name": "Pydantic V2",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Pydantic is like a bouncer at a club that checks if your data looks correct before letting it into your Python program.",
  "description": "Pydantic V2 is a data validation and serialization library for Python that uses type hints to ensure data integrity and convert between different formats. It powers FastAPI and is essential for building robust APIs and data pipelines.",
  "use_cases": [
   "API request/response validation in FastAPI applications",
   "Configuration management and environment variable parsing",
   "Data transformation and serialization in ML pipelines",
   "Type-safe database model definitions"
  ],
  "used_by": [
   "Shopify",
   "Microsoft",
   "Databricks",
   "Stripe"
  ],
  "docs": "https://docs.pydantic.dev/latest/",
  "tutorial": "https://docs.pydantic.dev/latest/concepts/models/",
  "added": "2026-06-29",
  "dex": 477
 },
 {
  "id": "python",
  "name": "Python",
  "category": "Language",
  "domain": "Backend",
  "eli5": "A language so easy to read it almost looks like plain English — the Swiss Army knife of programming.",
  "description": "High-level, dynamically typed, interpreted language known for readability and a massive ecosystem. Dominant in data science, ML, and scripting. Slower than compiled languages but fast enough when paired with C-extension libraries (NumPy, PyTorch).",
  "use_cases": [
   "Data science and ML: NumPy, Pandas, scikit-learn, PyTorch, TensorFlow",
   "Backend APIs with FastAPI, Django, or Flask",
   "Automation, scripting, and glue code for DevOps pipelines",
   "Data engineering: PySpark, Apache Airflow DAGs, dbt Python models",
   "LLM applications and AI agent frameworks (LangChain, LlamaIndex)"
  ],
  "used_by": [
   "Google",
   "Instagram",
   "Spotify",
   "Netflix",
   "Dropbox",
   "NASA"
  ],
  "docs": "https://docs.python.org/3/",
  "tutorial": "https://docs.python.org/3/tutorial/",
  "added": "2026-06-08",
  "dex": 478
 },
 {
  "id": "resend",
  "name": "Resend",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "Resend is like a postman service for emails that helps developers send beautiful, reliable emails from their apps without worrying about spam filters.",
  "description": "Resend is a modern email API designed for developers, providing reliable email delivery with built-in templates, previews, and easy integration. It focuses on developer experience with intuitive SDKs and high deliverability rates.",
  "use_cases": [
   "Transactional email delivery for web and mobile applications",
   "Marketing and notification emails with templates",
   "Email verification and authentication workflows",
   "Batch email campaigns with detailed analytics"
  ],
  "used_by": [
   "Vercel",
   "Figma",
   "Stripe",
   "Notion"
  ],
  "docs": "https://resend.com/docs",
  "tutorial": "https://resend.com/blog/getting-started",
  "added": "2026-07-03",
  "dex": 479
 },
 {
  "id": "resend-api",
  "name": "Resend API",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like a mailbox service that makes sure your emails get to people's inboxes instead of spam folders.",
  "description": "Resend is a transactional email platform designed for developers, providing reliable email delivery with built-in templates, webhooks, and analytics. It simplifies sending application emails like password resets and notifications with a clean API.",
  "use_cases": [
   "Transactional email delivery for authentication and password resets",
   "Marketing campaign emails and newsletters",
   "Order confirmations and delivery notifications",
   "User onboarding and engagement emails"
  ],
  "used_by": [
   "Vercel",
   "Supabase",
   "Stripe",
   "Linear"
  ],
  "docs": "https://resend.com/docs",
  "tutorial": "https://resend.com/docs/quickstart",
  "added": "2026-08-09",
  "dex": 480
 },
 {
  "id": "resend-email",
  "name": "Resend Email Service",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like hiring a mail carrier service specifically trained to deliver your digital letters and make sure they actually show up in people's inboxes.",
  "description": "Resend is a modern email API platform built for developers, handling transactional emails with a focus on deliverability, reliability, and developer experience. It provides built-in templates, analytics, and integrations for sending emails from applications.",
  "use_cases": [
   "Transactional email delivery (password resets, confirmations)",
   "Marketing email campaigns with analytics",
   "Notification systems for user alerts",
   "Invoice and receipt generation and delivery"
  ],
  "used_by": [
   "Vercel",
   "Stripe",
   "Supabase"
  ],
  "docs": "https://resend.com/docs",
  "tutorial": "https://resend.com/docs/send-with-nextjs",
  "added": "2026-08-07",
  "dex": 481
 },
 {
  "id": "river",
  "name": "River",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "River is like a reliable mailbox for your computer tasks—it makes sure every job gets delivered and done even if things go wrong temporarily.",
  "description": "River is a fast, reliable job queue for Go and Python that provides guaranteed job delivery, automatic retries, and job expiration. It simplifies background job processing with minimal configuration while maintaining high throughput and low latency.",
  "use_cases": [
   "Processing asynchronous background jobs reliably",
   "Sending scheduled notifications and emails",
   "Coordinating multi-step workflows with retries",
   "Handling time-sensitive batch processing tasks"
  ],
  "used_by": [
   "Various Go/Python startups",
   "Fintech applications",
   "SaaS platforms"
  ],
  "docs": "https://riverqueue.com/docs",
  "tutorial": "https://riverqueue.com/docs/getting-started",
  "added": "2026-07-14",
  "dex": 482
 },
 {
  "id": "riza",
  "name": "Riza",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Riza is like a safe sandbox where you can run code written by other people on the internet without worrying it will break your computer or steal your data.",
  "description": "Riza is a serverless sandboxed code execution platform that allows safe execution of untrusted code (Python, JavaScript, TypeScript, etc.) with automatic resource limits and security isolation. It's used for AI agent frameworks, user-generated code evaluation, and dynamic scripting.",
  "use_cases": [
   "Safely running Python/JavaScript code generated by LLMs in production",
   "Building user-facing code editors and execution environments",
   "Automated testing and validation of student submissions",
   "Dynamic workflow engines that require arbitrary code execution"
  ],
  "used_by": [
   "LangChain, AI startups, Educational platforms"
  ],
  "docs": "https://docs.riza.io/",
  "tutorial": "https://docs.riza.io/quickstart",
  "added": "2026-07-11",
  "dex": 483
 },
 {
  "id": "rails",
  "name": "Ruby on Rails",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Rails is like a fully-assembled LEGO kit for building web apps — the pieces are already connected in the right way so you can build fast without reading a manual.",
  "description": "Full-stack web framework for Ruby following convention over configuration. Includes ORM (ActiveRecord), routing, templating, mailers, and a strong developer productivity focus.",
  "use_cases": [
   "Rapid full-stack web application development",
   "Monolithic apps with built-in admin and auth",
   "REST and GraphQL API backends",
   "Startups and MVPs with fast time-to-market"
  ],
  "used_by": [
   "GitHub",
   "Shopify",
   "Airbnb",
   "Basecamp"
  ],
  "docs": "https://guides.rubyonrails.org/",
  "tutorial": "https://guides.rubyonrails.org/getting_started.html",
  "added": "2026-06-14",
  "dex": 484
 },
 {
  "id": "rust",
  "name": "Rust",
  "category": "Language",
  "domain": "Backend",
  "eli5": "A language that won't let you write dangerous code — like a strict teacher who catches your mistakes before class starts.",
  "description": "Systems programming language focused on memory safety, performance, and fearless concurrency — without a garbage collector. The borrow checker catches memory bugs at compile time. Consistently voted most loved language in Stack Overflow surveys.",
  "use_cases": [
   "Systems programming: OS components, drivers, embedded firmware",
   "WebAssembly (WASM) compilation for near-native browser performance",
   "High-performance networking and proxies (Cloudflare, AWS, Discord)",
   "CLI tools requiring minimal binary size and zero runtime overhead",
   "Replacing C/C++ in safety-critical code (Linux kernel, Android, Windows)"
  ],
  "used_by": [
   "Mozilla",
   "Cloudflare",
   "Discord",
   "AWS",
   "Microsoft",
   "Linux Foundation"
  ],
  "docs": "https://doc.rust-lang.org/book/",
  "tutorial": "https://rustlings.cool/",
  "added": "2026-06-08",
  "dex": 485
 },
 {
  "id": "rustler",
  "name": "Rustler",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "A bridge that lets Rust (the super-fast language) work inside Elixir programs, like connecting a speedy race car engine to a reliable truck.",
  "description": "Rustler is an Elixir library that enables seamless integration of Rust code for performance-critical operations, allowing developers to write safe, concurrent Elixir applications with Rust's speed for compute-heavy tasks. It handles the FFI complexity automatically.",
  "use_cases": [
   "Image processing and computer vision in Elixir applications",
   "Cryptographic operations requiring high performance",
   "Real-time data processing in telecom and financial systems",
   "CPU-intensive algorithms while maintaining Elixir's concurrency model"
  ],
  "used_by": [
   "Discord",
   "Mux",
   "Qdrant",
   "Akord"
  ],
  "docs": "https://hexdocs.pm/rustler/",
  "tutorial": "https://github.com/rusterlium/rustler/blob/master/README.md",
  "added": "2026-08-10",
  "dex": 486
 },
 {
  "id": "rustlings",
  "name": "Rustlings",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a interactive workbook that teaches you Rust programming by fixing small broken exercises, like a game where you solve puzzles to learn.",
  "description": "Rustlings is a collection of small exercises to get you used to reading and writing Rust code. It's designed as a hands-on learning tool for developers learning the Rust language through practical, incremental challenges.",
  "use_cases": [
   "Onboarding developers to Rust programming",
   "Teaching Rust language fundamentals in team environments",
   "Self-paced learning for individual developers",
   "Building confidence with Rust syntax and ownership rules"
  ],
  "used_by": [
   "Mozilla",
   "Google",
   "Amazon",
   "Microsoft"
  ],
  "docs": "https://github.com/rust-lang/rustlings",
  "tutorial": "https://github.com/rust-lang/rustlings#readme",
  "added": "2026-07-02",
  "dex": 487
 },
 {
  "id": "seaorm",
  "name": "SeaORM",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "SeaORM is like a translator that lets you talk to databases using Rust instead of learning database languages.",
  "description": "SeaORM is an async Object-Relational Mapping (ORM) library for Rust that provides a safe, ergonomic way to interact with SQL databases. It supports multiple databases including PostgreSQL, MySQL, and SQLite with compile-time query validation.",
  "use_cases": [
   "Building type-safe backend APIs in Rust with minimal boilerplate",
   "Database schema migrations and management in async Rust applications",
   "Creating production web services with guaranteed compile-time database safety",
   "Rapid development of microservices that need concurrent database access"
  ],
  "used_by": [
   "Toggl Track",
   "Navi",
   "Shuttle"
  ],
  "docs": "https://www.sea-orm.io/docs",
  "tutorial": "https://www.sea-orm.io/docs/tutorials",
  "added": "2026-09-25",
  "dex": 488
 },
 {
  "id": "shuttle",
  "name": "Shuttle",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Shuttle is like a helpful moving company that takes your Rust backend code and automatically sets up all the servers and databases you need, then runs it for you.",
  "description": "Shuttle is a serverless platform built for Rust that handles infrastructure provisioning, deployment, and scaling automatically. It integrates with popular Rust frameworks like Axum and Actix and manages databases and secrets out of the box.",
  "use_cases": [
   "Deploying Rust web services without managing servers or containers",
   "Building event-driven microservices with automatic scaling",
   "Rapid prototyping of production-grade Rust APIs",
   "Running resource-constrained workloads cost-effectively"
  ],
  "used_by": [
   "Rust startups, Open-source projects, Backend teams migrating to Rust"
  ],
  "docs": "https://docs.shuttle.rs/",
  "tutorial": "https://docs.shuttle.rs/getting-started",
  "added": "2026-07-11",
  "dex": 489
 },
 {
  "id": "spring-boot",
  "name": "Spring Boot",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Spring Boot is like a pre-configured Java starter kit — it sets up all the boring boilerplate so you can start writing business logic on day one.",
  "description": "Convention-over-configuration Java framework for building production-ready microservices and web apps. Embeds Tomcat/Jetty and auto-configures the Spring ecosystem.",
  "use_cases": [
   "Enterprise REST APIs and microservices",
   "Event-driven applications with Spring Kafka/RabbitMQ",
   "Batch processing pipelines",
   "Cloud-native apps with Spring Cloud"
  ],
  "used_by": [
   "Netflix",
   "Amazon",
   "Google",
   "Microsoft"
  ],
  "docs": "https://docs.spring.io/spring-boot/docs/current/reference/html/",
  "tutorial": "https://spring.io/quickstart",
  "added": "2026-06-12",
  "dex": 490
 },
 {
  "id": "sqlalchemy",
  "name": "SQLAlchemy",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a translator that lets your Python code talk to databases in a simpler way without writing raw database language.",
  "description": "SQLAlchemy is a mature Python SQL toolkit and Object-Relational Mapping (ORM) library that provides a full suite of patterns for accessing databases, from simple queries to complex relationships. It supports multiple database backends and is the standard for database interaction in Python.",
  "use_cases": [
   "Building data models and relationships in Python web applications",
   "Abstracting database logic from application code",
   "Writing database-agnostic code that works across PostgreSQL, MySQL, SQLite",
   "Complex query building and optimization with expressions"
  ],
  "used_by": [
   "Uber",
   "Instacart",
   "Airbnb",
   "Spotify"
  ],
  "docs": "https://docs.sqlalchemy.org/",
  "tutorial": "https://docs.sqlalchemy.org/en/20/tutorial/",
  "added": "2026-07-21",
  "dex": 491
 },
 {
  "id": "sqlc",
  "name": "sqlc",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a translator that reads your SQL queries and automatically writes the boring code to use them safely in your program.",
  "description": "sqlc generates type-safe code from SQL queries without an ORM. It parses SQL queries and produces strongly-typed, idiomatic Go, Python, or other language code that prevents SQL injection and type mismatches at compile time.",
  "use_cases": [
   "Building type-safe database layers in Go microservices",
   "Generating Python database clients from SQL migrations",
   "Replacing ORM boilerplate with raw SQL safety guarantees",
   "Enforcing schema correctness across database operations"
  ],
  "used_by": [
   "Stripe",
   "Vercel",
   "GitLab",
   "Segment"
  ],
  "docs": "https://docs.sqlc.dev/",
  "tutorial": "https://docs.sqlc.dev/en/stable/tutorials/getting-started.html",
  "added": "2026-06-14",
  "dex": 492
 },
 {
  "id": "stripe",
  "name": "Stripe",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Stripe is like a digital cash register that lets your online store accept and manage money from customers all over the world.",
  "description": "Stripe is a payment processing platform that enables businesses to accept payments online and in-person. It provides APIs for payment collection, subscription management, and financial operations at scale.",
  "use_cases": [
   "E-commerce checkout and payment processing",
   "Subscription and recurring billing management",
   "Marketplace and platform payouts",
   "Global payment processing with multi-currency support"
  ],
  "used_by": [
   "Amazon",
   "Shopify",
   "Lyft",
   "DoorDash"
  ],
  "docs": "https://stripe.com/docs",
  "tutorial": "https://stripe.com/docs/payments/quickstart",
  "added": "2026-06-18",
  "dex": 493
 },
 {
  "id": "stripe-cli",
  "name": "Stripe CLI",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a telephone that lets your computer talk to Stripe's payment computers so you can test sending money without using real money.",
  "description": "Stripe CLI is a command-line tool that enables developers to interact with Stripe's API locally, test webhooks, trigger events, and manage resources from the terminal. It's essential for payment integration development and testing without hitting production systems.",
  "use_cases": [
   "Testing webhook events during local development",
   "Debugging payment flows in non-production environments",
   "Triggering test charges and subscription events",
   "Managing API keys and authentication securely"
  ],
  "used_by": [
   "Shopify",
   "Slack",
   "DoorDash",
   "Lyft"
  ],
  "docs": "https://stripe.com/docs/stripe-cli",
  "tutorial": "https://stripe.com/docs/stripe-cli/get-started",
  "added": "2026-07-21",
  "dex": 494
 },
 {
  "id": "stripe-elements",
  "name": "Stripe Elements",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a pre-built payment checkout form that you can drop into your website, so you don't have to build it from scratch.",
  "description": "Stripe Elements is a set of pre-built UI components for collecting payment information securely without handling raw card data. It simplifies PCI compliance and payment integration for web and mobile applications.",
  "use_cases": [
   "Building custom checkout experiences while maintaining PCI compliance",
   "Integrating payment processing into SaaS platforms",
   "Creating mobile payment flows with tokenized card handling",
   "Reducing fraud through Stripe's built-in security features"
  ],
  "used_by": [
   "Shopify",
   "DoorDash",
   "Figma",
   "Notion"
  ],
  "docs": "https://stripe.com/docs/stripe-js/elements/payment-element",
  "tutorial": "https://stripe.com/docs/payments/payment-element",
  "added": "2026-09-13",
  "dex": 495
 },
 {
  "id": "stripe-test-clocks",
  "name": "Stripe Test Clocks",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a time machine that lets you jump to any date in the future to test what happens when subscriptions expire or billing dates arrive.",
  "description": "Stripe's testing feature that enables simulating time-dependent billing scenarios in test mode. Developers can accelerate billing cycles, test subscription renewals, and validate time-sensitive payment logic without waiting days or months.",
  "use_cases": [
   "Testing subscription renewal and cancellation flows",
   "Validating trial period transitions and billing cycles",
   "Testing dunning and retry logic for failed payments",
   "Simulating invoice generation at specific future dates"
  ],
  "used_by": [
   "Netflix",
   "Slack",
   "Figma",
   "Notion"
  ],
  "docs": "https://stripe.com/docs/billing/testing",
  "tutorial": "https://stripe.com/docs/billing/testing#test-clocks",
  "added": "2026-08-08",
  "dex": 496
 },
 {
  "id": "supabase-realtime",
  "name": "Supabase Realtime",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "Supabase Realtime is like a magic bulletin board that instantly tells everyone watching it whenever something new gets posted, so everyone sees updates at the same time.",
  "description": "Supabase Realtime is a PostgreSQL-native pub/sub system that enables real-time data synchronization across connected clients. It powers instant updates for collaborative applications, live notifications, and reactive features without additional infrastructure.",
  "use_cases": [
   "Real-time collaborative editing and presence tracking",
   "Live notifications and activity feeds for user dashboards",
   "Instant data synchronization across web and mobile clients",
   "Live multiplayer features in web applications"
  ],
  "used_by": [
   "Slack",
   "Figma integrations",
   "Vercel"
  ],
  "docs": "https://supabase.com/docs/guides/realtime",
  "tutorial": "https://supabase.com/docs/guides/realtime/quickstart",
  "added": "2026-06-22",
  "dex": 497
 },
 {
  "id": "svix",
  "name": "Svix",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Svix is like a reliable postal service that makes sure your messages about important events get delivered to all the right places.",
  "description": "Svix is a webhook platform as a service that provides reliable event delivery, automatic retries, and webhook management for developers. It handles the complexity of building and maintaining webhooks at scale with built-in security, monitoring, and testing tools.",
  "use_cases": [
   "Event-driven architecture and webhooks",
   "Third-party integrations and notifications",
   "Real-time data synchronization",
   "Payment and transactional notifications"
  ],
  "used_by": [
   "Slack",
   "Zapier",
   "Stripe"
  ],
  "docs": "https://docs.svix.com/",
  "tutorial": "https://docs.svix.com/getting-started",
  "added": "2026-07-05",
  "dex": 498
 },
 {
  "id": "svix-webhooks",
  "name": "Svix Webhooks",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like having a mailman who guarantees your messages get delivered to your neighbors, and if they're not home, he keeps trying until they pick up.",
  "description": "Svix is a webhook-as-a-service platform that handles reliable event delivery, retries, and webhook management for applications, eliminating the complexity of building robust webhook infrastructure. It provides built-in security, monitoring, and developer tools for webhook operations.",
  "use_cases": [
   "Delivering real-time notifications from SaaS platforms to customers",
   "Building event-driven architectures with guaranteed delivery",
   "Testing webhooks in development without complex setup",
   "Monitoring webhook health and debugging delivery failures"
  ],
  "used_by": [
   "Supabase",
   "Zapier",
   "Retool",
   "Plane"
  ],
  "docs": "https://docs.svix.com/",
  "tutorial": "https://docs.svix.com/getting-started",
  "added": "2026-09-13",
  "dex": 499
 },
 {
  "id": "tailcall",
  "name": "Tailcall",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "Tailcall is like a super-smart mail sorter that combines letters from many different mailboxes into one organized pile so you only have to pick it up once.",
  "description": "Tailcall is an open-source GraphQL engine that efficiently composes multiple data sources and APIs into a single GraphQL schema. It enables rapid API composition and data aggregation with built-in caching and performance optimization.",
  "use_cases": [
   "Aggregating data from multiple microservices into a unified GraphQL API",
   "Reducing latency by composing REST APIs without writing backend code",
   "Building API gateways that combine internal and third-party APIs",
   "Creating real-time data pipelines that fetch from heterogeneous sources"
  ],
  "used_by": [
   "Shopify",
   "Hasura customers",
   "Enterprise API teams"
  ],
  "docs": "https://tailcall.run/docs",
  "tutorial": "https://tailcall.run/docs/getting-started",
  "added": "2026-07-17",
  "dex": 500
 },
 {
  "id": "temporal",
  "name": "Temporal",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Temporal is like a very reliable calendar and task manager for computers that remembers what they need to do, even if the power goes off.",
  "description": "Temporal is an open-source workflow orchestration platform that allows developers to build resilient, scalable applications with complex business logic using a durable execution model. It handles retries, timeouts, and state management automatically.",
  "use_cases": [
   "Order processing and fulfillment workflows",
   "Payment processing and financial transactions",
   "User onboarding and provisioning workflows",
   "Long-running asynchronous job orchestration"
  ],
  "used_by": [
   "Stripe",
   "Databricks",
   "Instacart",
   "Uber"
  ],
  "docs": "https://docs.temporal.io/",
  "tutorial": "https://docs.temporal.io/getting-started",
  "added": "2026-06-15",
  "dex": 501
 },
 {
  "id": "temporal-cloud",
  "name": "Temporal Cloud",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like a reliable postal service for your code—it guarantees your tasks get delivered and executed, even if things go wrong along the way.",
  "description": "A fully managed SaaS platform for Temporal, an open-source workflow orchestration engine that ensures reliable, scalable execution of complex distributed workflows with built-in resilience and visibility. It eliminates the need to operate Temporal infrastructure while providing enterprise-grade reliability.",
  "use_cases": [
   "Long-running payment processing workflows that must complete reliably",
   "Coordinating multi-step microservice interactions across teams",
   "Building audit trails for critical business processes",
   "Automatically retrying failed operations with exponential backoff"
  ],
  "used_by": [
   "Datadog",
   "Stripe",
   "Figma"
  ],
  "docs": "https://docs.temporal.io/cloud",
  "tutorial": "https://docs.temporal.io/cloud/get-started",
  "added": "2026-08-05",
  "dex": 502
 },
 {
  "id": "temporal-state-machines",
  "name": "Temporal State Machines",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "It's like a flowchart that remembers where you are in a process and automatically knows what to do next, even if the computer crashes and restarts.",
  "description": "An extension to Temporal.io that provides durable, fault-tolerant workflow orchestration using state machine patterns in TypeScript. It enables building reliable microservices workflows that survive failures and provide strong consistency guarantees.",
  "use_cases": [
   "Distributed saga transactions across microservices",
   "Long-running business process orchestration",
   "Order processing and fulfillment workflows",
   "Payment processing and financial transaction pipelines"
  ],
  "used_by": [
   "Stripe",
   "Databricks",
   "Figma"
  ],
  "docs": "https://docs.temporal.io/",
  "tutorial": "https://learn.temporal.io/",
  "added": "2026-08-21",
  "dex": 503
 },
 {
  "id": "trigger-dev",
  "name": "Trigger.dev",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "It's like hiring a reliable assistant who watches for specific events and automatically does tasks for you whenever those events happen.",
  "description": "Trigger.dev is a platform for building, running, and monitoring background jobs and event-driven workflows with a developer-friendly SDK and built-in reliability features. It simplifies complex async operations, integrations, and long-running tasks without requiring infrastructure management.",
  "use_cases": [
   "Building webhook receivers and event-driven architectures",
   "Creating scheduled jobs and cron tasks with retries",
   "Integrating third-party APIs with automatic error handling",
   "Processing background work at scale without managing queues"
  ],
  "used_by": [
   "Vercel",
   "Stripe ecosystem",
   "Supabase partners",
   "Resend"
  ],
  "docs": "https://trigger.dev/docs",
  "tutorial": "https://trigger.dev/docs/tutorials",
  "added": "2026-08-14",
  "dex": 504
 },
 {
  "id": "trpc",
  "name": "tRPC",
  "category": "Framework",
  "domain": "Backend",
  "eli5": "tRPC is like an API where you never have to write documentation — your frontend automatically knows the exact types of every function your backend exposes.",
  "description": "TypeScript RPC framework that gives end-to-end type safety between client and server with zero code generation. Eliminates REST/GraphQL schemas for TypeScript full-stack apps.",
  "use_cases": [
   "Full-stack TypeScript apps with shared types",
   "Next.js and Remix API layers without REST overhead",
   "Replacing REST or GraphQL in mono-repo TS stacks",
   "Type-safe subscriptions and streaming"
  ],
  "used_by": [
   "Cal.com",
   "Ping.gg",
   "Newline"
  ],
  "docs": "https://trpc.io/docs",
  "tutorial": "https://trpc.io/docs/quickstart",
  "added": "2026-06-14",
  "dex": 505
 },
 {
  "id": "uv",
  "name": "uv",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's a super fast assistant that organizes all your Python project's ingredients and makes sure everyone gets the right versions.",
  "description": "uv is an extremely fast Python package installer and dependency resolver written in Rust. It replaces pip, pip-tools, and virtualenv with a single blazingly fast tool that significantly reduces Python project setup and dependency resolution time.",
  "use_cases": [
   "Accelerating CI/CD pipelines for Python projects",
   "Managing Python dependencies in monorepos",
   "Replacing pip for faster local development iteration",
   "Building reproducible Python environments with lock files"
  ],
  "used_by": [
   "Anthropic",
   "Hugging Face",
   "Astronomer",
   "Glyph"
  ],
  "docs": "https://docs.astral.sh/uv/",
  "tutorial": "https://docs.astral.sh/uv/getting-started/",
  "added": "2026-06-14",
  "dex": 506
 },
 {
  "id": "uv-pip",
  "name": "uv (pip replacement)",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a super-fast delivery person for Python packages instead of the normal mailman - same job, way quicker.",
  "description": "uv is an extremely fast Python package installer and resolver written in Rust that replaces pip and pip-tools. It provides 10-100x faster dependency resolution while maintaining compatibility with standard Python packaging.",
  "use_cases": [
   "Accelerating CI/CD pipelines for Python projects",
   "Improving developer experience in large monorepos with many dependencies",
   "Reducing container build times for Python applications",
   "Managing complex dependency resolution in data science projects"
  ],
  "used_by": [
   "Astral",
   "GitHub",
   "Meta",
   "Google"
  ],
  "docs": "https://docs.astral.sh/uv/",
  "tutorial": "https://docs.astral.sh/uv/getting-started/",
  "added": "2026-08-29",
  "dex": 507
 },
 {
  "id": "uv-python-package-manager",
  "name": "uv (Python Package Manager)",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "uv is like a super-fast librarian that finds and organizes all the books your Python project needs, much faster than the old librarian.",
  "description": "uv is an extremely fast Python package installer and resolver written in Rust, designed as a drop-in replacement for pip and pip-tools. It dramatically reduces dependency resolution time and installation overhead for Python projects.",
  "use_cases": [
   "Accelerating CI/CD pipelines for Python projects",
   "Local development with faster dependency installation",
   "Managing complex monorepo Python dependencies",
   "Building Python applications with strict performance requirements"
  ],
  "used_by": [
   "Astral",
   "GitHub",
   "Meta",
   "Google"
  ],
  "docs": "https://docs.astral.sh/uv/",
  "tutorial": "https://docs.astral.sh/uv/getting-started/",
  "added": "2026-09-08",
  "dex": 508
 },
 {
  "id": "uv-builder",
  "name": "uv Builder",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a super-fast assembly line that packages your Python code and all its ingredients into a box that other people can use.",
  "description": "uv is an extremely fast Python package installer and resolver written in Rust that replaces pip and pip-tools. It dramatically accelerates dependency resolution and installation while maintaining compatibility with the Python packaging ecosystem.",
  "use_cases": [
   "Speeding up CI/CD pipelines for Python projects",
   "Managing monorepo dependencies across multiple Python packages",
   "Local development with instant package installation",
   "Building isolated Python environments for data science workflows"
  ],
  "used_by": [
   "Anthropic",
   "Hugging Face",
   "FastAPI community"
  ],
  "docs": "https://docs.astral.sh/uv/",
  "tutorial": "https://docs.astral.sh/uv/getting-started/",
  "added": "2026-09-20",
  "dex": 509
 },
 {
  "id": "uv-workspace",
  "name": "uv Workspace",
  "category": "Tool",
  "domain": "Backend",
  "eli5": "It's like a shared notebook where multiple Python projects can work together and share their pencils (dependencies) without getting in each other's way.",
  "description": "uv workspace is a monorepo tool for Python that manages multiple interdependent packages with unified dependency resolution and optimized build caching. It enables teams to organize large Python codebases with shared dependencies while maintaining isolation and reproducibility.",
  "use_cases": [
   "Managing large Python monorepos with multiple interdependent services",
   "Organizing machine learning projects with shared model and utility libraries",
   "Building plugin ecosystems where core library and plugins share versions",
   "Coordinating dependencies across data engineering pipelines"
  ],
  "used_by": [
   "Astral",
   "Fast.ai",
   "Hugging Face"
  ],
  "docs": "https://docs.astral.sh/uv/concepts/workspaces/",
  "tutorial": "https://docs.astral.sh/uv/guides/workspaces/",
  "added": "2026-09-18",
  "dex": 510
 },
 {
  "id": "windmill-workflow",
  "name": "Windmill",
  "category": "Platform",
  "domain": "Backend",
  "eli5": "Windmill is like a visual flowchart maker for creating automated jobs that run tasks in sequence without writing traditional code.",
  "description": "Windmill is an open-source workflow orchestration platform that enables rapid development of internal tools, APIs, and automated workflows using visual flows combined with Python, JavaScript, or SQL scripts. It provides instant UIs and APIs for data operations without boilerplate.",
  "use_cases": [
   "Building internal tools and dashboards for operations teams",
   "Automating data pipelines and ETL workflows",
   "Creating scheduled jobs for background processing",
   "Rapid API development for business logic without frontend boilerplate"
  ],
  "used_by": [
   "Airbnb",
   "Notion",
   "Zapier",
   "Twilio"
  ],
  "docs": "https://docs.windmill.dev/",
  "tutorial": "https://docs.windmill.dev/docs/getting_started/quickstart",
  "added": "2026-08-18",
  "dex": 511
 },
 {
  "id": "apache-kafka",
  "name": "Apache Kafka",
  "category": "Tool",
  "domain": "Messaging",
  "description": "Distributed event streaming platform designed for high-throughput, fault-tolerant, ordered message delivery. Stores events durably as an immutable log. De facto standard for real-time data pipelines and event-driven architectures.",
  "use_cases": [
   "Real-time event streaming between microservices (decoupled producers/consumers)",
   "Change Data Capture (CDC) from databases via Kafka Connect + Debezium",
   "Stream processing pipelines with Kafka Streams or Apache Flink",
   "Activity tracking: clickstreams, metrics, audit logs at massive scale",
   "Event sourcing and CQRS as the persistent event log",
   "Multi-datacenter replication with MirrorMaker 2"
  ],
  "used_by": [
   "LinkedIn",
   "Uber",
   "Netflix",
   "Airbnb",
   "Goldman Sachs",
   "Cloudflare"
  ],
  "docs": "https://kafka.apache.org/documentation/",
  "tutorial": "https://kafka.apache.org/quickstart",
  "added": "2026-06-08",
  "eli5": "A conveyor belt that carries messages between apps without dropping any.",
  "dex": 512
 },
 {
  "id": "apache-pulsar",
  "name": "Apache Pulsar",
  "category": "Tool",
  "domain": "Messaging",
  "eli5": "Pulsar is like a post office that can hold letters forever and deliver them to any subscriber at any time — even ones who weren't listening when the letter arrived.",
  "description": "Cloud-native distributed messaging and streaming platform with multi-tenancy, geo-replication, and built-in tiered storage. Decouples compute from storage.",
  "use_cases": [
   "Multi-tenant event streaming across teams",
   "Long-term message retention with tiered storage",
   "Geo-replicated pub/sub for global deployments",
   "Serverless stream processing with Pulsar Functions"
  ],
  "used_by": [
   "Yahoo",
   "Tencent",
   "Overstock",
   "Narvar"
  ],
  "docs": "https://pulsar.apache.org/docs/",
  "tutorial": "https://pulsar.apache.org/docs/getting-started-standalone/",
  "added": "2026-06-12",
  "dex": 513
 },
 {
  "id": "zookeeper",
  "name": "Apache ZooKeeper",
  "category": "Tool",
  "domain": "Messaging",
  "eli5": "ZooKeeper is like a referee who keeps track of all the rules and makes sure every player in a big game follows them.",
  "description": "Apache ZooKeeper is a centralized service for maintaining configuration information, naming, providing distributed synchronization, and group services over large clusters. It's critical for coordinating distributed systems like Kafka, HBase, and Hadoop.",
  "use_cases": [
   "Coordinating leader election in distributed databases",
   "Managing configuration across Kafka brokers",
   "Service discovery and health checking",
   "Distributed locking and synchronization"
  ],
  "used_by": [
   "Netflix",
   "Yahoo",
   "Twitter",
   "Salesforce"
  ],
  "docs": "https://zookeeper.apache.org/",
  "tutorial": "https://zookeeper.apache.org/doc/current/zookeeperStarted.html",
  "added": "2026-06-16",
  "dex": 514
 },
 {
  "id": "celery",
  "name": "Celery",
  "category": "Framework",
  "domain": "Messaging",
  "eli5": "Celery is like a restaurant ticket system — the server hands off orders to kitchen workers in the background so the front desk stays fast and free.",
  "description": "Distributed task queue for Python. Executes async and scheduled tasks in background workers, backed by brokers like Redis or RabbitMQ. Essential for offloading slow work from web requests.",
  "use_cases": [
   "Async task processing (email, image resize, PDF generation)",
   "Periodic/scheduled jobs (cron replacement)",
   "Distributed computation across multiple workers",
   "Rate-limited API call queuing"
  ],
  "used_by": [
   "Instagram",
   "Mozilla",
   "Coursera",
   "Robinhood"
  ],
  "docs": "https://docs.celeryq.dev/",
  "tutorial": "https://docs.celeryq.dev/en/stable/getting-started/first-steps-with-celery.html",
  "added": "2026-06-12",
  "dex": 515
 },
 {
  "id": "cloud-pubsub",
  "name": "Google Cloud Pub/Sub",
  "category": "Platform",
  "domain": "Messaging",
  "eli5": "Google's message bus — apps shout a message, other apps listen and catch it.",
  "description": "GCP's fully managed real-time messaging service. Decouples producers from consumers with at-least-once delivery, ordering guarantees, and replay. Global by default — messages published in one region can be consumed anywhere with sub-100ms latency.",
  "use_cases": [
   "Event-driven microservices: decouple services with async message passing",
   "Streaming ingest pipeline: Pub/Sub → Dataflow → BigQuery",
   "Fan-out: one publisher, many independent subscriber services",
   "Dead letter topics for handling unprocessable messages",
   "GCS/Cloud Run/Cloud Functions event triggers via Eventarc"
  ],
  "used_by": [
   "Spotify",
   "The New York Times",
   "Mercado Libre",
   "Twitch",
   "Wix"
  ],
  "docs": "https://cloud.google.com/pubsub/docs",
  "tutorial": "https://cloud.google.com/pubsub/docs/publish-receive-messages-console",
  "added": "2026-06-08",
  "dex": 516
 },
 {
  "id": "nats",
  "name": "NATS",
  "category": "Tool",
  "domain": "Messaging",
  "eli5": "NATS is like a super-fast walkie-talkie system where any app can shout a message and anyone listening on that channel instantly hears it.",
  "description": "High-performance, lightweight messaging system for cloud-native applications, IoT, and microservices. Supports pub/sub, request/reply, and durable JetStream streaming.",
  "use_cases": [
   "Microservice-to-microservice messaging",
   "IoT telemetry ingestion",
   "Event-driven architectures",
   "Distributed systems coordination"
  ],
  "used_by": [
   "Apcera",
   "Netlify",
   "Mastercard",
   "Synadia"
  ],
  "docs": "https://docs.nats.io/",
  "tutorial": "https://docs.nats.io/nats-concepts/overview",
  "added": "2026-06-11",
  "dex": 517
 },
 {
  "id": "nats-jetstream",
  "name": "NATS JetStream",
  "category": "Platform",
  "domain": "Messaging",
  "eli5": "It's like a mailbox system where messages wait safely until someone picks them up, even if the recipient isn't home right now.",
  "description": "NATS JetStream adds persistent message storage and replay capabilities to NATS, providing message streaming with exactly-once delivery semantics. It enables sophisticated event streaming patterns while maintaining NATS's core simplicity and performance.",
  "use_cases": [
   "Event sourcing and temporal event replay",
   "Stream processing with delivery guarantees",
   "Log aggregation and audit trails",
   "Microservice choreography with replay capabilities"
  ],
  "used_by": [
   "Synadia",
   "Cloudflare",
   "Dell Technologies"
  ],
  "docs": "https://docs.nats.io/nats-concepts/jetstream",
  "tutorial": "https://docs.nats.io/using-nats/developer/develop_jetstream",
  "added": "2026-08-07",
  "dex": 518
 },
 {
  "id": "rabbitmq",
  "name": "RabbitMQ",
  "category": "Tool",
  "domain": "Messaging",
  "description": "Open-source message broker implementing AMQP, MQTT, and STOMP. Routes messages through exchanges and queues with flexible binding rules. Reliable delivery with acknowledgements, dead-letter queues, and persistence.",
  "use_cases": [
   "Task queues for background job processing (Celery, Sidekiq backends)",
   "Decoupling microservices with publish/subscribe via fanout exchanges",
   "Priority queues and delayed message delivery",
   "Dead-letter queues for failed message handling and retry logic",
   "RPC-over-AMQP pattern for synchronous request-reply messaging"
  ],
  "used_by": [
   "Reddit",
   "Instagram",
   "Mozilla",
   "VMware",
   "Zalando"
  ],
  "docs": "https://www.rabbitmq.com/documentation.html",
  "tutorial": "https://www.rabbitmq.com/tutorials",
  "added": "2026-06-08",
  "eli5": "A post office — apps drop letters in, other apps pick them up later.",
  "dex": 519
 },
 {
  "id": "rabbitmq-amqp",
  "name": "RabbitMQ AMQP",
  "category": "Platform",
  "domain": "Messaging",
  "eli5": "RabbitMQ is like a postal service that delivers messages between different parts of your application, making sure nothing gets lost.",
  "description": "RabbitMQ is an open-source message broker that implements the Advanced Message Queuing Protocol (AMQP) to enable reliable, asynchronous communication between distributed systems. It's a foundational tool for building event-driven architectures and decoupling microservices.",
  "use_cases": [
   "Asynchronous task processing and job queues",
   "Event streaming between microservices",
   "Log aggregation and metrics collection",
   "Request-response patterns with message routing"
  ],
  "used_by": [
   "Cloudflare",
   "Zalando",
   "Mozilla",
   "WeChat"
  ],
  "docs": "https://www.rabbitmq.com/documentation.html",
  "tutorial": "https://www.rabbitmq.com/tutorials/tutorial-one-python.html",
  "added": "2026-09-26",
  "dex": 520
 },
 {
  "id": "redpanda",
  "name": "Redpanda",
  "category": "Tool",
  "domain": "Messaging",
  "eli5": "Redpanda is like Kafka but without the complicated setup — it does the same job of passing messages between apps, just simpler and faster.",
  "description": "Kafka-compatible streaming data platform written in C++. No ZooKeeper, no JVM — delivers 10x lower latency and simpler operations than Apache Kafka.",
  "use_cases": [
   "Drop-in Kafka replacement with lower ops overhead",
   "High-throughput event streaming",
   "Real-time analytics ingestion",
   "Embedded streaming in edge or IoT deployments"
  ],
  "used_by": [
   "Loft Labs",
   "Aiven",
   "Forma.ai",
   "Starburst"
  ],
  "docs": "https://docs.redpanda.com/",
  "tutorial": "https://docs.redpanda.com/current/get-started/quick-start/",
  "added": "2026-06-12",
  "dex": 521
 },
 {
  "id": "aws-s3",
  "name": "AWS S3",
  "category": "Platform",
  "domain": "Storage",
  "description": "Amazon Simple Storage Service — infinitely scalable object storage with 99.999999999% durability. Store and retrieve any amount of data from anywhere. Foundational AWS service used as a data lake, static site host, and backup store.",
  "use_cases": [
   "Data lake storage for analytics pipelines (Athena, Spark, Redshift Spectrum)",
   "Static website and frontend asset hosting with CloudFront CDN",
   "Backup, archival, and disaster recovery (S3 Glacier tiers)",
   "ML training dataset and model artifact storage",
   "Event-driven pipelines via S3 event notifications to Lambda/SQS"
  ],
  "used_by": [
   "Netflix",
   "Airbnb",
   "Dropbox",
   "Pinterest",
   "Slack"
  ],
  "docs": "https://docs.aws.amazon.com/s3/",
  "tutorial": "https://docs.aws.amazon.com/AmazonS3/latest/userguide/GetStartedWithS3.html",
  "added": "2026-06-08",
  "eli5": "A magic box that holds unlimited files and never loses them.",
  "dex": 522
 },
 {
  "id": "azure-blob",
  "name": "Azure Blob Storage",
  "category": "Platform",
  "domain": "Storage",
  "eli5": "Microsoft's giant cloud bucket for storing any file — like S3 but Azure's version.",
  "description": "Azure's massively scalable object storage for unstructured data. Three access tiers (Hot, Cool, Archive) for cost optimization. Supports hierarchical namespace (Azure Data Lake Storage Gen2) for big data analytics. Integrates with Azure CDN, Databricks, and Synapse Analytics.",
  "use_cases": [
   "Data lake foundation with ADLS Gen2 hierarchical namespace for analytics",
   "Backup and disaster recovery with geo-redundant storage options",
   "Static website hosting with Azure CDN integration",
   "ML dataset and model storage for Azure Machine Learning pipelines",
   "Tiered storage lifecycle policies: Hot → Cool → Archive as data ages"
  ],
  "used_by": [
   "Unilever",
   "3M",
   "Ecolab",
   "FedEx",
   "Walgreens Boots Alliance"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/storage/blobs/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/storage/blobs/storage-quickstart-blobs-portal",
  "added": "2026-06-08",
  "dex": 523
 },
 {
  "id": "cadaver",
  "name": "Cadaver",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "It's like a file browser that works over the internet, letting you upload, download, and organize files on a WebDAV server.",
  "description": "Cadaver is a command-line WebDAV client that enables users to access and manage files on WebDAV servers through an interactive shell interface. It's widely used for remote file synchronization and management in enterprise environments.",
  "use_cases": [
   "Managing files on WebDAV-enabled collaboration servers",
   "Automating file uploads and downloads to remote repositories",
   "Synchronizing document management systems with local systems",
   "Scripting batch operations on centralized file storage"
  ],
  "used_by": [
   "Apache Foundation",
   "SugarCRM",
   "Alfresco",
   "ownCloud"
  ],
  "docs": "https://www.webdav.org/cadaver/",
  "tutorial": "http://www.webdav.org/cadaver/guide/using.html",
  "added": "2026-07-28",
  "dex": 524
 },
 {
  "id": "ceph",
  "name": "Ceph",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "Ceph is like building your own private S3 in your data center from regular hard drives — it spreads your files across many disks so nothing gets lost.",
  "description": "Open-source distributed storage system providing object, block, and file storage in a single cluster. Designed for self-healing and no single point of failure.",
  "use_cases": [
   "On-premises object storage (S3-compatible via RadosGW)",
   "Persistent block storage for Kubernetes (via Rook)",
   "OpenStack Cinder backend",
   "Large-scale petabyte storage clusters"
  ],
  "used_by": [
   "CERN",
   "Red Hat",
   "Deutsche Telekom",
   "Bloomberg"
  ],
  "docs": "https://docs.ceph.com/",
  "tutorial": "https://docs.ceph.com/en/latest/install/",
  "added": "2026-06-12",
  "dex": 525
 },
 {
  "id": "gcs",
  "name": "Google Cloud Storage",
  "category": "Platform",
  "domain": "Storage",
  "eli5": "Google's magic bucket that holds unlimited files — like AWS S3 but made by Google.",
  "description": "GCP's unified object storage service with 99.999999999% durability. Four storage classes (Standard, Nearline, Coldline, Archive) for tiered cost/access tradeoffs. Strongly consistent, globally accessible, and tightly integrated with BigQuery and Dataflow.",
  "use_cases": [
   "Data lake for BigQuery, Dataflow, and Spark/Dataproc analytics pipelines",
   "ML training dataset and model artifact storage",
   "Static website hosting and CDN origin via Cloud CDN",
   "Cross-region replication with dual-region and multi-region buckets",
   "Backup and archival with Coldline/Archive storage classes"
  ],
  "used_by": [
   "Spotify",
   "Snap",
   "New York Times",
   "Lush",
   "King"
  ],
  "docs": "https://cloud.google.com/storage/docs",
  "tutorial": "https://cloud.google.com/storage/docs/quickstart-console",
  "added": "2026-06-08",
  "dex": 526
 },
 {
  "id": "memcached",
  "name": "Memcached",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "It's like a super-fast sticky note board that your computer uses to remember frequently used information so it doesn't have to look it up every time.",
  "description": "Memcached is a distributed memory caching system widely used to speed up dynamic web applications by temporarily storing frequently accessed data in RAM. It reduces database load and improves response times through simple key-value storage.",
  "use_cases": [
   "Caching database query results to reduce load",
   "Session storage for web applications",
   "Rate limiting and throttling data",
   "Leaderboard and scoring caches for gaming platforms"
  ],
  "used_by": [
   "Facebook",
   "Twitter",
   "Airbnb",
   "YouTube"
  ],
  "docs": "https://memcached.org/about",
  "tutorial": "https://memcached.org/wiki/NewStart",
  "added": "2026-08-16",
  "dex": 527
 },
 {
  "id": "minio",
  "name": "MinIO",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "MinIO is like having your own private Amazon S3 in your own data center — same interface, your rules.",
  "description": "High-performance, S3-compatible object storage designed for on-premises and private cloud deployments. Supports erasure coding and runs on commodity hardware.",
  "use_cases": [
   "On-premises object storage with S3 API",
   "Data lake storage tier",
   "Backup and archival for Kubernetes workloads",
   "ML training data and model artifact storage"
  ],
  "used_by": [
   "Verizon",
   "CERN",
   "NVIDIA",
   "Goldman Sachs"
  ],
  "docs": "https://min.io/docs/minio/",
  "tutorial": "https://min.io/docs/minio/linux/operations/install-deploy-manage/deploy-minio-single-node-single-drive.html",
  "added": "2026-06-11",
  "dex": 528
 },
 {
  "id": "mux-streaming",
  "name": "Mux Stream",
  "category": "Platform",
  "domain": "Storage",
  "eli5": "It's like having a professional video store that automatically makes your videos play smoothly whether someone's watching on a phone over Wi-Fi or on a TV with fast internet.",
  "description": "Mux is a video infrastructure platform that handles transcoding, delivery, and streaming at scale with built-in analytics and quality monitoring. It abstracts away the complexity of video processing and CDN management for developers.",
  "use_cases": [
   "Live streaming video content with adaptive bitrate",
   "On-demand video delivery with automatic format optimization",
   "Real-time video analytics and performance monitoring",
   "Building video platforms with playback insights"
  ],
  "used_by": [
   "Discord",
   "Figma",
   "Webflow",
   "Slack"
  ],
  "docs": "https://docs.mux.com",
  "tutorial": "https://docs.mux.com/guides",
  "added": "2026-09-03",
  "dex": 529
 },
 {
  "id": "restic",
  "name": "Restic",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "It's like taking snapshots of your files and storing them in a secure locker that you can open from anywhere to restore what you lost.",
  "description": "Restic is a modern backup program that is fast, secure, and efficient for backing up files to various backends including AWS S3, Google Cloud Storage, and self-hosted repositories. It provides incremental backups, deduplication, and end-to-end encryption out of the box.",
  "use_cases": [
   "Automated backup of application data to cloud storage",
   "Server disaster recovery with encrypted backups",
   "Personal and enterprise data archival",
   "Multi-destination backup strategies with deduplication"
  ],
  "used_by": [
   "Proxmox",
   "Home Assistant",
   "Enterprise backup users",
   "Open-source projects"
  ],
  "docs": "https://restic.readthedocs.io/",
  "tutorial": "https://restic.readthedocs.io/en/stable/030_preparing_a_new_repo.html",
  "added": "2026-07-19",
  "dex": 530
 },
 {
  "id": "rook",
  "name": "Rook",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "Rook is like a translator between Kubernetes and Ceph — it makes your cluster automatically manage its own distributed storage without any manual disk wrangling.",
  "description": "Cloud-native storage orchestrator for Kubernetes. Turns Ceph into a self-managing, self-scaling distributed storage system via Kubernetes operators and CRDs.",
  "use_cases": [
   "Persistent block storage for stateful Kubernetes workloads",
   "Self-healing distributed object storage (S3-compatible)",
   "Shared filesystem for multi-pod access (CephFS)",
   "Database persistent volumes on bare metal"
  ],
  "used_by": [
   "CERN",
   "Roblox",
   "Red Hat OpenShift",
   "Giantswarm"
  ],
  "docs": "https://rook.io/docs/rook/latest-release/",
  "tutorial": "https://rook.io/docs/rook/latest-release/Getting-Started/intro/",
  "added": "2026-06-14",
  "dex": 531
 },
 {
  "id": "seaweedfs",
  "name": "SeaweedFS",
  "category": "Tool",
  "domain": "Storage",
  "eli5": "It's like having a really smart filing system that stores lots of files across many computers and finds them super fast.",
  "description": "SeaweedFS is a fast distributed object storage system written in Go that scales horizontally and handles massive amounts of unstructured data. It provides object storage capabilities similar to S3 but with simpler architecture and superior performance for many use cases.",
  "use_cases": [
   "Distributed image and media storage for content platforms",
   "Replacement for expensive S3-like services in private clouds",
   "High-throughput file serving in IoT and edge computing",
   "Cost-effective backup and archival systems"
  ],
  "used_by": [
   "Allegro",
   "Roblox",
   "large media platforms",
   "enterprise data centers"
  ],
  "docs": "https://github.com/seaweedfs/seaweedfs/wiki",
  "tutorial": "https://github.com/seaweedfs/seaweedfs/wiki/Getting-Started",
  "added": "2026-06-19",
  "dex": 532
 },
 {
  "id": "argo-workflows",
  "name": "Argo Workflows",
  "category": "Tool",
  "domain": "CI/CD",
  "description": "Kubernetes-native workflow engine for orchestrating parallel jobs as DAGs or step sequences. Each step runs as a container. Powers ML pipelines, data engineering, and CI/CD automation on Kubernetes. Part of the Argo project (CNCF graduated).",
  "use_cases": [
   "ML training pipelines: data prep → feature engineering → train → evaluate as a DAG",
   "CI/CD pipelines running directly on Kubernetes without a separate CI server",
   "Data engineering batch jobs with fan-out parallelism and artifact passing",
   "Scheduled recurring workflows via CronWorkflow resources",
   "Event-driven workflow triggers via Argo Events integration"
  ],
  "used_by": [
   "Intuit",
   "Adobe",
   "Google",
   "Alibaba",
   "BlackRock"
  ],
  "docs": "https://argo-workflows.readthedocs.io/en/latest/",
  "tutorial": "https://argo-workflows.readthedocs.io/en/latest/quick-start/",
  "added": "2026-06-08",
  "eli5": "A flowchart that runs jobs on Kubernetes in the right order, automatically.",
  "dex": 533
 },
 {
  "id": "argocd",
  "name": "ArgoCD",
  "category": "Tool",
  "domain": "CI/CD",
  "description": "Declarative GitOps continuous delivery tool for Kubernetes. Continuously reconciles cluster state with Git as the source of truth. CNCF graduated project. Pairs with Argo Rollouts for progressive delivery.",
  "use_cases": [
   "GitOps deployments: Git PR merge triggers automatic cluster sync",
   "Multi-cluster application delivery from a single control plane",
   "Canary and blue/green deployments with Argo Rollouts",
   "App-of-apps pattern for managing many microservices",
   "Drift detection and auto-remediation for cluster state"
  ],
  "used_by": [
   "Intuit",
   "Red Hat",
   "Tesla",
   "Alibaba",
   "Adobe"
  ],
  "docs": "https://argo-cd.readthedocs.io/en/stable/",
  "tutorial": "https://argo-cd.readthedocs.io/en/stable/getting_started/",
  "added": "2026-06-08",
  "eli5": "A robot that watches Git and keeps your cluster looking exactly like the blueprint.",
  "dex": 534
 },
 {
  "id": "azure-devops",
  "name": "Azure DevOps",
  "category": "Platform",
  "domain": "CI/CD",
  "eli5": "Microsoft's all-in-one project tracker, code host, and pipeline runner — like GitHub + Jenkins bundled together.",
  "description": "Microsoft's integrated DevOps platform: Boards (project tracking), Repos (Git hosting), Pipelines (CI/CD), Test Plans, and Artifacts (package registry). Common in enterprises already using Microsoft tooling. Pipelines support any cloud and self-hosted agents.",
  "use_cases": [
   "CI/CD pipelines for .NET, Java, Python, Node via YAML or classic editor",
   "Azure Boards for Agile/Scrum sprint planning and work item tracking",
   "Azure Artifacts for NuGet, npm, Maven, Python, and Universal packages",
   "Self-hosted agents in VNets for builds needing private network access",
   "Integration with GitHub for source control while using Azure Pipelines for CI/CD"
  ],
  "used_by": [
   "Microsoft",
   "HP",
   "3M",
   "Accenture",
   "Progressive Insurance"
  ],
  "docs": "https://learn.microsoft.com/en-us/azure/devops/",
  "tutorial": "https://learn.microsoft.com/en-us/azure/devops/pipelines/get-started/pipelines-get-started",
  "added": "2026-06-08",
  "dex": 535
 },
 {
  "id": "bazel-build",
  "name": "Bazel",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "It's like a smart construction manager that only rebuilds the exact parts of your house that actually changed.",
  "description": "Build system that enables fast, incremental builds with hermetic, reproducible outputs across multiple languages and platforms. Used extensively in large monorepos to parallelize and cache build artifacts efficiently.",
  "use_cases": [
   "Managing builds in large monorepos with mixed languages and targets",
   "Ensuring reproducible builds and deployment artifacts",
   "Accelerating CI/CD pipelines through intelligent caching and parallelization",
   "Enforcing dependency management and sandboxing across polyglot projects"
  ],
  "used_by": [
   "Google",
   "Stripe",
   "Twitter",
   "Databricks"
  ],
  "docs": "https://bazel.build/docs",
  "tutorial": "https://bazel.build/start",
  "added": "2026-08-17",
  "dex": 536
 },
 {
  "id": "buf-breaking-changes",
  "name": "Buf Breaking Change Detection",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "It's like a spell-checker that catches when you accidentally change the shape of messages your programs send to each other, before anyone's code breaks.",
  "description": "Buf breaking change detection automatically identifies backward-incompatible changes in Protocol Buffers and gRPC service definitions before they're deployed, preventing runtime failures in distributed systems. It integrates into CI/CD pipelines to enforce API compatibility standards.",
  "use_cases": [
   "Preventing breaking changes in microservices API contracts",
   "Enforcing semantic versioning for gRPC service upgrades",
   "Automating API compatibility checks in CI/CD workflows",
   "Detecting protobuf schema changes that would break client applications"
  ],
  "used_by": [
   "Google Cloud",
   "Stripe",
   "Figma",
   "Twilio"
  ],
  "docs": "https://buf.build/docs/breaking/overview",
  "tutorial": "https://buf.build/docs/breaking/configuration",
  "added": "2026-09-18",
  "dex": 537
 },
 {
  "id": "buildkite",
  "name": "Buildkite",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "Buildkite is like a CI system where the build machines are yours — Buildkite just tells them what to do, so your code never leaves your infrastructure.",
  "description": "Hybrid CI/CD platform where the orchestration is SaaS but build agents run on your own infrastructure. Enables fast, scalable pipelines without sending code to a third party.",
  "use_cases": [
   "Fast parallel test suites on self-hosted agents",
   "CI/CD for security-sensitive regulated environments",
   "Large monorepo pipelines with dynamic step generation",
   "GPU and specialized hardware build agents"
  ],
  "used_by": [
   "Shopify",
   "Airbnb",
   "PagerDuty",
   "Canva"
  ],
  "docs": "https://buildkite.com/docs/",
  "tutorial": "https://buildkite.com/docs/tutorials/getting-started",
  "added": "2026-06-12",
  "dex": 538
 },
 {
  "id": "circleci",
  "name": "CircleCI",
  "category": "Platform",
  "domain": "CI/CD",
  "eli5": "A cloud service that runs your tests and deploys your code automatically every time you push — like a robot QA engineer.",
  "description": "Cloud-native CI/CD platform known for speed and developer experience. Pipelines defined in `.circleci/config.yml`. Offers Docker layer caching, test parallelism, reusable Orbs (pre-built pipeline packages), and self-hosted runners. Strong GitHub/Bitbucket integration.",
  "use_cases": [
   "Fast CI with Docker layer caching and test splitting across parallel containers",
   "Orbs: reusable config packages for AWS, GCP, Slack, Snyk, and 1000+ integrations",
   "Scheduled pipelines for nightly builds, dependency updates, and reports",
   "Self-hosted runners for compliance, on-prem, or GPU workloads",
   "Dynamic config: generate pipeline config at runtime based on changed files"
  ],
  "used_by": [
   "Spotify",
   "PagerDuty",
   "Stitch Fix",
   "Lyft",
   "Coinbase"
  ],
  "docs": "https://circleci.com/docs/",
  "tutorial": "https://circleci.com/docs/getting-started/",
  "added": "2026-06-08",
  "dex": 539
 },
 {
  "id": "earthly",
  "name": "Earthly",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "Earthly is like a recipe book that tells computers exactly how to build your software the same way every time, whether you're cooking at home or in a restaurant.",
  "description": "Earthly is a build automation tool that creates reproducible builds using a simple, Docker-based syntax. It simplifies CI/CD pipelines by allowing developers to define builds once and run them consistently across local machines and CI systems.",
  "use_cases": [
   "Creating reproducible Docker images and artifacts",
   "Simplifying multi-language monorepo builds",
   "Running the same build locally as in CI/CD pipelines",
   "Reducing CI/CD configuration complexity"
  ],
  "used_by": [
   "Stripe",
   "Shopify",
   "Segment"
  ],
  "docs": "https://docs.earthly.dev/",
  "tutorial": "https://docs.earthly.dev/basics/part-1-a-simple-earthfile",
  "added": "2026-06-15",
  "dex": 540
 },
 {
  "id": "earthly-build-system",
  "name": "Earthly Build System",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "Earthly is like a recipe book for building software that works the same way on your computer and on a server, so nothing breaks when you switch places.",
  "description": "Earthly is a build automation tool that uses containerization to create reproducible builds across different environments. It combines the power of Docker and Makefiles to provide consistent build results whether running locally or in CI/CD pipelines.",
  "use_cases": [
   "Creating reproducible builds that work identically across developer machines and CI systems",
   "Building multi-language projects with complex dependencies in a single unified pipeline",
   "Caching build artifacts and dependencies efficiently using container layers",
   "Integrating with existing CI/CD systems like GitHub Actions, GitLab CI, and Jenkins"
  ],
  "used_by": [
   "Stripe",
   "Segment",
   "Twilio",
   "Auth0"
  ],
  "docs": "https://earthly.dev/",
  "tutorial": "https://earthly.dev/doc/basic",
  "added": "2026-09-24",
  "dex": 541
 },
 {
  "id": "earthly-ci-cd-containerization",
  "name": "Earthly CI/CD",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "It's like a recipe card that tells computers exactly how to build your software in the exact same way every single time, no matter where they build it.",
  "description": "Earthly is a containerized CI/CD framework that uses a language similar to Dockerfile and Makefile to define reproducible builds. It ensures consistent build outputs across local machines and CI/CD pipelines.",
  "use_cases": [
   "Running identical builds locally and in CI/CD pipelines",
   "Building multi-language projects with complex dependencies",
   "Creating reproducible Docker image builds",
   "Parallelizing build tasks across containers"
  ],
  "used_by": [
   "Datadog",
   "Notion",
   "Twitch",
   "Segment"
  ],
  "docs": "https://docs.earthly.dev/",
  "tutorial": "https://docs.earthly.dev/basics/overview",
  "added": "2026-09-22",
  "dex": 542
 },
 {
  "id": "flux-cd",
  "name": "Flux CD",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "A robot that watches Git and keeps your Kubernetes cluster matching exactly what's in the repo — like ArgoCD but different.",
  "description": "GitOps toolkit for Kubernetes by Weaveworks. Continuously reconciles cluster state with Git repositories. Modular design (Source, Kustomize, Helm, Notification controllers). CNCF graduated project. Integrates natively with kustomize and Helm.",
  "use_cases": [
   "GitOps: cluster state continuously synced from Git without manual kubectl apply",
   "Multi-tenancy: separate Flux instances per team with isolated Git repos",
   "Progressive delivery with Flagger: canary, blue/green, A/B via traffic shifting",
   "Image automation: auto-update Git when new container images are published",
   "Multi-cluster GitOps from a single management cluster"
  ],
  "used_by": [
   "Weaveworks",
   "Deutsche Telekom",
   "Lunar",
   "RingCentral",
   "Digital Ocean"
  ],
  "docs": "https://fluxcd.io/flux/",
  "tutorial": "https://fluxcd.io/flux/get-started/",
  "added": "2026-06-08",
  "dex": 543
 },
 {
  "id": "forgejo",
  "name": "Forgejo",
  "category": "Platform",
  "domain": "CI/CD",
  "eli5": "It's like GitHub but you can run it on your own computer or server, so you own all your code instead of renting space.",
  "description": "Forgejo is a lightweight, self-hosted Git service forked from Gitea that provides a GitHub-like experience without cloud dependency. It's ideal for organizations needing on-premise source code management with integrated CI/CD capabilities.",
  "use_cases": [
   "Self-hosted source code management for regulated industries",
   "Organizations avoiding cloud vendor lock-in",
   "Integrated CI/CD pipelines on private infrastructure",
   "Small to medium teams requiring GitHub-like features without SaaS costs"
  ],
  "used_by": [
   "Various government agencies",
   "financial institutions",
   "privacy-focused companies"
  ],
  "docs": "https://forgejo.org/docs",
  "tutorial": "https://forgejo.org/docs/latest/admin/",
  "added": "2026-06-19",
  "dex": 544
 },
 {
  "id": "github-actions",
  "name": "GitHub Actions",
  "category": "Platform",
  "domain": "CI/CD",
  "description": "GitHub-native CI/CD and automation platform. Workflows defined in YAML trigger on GitHub events (push, PR, release, schedule). Hosted runners for Linux/macOS/Windows; self-hosted runners for custom environments.",
  "use_cases": [
   "CI pipelines: lint, test, build on every PR",
   "CD pipelines: deploy to cloud on merge to main",
   "Automated release workflows: semantic versioning, changelog, publish to npm/PyPI",
   "OIDC-based passwordless auth to AWS, GCP, Azure — no stored credentials",
   "Matrix builds for cross-platform and multi-version testing"
  ],
  "used_by": [
   "Microsoft",
   "Google",
   "Meta",
   "Stripe",
   "Vercel"
  ],
  "docs": "https://docs.github.com/en/actions",
  "tutorial": "https://docs.github.com/en/actions/quickstart",
  "added": "2026-06-08",
  "eli5": "A helper that automatically does chores (tests, deploys) whenever you push code.",
  "dex": 545
 },
 {
  "id": "gitlab-ci",
  "name": "GitLab CI/CD",
  "category": "Platform",
  "domain": "CI/CD",
  "eli5": "GitHub Actions but built into GitLab — your code and your pipelines live in the same house.",
  "description": "Built-in CI/CD platform in GitLab, configured via `.gitlab-ci.yml`. Tight integration with GitLab's SCM, container registry, security scanning, and deployment environments. Supports self-hosted runners and Auto DevOps for zero-config pipelines.",
  "use_cases": [
   "CI/CD pipelines defined as YAML in the same repo as application code",
   "Auto DevOps: zero-config pipelines for build, test, SAST, DAST, deploy",
   "GitLab Environments and Deployments for tracking where code is deployed",
   "Self-managed runners on any infrastructure (Kubernetes, shell, Docker)",
   "Security scanning (SAST, DAST, dependency, container) built into pipelines"
  ],
  "used_by": [
   "Goldman Sachs",
   "Airbus",
   "T-Mobile",
   "Alibaba",
   "CERN"
  ],
  "docs": "https://docs.gitlab.com/ee/ci/",
  "tutorial": "https://docs.gitlab.com/ee/ci/quick_start/",
  "added": "2026-06-08",
  "dex": 546
 },
 {
  "id": "goreleaser",
  "name": "GoReleaser",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "It's like a automated factory that takes your code and packages it up perfectly for every different computer (Mac, Windows, Linux) and publishes it everywhere at once.",
  "description": "GoReleaser automates the entire release process for Go applications, handling cross-compilation, artifact creation, changelog generation, and multi-repository publishing. It's become the standard for Go project maintainers to deliver high-quality releases reliably.",
  "use_cases": [
   "Automating releases of Go CLI tools across Linux, macOS, and Windows",
   "Publishing binaries to multiple repositories (GitHub, GitLab, Artifactory) in one workflow",
   "Generating and managing changelogs with conventional commits",
   "Creating Docker images and signing artifacts for secure distribution"
  ],
  "used_by": [
   "Kubernetes project ecosystem",
   "Docker",
   "CNCF projects",
   "Numerous open-source Go projects"
  ],
  "docs": "https://goreleaser.com/intro/",
  "tutorial": "https://goreleaser.com/quick-start/",
  "added": "2026-08-02",
  "dex": 547
 },
 {
  "id": "jenkins",
  "name": "Jenkins",
  "category": "Tool",
  "domain": "CI/CD",
  "description": "Open-source automation server — the original CI/CD tool, extensible via 1800+ plugins. Self-hosted, runs pipelines defined in Groovy-based Jenkinsfiles. Highly flexible but operationally heavy.",
  "use_cases": [
   "Build, test, and deploy pipelines via declarative/scripted Jenkinsfile",
   "Integration with any VCS, artifact registry, or deployment target via plugins",
   "Multi-branch pipelines for PR-based workflows",
   "Distributed builds across master-agent clusters",
   "Legacy enterprise CI/CD where self-hosted control is required"
  ],
  "used_by": [
   "LinkedIn",
   "Netflix",
   "eBay",
   "Bosch",
   "IBM"
  ],
  "docs": "https://www.jenkins.io/doc/",
  "tutorial": "https://www.jenkins.io/doc/pipeline/tour/getting-started/",
  "added": "2026-06-08",
  "eli5": "A robot that builds and tests your code every time you save a change.",
  "dex": 548
 },
 {
  "id": "moto",
  "name": "Moto",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "It's a fake AWS service that lives in your computer so you can test cloud code without paying real money.",
  "description": "Moto is a library that mocks AWS services for testing purposes, allowing developers to test AWS-dependent code locally without incurring costs. It supports hundreds of AWS services and is essential for CI/CD pipelines testing infrastructure as code.",
  "use_cases": [
   "Unit and integration testing of AWS-dependent applications",
   "Local development without AWS credentials",
   "CI/CD pipeline testing before deployment",
   "Cost reduction by avoiding test infrastructure charges"
  ],
  "used_by": [
   "Amazon",
   "Segment",
   "Twilio",
   "CloudFormation users"
  ],
  "docs": "https://docs.getmoto.org/",
  "tutorial": "https://docs.getmoto.org/en/latest/docs/getting_started.html",
  "added": "2026-06-26",
  "dex": 549
 },
 {
  "id": "renovate",
  "name": "Renovate",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "Renovate is like a robot assistant that automatically opens pull requests whenever your dependencies have updates, so you never fall behind.",
  "description": "Automated dependency update tool that creates PRs for outdated packages across npm, Docker, Helm, Terraform, and 100+ package managers. Highly configurable update policies.",
  "use_cases": [
   "Automated dependency updates across all package ecosystems",
   "Scheduled batching of minor/patch updates",
   "Security patch automation",
   "Monorepo dependency management"
  ],
  "used_by": [
   "WhiteSource",
   "GitLab",
   "Mend",
   "HashiCorp"
  ],
  "docs": "https://docs.renovatebot.com/",
  "tutorial": "https://docs.renovatebot.com/getting-started/installing-onboarding/",
  "added": "2026-06-12",
  "dex": 550
 },
 {
  "id": "tekton",
  "name": "Tekton",
  "category": "Tool",
  "domain": "CI/CD",
  "eli5": "A pipeline builder that lives inside Kubernetes — each step of your CI/CD runs as its own container.",
  "description": "Cloud-native CI/CD framework for Kubernetes. Defines pipelines as Kubernetes CRDs (Tasks, Pipelines, PipelineRuns). Each step is a container — portable, reusable, and cloud-agnostic. Foundation of OpenShift Pipelines and many enterprise CI/CD platforms.",
  "use_cases": [
   "Kubernetes-native CI/CD where each pipeline step is an OCI container",
   "Reusable Tasks shared via Tekton Hub across teams and orgs",
   "Supply chain security: Tekton Chains signs pipeline artifacts with Sigstore",
   "Multi-cloud pipelines that run identically on any Kubernetes cluster",
   "Foundation for OpenShift Pipelines and other enterprise CI platforms"
  ],
  "used_by": [
   "Google",
   "Red Hat",
   "IBM",
   "Puppet",
   "Broadcom"
  ],
  "docs": "https://tekton.dev/docs/",
  "tutorial": "https://tekton.dev/docs/getting-started/",
  "added": "2026-06-08",
  "dex": 551
 },
 {
  "id": "auth0",
  "name": "Auth0",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "Auth0 is like hiring a professional security guard for your app — you don't build the login system yourself, you let experts handle it.",
  "description": "Managed identity platform providing authentication, authorization, and user management as a service. Supports social login, MFA, SSO, and enterprise federation.",
  "use_cases": [
   "Outsourcing authentication to a managed service",
   "Social login (Google, GitHub, Facebook)",
   "B2B SSO with SAML and OIDC",
   "Role-based access control (RBAC)"
  ],
  "used_by": [
   "Mozilla",
   "Siemens",
   "AMD",
   "Atlassian"
  ],
  "docs": "https://auth0.com/docs/",
  "tutorial": "https://auth0.com/docs/quickstarts",
  "added": "2026-06-12",
  "dex": 552
 },
 {
  "id": "authelia",
  "name": "Authelia",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "Authelia is like a bouncer at a club who checks your ID and makes sure you're allowed in, and remembers you so you don't have to show your ID every time.",
  "description": "Authelia is an open-source single sign-on and authorization server that provides multi-factor authentication, session management, and fine-grained access control for protecting internal applications and APIs. It integrates seamlessly with reverse proxies like Nginx and Traefik.",
  "use_cases": [
   "Centralized authentication and authorization for internal tools and applications",
   "Implementing multi-factor authentication without proprietary services",
   "Fine-grained access control based on user groups and policies",
   "Session management and single sign-on across multiple applications"
  ],
  "used_by": [
   "self-hosted communities",
   "enterprises",
   "open-source projects",
   "ISPs"
  ],
  "docs": "https://www.authelia.com/",
  "tutorial": "https://www.authelia.com/integration/deployment/supported-proxies/nginx/",
  "added": "2026-07-31",
  "dex": 553
 },
 {
  "id": "authentik",
  "name": "Authentik Identity Provider",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "It's like a security guard at the door of your applications who checks everyone's ID and decides who gets in and what they can do.",
  "description": "Authentik is an open-source identity provider offering SSO, multi-factor authentication, and authorization without vendor lock-in. It provides modern authentication capabilities comparable to enterprise solutions while remaining self-hostable.",
  "use_cases": [
   "Single sign-on (SSO) for multiple applications",
   "Multi-factor authentication enforcement",
   "User provisioning and lifecycle management",
   "SAML and OAuth2 protocol support for legacy integrations"
  ],
  "used_by": [
   "Proxmox",
   "Metabase",
   "Various self-hosted enterprises"
  ],
  "docs": "https://goauthentik.io/docs/",
  "tutorial": "https://goauthentik.io/docs/guides/basic-flows",
  "added": "2026-08-07",
  "dex": 554
 },
 {
  "id": "aws-iam",
  "name": "AWS IAM",
  "category": "Platform",
  "domain": "Identity",
  "description": "AWS Identity and Access Management — controls who can do what in AWS. Define users, roles, and policies to grant least-privilege access to every AWS resource. Central to AWS security posture.",
  "use_cases": [
   "Least-privilege policies for services, users, and cross-account access",
   "IAM roles for EC2/Lambda/EKS workloads (no long-lived credentials)",
   "Service Control Policies (SCPs) in AWS Organizations for guardrails",
   "OIDC federation for GitHub Actions / Kubernetes workload identity",
   "Auditing access with IAM Access Analyzer and CloudTrail"
  ],
  "used_by": [
   "All AWS customers"
  ],
  "docs": "https://docs.aws.amazon.com/iam/",
  "tutorial": "https://docs.aws.amazon.com/IAM/latest/UserGuide/getting-started.html",
  "added": "2026-06-08",
  "eli5": "The bouncer who checks IDs and decides who gets into which room.",
  "dex": 555
 },
 {
  "id": "azure-entra-id",
  "name": "Azure Entra ID",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "Microsoft's bouncer that remembers all your company's employees and lets them into all the apps with one password.",
  "description": "Microsoft's cloud identity platform (formerly Azure Active Directory). Enterprise SSO, MFA, conditional access, and B2B/B2C identity for 500M+ users. The identity backbone of Microsoft 365, Azure, and thousands of SaaS apps. OIDC/SAML/OAuth2 federation hub.",
  "use_cases": [
   "Enterprise SSO across Microsoft 365, Azure, and 3000+ pre-integrated SaaS apps",
   "Conditional access policies: enforce MFA based on user, location, device risk",
   "B2B collaboration: external partners access internal apps with their own credentials",
   "B2C identity: customer-facing app login with social providers (Google, Facebook)",
   "Workload identity federation for GitHub Actions and Kubernetes to access Azure"
  ],
  "used_by": [
   "Fortune 500 companies",
   "Government agencies",
   "Universities",
   "Healthcare systems"
  ],
  "docs": "https://learn.microsoft.com/en-us/entra/identity/",
  "tutorial": "https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-web-app-sign-in",
  "added": "2026-06-08",
  "dex": 556
 },
 {
  "id": "dex",
  "name": "Dex",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "Dex is like a universal translator for logins — it sits in front of your apps and lets users sign in with GitHub, LDAP, or SAML, giving your app a single clean OIDC interface.",
  "description": "Federated OpenID Connect provider that bridges OIDC/OAuth2 to upstream identity providers like LDAP, SAML, GitHub, and Google. Acts as an identity hub for Kubernetes and apps.",
  "use_cases": [
   "OIDC front-end for Kubernetes kubectl authentication",
   "Federating LDAP/Active Directory into OIDC",
   "Single OIDC provider aggregating multiple IdPs",
   "GitOps tooling auth (ArgoCD, Flux)"
  ],
  "used_by": [
   "Red Hat",
   "Kubernetes community",
   "Argo Project"
  ],
  "docs": "https://dexidp.io/docs/",
  "tutorial": "https://dexidp.io/docs/getting-started/",
  "added": "2026-06-14",
  "dex": 557
 },
 {
  "id": "keto",
  "name": "Keto",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "It's like a rulebook that decides who is allowed to do what in your application, based on relationships like 'Alice manages Bob' or 'admins can delete users'.",
  "description": "Keto is an open-source, cloud-native authorization service that implements fine-grained access control through relation-based access control (ReBAC) and Zanzibar-inspired permission graphs. It decouples authorization logic from applications, enabling scalable and maintainable permission systems.",
  "use_cases": [
   "Multi-tenant SaaS applications with complex permission hierarchies",
   "Role-based and attribute-based access control implementations",
   "Building permission systems that scale with organizational complexity",
   "Authorization as a dedicated microservice in zero-trust architectures"
  ],
  "used_by": [
   "Ory ecosystem",
   "Enterprise SaaS platforms",
   "Zero-trust initiatives"
  ],
  "docs": "https://docs.ory.sh/keto/",
  "tutorial": "https://docs.ory.sh/keto/quickstart",
  "added": "2026-07-25",
  "dex": 558
 },
 {
  "id": "keycloak",
  "name": "Keycloak",
  "category": "Tool",
  "domain": "Identity",
  "description": "Open-source Identity and Access Management (IAM) solution by Red Hat. Provides SSO, OAuth 2.0, OpenID Connect, and SAML out of the box. Self-hosted alternative to Auth0 and Okta for teams needing full control over identity infrastructure.",
  "use_cases": [
   "SSO across multiple applications with a single login via OIDC/SAML",
   "Social login federation (Google, GitHub, LDAP, Active Directory)",
   "Fine-grained authorization with roles, groups, and resource-based policies",
   "Token exchange and service account auth for machine-to-machine flows",
   "Multi-tenant identity with separate realms per customer or environment"
  ],
  "used_by": [
   "Red Hat",
   "Bosch",
   "Airbus",
   "Deutsche Telekom",
   "Zalando"
  ],
  "docs": "https://www.keycloak.org/documentation",
  "tutorial": "https://www.keycloak.org/getting-started/getting-started-docker",
  "added": "2026-06-08",
  "eli5": "One front door for all your apps — log in once, get into everything.",
  "dex": 559
 },
 {
  "id": "okta",
  "name": "Okta",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "Okta is like a master keycard for enterprise apps — employees log in once and Okta silently opens every door they're allowed through.",
  "description": "Enterprise identity and access management platform. Provides SSO, MFA, lifecycle management, and API security for workforce and customer identity use cases.",
  "use_cases": [
   "Workforce SSO across SaaS apps",
   "Customer identity and access management (CIAM)",
   "Zero-trust policy enforcement",
   "SCIM-based user provisioning and deprovisioning"
  ],
  "used_by": [
   "FedEx",
   "T-Mobile",
   "jetBlue",
   "Nordstrom"
  ],
  "docs": "https://developer.okta.com/docs/",
  "tutorial": "https://developer.okta.com/docs/guides/",
  "added": "2026-06-12",
  "dex": 560
 },
 {
  "id": "openfga",
  "name": "OpenFGA",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "It's like a permission system that decides who can do what, written as simple sentences that are easy to understand.",
  "description": "OpenFGA is an open-source authorization engine using the Relationship-Based Access Control (ReBAC) model, enabling scalable fine-grained permission management. It can handle complex permission hierarchies and is used by companies needing sophisticated access control.",
  "use_cases": [
   "Multi-tenant SaaS permission systems",
   "Hierarchical team and role management",
   "Marketplace platforms with complex sharing rules",
   "Document collaboration with granular permissions"
  ],
  "used_by": [
   "Auth0",
   "Okta",
   "Slack",
   "Figma"
  ],
  "docs": "https://openfga.dev/docs",
  "tutorial": "https://openfga.dev/docs/getting-started",
  "added": "2026-07-12",
  "dex": 561
 },
 {
  "id": "ory",
  "name": "Ory",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "Ory is like Auth0 but open-source — a complete identity toolkit you can self-host or use as a cloud service, covering login, permissions, and API security.",
  "description": "Open-source identity infrastructure stack: Kratos (user management), Hydra (OAuth2/OIDC), Keto (permissions), and Oathkeeper (API gateway). Cloud-native and self-hostable.",
  "use_cases": [
   "Self-hosted OAuth2 and OpenID Connect server",
   "User registration, login, and 2FA flows",
   "Fine-grained permissions with Google Zanzibar model",
   "Zero-trust API access control"
  ],
  "used_by": [
   "Segment",
   "Sainsbury's",
   "various open-source projects"
  ],
  "docs": "https://www.ory.sh/docs/",
  "tutorial": "https://www.ory.sh/docs/getting-started/overview",
  "added": "2026-06-12",
  "dex": 562
 },
 {
  "id": "spicedb",
  "name": "SpiceDB",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "SpiceDB is like a permission rulebook that instantly tells you who can do what, just like a bouncer checks a VIP list at a club.",
  "description": "SpiceDB is an open-source, Google Zanzibar-inspired authorization system that stores and evaluates fine-grained permissions at scale. It enables applications to implement complex access control policies with millisecond-latency permission checks.",
  "use_cases": [
   "Multi-tenant SaaS platforms with complex role-based access control",
   "Real-time permission evaluation for APIs and microservices",
   "Organizations migrating from policy-as-code to graph-based authorization",
   "Enterprise applications requiring audit trails for permission decisions"
  ],
  "used_by": [
   "Auth0",
   "Okta",
   "Cisco",
   "Carta"
  ],
  "docs": "https://docs.authzed.com/",
  "tutorial": "https://docs.authzed.com/getting-started",
  "added": "2026-08-03",
  "dex": 563
 },
 {
  "id": "spiffe",
  "name": "SPIFFE / SPIRE",
  "category": "Tool",
  "domain": "Identity",
  "description": "SPIFFE (Secure Production Identity Framework For Everyone) is a CNCF standard for workload identity. SPIRE is its reference implementation. Issues short-lived X.509 SVIDs and JWT-SVIDs to workloads — cryptographic identity without secrets or passwords.",
  "use_cases": [
   "Workload identity for microservices: every pod/VM gets a cryptographic SPIFFE ID",
   "mTLS between services using automatically rotated X.509 SVIDs",
   "Eliminating static credentials: services authenticate by identity, not secrets",
   "Federation across clouds and on-prem via SPIFFE federation bundles",
   "Foundation for Istio, Consul Connect, and Envoy SDS identity"
  ],
  "used_by": [
   "Bloomberg",
   "GitHub",
   "Square",
   "Pinterest",
   "Uber"
  ],
  "docs": "https://spiffe.io/docs/latest/spiffe-about/overview/",
  "tutorial": "https://spiffe.io/docs/latest/try/getting-started-linux-macos-x/",
  "added": "2026-06-08",
  "eli5": "An ID card factory that gives every microservice a passport so they trust each other.",
  "dex": 564
 },
 {
  "id": "supabase-auth",
  "name": "Supabase Auth",
  "category": "Tool",
  "domain": "Identity",
  "eli5": "Supabase Auth is like a bouncer at a nightclub who checks your ID and remembers you so you don't have to prove who you are every time.",
  "description": "An open-source authentication and authorization system built on PostgreSQL Row Level Security (RLS), providing email/password, OAuth, magic links, and multi-factor authentication out of the box. It eliminates the need for external identity providers while maintaining enterprise-grade security.",
  "use_cases": [
   "Building modern web and mobile applications with built-in user authentication",
   "Implementing fine-grained row-level access control directly in the database",
   "Enabling social login through OAuth providers without third-party IAM platforms",
   "Securing real-time applications with token-based authentication"
  ],
  "used_by": [
   "Vercel",
   "Figma",
   "ProductHunt"
  ],
  "docs": "https://supabase.com/docs/guides/auth",
  "tutorial": "https://supabase.com/docs/guides/auth/quickstarts",
  "added": "2026-06-27",
  "dex": 565
 },
 {
  "id": "supertokens",
  "name": "SuperTokens",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "It's like a bouncer at a club who checks your ID and remembers you so you don't have to show it every time you enter a room.",
  "description": "SuperTokens is an open-source authentication and session management platform that provides secure user login, signup, and multi-factor authentication capabilities. It eliminates the need to build authentication from scratch with pre-built SDKs for web and mobile applications.",
  "use_cases": [
   "Implementing passwordless authentication flows",
   "Managing user sessions and refresh tokens securely",
   "Adding multi-factor authentication to applications",
   "Handling complex OAuth integrations with social providers"
  ],
  "used_by": [
   "Hasura",
   "Supabase",
   "Auth0 competitors",
   "Enterprise SaaS platforms"
  ],
  "docs": "https://supertokens.com/docs",
  "tutorial": "https://supertokens.com/docs/guides",
  "added": "2026-09-03",
  "dex": 566
 },
 {
  "id": "zitadel",
  "name": "ZITADEL",
  "category": "Platform",
  "domain": "Identity",
  "eli5": "It's like a security guard at a club who checks your ID, remembers who you are, and decides what you're allowed to access without asking again.",
  "description": "ZITADEL is an open-source identity and access management (IAM) platform that provides authentication, authorization, and user management capabilities. It offers enterprise-grade security features with a cloud-native, Kubernetes-ready architecture.",
  "use_cases": [
   "Enterprise authentication and single sign-on (SSO) for applications",
   "Managing user roles, permissions, and access control policies",
   "Integrating OAuth 2.0 and OpenID Connect authentication",
   "Implementing multi-tenant identity management at scale"
  ],
  "used_by": [
   "Roche",
   "Boehringer Ingelheim",
   "Multiple enterprises",
   "Open-source community"
  ],
  "docs": "https://zitadel.com/docs",
  "tutorial": "https://zitadel.com/docs/quickstarts/start-coding",
  "added": "2026-06-17",
  "dex": 567
 },
 {
  "id": "cal-com",
  "name": "Cal.com",
  "category": "Tool",
  "domain": "Other",
  "eli5": "It's like a smart calendar that automatically finds free time between you and someone else so you can schedule meetings without the back-and-forth.",
  "description": "Cal.com is an open-source scheduling infrastructure and calendar tool that replaces Calendly, enabling customizable booking experiences and integrating directly with applications. It provides scheduling as a composable component.",
  "use_cases": [
   "SaaS product embedded scheduling and calendar booking",
   "Sales and support team meeting coordination",
   "Event and interview scheduling automation",
   "White-label calendar solutions for platforms"
  ],
  "used_by": [
   "Notion",
   "Figma",
   "OpenAI",
   "Slack"
  ],
  "docs": "https://cal.com/docs",
  "tutorial": "https://cal.com/docs/introduction",
  "added": "2026-08-27",
  "dex": 568
 },
 {
  "id": "hyperswitch",
  "name": "Hyperswitch",
  "category": "Platform",
  "domain": "Other",
  "eli5": "It's like a smart traffic director that sends your money payment to whichever payment processor will handle it best and fastest.",
  "description": "An open-source payment orchestration platform that routes transactions across multiple payment processors to optimize success rates, reduce costs, and improve reliability. It abstracts the complexity of managing multiple payment gateways with a unified API.",
  "use_cases": [
   "Intelligent payment routing across multiple processors",
   "Reducing payment failure rates through smart retry logic",
   "Consolidating multiple payment gateway integrations",
   "PCI compliance simplification and payment tokenization"
  ],
  "used_by": [
   "Stripe",
   "Adyen",
   "Juspay",
   "Wise"
  ],
  "docs": "https://hyperswitch.io/docs/",
  "tutorial": "https://hyperswitch.io/docs/quickstart",
  "added": "2026-08-06",
  "dex": 569
 },
 {
  "id": "mattermost",
  "name": "Mattermost",
  "category": "Platform",
  "domain": "Other",
  "eli5": "It's like Slack, but you can install it on your own computer instead of trusting someone else to keep your messages safe.",
  "description": "Mattermost is an open-source team collaboration and messaging platform that can be self-hosted, providing Slack-like functionality with full control over data and infrastructure. It's widely used in enterprises requiring on-premise solutions.",
  "use_cases": [
   "Internal team communication and chat",
   "DevOps and incident response coordination",
   "Compliance-requiring organizations needing data control",
   "Integration with development tools and workflows"
  ],
  "used_by": [
   "AWS",
   "SolarWinds",
   "OpenStack",
   "Hitachi Vantara"
  ],
  "docs": "https://docs.mattermost.com/",
  "tutorial": "https://docs.mattermost.com/guides/deployment.html",
  "added": "2026-07-01",
  "dex": 570
 },
 {
  "id": "mermaid",
  "name": "Mermaid",
  "category": "Tool",
  "domain": "Other",
  "eli5": "It's like a magic notebook where you can write simple instructions and it automatically draws diagrams, flowcharts, and pictures for you.",
  "description": "Mermaid is a JavaScript-based diagramming and charting tool that uses a simple, markdown-inspired syntax to create and modify diagrams dynamically. It enables developers to document architecture, workflows, and systems as code within their projects.",
  "use_cases": [
   "Creating system architecture and flowchart documentation in version control",
   "Embedding diagrams directly in markdown documentation and README files",
   "Visualizing database schemas, user journeys, and deployment workflows",
   "Generating sequence diagrams and Gantt charts for project planning"
  ],
  "used_by": [
   "GitHub",
   "GitLab",
   "Notion",
   "Confluence"
  ],
  "docs": "https://mermaid.js.org/",
  "tutorial": "https://mermaid.js.org/ecosystem/integrations.html",
  "added": "2026-06-14",
  "dex": 571
 },
 {
  "id": "mermaid-live",
  "name": "Mermaid Live",
  "category": "Tool",
  "domain": "Other",
  "eli5": "Mermaid is like a magic sketchpad where you write simple words and it automatically draws charts, flowcharts, and diagrams for you.",
  "description": "Mermaid is a JavaScript-based diagramming and charting tool that uses a Markdown-inspired syntax to create and modify diagrams dynamically. It's widely used for documentation, architecture diagrams, and workflow visualization.",
  "use_cases": [
   "Creating architecture and system design diagrams",
   "Generating flowcharts and process workflows",
   "Documenting user journey and sequence flows",
   "Building organizational charts and timeline diagrams"
  ],
  "used_by": [
   "GitHub",
   "GitLab",
   "Notion",
   "Confluence"
  ],
  "docs": "https://mermaid.js.org/",
  "tutorial": "https://mermaid.live/",
  "added": "2026-06-18",
  "dex": 572
 },
 {
  "id": "polar",
  "name": "Polar",
  "category": "Platform",
  "domain": "Other",
  "eli5": "It's like a payment processor for open-source developers, similar to how a store handles credit cards so customers can buy things without giving money directly to the shopkeeper.",
  "description": "A platform that enables open-source developers and creators to monetize their work through sponsorships, product sales, and subscriptions while maintaining their open-source commitment. It handles payments, licensing, and community management in one integrated system.",
  "use_cases": [
   "Open-source projects generating recurring revenue from supporters",
   "Selling digital products and merchandise directly to users",
   "Managing sponsorship tiers with different benefit levels",
   "Funding independent developers without creating closed-source forks"
  ],
  "used_by": [
   "Astro",
   "Zod",
   "Trigger.dev"
  ],
  "docs": "https://docs.polar.sh",
  "tutorial": "https://docs.polar.sh/getting-started",
  "added": "2026-08-05",
  "dex": 573
 },
 {
  "id": "risc-v-specification",
  "name": "RISC-V ISA",
  "category": "Concept",
  "domain": "Other",
  "eli5": "It's like an open instruction manual that any company can use to build their own computer processors without paying licensing fees.",
  "description": "RISC-V is an open-source instruction set architecture (ISA) that enables anyone to design, manufacture, and deploy processors without licensing restrictions. It's gaining adoption in embedded systems, IoT devices, and server processors.",
  "use_cases": [
   "Building custom processors for embedded systems and IoT",
   "Reducing licensing costs for chip manufacturers",
   "Academic research in computer architecture",
   "Edge computing and specialized hardware acceleration"
  ],
  "used_by": [
   "SiFive",
   "Western Digital",
   "Google",
   "NVIDIA"
  ],
  "docs": "https://riscv.org/",
  "tutorial": "https://riscv.org/technical/specifications/",
  "added": "2026-08-24",
  "dex": 574
 },
 {
  "id": "sablier",
  "name": "Sablier",
  "category": "Tool",
  "domain": "Other",
  "eli5": "It's like a smart piggy bank for cryptocurrency that automatically releases money over time, like getting a paycheck spread out into tiny daily deposits.",
  "description": "Sablier is a protocol for streaming payments on blockchain, enabling continuous money flows instead of lump-sum transfers. It's widely used for salary automation, vesting schedules, and continuous funding mechanisms in Web3.",
  "use_cases": [
   "Automating token vesting for employees and investors",
   "Creating continuous salary or subscription payments on-chain",
   "Setting up real-time revenue sharing between parties",
   "Managing grant disbursements with time-based release schedules"
  ],
  "used_by": [
   "Aave",
   "Uniswap",
   "Yearn Finance"
  ],
  "docs": "https://docs.sablier.com",
  "tutorial": "https://github.com/sablier-labs/v2-core",
  "added": "2026-07-10",
  "dex": 575
 },
 {
  "id": "vscode",
  "name": "Visual Studio Code",
  "category": "Tool",
  "domain": "Other",
  "eli5": "A smart notebook for writing code that helps you catch mistakes and find the right words, like autocorrect but way smarter.",
  "description": "VS Code is a lightweight, extensible code editor built by Microsoft that dominates modern software development with its speed, debugging capabilities, and vast extension ecosystem. It supports virtually every programming language and framework through plugins, making it the most widely adopted editor in the industry.",
  "use_cases": [
   "Full-stack web development with Node.js, React, and Vue frameworks",
   "Python development with linting, formatting, and debugging tools",
   "Remote development and tunneling via VS Code Remote extensions",
   "Infrastructure-as-Code editing with Terraform, CloudFormation, and Ansible support"
  ],
  "used_by": [
   "Microsoft",
   "Google",
   "Netflix",
   "Stripe"
  ],
  "docs": "https://code.visualstudio.com/docs",
  "tutorial": "https://code.visualstudio.com/learn",
  "added": "2026-08-13",
  "dex": 576
 },
 {
  "id": "vscode-extension-marketplace",
  "name": "VS Code Extension Marketplace",
  "category": "Platform",
  "domain": "Other",
  "eli5": "It's like an app store for your text editor where you can download add-ons that give it superpowers.",
  "description": "VS Code's official marketplace hosting thousands of extensions that enhance code editing, debugging, and development workflows. It's a critical ecosystem that extends VS Code's functionality for virtually every programming language and framework.",
  "use_cases": [
   "Installing language support extensions (Python, Go, Rust, etc.)",
   "Integrating Git workflows and version control tools",
   "Adding cloud provider tooling (AWS, Azure, GCP)",
   "Enhancing productivity with formatters, linters, and testing frameworks"
  ],
  "used_by": [
   "GitHub",
   "Microsoft",
   "Google",
   "Amazon"
  ],
  "docs": "https://marketplace.visualstudio.com/",
  "tutorial": "https://code.visualstudio.com/docs/editor/extension-marketplace",
  "added": "2026-09-22",
  "dex": 577
 }
];
